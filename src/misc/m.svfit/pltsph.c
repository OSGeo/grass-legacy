...
#define PLT_STR "x0(t),f0(x0(t)) 2 2,x1(t),f1(x1(t)) 2 2,x2(t),f2(x2(t)) 2 2"

/* int plot_spherical (HGN *list) */

int plot_spherical (list, parameters, nwork, verbose, save)
  char *save;
  int nwork, verbose;
  HGN *list;
  PARAM parameters;
 ...
 * file for plotting the working list with a spherical semivariogram model
 * defined in structure {\tt parameters}. Does not check to see if
 ...
 */
{
  ...
  fprintf (tmp_file, "set title 'Spherical Semivariogram'\n");
  ...
  fprintf (tmp_file, "d=%g\n", parameters.range);
  fprintf (tmp_file, "e=%g\n", parameters.range);
  /*
   * the following assume that working list is sorted in ascending order
   */
  fprintf (tmp_file, "f=%g\n", list[nwork - 1].h);
  fprintf (tmp_file, "x0(t) = a+(b-a)*t\n");
  fprintf (tmp_file, "x1(t) = c+(d-c)*t\n");
  fprintf (tmp_file, "x2(t) = e+(f-e)*t\n");
  fprintf (tmp_file, "f0(x) = %g*x/b\n", parameters.c0);
  fprintf (tmp_file, "f1(x) = %g+%g*(1.5*x/%g - 0.5*(x/%g)**3)\n",
       parameters.c0, parameters.c1, parameters.range, parameters.range);
  fprintf (tmp_file, "f2(x) = %g\n", parameters.sill);

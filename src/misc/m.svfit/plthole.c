...
/* int plot_hole_effect (HGN *list) */

int plot_hole_effect (list, parameters, nwork, verbose, save)
  char *save;
  int nwork, verbose;
  HGN *list;
  PARAM parameters;
 ...
 * file for plotting the working list with a hole effect semivariogram
 ...
 */
{
  ...
  fprintf (tmp_file, "set title 'Hole Effect (Wave) Semivariogram'\n");
  ...
  fprintf (tmp_file, "f1(x) = %g+%g*(1.0-r*sin(x/r)/x)\n",
       parameters.c0, parameters.c1);

  /******************************************/
  /* BUG BUG need to define x2(t) and f2(x) */
  /******************************************/

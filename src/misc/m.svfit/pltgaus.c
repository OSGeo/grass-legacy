...
/* int plot_gaussian (HGN *list) */

int plot_gaussian (list, parameters, nwork, verbose, save)
  char *save;
  int nwork, verbose;
  HGN *list;
  PARAM parameters;
 ...
 * file for plotting the working list with a gaussian semivariogram model
 ...
 */
{
  ...
  fprintf (tmp_file, "set title 'Gaussian Semivariogram'\n");
  ...
  fprintf (tmp_file, "f1(x) = %f+%f*(1.0-exp(-3.0*(x/%f)**2))\n",

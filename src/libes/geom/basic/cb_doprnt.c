...
/* NOTE: This code is based on Steve Summit's _doprnt() implementation.
         See his copyright notice and short discription below.
         For more details (incl original files) see "Summit" subdirectory.
         --Ernst. */
...
#ifndef lint   /* don't worry about lint on this one */
...
/* #include "printf.h" */
#define PRF_LONG        01
#define PRF_NUMSGN      010
#define PRF_LADJUST     020
#define PRF_ZEROPAD     040
#define PRF_CAPITAL     01000
...
#define basic_isdigit(d) ((d) >= '0' && (d) <= '9')
#define Ctod(c) ((c) - '0')
...
return basic_cb_do2prnt(0, fmt, &argp /*,fd*/);
}

/* only broken up into two routines #ifdef ASKF */

int basic_cb_do2prnt(askf, fmt, argpp /*,fd*/)
int askf,
register char *fmt,
va_list *argpp /* , */
/*FILE *fd;*/
...
#ifndef NORETURNVALUE
#define Putc(c, fd) {basic_cb_putc((c) /*,(fd)*/); nprinted++; }
#else
#define Putc(c, fd)  basic_cb_putc((c) /*,(fd)*/)
#endif
...
while(*fmt != '\0')
        ...
        while(TRUE)
                ...
                else if(basic_isdigit(*fmt))
                        {
                        while(basic_isdigit(*fmt))
...
static void tack (int d, char *digs, char **p)
{
if(d == 0) return;
if(d >= 1 && d <= 3)
        {
...
static void doit (int d, int one, char **p)
{
int i;

...
#ifndef lint
static char RCSID[] = "@(#)$Id: PJ_lsat.c,v 4.5 1992/07/14 01:27:35 gie Exp $";
#endif
...
ENTRY(pj_lsat) {
	...
	land = pj_param("ilandsat", "")->i;
	if (land <= 0 || land > 5) {
		emess(-1,"landsat value of %d not in 1 to 5 range",land);
		E_ERROR;
	}
	path = pj_param("ipath", "")->i;
	if (path <= 0 || path > (land <= 3 ? 251 : 233)) {
		emess(-1,"path %d not in LANDSAT %d range",path,land);
		E_ERROR;
	}
	if (land <= 3) {

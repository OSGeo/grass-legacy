...
/*****************************************************************/
void
gsd_bgn_legend_viewport(GLint wl, GLint wb, GLint wr, GLint wt)
{
...
/*****************************************************************/
int
gsd_get_nice_range(float lownum, float highnum, int numvals, 
		float *vals)
{
...
/*****************************************************************/
int 
gsd_make_nice_number(float *num)
{
...
/*****************************************************************/
/*
gsd_put_legend(geosurf *gs, char *name, GLint sl, GLint sr, GLint sb, GLint st, 
int legend_type, float *rangef, float *list, int nlist)
*/

void 
gsd_put_legend(char *name, char *font, int *flags, float *rangef, int *pt)
{
GLint sl, sr, sb, st;
...
*/
	...
	if (discrete){
	    ...
	    else if(numlabs > 1.2 * lleg/gsd_get_txtheight())
	...
	if(cat_labs || cat_vals){ 
	    ...
	    for(k=0; k<numlabs; k++){
			...
			labpt[X] = labpos * (sr - sl) + xoff - 
				    gsd_get_txtwidth(buff)/2 - get_txtxoffset();
			labpt[Y] = st - sb + yoff + 3 + gsd_get_txtheight()/2;
		    }
		    else{

...
*  long dig_input();      manages all graphics input, screen disp, & button str*
...
long raw_diginput()
{
    ...
    if(digdevice.mode == QUERY_MODE && digdevice.no_query_actions)
    {
        ...
        /************* find a good packet (1 read max, if necess) ***/
    readflag = 0;
	...
	for(;;)
            ...
            else if(ptr2 < endptr)    /*** not a good packet ***/
	    {
            ...
            {
		if (readflag)           /*** read already tried ***/
		{
                    return(0);
		}
...
#endif
    {
	sprintf(tmp_msg, "Cannot open %s.  Check the digX_adr field \
in the config file.", dev_path);
...
ACTION *act;
{
    ...
    {
        act->action = 0;
        act->value = bslash(p2); 	/* there is a BUG IN blash stuff -dpg */
...
*   Parameters:   none                                                         *
*                                                                              *
*   Values Returned:    -1 = error                                             *
*                        0 = no digitizer input                                *
*                        1 = new digitizer input, but no button press          *
*                        2 = button press, defined in cursor_key[][]           *
*                                                                              *

...

{
  ...
  if (xRedundant) {
    for (z = 0; z < depths; z++) {
      for (y = 0; y < cols; y++) {
	if (! G3d_copyFromXdr (rows, tile)) {
	  G3d_error ("G3d_xdrTile2tile: error in G3d_copyFromXdr");
	  return 0;
	}
	tile += rows * G3d_length (type);
	G3d_setNullValue (tile, xRedundant, type);
	tile += xRedundant * G3d_length (type);
      }
      if (yRedundant) {
  ...
  if (yRedundant) {
    for (z = 0; z < depths; z++) {
      if (! G3d_copyFromXdr (map->tileX * cols, tile)) {
	G3d_error ("G3d_xdrTile2tile: error in G3d_copyFromXdr");
	return 0;
      }
      tile += map->tileX * cols * G3d_length (type);
      G3d_setNullValue (tile, map->tileX * yRedundant, type);
      tile += map->tileX * yRedundant * G3d_length (type);
    }
    if (! zRedundant) return 1;
  ...
  tile += map->tileXY * depths * G3d_length (type);
  G3d_setNullValue (tile, map->tileXY * zRedundant, type);
...

{
  ...
  G3d_cache_set_minUnlock (map->cache, minUnlocked);
}

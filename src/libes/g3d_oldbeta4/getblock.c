...

{
  ...
  if (! map->useCache) tile = G3d_allocTilesType (map, 1, type);
  if (tile == NULL)
  ...
  for (tz = tileZ0; tz <= tileZ1; tz++) {
	...
	if (G3d_tileIndexInRange (map, tileIndex)) 
	  if (map->useCache) {
	    ...
	    if (tile == NULL) 
	      G3d_fatalError ("G3d_getBlockNocache: error in G3d_getTilePtr");
	  } else {
	    if (! G3d_readTile (map, tileIndex, tile, map->typeIntern))
	      G3d_fatalError ("G3d_getBlockNocache: error in G3d_readTile");
	  }
	      
	else
	  G3d_setNullTile (map, tile);

	rows = (tx == tileX1 ? tileOffsX1 : map->tileX - 1);
	cols = (ty == tileY1 ? tileOffsY1 : map->tileY - 1);
	depths = (tz == tileZ1 ? tileOffsZ1 : map->tileZ - 1);

	x = (tx == tileX0 ? tileOffsX0 : 0);

	for (z = (tz == tileZ0 ? tileOffsZ0 : 0); z <= depths; z++) 
	  for (y = (ty == tileY0 ? tileOffsY0 : 0); y <= cols; y++) {
	    G3d_copyValues (tile, 
			    z * map->tileXY + y * map->tileX + x, 
			    map->typeIntern, 
			    block, 
			    (z + dz) * nx * ny + (y + dy) * nx + (x + dx),
			    type,
			    rows - x + 1);
	  }
      }
  ...
  if (! map->useCache) G3d_freeTiles (tile);
}
...

{
  ...
  x1 = G3D_MIN (x0 + nx, map->region.rows);
  y1 = G3D_MIN (y0 + ny, map->region.cols);
  ...
  for (z = z0; z < z1; z++) {
    for (y = y0; y < y1; y++) {
      for (x = x0; x < x1; x++) {
	G3d_getValue (map, x, y, z, block, type);
	block += length;
      }
      nNull = x0 + nx - x;

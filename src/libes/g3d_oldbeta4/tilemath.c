...
{
  return (x >= 0) && (x < map->region.rows) && (y >= 0) && (y < map->region.cols) &&
...

{
  ...
  if (x != map->clipX) {
    ...
    *rows = (map->region.rows - 1) % map->tileX + 1;
    *xRedundant = map->tileX - *rows;
  }
  if (y != map->clipY) {
    *cols = map->tileY; 
    *yRedundant = 0;
  } else {
    *cols = (map->region.cols - 1) % map->tileY + 1;
    *yRedundant = map->tileY - *cols;
  }
  if (z != map->clipZ) {
    *depths = map->tileZ; 
    *zRedundant = 0;
...

{
  return ((north >= map->region.south) && (north <= map->region.north) &&
	  (west >= map->region.west) && (west <= map->region.east) &&
	  (((bottom >= map->region.bottom) && (bottom <= map->region.top)) ||
	   ((bottom <= map->region.bottom) && (bottom >= map->region.top))));
}
...
void
G3d_location2coord (map, north, west, bottom, x, y, z)
...
{
  if (! G3d_isValidLocation (map, north, west, bottom))
    G3d_fatalError ("location2coord: location not in region");

  *x = (north - map->region.south) / (map->region.north - map->region.south) * 
       (map->region.rows - 1);
  *y = (west - map->region.west) / (map->region.east - map->region.west) * 
        (map->region.cols - 1);
  *z = (bottom - map->region.bottom) / (map->region.top - map->region.bottom) * 
       (map->region.depths - 1);
}

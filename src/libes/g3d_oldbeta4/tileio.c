...

{
  ...
  if (map->typeIntern == G3D_DOUBLE) 
    return (float) G3d_getDouble (map, x, y, z);

  G3d_coord2tileIndex (map, x, y, z, &tileIndex, &offs);
  tile = (float *) G3d_getTilePtr (map, tileIndex);

  if (tile == NULL) G3d_fatalError ("G3d_getFloat: error in G3d_getTilePtr");
...

{
  ...
  if (map->typeIntern == G3D_FLOAT) 
    return (double) G3d_getFloat (map, x, y, z);

  G3d_coord2tileIndex (map, x, y, z, &tileIndex, &offs);
  tile = (double *) G3d_getTilePtr (map, tileIndex);
  
  if (tile == NULL) G3d_fatalError ("G3d_getDouble: error in G3d_getTilePtr");

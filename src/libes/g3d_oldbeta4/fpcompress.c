...

{
  ...
  for (i = 0; i < numBits; i++) {
    fprintf (stdout,"%d", (*((unsigned char *) c) & bit) != 0);
    bit >>= 1;
  }
}
...

{
  ...
  f = (float *) src;
  nofNull = 0;
  while (srcStop != (unsigned char *) f) nofNull += G3d_isXdrNullFloat (f++);
...
/* fprintf (stdout,"write src cp0 %d %d (%d %d) %d\n", *src, *cp0, src, cp0, nullBits); */
	...
	*cp1 = 
	  (unsigned char) ((unsigned char) ((*src & mask)) << (8 - nBits));
	nBits += precision - 8;
      } else {
...

{
  ...
  d = (double *) src;
  nofNull = 0;
  while (srcStop != (unsigned char *) d) nofNull += G3d_isXdrNullDouble (d++);
  ...
  *length = 1;
  ...
  if (nofNull) *length += size / 8 + ((size % 8) != 0);
...

{
  ...
  if (*src == (unsigned char) ALL_NULL_CODE) {
    f = (float *) dst;
    while (dstStop != (unsigned char *) f) G3d_setXdrNullFloat (f++);
...

{
  ...
  if (isFloat) 
    G_fpcompress_rearrangeDecodeFloats (compressBuf, nofNum, precision, dst);
  else
    G_fpcompress_rearrangeDecodeDoubles (compressBuf, nofNum, precision, dst);

...

{
  ...
  for (z = 0; z < nz * tileZ; z++) {
	...
	if (! correct) {
	  ...
	  G3d_coord2tileCoord (map2, x, y, z, &xTile, &yTile, &zTile, 
			       &xOffs, &yOffs, &zOffs);
	  fprintf (stdout,"(%d %d %d) (%d %d %d) (%d %d %d) %.20lf %.20lf\n", 
  ...
  fprintf (stdout,"Files are identical up to precision.\n");
}
...

{
  ...
  map = G3d_openCellOld (f1, mapset1, G3D_TILE_SAME_AS_FILE, 
  ...
  map2 = G3d_openCellOld (f2, mapset2, G3D_TILE_SAME_AS_FILE, 
			 G3D_USE_CACHE_DEFAULT);
  if (map2 == NULL) 
    G3d_fatalError ("G3d_compareFiles: error in G3d_openCellOld");
  ...
  G3d_autolockOn (map);  
  G3d_autolockOn (map2);  
  G3d_minUnlocked (map, rows / tileX + 1); 

  G3d_getCoordsMap (map2, &rows, &cols, &depths);
  G3d_minUnlocked (map2, rows / tileX + 1); 
  ...
  for (z = 0; z < depths; z++) {
    ...
    for (y = 0; y < cols; y++)
      for (x = 0; x < rows; x++) {
	G3d_getValue (map, x, y, z, n1p, typeIntern);
	G3d_getValue (map2, x, y, z, n2p, typeIntern2);

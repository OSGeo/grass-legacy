 ...
 *	form also includes a list of keyword synonymns
 *    
...
G_parse_command_usage (progname, keys, full)
    char *progname;
    struct Command_keys keys[];
    int full;			/* use long form if true */
{
    register int i, j;
    int maxkey;
    int longest;
	...
	if ((len = strlen (keys[i].alias)) > longest)
	    longest = len;
    }
	...
	for (position = 1 ; position <= maxkey ; position++) 
		...
		if (keys[j].alias[0] != '\0' && keys[j].position == position)
		{
		    fprintf (stderr, "[%s%s] ", keys[j].alias, i==0?"":"=opt");

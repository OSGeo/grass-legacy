...
/*
 * set first byte of cell data buf to no data.
 * Data will be read into this buffer starting at the second cell.
 */
    data = DATA_BUF;
...
printf ("read row %d\n", r);
#endif
	if(read_data (fd, ROW=r, data, &FCB.cur_nbytes) < 0)
	{
	    i = WINDOW.cols;
	    while(i-- > 0)
		*cell++ = 0;
...
printf ("   reclass\n\r");
	...
	while (i-- > 0)
	{
	    v = cell_value(data, (int) *c++, FCB.cur_nbytes);
	    if (v < min || v > max)
		*cell++ = 0;
	    else
    ...
    else                                    /* no reclass */
    {
        ...
        i = WINDOW.cols;
	while (i-- > 0)
	    *cell++ = cell_value(data, (int) *c++, FCB.cur_nbytes);
    }
    ...
    register int nbytes;
{
    ...
    data += offset * nbytes;
    if (nbytes >= sizeof (CELL) && (*data & 0200))
    {
        neg = 1;
        v = *data++ & 0177;
    }

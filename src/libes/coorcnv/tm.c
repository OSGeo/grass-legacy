 ...
 * lat/lon to tm conversion
 *
 * CC_tm2ll_spheroid (spheroid_name)
 *     char *spheroid_name
 *
 ...
 *
 * CC_tm2ll_spheroid_parameters (a,e2)
 *    double a,e2;
 *
 *    called by CC_tm2ll_spheroid() to set the ellipsoid major axis 'a'
 *    and eccentricity squared 'e'. can be called directly for
 *    unknown ellipsoids.
 *    returns -2 illegal values for a or e, 1 otherwise
 *
 * CC_tm2ll_zone (zone)
 *   must be called before CC_tm2ll_north()
 *   set tm 'zone' must be non-zero.
 *   positive means northern hemisphere
 *   negative means southern hemisphere
 *   used to set the longitude of the central merdian
 *   (only used for tm to lat/lon conversions)
 *
 * CC_tm2ll_north (north)
 *   double north
 *
 *   set the tm north. must be called before CC_tm2ll.
 *   returns -1 if related rectifying latitude exceed 1.47 radians
 *            1 otherwise
 *
 * CC_tm2ll (east, lat, lon)
 *  double east
 *  double *lat, *lon
 *
 *  compute lat,lon from east
 *  CC_tm2ll_north() already called with north.
 *  returns -1 if lon is more than .16 radians from center of tm zone
 *           1 otherwise
 ***********************************************************************/
...
static double a5 = 5.0e5;	/* false easting (TM) */
static double a6;		/* false northing */
static double a7 = 0;		/* M0? */
static double a8 = 1.0;	        /* scale factor at central meridian */
...
int 
CC_tm2ll_spheroid (char *spheroid_name)
{
    double a, e2;

    if (CC_get_spheroid (spheroid_name, &a, &e2))
	return CC_tm2ll_spheroid_parameters (a, e2);
    return -1;	   /* unknown spheroid */
}

int 
CC_tm2ll_spheroid_parameters (double a, double e2)
{
    double x, x2, x3, x4;
...
int 
CC_tm2ll (double east, double *lat, double *lon)
{
    double b9, b10;

    b9 = ((a5 - east) * 1.0e-6) / a8 ;
    if (dabs(b9) > a15 * 2.0e-7)
	return -1;   /* easting too far from center of zone */
...
int 
CC_ll2tm (double lat, double lon, double *east, double *north, int *zone)
{
    int deg;
    double sinp, cosp;
    double etas, t, ts, rn;
    ...
    if (dabs(b10) > .16)
	return -2;   /* longitude to far from center of zone */

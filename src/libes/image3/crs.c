...
/** mbaba -- changed string.h to strings.h **/
#include "strings.h"
#include "math.h"

/** mbaba -- included "sys/types.h" **/
#include "sys/types.h"
#include "limits.h"
...
*/
  ...
  FILE *fp;
static int 
checkgeoref (Control_Points_2D *cp, double E[], double N[], int order, int forward, FILE *fp)
#endif
...
*/
static int exactdet (Control_Points_2D *,struct MATRIX *,DOUBLE *,DOUBLE *,double *,double *);
static int solvemat (struct MATRIX *, DOUBLE *, DOUBLE *, double *, double *);
static DOUBLE term (int, double, double);
static int calcls (Control_Points_2D *,struct MATRIX *, DOUBLE *, DOUBLE *, double *, double *);
static int calccoef (Control_Points_2D *, double *, double *, int);
/***************************************************************************/
...
int CRS_compute_georef_equations (
  ...
  double *E12, double *N12, double *E21, double *N21, int order)
  {
  double *tempptr;
  int status;
...
static int 
calccoef (Control_Points_2D *cp, double E[], double N[], int order)
  {
  struct MATRIX m;
  DOUBLE *a;
  DOUBLE *b;
  ...
  /** mbaba -- require 1 more point than the minimum **/
  if(numactive < (m.n + 1))
    return(MNPTERR);

...
/* Check for acceptance of existing attributes */
	for (i=0; i<node[choice].n_atts; i++)
	{
		...
		sprintf(buffer," Retain: %d %d\n",
			node[choice].atts[i*2],
			node[choice].atts[i*2+1]) ; 
		...
		while(incorrect)
		{
			...
			{
				case 'y':
					atts[ptr++] = node[choice].atts[i*2] ;
					atts[ptr++] = node[choice].atts[i*2+1] ;
					incorrect = 0 ;
					break ;
				case 'n':
					incorrect = 0 ;
...
/* Free old atts list and allocate space for new */
	free((char*)node[choice].atts) ;
	node[choice].atts = (int *)(calloc(ptr,sizeof(int))) ;

/* update atts and n_atts */
	node[choice].n_atts = ptr/2 ;
	for (i=0; i<ptr; i++)
		node[choice].atts[i] = atts[i] ;
}

...
main ()
{
	...
	fprintf (stdout,"  High or low resolution, default is high. (h/l):  ") ;
	...
	fprintf (stdout,"     X           Y      reads:   good       bad     none  Nread\n") ;
...
while(1)
{
	...
	if ( (N_read<0)	 ||   (N_read != (CHARS_RD - 1)))
	 {
		++bad_r ;
		fprintf (stdout,"\r   %35s  bad read:  %g           N_read:   %d ",
				"", bad_r, N_read) ;
		continue ;
	 }
	***************/
	...
	fprintf (stdout,"\r    %d       %d              %ld,       %ld        %ld     %d    ",

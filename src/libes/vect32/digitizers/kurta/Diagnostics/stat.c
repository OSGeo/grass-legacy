...
/*  says to read 11 characters.  works better reading 12 or greater  */

#define	ser_name	"/dev/tty1"
#define	CHARS_RD	12
#define	ALARM_TIME	3
...
while (gets(buf) != NULL)
{
	...
	fprintf (stdout," should have written one character.  wrote: %d.\n", i) ;
	...
	for ( i = 0; i<8; i++)
	 {
		fprintf (stdout," %d", (c & 0x1)) ;
		c >>= 1 ;
	 }
	fprintf (stdout,"\n switch number :  1 2 3 4 5 6 7 8\n\n") ;

	fprintf (stdout," Unit Type Number(3): %c\n", InBuffer[7]) ;
	fprintf (stdout," Revision Number(A): %c\n", InBuffer[8]) ;
	fprintf (stdout," Unit Part Number(3): %c\n", InBuffer[9]) ;
	...
	/**
		...
		for (i=0; i<N_read+2; i++)
		 {
			fprintf (stdout," [%d] = %x ", i, InBuffer[i]) ;
			if (!(i%5))
				fprintf (stdout,"\n") ;
			InBuffer[i] = NULL ;
	 }
...
*/
	...
	if ( timedout ) 	/*  did it timeout on read  */
		return (-1);

	alarm (ALARM_TIME);
	N_read = read (IORser, InBuffer, CHARS_RD);	/*  read the remote machine  */
	alarm (0);

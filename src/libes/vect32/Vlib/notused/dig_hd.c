    ...
    struct dig_head *head;
{
    ...
    fp = G_fopen_old ("dig_hd", name, mapset);
    if (fp == NULL)
    {
	char buf[300];

	sprintf (buf, "Can't open header file for vector file [%s in %s]", name,
	    mapset);
	G_warning (buf);
	return -1;
    ...
    struct dig_head *head;
{
	...
	*/
	...
	V__read_head_file (fp, &tmp_head);
	V__copy_portable_info (&tmp_head, head); 	/* from -> to */
	fclose (fp);
    }
    else
    {
	...
	*/
	...
	V__copy_portable_info (&ptmp_head, head); 	/* from -> to */
    }

    return V__put_head (name, head);
}
...
Vect__put_head (name, head)
    ...
    struct dig_head *head;
{
    fp = G_fopen_new ("dig_hd", name, mapset);
    ...
    struct dig_head *head;
{
    ...
    dig_struct_copy (Vect__get_def_out_head (), head, sizeof (struct head));
}

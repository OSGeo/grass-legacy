    ...
    int tell;
{
    ...
    {
	char buf[100];
	sprintf (buf, "Cannot open '%s' for write\n", CM->plus_file);
	make_monolog (1, buf);
	return (-1);
    }
    ...
    phead.all_isles = 0;
    if (tell)
        write_info (1, "Writing Plus File ...");
    dig_map_to_head (CM, &phead);
    if (0 > dig_write_plus_file (fp, CM, &phead))
    {
	make_monolog (1, "Could not write Plus file");
    ...
    fclose (fp);
    if (tell)
	write_info (1, "*Writing Plus File ... DONE.");
    return (0);
}
...
/*DEBUG*/ debugf ("Checkpoint A\n");
    ...
    */
    ...
    if (tell)
    write_info (1, " Compressing NODES.");
...
/*DEBUG*/ debugf ("Deleted %d NODES\n", map->n_nodes - new_count);
    ...
    if (tell)
    write_info (1, " Compressing NODES.  LINES.");
...
/*DEBUG*/ debugf ("Deleted %d LINES\n", map->n_lines - new_count);
    ...
    if (tell)
    write_info (1, " Compressing NODES.  LINES.  AREAS.");
...
/*DEBUG*/ debugf ("Deleted %d AREAS\n", map->n_areas - new_count);
    ...
    if (tell)
    write_info (1, " Compressing NODES.  LINES.  AREAS.  ISLES.");
...
/*DEBUG*/ debugf ("Deleted %d ISLES\n", map->n_isles - new_count);
    ...
    if (tell)
    write_info (1, " Compressing NODES.  LINES.  AREAS.  ISLES. ATTS.");
...
/*DEBUG*/ debugf ("Deleted %d ATTS\n", map->n_atts - new_count);
    ...
    if (tell)
    {
	write_info (1, "Compressing Data:   DONE. ");
    }
}

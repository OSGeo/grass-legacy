...
#define LIST_SIZE 50
	...
	label1 = 0;
	ask_value("Line is unlabelled. Enter value:", &label1);
	if (label1 ==0)
	    return (0);
    }
    else
	...
	label2 = 0.0;
	while (label2 == 0.0)
	ask_value("Line is unlabelled. Enter value:", &label2);
	if (label2 ==0)
	    return (0);
...
/*DEBUG*/ debugf ("Crosses line %d  Cnt %d\n", line, cnt+1);
    ...
    {
	make_monolog (1, "ERROR: Crosses same line more than once");
    }
...
/* n_interve = 1 + number of internal lines */
    ...
    {
    /*DEBUG*/ debugf ("interval %d n_interval %d labeldiff %d\n",
		     interval, n_interv, label_diff);
	make_monolog (1, "Not correct number of intervals. No action taken.");
	goto ending;
    }
...
/************************************************************************/
	...
	if (LINE_LABELED (Line))
	{
	    ...
	    {
		make_monolog (1, "Labelled line does not agree. No action taken");
    ...
    */
	...
	for (n = 0 ; n < 2 ; n++)	/* do both directions */
	{
		...
		if (LINE_LABELED (Line) && map->Att[Line->att].cat != label)
		{
		    ...
		    make_monolog (1, "Ran into a line of different value");

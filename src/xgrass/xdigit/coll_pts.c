...
#define		DIGIT_POINT	1
#define		STOP_DIG	2
#define		UPDATE_MON	3
#define		TOGGLE_MODE	4


Collect_points(mode, ctype, p)
    int mode;
    char ctype;
    struct line_pnts *p;
{
	...
	/*DEBUG*/ debugf ("Collect_points\n");
    set_priority ();
    n_points =  &(p->n_points);
    set_lit_line (p, ctype, 0);

    type = (int)ctype;
	if (! alloc_points)
	{
		alloc_points     = 1000 ;
		p->x= (double *) dig_falloc(alloc_points, sizeof(double)) ;
		p->y = (double *) dig_falloc(alloc_points, sizeof(double)) ;
	}
	xarray = p->x;
	yarray = p->y;
...
/*  delay a heartbeat whilst they take finger off the keys, clear digitizer  */
	...
	if (run_mode == STREAM)
	{
		...
		while ( xdig_readall (&Xraw, &Yraw) < 0)
			 ;

		xdig_readall (&Xraw, &Yraw);
		xdig_readall (&Xraw, &Yraw);
		...
		*n_points = 1 ;
		stream_mode = 1 ;
		sprintf(message, "   %6d   %12.2lf %12.2lf", 
		     *n_points, *xptr, *yptr);
		add_info(1, message) ;
		xptr++ ;
		yptr++ ;
		write_info(2, " STREAM mode ") ;
	}
	else
	{
		Xlast = 0.0 ;
...
/*  digitizing loop  */
	while (loop)
	{
		...
		while ( (KeyHit = xdig_readall (&Xraw, &Yraw)) < 0)
			 ;
	...
	*/
	...
	/*  hit a cursor key  */
			...
			switch (KeyHit)
			 {
				...
				case UPDATE_MON:
					write_info(1, "    Updating  MONITOR") ;
					plot_points
					    (type, *n_points, xarray, yarray,
					     CLR_HIGHLIGHT, 0);
					write_info(1, 
					   "finished: continue  digitizing...");
					last_key = KeyHit ;
					continue ;
					break ;
		...
		if (run_mode == POINT  && KeyHit != DIGIT_POINT )
			continue ;
		...
		if (*n_points + 2 == alloc_points)
		{
			...
			old = alloc_points;
			alloc_points += 100 ;
			p->x = xarray = (double *)dig_frealloc((char *)xarray, 
					   alloc_points, sizeof(double), old);
			p->y = yarray = (double *)dig_frealloc((char *)yarray, 
					   alloc_points, sizeof(double), old);
		...
		sprintf(message, 
		    "   %6d   %12.2lf %12.2lf", *n_points, *xptr, *yptr);
    ...
    write_info(2, " ") ;
    /*  black out the line on the monitor  exactly as it is now  */

    ...
    int mode;
{
    ...
    if (area_n_points == 0)
    {
	Pnts.alloc_points = 0;	/* executed only once */
	area_n_points = -1; 
    }
	...
	if (!line_in_window (&(map->Line[line])))
	    continue;
	if (0 > V1_read_line (map, &Pnts, map->Line[line].offset))
	{
	...
	switch (mode) {
	    case DISPLAY:
		_color_line (map->Line[line].type, &Pnts, line, map, CLR_LAREA);
		break;
	    case ERASE:
		_erase_line (map->Line[line].type, &Pnts, line, map);
		break;
	    case HIGHLIGHT:
		_highlight_line (map->Line[line].type, &Pnts, line, map);
		break;
	    case RESET:
		/* 
		if (!Disp_lines)
		...
		*/
		_display_line (map->Line[line].type, &Pnts, line, map);
		break;
	    default:
		break;
	}

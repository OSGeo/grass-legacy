...
static Widget  legendBoxW;
static short   legendObjectSet;
...
#endif
{
  ...
  SetOptMenuToPixel(fgColor, Global.foreground);
  SetOptMenuToPixel(bColor, Global.foreground);
  XmToggleButtonSetState(togButton, False, False);
  XmTextFieldSetString(columnSpacing , val);
  XmToggleButtonSetState(tog1, True, False);
  XmToggleButtonSetState(tog2, False, False);
  XmToggleButtonSetState(tog3, False, False);
  legendObjectSet = 0;
}
...
#endif
{
  if (SelectedObjectCount() == 1){
     if (Global.selectedObjects->object->type == XGD_LEGEND){
       ...
       legendBoxW = CreateLegendBox(Global.selectedObjects->object);
       
       XtManageChild(legendBoxW);
     } else 

...
{ 
        ...
        zzGlobal->_xc_Data.prog = _XgStrDup(yyvsp[0].cval);
    }
break;
case 5:
#line 136 "xc.grammar.y"
{
        ...
        zzGlobal->_xc_Data.title = _XgStrDup(yyvsp[0].cval);
    }
break;
case 6:
#line 143 "xc.grammar.y"
{
        zzGlobal->_xc_Data.argString = _XgStrDup(yyvsp[0].cval);
    }
break;
case 12:
#line 169 "xc.grammar.y"
{
        ...
        zzGlobal->_xc_Data.description = _XgStrDup(yyvsp[0].cval);
    }
break;
case 13:
#line 178 "xc.grammar.y"
{
        ...
        zzGlobal->_xc_Data.help = _XgStrDup(yyvsp[0].cval);
    }
break;
case 14:
#line 186 "xc.grammar.y"
{
        ...
        zzGlobal->_xc_Data.helpFile.filename = _XgStrDup(yyvsp[0].cval);
    }
break;
case 15:
#line 194 "xc.grammar.y"
{
	char *ptr = _XgStrDup(yyvsp[0].cval);
...
#line 221 "xc.grammar.y"
{
        ...
        zzGlobal->_xc_Data.capture = (ISONSTR(yyvsp[0].cval) || ISTRUESTR(yyvsp[0].cval));
    }
break;
case 17:
#line 229 "xc.grammar.y"
{
        ...
        zzGlobal->_xc_Data.nerrcodes = _XcParseErrorCodes(yyvsp[0].cval,&(zzGlobal->_xc_Data.errorcodes));
	if ( zzGlobal->_xc_Data.nerrcodes == -1 ) {
          sprintf(errorbuf,"Illegal errorcode string [%s]", yyvsp[0].cval);
          yyerror(errorbuf);
	}
    }
break;
...
#line 276 "xc.grammar.y"
{
        ...
        */
	    ...
	    curPtr->name = _XgStrDup(yyvsp[0].cval);
        }  else {
	    tempPtr = 
		(XCInterfaceData *)_XgMalloc(sizeof(XCInterfaceData));
...
{
	curPtr->desc = _XgStrDup(yyvsp[0].cval);
        if ( XCverbose ) {
...
#line 348 "xc.grammar.y"
{
        ...
        curPtr->help = _XgStrDup(yyvsp[0].cval);
    }
break;
case 31:
#line 356 "xc.grammar.y"
{
        ...
        curPtr->helpFile.filename = _XgStrDup(yyvsp[0].cval);
    }
break;
case 32:
...
{ 
	...
	*/
	if ( !_XcUniqueName(interface, yyvsp[0].cval) ) {
	    sprintf(errorbuf,"Non-unique flag or parameter name [%s]",yyvsp[0].cval);
	    yyerror(errorbuf);
	}
        /*
...
{ 
	CURFLAG->key = _XgStrDup(yyvsp[0].cval);
        if ( XCverbose ) {
...
{
	CURFLAG->desc = _XgStrDup(yyvsp[-4].cval);
        if ( XCverbose ) {
            DoFprintf(dialogLevel,"\tDesc: %s\n",yyvsp[0].cval);
...
{
	CURFLAG->help = _XgStrDup(yyvsp[0].cval);
        if ( XCverbose ) {
            DoFprintf(dialogLevel,"\t\tHelp: %s\n",yyvsp[0].cval);
        }
    }
...
{
	CURFLAG->helpFile.filename = _XgStrDup(yyvsp[0].cval);
        if ( XCverbose ) {
            DoFprintf(dialogLevel,"\t\tHelpFile: %s\n",yyvsp[0].cval);
        }
    }
...
{
	CURFLAG->def = ISONSTR(yyvsp[0].cval);
...
#line 474 "xc.grammar.y"
{
	if (CURFLAG->precludes ) {
	    ...
	    rPtr = rPtr->next;
	    rPtr->isFlag = ISFLAGSTR(yyvsp[-2].cval);
	    rPtr->name = _XgStrDup(yyvsp[0].cval);
	    rPtr->next = NULL;
	} else {
	    CURFLAG->precludes = True;
	    CURFLAG->preclude.isFlag = ISFLAGSTR(yyvsp[-2].cval);
	    CURFLAG->preclude.name = _XgStrDup(yyvsp[0].cval);
...
#line 498 "xc.grammar.y"
{
	if (CURFLAG->requirements ) {
	    ...
	    CURFLAG->requires.isFlag = ISFLAGSTR(yyvsp[-2].cval);
	    CURFLAG->requires.name = _XgStrDup(yyvsp[0].cval);
...
{
	CURPARM->desc = _XgStrDup(yyvsp[0].cval);
        if ( XCverbose ) {
            DoFprintf(dialogLevel,"\tDesc: %s\n",yyvsp[0].cval);
        }
    }
...
#line 622 "xc.grammar.y"
{
	...
	CURPARM->hasModifier = True;
        XcModifierParse(&CURPARM->modifier,XC_TYPE_ENUMERATE,yyvsp[0].cval);
        if ( XCverbose ) {
...
{
        XcModifierParse(&CURPARM->modifier,XC_TYPE_INTEGER,yyvsp[0].cval);
        CURPARM->hasModifier = True;
        if ( XCverbose ) {
            DoFprintf(dialogLevel,"Modifiers: %s",yyvsp[0].cval);
        }
    }
break;
case 65:
...
{
        XcModifierParse(&CURPARM->modifier,XC_TYPE_DOUBLE,yyvsp[0].cval);
        CURPARM->hasModifier = True;
        if ( XCverbose ) {
            DoFprintf(dialogLevel,"Modifiers: %s",yyvsp[0].cval);
        }
...
{
        XcModifierParse(&CURPARM->modifier,XC_TYPE_LOGICAL,yyvsp[0].cval);
        CURPARM->hasModifier = True;
        if ( XCverbose ) {
            DoFprintf(dialogLevel,"Modifiers: %s",yyvsp[0].cval);
        }
...
{
        XcModifierParse(&CURPARM->modifier,XC_TYPE_FILENAME,yyvsp[0].cval);
        CURPARM->hasModifier = True;
        if ( XCverbose ) {
            DoFprintf(dialogLevel,"Modifiers: %s",yyvsp[0].cval);
        }
...
#line 935 "xc.grammar.y"
{
        ...
        case XC_TYPE_CHARACTER:
            CURPARM->def.cval = _XgStrDup(yyvsp[0].cval);
            CURPARM->value.cval = _XgStrDup(yyvsp[0].cval);
            break;
        case XC_TYPE_ENUMERATE:
            CURPARM->def.cval = _XgStrDup(
		_XcEnumStringToKey(CURPARM->modifier,yyvsp[0].cval));
            CURPARM->value.cval = _XgStrDup(CURPARM->def.cval);
            break;
        case XC_TYPE_INTEGER:
            CURPARM->def.ival = atoi(yyvsp[0].cval);
            CURPARM->value.ival = CURPARM->def.ival;
            break;
        case XC_TYPE_FLOAT:
#ifndef mips
            CURPARM->def.dval = strtod(yyvsp[0].cval,(char **)NULL);
#else
            CURPARM->def.dval = atof(yyvsp[0].cval);
#endif
        ...
        case XC_TYPE_LOGICAL:
            CURPARM->def.bval = (ISONSTR(yyvsp[0].cval) || ISTRUESTR(yyvsp[0].cval));
            CURPARM->value.bval = CURPARM->def.bval;
            break;
        case XC_TYPE_FILENAME:
            CURPARM->def.cval = _XgStrDup(yyvsp[0].cval);
            if ( CURPARM->isInput && access(CURPARM->def.cval,0) != 0 ) {
		sprintf(errorbuf,"invalid default input file name [%s]",yyvsp[0].cval);
		yyerror(errorbuf);
            }
            CURPARM->value.cval = _XgStrDup(yyvsp[0].cval);
            break;
        case XC_TYPE_DB_ELEMENT:
            CURPARM->def.dbval.desc = _XgStrDup(yyvsp[0].cval);
            CURPARM->value.dbval.desc = _XgStrDup(yyvsp[0].cval);
            break;
        }
...
{
	CURPARM->help = _XgStrDup(yyvsp[0].cval);
        if ( XCverbose  ) {
            DoFprintf(dialogLevel,"\tOptional Parm Elements:\n");
            DoFprintf(dialogLevel,"\t\tHelp: %s\n",yyvsp[0].cval);
        }
...
{
	CURPARM->helpFile.filename = _XgStrDup(yyvsp[0].cval);
        if ( XCverbose ) {
            DoFprintf(dialogLevel,"\tOptional Parm Elements:\n");
            DoFprintf(dialogLevel,"\t\tHelpFile: %s\n",yyvsp[0].cval);
        }
...
#line 1015 "xc.grammar.y"
{
        if (CURPARM->precludes ) {
            ...
            CURPARM->preclude.isFlag = ISFLAGSTR(yyvsp[-2].cval);
            CURPARM->preclude.name = _XgStrDup(yyvsp[0].cval);
...
#line 1040 "xc.grammar.y"
{
        if (CURPARM->requirements ) {
            ...
            CURPARM->requires.isFlag = ISFLAGSTR(yyvsp[-2].cval);
            CURPARM->requires.name = _XgStrDup(yyvsp[0].cval);
...
#line 1062 "xc.grammar.y"
{
      if ( CURPARM->type == XC_TYPE_FILENAME ||
           CURPARM->type == XC_TYPE_DB_ELEMENT ||
           CURPARM->type == XC_TYPE_ENUMERATE ) {
          CURPARM->multiple = (ISONSTR(yyvsp[0].cval) || ISTRUESTR(yyvsp[0].cval));
      } else {
...
{
        CURPARM->isInput = (ISONSTR(yyvsp[0].cval) || ISTRUESTR(yyvsp[0].cval));
        if ( XCverbose ) {
            DoFprintf(dialogLevel,"\tOptional Parm Elements:\n");
...
{
        CURPARM->optional  = (ISONSTR(yyvsp[0].cval) || ISTRUESTR(yyvsp[0].cval));
        if ( XCverbose ) {
            DoFprintf(dialogLevel,"\tOptional Parm Elements:\n");

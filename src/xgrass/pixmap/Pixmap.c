	...
	if (status == XpmPixmapSuccess) 
	  {
	      ...
	      for (i = 0; i < attribs.ncolors; i++)
		{
		  if (_PWDEBUG) {
		    ...
		    fprintf (stdout,"Color name %s\n", 
			   *(*(attribs.colorTable+i)+offset));
		    fprintf (stdout,"colorTable[%d][0] %s\n", i,
			   **(attribs.colorTable+i));
		    fprintf (stdout,"colorTable[%d][1] %s\n", i, 
			   *(*(attribs.colorTable+i)+1));
		    fprintf (stdout,"colorTable[%d][2] %s\n", i, 
			   *(*(attribs.colorTable+i)+2));
		    fprintf (stdout,"colorTable[%d][3] %s\n", i, 
			   *(*(attribs.colorTable+i)+3));
		    fprintf (stdout,"colorTable[%d][4] %s\n", i, 
			   *(*(attribs.colorTable+i)+4));
		    fprintf (stdout,"colorTable[%d][5] %s\n", i, 

...
GRASS commands operate on three basic forms of data:
  ...
  @ref("-  site data ",02.db.setup/02.03.3.fmt.s)
...
vector, and sites formats:
      ...
      @ref("-  r.line ",Commands.def/rline.def)            @ref("-  s.in.ascii ",Commands.def/sinascii.def)      @ref("-  v.in.ascii ",Commands.def/vinasc.def)
      @ref("-  r.poly ",Commands.def/rpoly.def)            @ref("-  s.out.ascii ",Commands.def/soutascii.def)     @ref("-  v.out.ascii ",Commands.def/voutasc.def)
      @ref("-  r.thin ",Commands.def/rthin.def)                                  @ref("-  v.to.rast ",Commands.def/v2rast.def)
      @ref("-  r.in.ascii ",Commands.def/rinasc.def)                              @ref("-  v.to.sites ",Commands.def/v2sites.def)

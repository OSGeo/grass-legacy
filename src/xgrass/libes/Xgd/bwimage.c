...
*/
  ...
  if (col_inc >= 1.0 && row_inc >= 1.0) {
	...
	while (col < ncols) {
	  switch
	    ((int) obj->Obj.GeoFrame.lookup_tbl[cell[col] - obj->Obj.GeoFrame.colors.fixed.min + 1]) {
	    case 0:
	...
	while (col < newbwwidth - 1) {
	  ...
	  if (rcol >= ncols)
	    rcol = ncols - 1;
	  switch
	    ((int) obj->Obj.GeoFrame.lookup_tbl[cell[rcol] - obj->Obj.GeoFrame.colors.fixed.min + 1]) {
	    case 0:
	      XPutPixel(obj->Obj.GeoFrame.image, (ccol - 1), crow,
			BlackPixel(obj->display, scrn));
	      XPutPixel(obj->Obj.GeoFrame.image, (ccol - 1), crow + 1,

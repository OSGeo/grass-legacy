...
#ifdef _NO_PROTO
XgdPlotLines(obj, points)
     XgdObject *obj;
     struct line_pnts *points;     
#else
XgdPlotLines(XgdObject *obj, struct line_pnts *points)
#endif
{
  XSegment *xPoints = (XSegment *)malloc(sizeof(XSegment)* points->n_points*2);
  int i, pntCnt = 0;
  double x1,x2,y1,y2;
  double north1, north2, east1, east2;
  ...
  for (i = 0; i < points->n_points - 1; i++){
	...
	east2 += 360.0;
      ...
      xPoints[pntCnt].x1 = (short) _xgdIFloor(x1+0.5);
      xPoints[pntCnt].y1 = (short) _xgdIFloor(y1+0.5);
      xPoints[pntCnt].x2 = (short) _xgdIFloor(x2+0.5);
      xPoints[pntCnt++].y2 = (short) _xgdIFloor(y2+0.5);
  ...
  XDrawSegments(obj->display, obj->Obj.GeoFrame.pixmap, obj->objgc,
	     xPoints, pntCnt);
}
/*
 ***************************************************************************

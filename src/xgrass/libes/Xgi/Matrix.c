...
static XtResource resources[] = {
    ...
    { XmNselectCellCallback, XmCCallback, XmRCallback,
	  ...
	  XmRCallback, NULL },
    { XmNseparator, XmCSeparator, XmRChar, sizeof (char),
          XtOffset (XbaeMatrixWidget, matrix.separator),
...
int row, column;
{
   ...
   {
      /* call SetCell for a text cell edited last 
       * added by Olga to fix a problem of the last field being ignored if
       * the return is not entered 
       *
       * Get the value the user entered in the textField (this is a copy)
       * and set this value
       */
    ...
    int row;
{
    ...
    coltext = (char **) XtCalloc(tb->matrix.columns, sizeof (char *));
    bzero((char *)coltext,tb->matrix.columns*sizeof (char *));

    for ( i = 0; i < tb->matrix.columns; i++ ) {
        element = ((row-1)+i*tb->matrix.rows);
	coltext[i] = XbaeMatrixGetCell(tb,row,i);
    ...
    for ( i = 0; i < tb->matrix.columns; i++ ) {
        ...
        if ( (i != (tb->matrix.rows - 1) || tb->matrix.rows == 1) &&
              i != (tb->matrix.columns - 1) ) {
            if ( tb->matrix.separator == '\0' )
                strcat(text," ");
            else {
                char sep[1];

                sprintf(sep,"%c",tb->matrix.separator);
                strcat(text,sep);
            }
        }
    }
...
void
XbaeMatrixSetRow(tb,row,values)
    XbaeMatrixWidget tb;
    int row;
    char *values[];
{
    ...
    for ( i = 0; i < tb->matrix.columns; i++ ) {
        XbaeMatrixSetCell(tb,row,i,values[i]);
    }
}

void
XbaeMatrixSetColumn(tb,column,values)
    XbaeMatrixWidget tb;
    int column;
    char *values[];
{
    ...
    for ( i = 0; i < tb->matrix.rows; i++ ) {
        XbaeMatrixSetCell(tb,i,column,values[i]);
    }
}

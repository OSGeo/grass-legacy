...
void EditMenu(), ShowMenu();
Widget toplevel;
     
void open_file(), fonts_cb();
...
#endif
{
  ...
  toplevel = XtVaAppInitialize(&app, "Editor",
  ...
  XmVaCreateSimplePulldownMenu(menubar, "fonts_menu", 3, fonts_cb,
...
#endif     
{
  Widget flist;
  void NewFontCB();
  Widget XgCreateInteractorFontListDialog();
  
  flist = XgCreateInteractorFontListDialog(toplevel, "Fonts", NULL, 0);

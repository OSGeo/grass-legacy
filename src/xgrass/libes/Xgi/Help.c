...

{
	...
	if ((dir = getenv("XGRASSHELPDIR")) == NULL) {
	    fprintf (stdout,"Error: Evnironment variable \"XGRASSHELPDIR\" not found\n");
	    exit(-1);
	}
...
#endif
{
    ...
    if (type == HOTWORD) {
        ...
        if (strncmp(&(str[*str_count]), "ref", 3) == 0) {
            ...
            fprintf (stdout,"illegal hot word\n");
            exit(-1);
        }
...
#endif
{
    switch (hot->type) {
        case REFERENCE:
        /*fprintf (stdout," reference hot word is at \"%s\"\n", hot->where);*/
        DoHotWord(w, hot);
        break;

    case GLOSSARY:
        /*fprintf (stdout," glossary hot word is at \"%s\"\n", hot->where);*/
        DoHotWord(w, hot);
        break;

...
int ask_gis_files (
       char *type,char *file,char *xname,char *xmapset,int position)
{
    static int use = 1;
...
/* start the mouse in the cancel box */
    ...
    dobox (&less,   "",       I_COLOR_WHITE, top, right, right+height,0);
    dobox (&more,   "",       I_COLOR_WHITE, bottom-height, right, right+height,0);
 ...
 */
	...
	*cur_mapset = 0;
	...
	while(1)
	{
		...
		dobox (&dummy, label, I_COLOR_WHITE, top+line*height, left, right, 0);
		line++;
	    }
	    if (col)
	    {
		dobox (&list[count].box, name, I_COLOR_GREY, top+line*height, left+width/2, right, 0);
		line++;
		col = 0;
	    }
	    else
	    {
		dobox (&list[count].box, name, I_COLOR_GREY, top+line*height, left, left+width/2, 0);
		col = 1;
	    }
	...
	downarrow (&more, page+1 < npages ? I_COLOR_BLACK : I_COLOR_WHITE);
	uparrow   (&less, page   > 0      ? I_COLOR_BLACK : I_COLOR_WHITE);
	which = -1;

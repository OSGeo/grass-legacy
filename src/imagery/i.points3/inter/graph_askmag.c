...
int ask_magnification (int *magnification)
{
    ...
    dotext ("MAGNIFICATION", top, top+height, left, right, I_COLOR_WHITE);
    dotext ("+", plus.top, plus.bottom, plus.left, plus.right, I_COLOR_GREY);
    dotext ("-", minus.top, minus.bottom, minus.left, minus.right, I_COLOR_GREY);
    dotext ("ACCEPT", accept.top, accept.bottom, accept.left, accept.right, I_COLOR_GREY);
    dotext ("CANCEL", cancel.top, cancel.bottom, cancel.left, cancel.right, I_COLOR_GREY);
    draw_mag();
...
int draw_mag()
{
    ...
    sprintf (buf, "%d", mag);
    dotext (buf, value.top, value.bottom, value.left, value.right, I_COLOR_WHITE);

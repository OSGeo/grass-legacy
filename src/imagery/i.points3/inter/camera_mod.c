        ...
        Camera *cam_info)
{
	...
	if( ! have_old)
		strcpy(cam_info->cam_name, "DEFAULT SYSTEMS CAMERA") ;
	...
	V_line(8,"        Point of Symmetry: Y-coordinate mm.") ;
        V_line(9,"        Maximum number of fiducail or reasue marks") ;
        V_line(11,"+-----------------------------------------------------------------------------+");	
	...
	V_clear() ;
	{
	    V_line(1,"               Please provide the following information:") ;
            V_line(2,"+--------------------------------------------------------------------------+");
	    V_line (4,"            Fid#     Fid Id          Xf              Yf" ) ;
        }
        endfid = startfid+NLINES <= cam_info->num_fid +1 ? startfid+NLINES : cam_info->num_fid;

...
int 
ask_window (struct Cell_head *window)
{
    double south, west ;
    char buff[64] ;
    short ok ;
...
/* do the same for the west */
    ...
    fprintf(stderr, "  north:       %12.2lf\n", window->north);
    fprintf(stderr, "  south:       %12.2lf", window->south);

    if (!visually_equal(window->south, south))
        fprintf(stderr, "  (Changed to match resolution)");
    fprintf(stderr, "\n");

    fprintf(stderr, "  east:        %12.2lf\n", window->east);
    fprintf(stderr, "  west:        %12.2lf", window->west);
    ...
    fprintf(stderr, "\n");
    fprintf(stderr, "  e-w res:     %12.2lf\n", window->ew_res);
    fprintf(stderr, "  n-s res:     %12.2lf\n", window->ns_res);
    ...
    {
	fprintf (stderr, "warning - north falls outside the minimal region\n");
	ok = 0;
    }
    if (window->south < minimal.south)
    {
	fprintf (stderr, "warning - south falls outside the minimal region\n");
	ok = 0;
    }
    if (window->east > minimal.east)
    {
	fprintf (stderr, "warning - east falls outside the minimal region\n");
	ok = 0;
    }
    if (window->west < minimal.west)
    {
	fprintf (stderr, "warning - west falls outside the minimal region\n");
	ok = 0;
    }
...
static 
visually_equal (double x, double y)
{
    char xs[40], ys[40];

...
static int get_histogram(register Colorbox *);
static Colorbox *largest_box(void);
static int splitbox(register Colorbox *);
static int quant_fsdither(void);
static int quant(void);
static int map_colortable(void);
static C_cell *create_colorcell(long,long,long);
static int shrinkbox(register Colorbox *);
...
static int get_histogram(register Colorbox *box)
{
    HIST *ptr;
    register long	i, j;
...
static int splitbox(register Colorbox *ptr)
{
...
static int shrinkbox(register Colorbox *box)
{
    HIST *histp;
    register long	ir, ig, ib;
...
static C_cell *create_colorcell(long red,long green,long blue)
{
...
static int map_colortable (void)
{
	...
	if (chatty) fprintf(stderr, "%ld\r", ir);
    }
    if (chatty) fprintf(stderr, "\nDone\n");
...
{ /*DEBUG*/
	...
	{
	    fprintf (stderr, "Scanline[%ld] = %d (%d,%d,%d)\n", x, scanline[x], *r, *g, *b);
	    scanline[x] = 0;
	}
} /*DEBUG*/
	    r++; g++; b++;
...
static int quant_fsdither (void)
{
	...
	if (chatty) fprintf(stderr, "%ld\r", i);
    }

...
poly_to_rast (input_file, raster_map, title, nrows)
    char *input_file, *raster_map, *title;
    int nrows;
{
    double *x, *y;
    int count;
	...
	if (npasses > 1)
	{
	    printf ("Pass #%d (of %d) ... ", pass, npasses);
	    fflush(stdout);
	}
	...
	printf ("writing raster map ... "); fflush (stdout);
    ...
    printf ("Creating support files for raster map <%s>\n", raster_map);
    G_close_cell(rfd);

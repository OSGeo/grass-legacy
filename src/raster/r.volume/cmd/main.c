  ...
  char *argv[])
{
  ...
  *site_list = 0;
  ...
  if (!data_mapset)
    G_fatal_error ("Can't find data map layer.");
  ...
  if (!clump_mapset)
    G_fatal_error ("Can't find clump map layer.");
  ...
  if (*site_list)
  {
    ...
    if (fd_sites == NULL)
      G_fatal_error ("Can't open sites list.");
  }
  ...
  if (*site_list)
  {
    ...
    if (site_info.desc==NULL || site_info.name==NULL || site_info.labels==NULL )
    ...
    sprintf (site_info.labels, "centroid east|centroid north|#cat vol avg t n");

...
/*******************************************************************/
 void norm_calc(nl,nx1,ltype,backdep,yp,qp,bel,
                xarea,xtw,xk,numhts,ht_spc,table_num,delx,
                NUMBPTS,normout,sslope,bottom,strick,yes_bkwater,grav)
...
------------------------------------------------------------------------*/
      ...
      int  nl,*backdep,*nx1,*ltype,*numhts,*table_num,NUMBPTS,yes_bkwater;
      float *yp,*qp,*bel,*xk,*ht_spc,delx,grav,*xarea,*xtw;
      float *normout,*sslope,*bottom,*strick;
 ...
 */
   ...
   double qds,yds,qus,yus,d_bel,d_bottom,d_sslope,d_strick;
   ...
   *normout=-0.9;
   ...
   if(ltype[nl]==1)  /* this is a trapezoidal link */
      ...
      while(diff>tol)
         ...
         if(count>250)
            {
            printf("WARNING NO CONVERGENCE IN NORMCALC!!!\n");
            printf("AFTER 250 ITERATIONS.  PROGRAM WIL\n");
            printf("FAIL.  CHECK OUTLET SLOPE.  MUST BE\n");
            printf("POSITIVE.\n");
            exit();
            }
         count++;
   ...
   if(yes_bkwater==TRUE)  /* calculate normal depths in the rest of the 
      ...
      for(j=nl;j>=1;j--)
         ...
         if(ltype[j]==2) 
            ...
            yp[1+j*NODES]=yus;
            if(yus<yw) printf("problem at weir in link %d\n",j);
            continue;
            }
         for(i=nx1[j];i>=1;i--)
            {
            ...
            if(fabs(diff)<=tol)
               ...
               /* printf("link %d node %d diff=%f iter=%d\n",j,i,diff,iter);*/
               continue;
               }
            else
               {
               printf("no convergence in norm_calc, link %d node %d\n",j,i);
               exit;
               }
            }               
         }
...
/*********************************************************************/
trap_section(i,j,d_bottom,d_sslope,d_strick,y,d_bel,d_area,
            d_konv,d_dkdy,d_width)
/*********************************************************************/

int i,j;
double d_bottom,d_sslope,d_strick,y,d_bel,*d_area,*d_konv,*d_dkdy,*d_width;
{
   double ydep,perim,r;
   double fith;
   double foth;
   ...
   if(ydep<0.0)
      {
      printf("zero depth in norm_calc, link %d, node %d\n",j,i);
      exit();
      }
...
/*********************************************************************/
brk_pnt_section(i,j,bel,yp,y,table_num,ht_spc,numhts,NUMBPTS,
                xarea,xtw,xk,ads,kds,d_dkdy,d_width)
/*********************************************************************/

int i,j,*table_num,*numhts,NUMBPTS;
float *bel,*yp,*ht_spc,*xarea,*xtw,*xk;
double *ads,*kds,*d_dkdy,*d_width;
double y;

{
   ...
   if(ydep<0.0)
      {
      printf("program stopped for negative depth, node %d, link %d\n",i,j);
      exit();
      }
   tnum=table_num[addr];
   ...
   printf("error in im_norm_calc, function brk_pnt_section\n");
   exit();
}

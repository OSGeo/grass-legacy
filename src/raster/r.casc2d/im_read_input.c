...
/*******************************************************************/
 void read_input(iunit,yunit,qunit,grav,alpha,beta,theta,delx,
		 delt,tt,nlinks,LINKS,NODES,NUM,depend,backdep,
		 nx1,strick,bottom,sslope,sslope2,bel,qp,yp,
		 ltype,ndep,maxnodes,chn_dep,
                 marea0,parea0,mtw0,ptw0,mk0,pk0,
                 marea1,parea1,mtw1,ptw1,mk1,pk1,numqpipe,yes_priess,yes_drain,
                 yes_bkwater,yes_table,table_num,highspot,lowspot,qmin)

FILE *iunit,*yunit,*qunit; 

int  *LINKS,*NODES,*NUM;
int  *nlinks,*maxnodes;
int  **nx1,
     **depend,**backdep,**ltype;
int  **ndep,**table_num;             /* table_num added by FLO 10-8-94 */ 

float *grav,*alpha,*beta,*theta,
      *delx,*delt,*tt,*qmin;
float **strick,**bottom,**chn_dep,**sslope,**sslope2,**bel,**qp,
      **yp;
float **marea0,**parea0,**mtw0,**ptw0,**mk0,**pk0;
float **marea1,**parea1,**mtw1,**ptw1,**mk1,**pk1;

int   **numqpipe;
int   yes_drain,yes_bkwater,*yes_table,yes_priess;

/* added by F.L. Ogden 10-12-94 to accomodate drain function */
float *highspot,*lowspot;
{
 ...
 */
   /******************************************************/
   /** first, allocate memory for arrays we know we need */
   /******************************************************/
   ...
   /****************************************************************/
   /*  Read the connectivity info, and what link types are present */
   /****************************************************************/
   for(j=1;j<=*nlinks;j++)
      {
      ...
      if(((*ltype)[j]==5)||((*ltype)[j]==6)) yes_apfa=1; /* there are APFA
                                                            links present */
      }

...
histograms (nbands, sum, product, histo, np, min, max, in_nstd, b_or_a)
     float *sum ;
     float **product;
     int **histo;
     int *min;
     ...
     int b_or_a;
{
  ...
  if (b_or_a == BEFORE_STD) {
    max_range = 1;
    for(b=0; b<nbands; b++)
      if (max[b]-min[b]>max_range) max_range = max[b]-min[b];
    old_range = max_range;
  }
  else
    max_range = old_range;
  ...
  if (histo_width%2 == 0) histo_width = histo_width - 1;
  if (histo_width<MIN_HISTO_WIDTH) histo_width = MIN_HISTO_WIDTH;
  else if (histo_width>MAX_HISTO_WIDTH) histo_width = MAX_HISTO_WIDTH;
  ...
  for (b = 0; b < nbands; b++)
	...
	sprintf(msg, "%-14s                    %s=%-3d  %s=%-3d", 
		lname[b], MIN, min[b], MAX, max[b]);
      else
	sprintf(msg, "%-14s                    %s=%-3d  %s=%-3d", 
		lname[b], RANGE_MIN, min[b], RANGE_MAX, max[b]);
      ...
      R_move_abs(x1,y1);
      for (bar = 0; bar < nbars && cat < MAX_CATS; bar++, cat++, x2 += histo_width)
	{
	  if (cat >= 0)
      ...
      x1 = h_left;
      R_font(GREEK_FONT);
      for (bar = 0; bar < nbars && cat < MAX_CATS; bar++, cat++, x1 += histo_width)
	{
	  int special;
	  float x;
	  int mp;
	  ...
	  if (cat == (int) (x = mean+.5))
	    ...
	    {
	      if (b_or_a == AFTER_STD) sprintf(msg,"%s", RANGE_MAX);
	      else sprintf(msg, "%s", MAX);
	      vh = .5;
	      color = RED;
	    }
	  else if (cat == (int) (x = (float) min[b] + .5))
	    {
	      if (b_or_a == AFTER_STD) sprintf(msg,"%s", RANGE_MIN);
	      else sprintf(msg, "%s", MIN);
	      vh = .5;
	      color = RED;
	    }
	  else

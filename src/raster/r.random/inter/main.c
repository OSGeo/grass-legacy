...
int random_cmd(char *, char *, char *, char *, int, int, long, int);
...
/* read input layer and count cells */
	...
	*err = 0;
    ...
    random_cmd (input, mapset, raster, sites, count, percent, total, cat_zero);

    return 0;
}

/*   Changed name from 'random' to 'random_cmd' to fix conflict
     with the stdlib 'random' function.
*/
int random_cmd (
    char *input,char *mapset,char *raster,char *sites,

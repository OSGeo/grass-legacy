...
void
contour (levels, nlevels, Map, z, Cell, Att, quiet)
...
***************************************************************************/
int
getnewcell(current, nrow, ncol, z)
	struct cell *current;
	int nrow, ncol;
	int **z;
{
	...
	{
		current->z[0] = (double ) z[current->r][current->c];
		current->z[1] = (double ) z[current->r][current->c + 1];
		current->z[2] = (double ) z[current->r + 1][current->c + 1];
		current->z[3] = (double ) z[current->r + 1][current->c];
...
************************************************************************/
void
getpoint(curr, level, Cell, Points)
	struct cell *curr;
	double level;
	struct Cell_head Cell;
	struct line_pnts *Points;

    ...
    char *argv[];
{
    ...
    char		*inmap, *outmap;
    FILEDESC    	infile = NULL, outfile = NULL;
    CELL		*inbuf1, *inbuf2, *outbuf; 
    char 		errbuf[100];
    int			row, col; 
    ...
    outbuf = G_allocate_cell_buf();

    /* open new map */
    if ((outfile = G_open_cell_new(rastout->answer)) < 0)
    {
	...
	for (row = 0; row < w.rows; row++) {
		...
		G_zero_cell_buf(outbuf); /* NULL? */
		G_put_map_row(outfile, outbuf);
		continue;
	    }
...
*/
		...
		outbuf[col] = (int)(new + .5);
/*
fprintf(stderr,"%f %f    ",t,u);
*/
	    }
	    G_put_map_row(outfile, outbuf);

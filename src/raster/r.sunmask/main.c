...
int main(int argc, char *argv[]) 
{
    ...
    parm.timezone->description = "timezone (east positive, offset from GMT, also use to adjust daylight savings)";
  ...
  if (use_solpos)
  {
      ...
      if (retval == 0) /* error check */
      {
       if( flag2->answer )
       {
        ...
        fprintf (stderr, " Sunrise time (without refraction): %.0f:%.0f\n", floor(pdat->sretr/60.), fmod(pdat->sretr, 60.));
        fprintf (stderr, " Sunset time  (without refraction): %.0f:%.0f\n", floor(pdat->ssetr/60.), fmod(pdat->ssetr, 60.));
       }

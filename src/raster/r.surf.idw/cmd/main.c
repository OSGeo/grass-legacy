...
SHORT	nrows, ncols;
{
    ...
    if (ll = (G_projection () == PROJECTION_LL)) {
        ...
        G_begin_geodesic_distance (nrows, a, e2);
...
interpolate (rowlist, nrows, ncols, datarows, npoints, out_fd, maskfd)
MELEMENT	rowlist[];
SHORT 		nrows, ncols, datarows;
int 		npoints, out_fd, maskfd;
{
    extern CELL *cell;
...
search (ewptr, head, row, col, npoints, neighbors, boundary, south)
...
find_neighbors (ewptr, nbr_head, row, col, npoints, neighbors)
EW		*ewptr;
NEIGHBOR        *nbr_head;
int             npoints;
SHORT           row, col,
...
MELEMENT *row_lists (rows, cols, datarows, npts, fd, cell)
/* Search and make array-indexed doubly-linked lists of original data points */
SHORT		rows, cols,	/* total rows and columns in window */
		*datarows;	/* number of rows with non-zero input data */

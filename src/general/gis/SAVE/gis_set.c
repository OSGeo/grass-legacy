...
/* Gather all existing variables **********************************************/
	...
	if (*gisdbase == 0)
	{
	    fprintf (stdout,"No DATABASE specified\n");
	...
	{
	    char temp[200];
	    fprintf (stdout,"DATABASE <%s> - must start with /\n", gisdbase);
	...
	{
	    fprintf (stdout,"DATABASE <%s> - not found\n", gisdbase);
	    continue;
	}
	G__setenv ("GISDBASE",      gisdbase) ;
	...
	{
	    fprintf (stdout,"LOCATION <%s> - illegal name\n", location_name);
	    continue;
	}
	if (*mapset && (G_legal_filename(mapset) < 0))
	{
	    fprintf (stdout,"MAPSET <%s> - illegal name\n", mapset);
	    continue;
	}
	...
	{
	    fprintf (stdout,"LOCATION <%s> - doesn't exist\n", location_name);
	    list_locations (gisdbase);
		...
		if(make_location (gisdbase, location_name))
		    fprintf (stdout,"LOCATION <%s> created!\n", location_name);
		else
		    fprintf (stdout,"LOCATION <%s> NOT created\n", location_name);
	    }
	    continue;
	}

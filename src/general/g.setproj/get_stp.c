...
FILE *fp;
	...
	sprintf(nad27,"%s/etc/state27\0",G_gisbase());
	fp = fopen (nad27,"r");
	if (fp == NULL)
        {	
...
{
	FILE *fipsfile;
        char FIPSfile[256], buff[256], buff1[256];
	int NUM_ZON, sfips, cfips, SFIPS=0, CFIPS=0, lookup, i;
	int record, ier, icode, reccnt, special_case;
	char STabbr[50], COname[50], answer[50];

	sprintf(FIPSfile,"%s/etc/FIPS.code\0",G_gisbase());
  ...
  if (SFIPS == 26)
  {
    if (special_case == 2) NUM_ZON = NUM_ZON + 10;
  }
...
int ask_fips(fp,s,c,sc)
FILE *fp;
int *s,*c,*sc;
{
  int i,ii, FIPS = 0, NUM_ZON, SFIPS, sfips, cfips;
  char STabbr[50], STabbr_prev[50], 
     COname[50], answer[50], buff[256], k, FIPSfile[256];
  ...
  *c = 0;
  ...
  if(in_stat !=0) G_fatal_error("reading sf key_value temp file");
  ...
  for (;;) {
    ...
    if (strcmp(answer,"list") == 0) {
      if (isatty(1)) sprintf(buff,"more %s",Tmp_file1);
      else sprintf(buff,"cat %s",Tmp_file1);
      system(buff);
    }
    else {
  ...
  if(in_stat !=0) G_fatal_error("reading cf key_value temp file");

  for (;;) {
    do {
	fprintf(stderr,"\nSpecify County FIPS (numeric) code \n"); 

...
#ifdef DEBUG
	fprintf(stdout,"Xserver screen resolution: x:%f dpi y:%f dpi\n", Xres, Yres);
#endif
        ...
        fprintf(stdout,"Scale of map displayed in GRASS monitor: \n"); /* add monitor name here */

#ifdef DEBUG
	fprintf(stdout,"\n True map size:  we_meters: %f ns_meters: %f \n", we_meters, ns_meters);
	fprintf(stdout," monitor size: screen_we_pix: %i screen_ns_pix: %i\n", r-l, b-t);

	fprintf(stdout," Pixel numbers used in screen: ns: %f  we:%f \n", s_rows, s_cols);
	fprintf(stdout," Map rows: %i  map cols: %i\n", window.rows, window.cols );

	fprintf(stdout,"\n Map height in true centimeter: %f\n", s_rows/ns_Xres*100);
	fprintf(stdout," Map width in true centimeter:  %f\n", s_cols/we_Xres*100);
	fprintf(stdout," (note: deviations may arise here from your screen adjustments)\n");
	
	fprintf(stdout,"\n Current map scale (note: Xserver x_res != y_res):\n");
	fprintf(stdout,"   y-scale: 1:%f\n", ns_meters/(s_rows/ns_Xres));
	fprintf(stdout,"   x-scale: 1:%f\n", we_meters/(s_cols/we_Xres));
#endif
 	...
 	/* put it out rounded: */
 	fprintf(stdout,"   Map scale: 1:%i (width: %.2fm, height: %.2fm)\n", (int)floor (av_scale + 0.5), we_meters, ns_meters);

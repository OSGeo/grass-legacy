...
main (void)
{
	...
	fprintf (stdout,"size of the cube  ");
	scanf("%d",&size);
	fprintf (stdout,"0");
...
/*
	mgifreesets();
	for (i = 1; i <= 5; i++) {
				fprintf (stdout,"color for side # %d  ",i);
				scanf("%s", color);
...
/* Set colors */
	...
	fprintf (stdout,"please enter the x, y, z rotation angles  ");
...
int 
copy (int sarray[5], int darray[8], int i1, int i2, int i3, int i4)
{
		sarray[0] = darray[i1];
		sarray[1] = darray[i2];
		sarray[2] = darray[i3];
...
int 
Reset_colors (float *red, float *grn, float *blu, int tot_colr)
{
	int incr ;
...
int 
Reset_color (double red, double grn, double blu, int number)
{
		mgicm(number+1,
			(_norm(red) << 20) +
			(_norm(grn) << 12) +

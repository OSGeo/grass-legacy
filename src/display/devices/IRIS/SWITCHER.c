...
#define debug(x)
...

	{
	    ...
	    /*
	    ** This code was added cuz, this is in a constant CPU loop
	    ** testing for window REDRAW.  It was slowing down other
	    ** applications.  
	    **
	    **  I use gsync to slow it down while waiting for 
	    **  next command to come down the pipe (this should be
	    **  machine independant, being tied to screen refresh.  
	    **  If a certain time elapses, I then call sleep to release 
	    **  the CPU.
	    **
	    **  This seems to work well.  With the sleep, things like
	    **  Dslide.show would take 1 second between each function
	    **  noticably slowing it down.  With just a straight loop
	    **  the gr_osview program would show 100% cpu utilization.
	    **
	    **  With this version, the meter shows very little cpu, while
	    **  Dslide.show runs relatively fast.   -dpg
	    */
	    loop_cnt++;
	    if (loop_cnt > 10)
	    {

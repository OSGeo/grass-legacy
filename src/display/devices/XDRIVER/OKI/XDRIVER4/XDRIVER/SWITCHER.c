...
static ITEM *new_item(), *find_item();
static PAD *find_pad();
...
#define LOOP_PER_SERVICE 15
	static int cmd_loop_count;  /* How many commands for each check of Xevents?? */
#endif /* OKI */
    ...
    while (1) {                 /* re-open upon EOF */
        ...
        while (eof <= 0 && !broken_pipe) {
            ...
            lc = c;
                                 ...
                                 * !broken_pipe)" block */
/* In conjunction with the modifications to Service_Xevent .....  w weber */
#ifdef OKI
		XSync(dpy,False);
#endif /* OKI */ 
...
#ifdef WWTRACE
		fprintf(stderr,"Re-fill fifo, %d chars\n",n_read);
#endif /* WWTRACE */
        if (n_read <= 0)
            return 1;           /* EOF */
    }
...
static append_item(pad, name, value)
PAD *pad;
char *name, *value;
{
    ITEM *item;

...
Cursor grcurse, grxh;
u_long gemask;
char *grasstr = "GRASS 4.0 - X Display Window";
...
char **argv;
{
    ...
    debug("Graph_Set XopenDisplay");
    if (!(dpy = XOpenDisplay(NULL))) {
        fprintf(stderr, "Graph_Set: can't open %s\n", XDisplayName(NULL));
        return 0;
    }
    debug("Graph_Set DefaultScreen");
    scrn = DefaultScreen(dpy);
    debug("Graph_Set backing store check");
    ...
    debug("Graph_Set XLoadQueryFont");
    ...
    debug("Graph_Set muck_with_colors");

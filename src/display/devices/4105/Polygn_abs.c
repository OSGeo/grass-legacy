...
/* Begin panel */
	...
	fprintf (stdout,"%cLP%c%c%c%c%c", ESC, s[0],s[1],s[2],s[3],s[4]) ;
	fprintf (stdout,"%c", '0') ;    /* Do not display panel boundaries */

/* Send coors */
	for(n=1; n<number; n++)
	{
		s = encode_xy(xarray[n],yarray[n]) ;
		fprintf (stdout,"%cLG%c%c%c%c%c", ESC, s[0],s[1],s[2],s[3],s[4]) ;
	}

/* End panel */
	fprintf (stdout,"%cLE", ESC) ;

...
const char *name;
{
	...
	scrd->cmap=cmap;
	scrd->white_pixel=0x00ffffff;
	scrd->black_pixel=0x00000000;
	...
	d->screens=scrd;
	d->max_keycode=255;
	((NT_window*)(scrd->root))->title_text=dn;
...
Window w;
{
	...
	if (VALID_WINDOW(window)) {
		...
		oldROP = SetROP2(hDC,R2_COPYPEN);
                hbrush = NT_GetWindowBrush ( window );
...
unsigned long bg, brd_col;
{
	...
	canvas->bgp=INVALID_HANDLE;
	canvas->hBitmap=INVALID_HANDLE;
	canvas->parent=(NT_window *)parent;
	canvas->title_text = display->display_name;
	SetWindowText(canvas->w, canvas->title_text);
...
StatusDef XGetWMName(dpy, win, prop)
Display *dpy;
Window  win;
XTextProperty *prop;
{
	...
	prop->format=8;
	prop->nitems=strlen((char *)(dpy->display_name));
  	return 1;
}


void XSetWMName(dpy, win, prop)	
Display *dpy;
Window  win;
XTextProperty *prop;
{
	...
	dpy->display_name= wn;
	ntw->title_text= wn;
	SetWindowText(ntw->w, ntw->title_text);
}
...
XGCValues *gc_values;
{
	...
	ptr = (char *)allocateMemory((size_t)2000);
	local_gc = (GC)ptr;
	...
	if (mask&GCBackground)
		...
		local_gc->values.background=display->screens->black_pixel;
	if (mask&GCCapStyle)
	...
	if (mask&GCForeground)
		...
		local_gc->values.foreground=display->screens->white_pixel;
	if (mask&GCFunction)
	{
		loc_function=0;
		if (GCFunction & GXclear);
		if (GCFunction & GXand) loc_function=loc_function|R2_MASKPEN;
		if (GCFunction & GXxor) loc_function=loc_function|R2_XORPEN;
		if (GCFunction & GXor) loc_function=loc_function|R2_NOTXORPEN;
		local_gc->values.function=loc_function;
	}
	else
...
unsigned int depth;
{
	...
	FillRect(w->hDC, &rct, GetStockObject(BLACK_BRUSH));
...
int x,y;
{
    return ximage->data[y*ximage->width+x];
}
int
...
unsigned long pixel;
{
    ...
    unsigned int offset = ( y * ( ximage->height ) + x ) * XIMAGEBYTEDEPTH;
...
/* THIS CODE ASSUMES THAT THE XIMAGES ARE 24 BIT PIXELS, AND
   SMALLER THAN THE DIB PIXEL SIZE */
...
{
    XImage *ximg = NULL;
    char *data =  (char *) allocateMemory ( width * height * XIMAGEBYTEDEPTH );

    NT_debug ("XGetImage\n");
        if ( NULL != data ) {
            if ( VALID_WINDOW ( drawable ) ) {
                HDC hDC = drawableGetDC ( drawable );
                BITMAP bm;
                BITMAPINFO *bmInfo;
                NT_window *wpix = (NT_window *)drawable;
                int bitmap_bytes_per_pix; /* Bytes per pixel for DC bitmap */

                GetObject ( wpix->hBitmap,
                            sizeof ( BITMAP ),
                            &bm );
                bitmap_bytes_per_pix = bm.bmBitsPixel>>3;
                bmInfo = allocateMemory ( sizeof(BITMAPINFO) +
                                          bitmap_bytes_per_pix * bm.bmWidth * bm.bmHeight );
                NT_debug ( "XGetImage: bmWidth %d, width %d, bmHeight %d, height %d, "
                ...
                if ( NULL != bmInfo ) {
                    ...
                    bits = allocateMemory ( bm.bmWidth * bitmap_bytes_per_pix * height );
                    if ( NULL != bits ) {
                        /* Get just the band we need starting at scan y, copying for height lines */
                        int res = GetDIBits ( wpix->hDC,
                                              wpix->hBitmap,
                                              y,
                                              height,
                                              bits,
                        ...
                        if ( res != height ) {
                            ...
                            char *s = bits + x * bitmap_bytes_per_pix; /* Offset to start of image */
                            char *d = data;                            /* Destination */
                            int i;
                            int sstepi = ( bm.bmWidth - width ) * bitmap_bytes_per_pix;
                            int sstepj = bitmap_bytes_per_pix;
                            int dstep = XIMAGEBYTEDEPTH;

                            NT_debug ( "Copying subimage: source %x, dest %x, sstepi %d, "
                                     "sstepj %d, dstep %d\n",
                                     s, d, sstepi, sstepj, dstep );
                            for ( i = 0; i < height; i++ ) {
                                int j;
                                for ( j = 0; j < width; j ++ ) {
                                    memcpy ( d, s, XIMAGEBYTEDEPTH );
#ifdef DEBUGGING_IMAGE
                                    if ( 0 == *d ) {
                                        NT_debug ( "0" );
                                    } else {
                                        NT_debug ( "1" );
            ...
            ximg = XCreateImage ( display, drawable, 24, 0, format, data,
                                  width, height, 24, width * XIMAGEBYTEDEPTH );
        } else {
            NT_log ( "XCreataImage: failed to allocate data\n" );
        }
    return ximg;
}
...
int bitmap_pad, bytes_per_line;
{
    ...
    if (img) {
        ...
        NT_log ( "XCreateImage: failed to allocate image\n" );
    }
...
unsigned int width,height;
{
    ...
    int res;
    int bytesperline   = width * XIMAGEBYTEDEPTH;
    int bplmodlongsize = bytesperline % sizeof ( LONG );
    char *bmp = image->data;
        ...
        if (VALID_WINDOW(w))
            {
...
#ifdef DEBUGGING_IMAGE
                /* Aim here is to catch the bitmap for one particular panel in i.points */
                /*if ( width==246 && height==18 )*/ {
                    int i;
                    char *d = image->data;
                    for ( i = 0; i < height; i++ ) {
                        int j;
...
#endif                
                /* If necessary, convert bitmap to LONG aligned row width
                 * per DIB standard. */
                if ( bplmodlongsize != 0 ) {
                    int newwidth = bytesperline + ( sizeof(LONG) - bplmodlongsize );
                    bmp = allocateMemory ( newwidth * height );
                    if ( NULL == bmp ) {
                        NT_log ( "XPutImage: Can't allocate memory to realign image data on LONG boundary.\n" );
                    } else {
                        int i;
                        char *s = image->data;
                        char *d = bmp;
                        memset ( bmp, 0, newwidth * height );
                        for ( i = 0; i < height; i++ ) {
                    ...
                    NT_debug ( "Converting to long aligned - sizeof(LONG) - bplmodlongsize %d, newwidth %d\n",
                             sizeof(LONG) - bplmodlongsize, newwidth );
                }                    
                ...
                if ( NULL != bmp ) {
                    ...
                    res = StretchDIBits ( hDC,
                                          ...
                                          SRCCOPY );
                    if ( res != height ) {
                        NT_log ( "XPutImage: StretchDIBits failed with %d scan lines, %s\n", res, GetLastError() );
                    }
                }
                drawableRelDC(w,hDC);
        } else {
            NT_log ( "XGetImage: invalid window %x\n", w );
        }
    return 0;
}
...
{
	NT_window *window = (NT_window *)w;
	NT_debug ("XSetWindowBackground\n");
	DeleteObject(window->bg);
	window->bg=CreateSolidBrush(CNUMTORGB(bg));
	DeleteObject(window->bgp);
        DeleteObject(window->hBitmap);
        window->bgp=INVALID_HANDLE;
        window->hBitmap=INVALID_HANDLE;
	return 0;
}
...
Pixmap background_tile;
{
	...
	NT_debug ("XSetWindowBackgroundPixmap\n");
	...
	else
	{
		...
		if (res==0) {
			NT_log ( "getDIBits failed %d\n", res );
                    }
		DeleteObject(window->bgp);
		window->bgp = CreateDIBPatternBrushPt(bmInfo, DIB_RGB_COLORS);
		DeleteObject(window->hBitmap);
		window->hBitmap = wpix->hBitmap;  
...
XSetWindowAttributes *attr;
{
	NT_debug ("XChangeWindowAttributes\n");
	return 0;
}
...
Window *ch;
{
	NT_debug ("XTranslateCoordinates\n");
	return (True);
}
...
int len;
{
	...
	NT_debug ("XGetErrorDatabaseText\n");
...
XMappingEvent *event;
{
	NT_debug ("XRefreshKeyboardMapping\n");
	return 0;
}
...
int XSetNormalHints(display,w,hints)
Display *display;
Window w;
XSizeHints *hints;
{
    ...
    if (VALID_WINDOW(ntw))
        {
            SetWindowPos(ntw->w,HWND_TOP,ntw->x,ntw->y,
                          ntw->wdth,ntw->hght,ff|SWP_SHOWWINDOW);
        }
    return 0;
}
...
XmbTextPropertyToTextList(
	...
	int *count_return)
{
	NT_debug ("XmbTextPropertyToTextList\n");
...
XSetICFocus(XIC ic)
{
	NT_debug ("XSetICFocus\n");
}
void
XUnsetICFocus(XIC ic)
{
	NT_debug ("XUnsetICFocus\n");
}
...
{
	char *wn;
	NT_debug ("XSetWMProperties\n");
	wn=strdup(window_name->value);
	((NT_window *)w)->title_text=wn;
	display->display_name=wn;
	SetWindowText(((NT_window *)w)->w,((NT_window *)w)->title_text);
	XSetNormalHints(display,w,normal_hints);
}
XDefineCursor(Display* display,Window w,Cursor cursor)
{
	NT_debug ("XDefineCursor\n");
	return 0;
}

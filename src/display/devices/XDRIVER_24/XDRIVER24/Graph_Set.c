...
#ifndef DO_NOT_REDECLARE_STDLIB
...
/*  ncolors = DisplayCells(dpy, scrn) ; */
    ...
    if (DefaultVisual(dpy,scrn)->class==TrueColor) {
      ...
      fprintf(stderr,"%d colors on Display\n",nlev*nlev*nlev);
...
AppendProperty(display, window, property, target, format, data, number_items)
    Display *display;
    Window window;
...
GetRealNumberOfColors(dpy,screen)
Display *dpy;
int screen;
{
    ...
    int value;
{
    ...
    if ((p=getenv(name)) && sscanf (p, "%d", n) == 1) return;
    *n = value;
}
/*** end Graph_Set.c ***/

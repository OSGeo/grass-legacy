...
modify()
{
    ...
    V_ques (config.skip,    's',RESET,  66, sizeof (config.skip) - 1);
    for (i = 0; i < TEXTLINES; i++)
	V_ques (config.text[i],'s',TEXT+i,6,sizeof(config.text[i])-1);

    V_ques (config.east,   's',EAST, 19,sizeof(config.east) - 1);
    V_ques (config.xoffset,'s',EAST, 47,sizeof(config.xoffset) - 1);
    V_ques (config.north,  's',NORTH,19,sizeof(config.north) - 1);
    V_ques (config.yoffset,'s',NORTH,47,sizeof(config.yoffset) - 1);
    V_ques (config.ref,    's',REF,19,sizeof(config.ref) - 1);

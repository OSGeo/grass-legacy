...
FILE *infile ;
{
	...
	for(;;)
	{
		...
		else if (! strncmp(text, "ref", 3))
		{
			...
			sscanf(text,"%*s %s", font) ;

		else if (! strncmp(text, "tex", 3))
		{
			int err ;
			if(strcmp("standard", font))
				R_font(font) ;
			else
				R_font("romand") ;
			err = show_it() ;
...
show_it()
{
	...
	if (scrR < (int)D_get_d_west() ||
		scrL > (int)D_get_d_east() ||
		scrB < (int)D_get_d_north() ||
		scrT > (int)D_get_d_south()) 
	{
		fprintf(stderr,"Please relocate label inside current map region\n") ;
		sleep(2) ;
		return(0) ;
	}

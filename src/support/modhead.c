...
/* open the cell file */
    ...
    if (fd < 0)
    {
	sprintf (buffer, "%s - can't open cell file\n", name);
	G_fatal_error (buffer);
	exit(1);
    }
...
/* determine file size */
    ...
    if (filesize == 0)
    {
	sprintf (buffer, "%s - empty cell file\n", name);
	G_fatal_error (buffer);
	exit(1);
    }
    if (filesize < 0)
    {
	sprintf (buffer, "%s - error reading cell file\n", name);
	G_fatal_error (buffer);
	exit(1);
    }
...
printf ("3.0 compression %sindicated\n", compressed_new?"":"not ");
printf ("pre 3.0 compression %sindicated\n", compressed_old?"":"not ");
hitreturn();
/*
*/
 ...
 */
	...
	if ((compressed_new || compressed_old) && G_yes (buffer, -1))
	{
		...
		while (1)
		{
		    printf ("Please indicate the type of compression\n");
		    printf ("  1. Pre 3.0 compression\n");
		    printf ("  2. 3.0 compression\n");
	...
	{
	    printf ("\n***\n");
	    printf ("The header for [%s] says the file is not compressed. ", name);
	    printf ("The file appears to be compressed.\n");
	    printf ("Most likely the header is wrong, but I want you to decide.\n");
	...
	{
	    printf ("\n*** WARNING ***\n\n");
	    printf ("The header for [%s] says the file is compressed. ", name);
	    printf ("The file does NOT appear to be compressed.\n");
	    printf ("Most likely the header is wrong, but I want you to decide.\n");
	...
	printf ("\n*** WARNING ***\n");
	printf ("Cell header indicates %d row%s in the cell file, but\n",
		cellhd.rows, cellhd.rows==1?"":"s");
	printf ("the actual file format indicates %d row%s\n",
		rows, rows==1?"":"s");
...
/* Write new header out */
    ...
    {
	sprintf (buffer, "unable to write cell header for %s", name);
	G_fatal_error(buffer) ;
    }
    else
	printf ("Cell header for [%s] updated\n", name);
    exit(0) ;
}

...
main(argc, argv) char *argv[];
{
    ...
    G_gisinit (argv[0]);
    if (G_ask_cell_in_mapset ("Enter name of cell file for which you will create/modify support files", name) == NULL)
	exit(0) ;
...
/* cell header */
    ...
    sprintf (buf, "Edit the cell header for [%s]? ", name);
...
/* color table */
    ...
    {
	G_clear_screen();
	sprintf (buf, "%s/etc/modcolr '%s in %s'", G_gisbase(), name, G_mapset());
	system (buf);
	hitreturn();
	G_clear_screen();
    }

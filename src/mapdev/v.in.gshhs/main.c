...
int main(int argc, char **argv)
{
    ...
    char buf[64], source;
    static char *slevel[] =
	{ "null", "land", "lake", "island_in_lake", "pond_in_island_in_lake" };
    FILE *fp, *dig_att;
    int k, i, max_east = 270000000;
    int cnt = 1;
    struct POINT_1 p; /* renamed to avoid conflict */
    struct GSHHS h;
	...
	"Automatically run \"v.support\" on vector file";

    flag.a = G_define_flag();
    flag.a->key = 'a';
    flag.a->description =
	"Import shoreline as line type Area (default = line)";
...
/* Lat Long Projection */
	if (!parm.w->answer || !parm.e->answer || !parm.s->answer
	    || !parm.n->answer) {
	    sprintf(errmsg,
		    "%s:  Missing region parameters ...\nMust supply n, s, e, w values or use '-g' flag\n",
		    progname);
...
/* Read Data Header Record and get # records */
    ...
    while (fread((void *) &h, (size_t) sizeof(struct GSHHS), (size_t) 1, fp)
	   == 1) {
...
/* Got Header now read in data structure */
	for (k = 0; k < h.n; k++) {
	    if (fread
		((void *) &p, (size_t) sizeof(struct POINT_1), (size_t) 1,
		 fp) != 1)
	    ...
	    lon = (h.greenwich
		   && p.x > max_east) ? p.x * 1.0e-6 - 360.0 : p.x * 1.0e-6;
	    if (lon > 180.)
		lon = lon - 360.;
	    lat = (p.y * 1.0e-6);

...
int readsites (FILE *fdsite, int all, int verbose, int field, bubblesite **xyz)
...
/* Reads a sites list into {\tt xyz}, returning the number of sites found.  */
{
  ...
  /*a window can be used to define a area for bubbling, it's not implemented
    yet */
  ...
  (*xyz) = (bubblesite *) G_malloc (allocated * sizeof (bubblesite ));
  ...
  while (G_site_get (fdsite, s) == 0) 
  {
    ...
    {
      allocated+=1000;
      (*xyz) = (bubblesite *) G_realloc ((*xyz), allocated * sizeof (bubblesite));
      if ((*xyz)==NULL) G_fatal_error("cannot allocate memory");
    }

...
dig_write_plus_file( fp_plus, Map, Plus)
	FILE   *fp_plus ;
	struct Map_info *Map ;
	struct Plus_head *Plus ;
{
	...
	if ( dig_Wr_Plus_head( Plus, fp_plus) < 0)
	{
	...
	struct Plus_head *Plus ;
{
	...
	{
		if ( dig_Wr_P_node( &Map->Node[i], plus) < 0)
			return(-1) ;
	}
	...
	struct Plus_head *Plus ;
{
	...
	{
		if ( dig_Wr_P_line( &Map->Line[i], plus) < 0)
			return(-1) ;
	}
	...
	struct Plus_head *Plus ;
{
	...
	{
		if ( dig_Wr_P_area( &Map->Area[i], plus) < 0)
			return(-1) ;
	}
	...
	struct Plus_head *Plus ;
{
	...
	{
		if ( dig_Wr_P_att( &Map->Att[i], plus) < 0)
			return(-1) ;
	}
	return(0) ;
}	/*  write_atts()  */

dig_write_isles( plus, Map, Plus)
	FILE   *plus ;
	struct Map_info *Map ;
	struct Plus_head *Plus ;
{
	...
	{
		if ( dig_Wr_P_isle( &Map->Isle[i], plus) < 0)
			return(-1) ;
	}

...
dig_load_plus (map, digit, do_checks)
    struct Map_info *map;
    FILE *digit;
    int do_checks;
{
    FILE *plus;
    register int dig_line_cnt;
    register int i;
    register int tot_points;
    int dig_lline_cnt;
    int dig_aline_cnt;
    int read_error;
    long offset;
    ...
    */
    ...
    if (do_checks)
	dig_do_file_checks(map->plus_file, map->digit_file, map->att_file);

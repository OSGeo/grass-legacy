...
dig_snap_line_to_node (map, line, node, points)
    struct Map_info *map;
    int line, node;
    struct line_pnts *points;
{
    ...
    dig_bound_box (points, &(MLN->N), &(MLN->S), &(MLN->E), &(MLN->W));
...
**	     
*/
dig_node_add_line (map, node, line, points, type)
    struct Map_info *map;
    P_NODE *node;
    int line;
    struct line_pnts *points;
    int type;	/* this is a psuedo type.  only checked for == DOT */
{
	...
	*/
	angle = dig_calc_end_angle (points, head.map_thresh);
    }
    else
    {
	/*  
	...
	*/
	angle = dig_calc_begin_angle (points, head.map_thresh);
    }

    if (angle == -9. && type != DOT)
    {
    /*DEBUG*/ fprintf (stderr, "NODE_ADD_LINE: Line %d  is degenerate. skipping\n", line);
	    return (0);
    }
    ...
    node->alive = 1;
    node->n_lines++;
/*DEBUG*/ debugf ("node_add_line: line %d added position %d n_lines: %d angle %f\n", line, i, node->n_lines, angle);
    return ((int) node->n_lines);
}
...
*/
dig_add_line_to_node (num, line, type, map, points)
    int num, line;
    char type;
    struct Map_info *map;
    struct line_pnts *points;

...
int 
do_dots (char *in_name, char *out_name, char *dot_name, int name_opt, int verbose)
{
	...
	if (name_opt)
	    ...
	    sprintf(label,"%s",cats.labels[cat]);
	    ptr = label;
	    while (*ptr && *ptr != ':')
	       ptr++;
	    if (*ptr != ':')
    ...
    if (size_opt)
      ...
      if (dot_ratio != 999999999.)
	 {
	    ...
	    fprintf(stderr,"\n\n");
            fprintf(stderr,
	     "\tsmallest area %.2f with dot count of %d\n",
		  min_area,area_cnt);
            fprintf(stderr,
	     "\tthis represents a ratio of :\n");
	    }
         fprintf (stdout,"\t%f", dot_ratio);
         if (verbose)
	    {

    ...
    struct Map_info *map;
{
	...
	do {
	    ...
	    if (0 > dig__Read_line (&Gpoints,map->digit,map->Line[line_num].offset))
		continue;
    ...
    if (rline_num < 0)
    {
	angle = dig_calc_end_angle (&Gpoints, head.map_thresh);
    }
    else
    {
	angle = dig_calc_begin_angle (&Gpoints, head.map_thresh);
    }

...
_Write_header_info()
{
    ...
    sprintf(buf,"%s in.", dig_float_point (tbuf, 6, head.digit_thresh)) ; wmove(BASE_WIN,7,18) ;  waddstr(BASE_WIN, buf);
    sprintf(buf,"%s %s", dig_float_point (tbuf, 6, head.map_thresh), G_unit_name (G_projection_units (G_projection()), 1));
    wmove(BASE_WIN,8,18) ;  waddstr(BASE_WIN, buf);
    /*
    sprintf(buf, "%20.20s", head.map_name) ;	wmove(BASE_WIN,4,16) ;  waddstr(BASE_WIN, buf);
    sprintf(buf, "%20d", head.orig_scale) ;      wmove(BASE_WIN,5,16) ;  waddstr(BASE_WIN, buf);
    sprintf(buf, "%20.20s", head.your_name) ;       wmove(BASE_WIN,6,16) ;  waddstr(BASE_WIN, buf);
    sprintf(buf,"%8.5lf", head.digit_thresh) ; wmove(BASE_WIN,7,16) ;  waddstr(BASE_WIN, buf);
    sprintf(buf,"%10.2lf", head.map_thresh) ;  wmove(BASE_WIN,8,16) ;  waddstr(BASE_WIN, buf);
    */

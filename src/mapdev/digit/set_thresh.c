...
/*DEBUG*/ head.digit_thresh = default_thresh;

/*DEBUG*/ debugf ("set_thresh: calling calc_thresh (%lf) [digit_thresh] \n", head.digit_thresh);
    calc_thresh (head.digit_thresh);
    calc_snap_thresh (head.digit_thresh);
    ...
    struct Map_info *map;
{
    ...
    */
    ...
    /* avoid FPE */
    if (head.orig_scale == 0 || conversion == 0.0)
	return (0);

    while (1)
     {
    
        cur_digit = map->snap_thresh / (head.orig_scale * conversion);

        Write_info ( 1, "  Current SNAPPING threshold:");

	sprintf ( buf, "         Map : %lf  %s. ", map->snap_thresh, G_unit_name (G_projection_units (G_projection()), 1));
        Write_info ( 2, buf);
...
/*DEBUG*/ debugf ("In RESET_THRESH ()\n");
    ...
    /* avoid FPE */
    if (head.orig_scale == 0 || dig_unit_conversion()  == 0.0)
	return (0);
    ...
    /*DEBUG*/        map->prune_thresh , head.orig_scale , dig_unit_conversion());

	cur_digit = map->prune_thresh / (dig_unit_conversion() * head.orig_scale);
    /*DEBUG*/ debugf ("reset_thresh: cur_digit = %lf\n", cur_digit);

	Write_info ( 1, "  Current thresholds:");

	sprintf ( buf, "         Map : %lf  %s. ", 2.0 * map->prune_thresh, G_unit_name (G_projection_units (G_projection()), 1));
	Write_info ( 2, buf);
...
/*DEBUG*/ debugf ("In CALC_SNAP(%lf)\n", new_thresh);
    tmp_thresh = new_thresh * head.orig_scale * dig_unit_conversion ();
...
/*DEBUG*/ debugf ("CALC_THRESH: snap_thresh = %lf\n", CM->snap_thresh);
    ...
    dig_units = (float)head.digit_thresh / scale;

    transform_a_into_b ((double) 0.0, (double) 0.0, &X1, &Y1);
    transform_a_into_b ((double) dig_units, (double) 0.0, &X2, &Y2);

    head.map_thresh = hypot(X2-X1, Y2-Y1);
    sample_thresh = head.map_thresh / 2.0;
#endif
}
...
/*DEBUG*/ debugf ("In CALC_THRESH(%lf)\n", new_thresh);
    head.digit_thresh = new_thresh ;     /* Threshold in inches on map */
    head.map_thresh = new_thresh * dig_unit_conversion() * head.orig_scale;

    /*
    if (CM->prune_thresh == 0.0)
    */
	CM->prune_thresh = head.map_thresh / 2.;
    ...
    double map_thresh;
{
    ...
    head.digit_thresh = _map_to_dig_thresh (map_thresh);
}
    ...
    double map_thresh;
{
	...
	dig_thresh = map_thresh / (dig_unit_conversion () * head.orig_scale);
    }
    return (dig_thresh);
}

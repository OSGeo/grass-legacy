...
#define LIST_SIZE 50
    ...
    {
	dig__Read_line (&Gpoints, map->digit, map->Line[line1].offset);
	...
	if (line_in_bbox (Line, N, S, E, W))
	{
	    ...
	    dig__Read_line (&Gpoints, map->digit, Line->offset);
...
/************************************************************************/
    ...
    */
    ...
    dig__Read_line (&Gpoints, map->digit, map->Line[line2].offset);
    if (!LINE_LABELED (&(map->Line[line2])))
    {

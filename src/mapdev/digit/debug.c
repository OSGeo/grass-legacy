    ...
    int line;
{
	...
	if (Line->att)
	{
	    ...
	    sprintf (buf, "  Att %3d (index %3d)   Category %3d  X %10.3lf Y %10.3lf",
    ...
    struct Map_info *map;
{
	...
	if(0 > dig__Read_line(&Gpoints, map->digit, map->Line[num].offset))
	{
    ...
    struct Map_info *map;
{
	...
	if (num)
	{
	    sprintf (buf, "Node# %d   X %lf (%x%x)  Y %lf (%x%x)", num,Node->x, Node->x, Node->y, Node->y);
	    Write_info (1, buf);
	}
    ...
    struct Map_info *map;
{
	...
	{
	    prev = i;
	    if(0 > dig__Read_line(&Gpoints, map->digit, map->Line[i].offset))
	    {
		Write_info (3, "Error reading line information");
		sleep (2);
		return (0);

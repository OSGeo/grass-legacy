...
Digitize ()
{
	...
	if ((command = get_menu_command (&M_digit, &chr)) > 0)
	{
		...
		case MDC_LABEL:
		    ...
		    break;
		case MDC_DIGIT:
		    do_digitize(CM, mode, type, CM->prune_thresh);
		    _Write_type_info();
		    show_mode(mode, type, auto_label);
		    break;
		default:
	...
	*/
	...
	if (type != DOT)
	{
	    if (Xpoints.n_points == 1 ||  /* are all points w/in snapping thresh? */
		 dig_is_line_degenerate (&Xpoints, head.map_thresh))
	    {
		Xpoints.n_points = 0;
		BEEP;
...
/*DEBUG*/ debugf ( "Check_nodes returns  N1 = %d  N2 = %d\n", node.N1, node.N2);
	...
	if (Digtiz_Device == MOUSE)
	    yes_no = mouse_yes_no("Do you accept this line? ") ;
	else
	    yes_no = ask_yes_no("Do you accept this line? ") ;

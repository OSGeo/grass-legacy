...
/*  set the monitor up for digit and give them something to look at  */
	...
	sprintf( command, "%s/%s/Dscreen", G_gisbase(), BIN) ;
	if ( system( command))
		exit(-1) ;
...
askagain:
	...
	printf ("    If name is entered that does not already exist, it\n");
	printf ("    will be created at this time.\n\n");
	...
	if (NULL == G_find_file ("dig", buf, G_mapset ()))
	{
		...
		sprintf (buff, "  Is this map in %s %s? ", 
		  G_projection_name (proj), G_unit_name (G_projection_units(proj)));
		if (!G_yes (buff, 1))
		{
...
/*  NOW execute the digit program  */
	...
	sprintf( command, "%s/etc/%s/%s/%s %s %s %d %s", G_gisbase(),
		DRIVER_DIR, Driver.name,  Driver.dig_program,

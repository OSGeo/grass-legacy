...
/* Make sure that the current projection is UTM. */
  if (G_projection() != 1)
   {
    char msg[256];      
    sprintf (msg, "%s:  Projection must be UTM since the vector file to be \
created will be UTM.  Change the value \"proj\" to 1 in file \"WIND\" \
(current region), then re-execute \"%s\".",   
  ...
  if ( strcmp(r_or_a,"r")==0 ) 
   {
    ...
    if (radius <= 0.0)
     {  
      char msg[256];
      sprintf(msg,"%s: \"%lf\" is an incorrect value for radius.\n",
...
(for feet), or \"mi\" (for miles)",
  ...
  else
   {
    ...
    if (area <= 0.0)
     {  
      char msg[256];
      sprintf(msg,"%s: \"%lf\" is an incorrect value for area.\n",
              G_program_name(), area);
      G_fatal_error (msg);
      exit(1);
     }  
...
"Enter vector file to be created (output)");
  ...
  if ( strcmp(r_or_a,"r")==0 ) 
   {
  sprintf(command,"%s/bin/%s %s radius=%lf radius_uom=%s sitefile=%s output=%s",
  G_gisbase(), G_program_name(), flag, radius, radius_uom, sitefile, output);
   }
  else
   {
    sprintf(command,"%s/bin/%s %s area=%lf area_uom=%s sitefile=%s output=%s",
...
int
double_prompt(answer_double,default_ch,variable,question,warn_msg)
...
int
ch_prompt(answer_ch,default_ch,variable,question,warn_msg)
  char *answer_ch;
  char *default_ch;
  char *variable;
  char *question;

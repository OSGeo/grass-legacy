...
*/
int
Vect__intersect_line_with_poly (Points, y, Inter)
...
** returns -1 on error
*/
Vect_point_in_islands (Map, area, cent_x, cent_y)
    struct Map_info *Map;
    int area;
    double cent_x, cent_y;
{
...
Vect_get_point_in_poly_isl(Points, IPoints, n_isles, att_x, att_y)
double *att_x, *att_y;
int n_isles;
...
/* returns 0 on success and -1 on failure */
{
    ...
    double cent_x, cent_y, a, b;
    register int i, j;
    double max, hi_y, lo_y;
    int maxpos;
    int point_in_sles=0, first_time=1, ret;
    P_AREA *Area;
    double diff;

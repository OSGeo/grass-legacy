...
try_again:
	...
	for(;;)
            ...
            else if(ptr2 < endptr)    /*** not a good packet ***/
	    {
            ...
            { 
		/*** read already tried ***/
		if (readflag > digdevice.packet_length[0])  
		{
                    return(0);
		}
...
long decode_digpkt (char string[])
{
    static char str[96];
...
/*DEBUG*/ fprintf (stderr, "\nbutton: '%c' upchar '%c'\nstring: <%s\n>\n", 
	...
	else
	{
	 /* else  up_char specified for button number field default: <space> */
	    ...
	    {
		val = buf[0] + digdevice.buttonoffset - '0';
...
long start_ginput (char dev_path[], int mode)
{
#ifdef HAVE_TERMIO_H
...
*                                                                              *
*******************************************************************************/
long ginput_ctl (long no_actions, ACTION *action)
{
#ifdef HAVE_TERMIO_H
    struct termio termvar;
#else
...
long read_value (char p1[], char p2[], ACTION *act)
{
    long j;
...
long ginput_setup (char *new_dev, char *ttypath, int mode)
{
    char path[300];
	...
	sprintf(path,"%s/etc/digitizers/%s",G_gisbase(), new_dev);
...
long dparse (char raw[], char delims[], char *parsed,
	long max_raw, long max_delims, long max_parsed, long max_p_elems)
{

...
FILE	*dxf_file;
{
	...
	if (header_flag)
	{
		...
		if (code == 9) /* only looking for 9 groups  */
		{
	    ...
	    dxf_entities (dxf_file);/* looks for ENTITIES in file */
	}
	pt_array = (POINT *) G_malloc (ARRAY_INCR * sizeof (POINT));
	arr_max = ARRAY_INCR;
	arr_size = 0;
	n = e = DBL_MIN;
	w = s = DBL_MAX;

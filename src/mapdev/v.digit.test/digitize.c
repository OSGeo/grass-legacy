/* updated to support dig_cats file creation 10/99 MN */
/* used SCS code for that */
...
static int	answer = 0;
static char auto_label_text[60];
...
/*DEBUG*/ debugf ("TYPE = %d\n", type);
		...
		case MDC_LABEL:
		    ...
		    auto_label = ask_cat ();                                   /* query cat number */
		    show_mode(mode, type, auto_label);
                                            if (!auto_label) break;

		    answer = ask_for_name(type, &lcats); /* cat label yes/no? */
                                            if (answer) /* o.k., read text label for selected auto_label*/
                                                sprintf(auto_label_text, "%d",  ask_name(auto_label, &lcats));  

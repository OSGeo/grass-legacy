/* upated to support dig_cats file creation 10/99 MN */
/*
...
int Label (void)
{
	...
	if ((command = get_menu_command (&M_label, &chr)) > 0)
	{
		...
		while (1)
		{
			...
			ans = ask_for_name(AREA, &cats); /* cat label yes/no? */
			if (ans)	/* o.k., read text label for selected auto_label*/
				ask_name(cat_number, &cats);

			label_area (CM, cat_number);
		}
		break;
	    case MLC_LLINE:
		...
		while (1)
		{
			...
			ans = ask_for_name(LINE, &cats); /* cat label yes/no? */
			if (ans)	/* o.k., read text label for selected auto_label*/
				ask_name(cat_number, &cats); 

			label_lines (CM, cat_number);
		}
		break;
	    case MLC_LSITE:
		...
		while (1)
		{
			...
			ans = ask_for_name(DOT, &cats); /* cat label yes/no? */
			if (ans) 	 /* o.k., read text label for selected auto_label*/
				ask_name(cat_number, &cats); 


			label_sites (CM, cat_number);
		}
		break;
	    case MLC_LLINES:
		Clear_base ();
		...
		while (1)
		{
		    ...
		    Clear_info ();
                    ...
                    if (ans) cat_number = ask_name(0, &cats);
                    else cat_number = ask_cat();

		    if (!cat_number) break;
		    label_mlines (CM, cat_number);
		}
		break;
...
int ask_for_name (int Type, struct Categories *pcats) /* ask, if text label shall be specified (yes / no)*/
{
    int ans, ans2, ier;
    char buffr[128];
    char *title;
	...
	if (Type == 1)         /* LINE */
	    ...
	    " Do you wish to enter line text attribute? ", 1);
	else if (Type == 2)    /* AREA */
	    ans = curses_yes_no_default(2,
	    " Do you wish to enter area text attribute? ", 1);
	else if (Type == 4)     /* DOT */
	    ans = curses_yes_no_default(2,
	    " Do you wish to enter site text attribute? ", 1);
	else
	    ans = 1;
	...
	/* read category file , if it exists*/
                ...
                ier = G_read_cats (N_name, G_mapset(), pcats);
...
int ask_name (int cat_number, struct Categories *pcats) /* query for text attribute */
{
    ...
    while (1)
      {
      Clear_info();
      Write_info( 4, "   Enter a text label (<CR> to quit): ");
	...
	/* find input string in category struct, assign category value to the
     ...
     if (!pcats->num)
            recd = cat_number;     /* no list existing yet, set max categories to the */
				   /* number currently defined by user*/
     else
            recd = pcats->num;    /* list exists, set the number of categories */

      for (i=0;i<recd;i++)              /* category search */
	{		 
	                                    /* get a category label */
        sscanf (pcats->labels[i], "%s", cat_name);
	...
	/* end of category search, NO category names match */
      ...
      Clear_info ();
      recd=cat_number;  /* assign user  selected cat number */
      sprintf(buffr," Use text label <%s> for category <%d>? ", nptr, recd);
      if (curses_yes_no_default (2, buffr, 1)) 
	 {                                      /* user said YES */

		/* create entry */
		G_set_cat ((CELL)recd, nptr, pcats);
		G_sort_cats (pcats);           /* required ?? 10/99*/
		ier = G_write_vector_cats(N_name, pcats);
		if (ier < 0)
		{
			sprintf(buffr," Error in writing dig_cats file: <%s>", N_name);
...
int label_psu (struct Map_info *map, int cat) /*unused 10/99*/
{
    int line, area, att;
    double x, y;
    int ret;

...
/* Initialize gis library */
	...
	new->description	= "raster output file";
	...
	dig_name = old->answer;
	cell_name = new->answer;

	if (!*dig_name  || !*cell_name )
	{
...
/*DEBUG*/ fprintf (stderr, "Size of tmpfile %ld\n", file_size);
	...
	pkgs = do_areas(&Map, &header, Points);
	if (pkgs < 0)
		G_fatal_error("Working on digit areas.");
	packages += pkgs;

	start_clock (NULL);
	pkgs = do_lines(&Map, &header, Points);
	if (pkgs < 0)
	...
	start_clock (NULL);
	pkgs = do_sites(&Map, &header, Points);
	if (pkgs < 0)
	...
	printf( "Step 5: Creating raster file...          ");
	start_clock (NULL);
	...
	if ( (newmap = G_open_cell_new (cell_name) ) == -1)
	{
...
finish:
	...
	if ( 0  >  update_hist(cell_name, Map.head.orig_scale) )
	{
		fprintf( stderr, "ERROR: Couldn't update history file : %s.\n", cell_name);
	}

	/*create color table the easy way*/
	sprintf (command, "r.colors -q map=%s color=random", cell_name); 
	system (command);

	G_suppress_warnings (1);
	stat = G_read_vector_cats (dig_name, mapset, &cats);
	...
	if (stat >= 0)	/* if cats file existed */
	{
	    ...
	    */
	    stat = G_write_cats (cell_name, &cats);
	}
	printf ( "                  Total time:          ");
	stop_clock (&timer);
	printf ( "\n");
...
/* note, this does not touch the files, just our copy of the data */
	...
	*/
	...
	process_area (Map, num_rows, num_cols, area, Points);
    }
    stop_clock (NULL);
    return (num_areas);
}
	...
	struct Cell_head *header;
{
	...
	{
	    fprintf (stderr, "Region not properly defined\n");
	    exit (-1);
	}
}
    ...
    int n;
{
	...
	*y =  (*y - south) / ns_res;
	...
	if ((int) (*y - round) == (int) *y)
	    *y *= 1.000000000001;	/* this is not real good */
	    /* *y += .00001; */
    }
}

...
/*DEBUG*/ fprintf (stderr, "YO!  walking too far down the buffer\n");
		...
		if (notfound)
		{
		    for ( ; InBuffer < cp ; InBuffer++ , ((len >= CHARS_RD) ? len = CHARS_RD : len++))
...
***/
	...
	if (N_read < 0 || ring)				  /*  did it timeout  */
		return (-1);

...
static int scan_doubles(int,char *,double *); 
...
**/
		...
		{
			case 6:
			tmp1[5] = tmp[5] * dlgstr.proj.int_params[0]
				-  tmp[4] * dlgstr.proj.int_params[1]
				+  dlgstr.proj.int_params[3] ;
			case 5:
			tmp1[4] = tmp[4] * dlgstr.proj.int_params[0]
				+  tmp[5] * dlgstr.proj.int_params[1]
				+  dlgstr.proj.int_params[2] ;

			case 4:
			tmp1[3] = tmp[3] * dlgstr.proj.int_params[0]
				-  tmp[2] * dlgstr.proj.int_params[1]
				+  dlgstr.proj.int_params[3] ;

			case 3:
			tmp1[2] = tmp[2] * dlgstr.proj.int_params[0]
				+  tmp[3] * dlgstr.proj.int_params[1]
				+  dlgstr.proj.int_params[2] ;

			case 2:
			tmp1[1] = tmp[1] * dlgstr.proj.int_params[0]
				-  tmp[0] * dlgstr.proj.int_params[1]
				+  dlgstr.proj.int_params[3] ;

			case 1:
			tmp1[0] = tmp[0] * dlgstr.proj.int_params[0]
				+  tmp[1] * dlgstr.proj.int_params[1]
				+  dlgstr.proj.int_params[2] ;
...
static int scan_doubles(int todo,char *coor_str,double tmp[])
{
...
/*
fprintf (stdout," str: '%s',", coors[i]) ;
*/

	/*  convert the coordinate  */
		tmp[i] = atof( coors[i]) ;
/*
fprintf (stdout,"    coor: %f\n", tmp[i]) ;
*/
	}
	...
	double *y_coors)
{
	...
	for (at_num=0; at_num<num; at_num++)
	{
		if (! (num_printed % 3) )
			if (num_printed)
				fprintf(file,"\n") ;
		fprintf(file,"%12.2f%12.2f",x_coors[at_num], y_coors[at_num]) ;
		num_printed++ ;
	}
	fprintf(file,"\n") ;
	...
	double buf[])
{
	...
	/* Write out the numbers in 'buf', 5 to a line */
	/*  Try this code instead  WB Hughes - 2-17-1999 */
	for(i=0;i<num;i++) {
		fprintf(file,"%12.2f",*(buf+i));
		if(!(i+1)%6) fprintf(file,"\n");
	}
	fprintf(file,"\n");
	return 0;
}
#ifdef OW_THIS_MAKES_MY_HEAD_HURT
	for(;;)
	{
		switch (num)
		{
		...
		case 2:
			...
			return(0) ;
		case 3:
			fprintf(file,"%12.2f%12.2f%12.2f%\n",
				*(buf+0), *(buf+1), *(buf+2)) ;
			return(0) ;
		case 4:
		...
		case 5:
			...
			return(0) ;
		default:
			fprintf(file,"%12.2f%12.2f%12.2f%12.2f%12.2f%12.2f\n",

...
int 
lines_intersect (long x1, long y1, long x2, long y2, long x3, long y3, long x4, long y4, long *x, long *y)
{
...
main (void)
{
    ...
    for (;;) {
        fprintf (stdout, "X1, Y1: " );
	scanf( "%ld %ld", &x1, &y1 );
        fprintf (stdout, "X2, Y2: " );
	scanf( "%ld %ld", &x2, &y2 );
        fprintf (stdout, "X3, Y3: " );
	scanf( "%ld %ld", &x3, &y3 );
        fprintf (stdout, "X4, Y4: " );
	scanf( "%ld %ld", &x4, &y4 );

        switch ( lines_intersect( x1, y1, x2, y2, x3, y3, x4, y4, &x, &y )) {
            case DONT_INTERSECT:
			 fprintf (stdout, "Lines don't intersect\n" );
			 break;
            case COLLINEAR:
                         fprintf (stdout, "Lines are collinear\n" );
                         break;
            case DO_INTERSECT:
			 fprintf (stdout, "Lines intersect at %ld,%ld\n", x, y );
                         break;
            }
        }
    } /* main */

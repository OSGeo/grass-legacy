...
/*DEBUG*/ debugf ("DEL_ATT: old type %02x new type %02x\n", (int)Att->type, (int)type);
    ...
    map->n_atts--;
    */
    fseek (map->att, Att->offset, 0);
    if (0>write_att (map->att, (char) dig_new_to_old_type (Att->type), Att->x, Att->y, Att->cat))
	return (-1);
    return (0);
}
    ...
    int cat;
{
    ...
    fseek (map->att, 0L, 2);
    offset = ftell (map->att);
    write_att (map->att, (char) dig_new_to_old_type (type), x, y, cat);
    ...
    int att;
{
    ...
    Att = &(map->Att[att]);
    fseek (map->att, Att->offset, 0);
    if (0>write_att (map->att, (char) dig_new_to_old_type (Att->type), Att->x, Att->y, Att->cat, Att->index))
	return (-1);
}

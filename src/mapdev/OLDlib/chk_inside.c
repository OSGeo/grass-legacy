...
double
dig_point_in_area(map, X, Y, Area)
    struct Map_info *map;
    double X, Y;
    P_AREA *Area;
{
...
/* Get-line loop */
	...
	*/
	...
	if (0 > dig__Read_line (&Points, map->digit, map->Line[at_line].offset))
	    return (-1.0);
...
/*DEBUG*/ debugf ("PNT_IN_AREA:  intersects = %d\n", n_intersects);
    if (n_intersects % 2)
	return(cur_min);
    else
	return(0.0);

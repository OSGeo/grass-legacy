...
#define FIRST  { if (First_Time) dig__Init_V (); }
/*
...
/* added below to Init_Portable_structs () */
...
** 
**  Portable_In  :   Iff a file is found that is 3.0 format, then this flag
**		     specifies how it should be read in.
*/

/* give access to this low level routine for Vlib */
dig__Init_Portable_structs ()
{
    return Init_Portable_structs () ;
}
	...
	struct dig_head *head;
{
    ...
    return dig_x_write_head_binary (digit, head);
}
	...
	struct dig_head *head;
{
    ...
    return dig_x_read_head_binary (digit, head);
}
...
{
    FIRST;
    return dig_x__Read_line (p, fp, offset);
}
...
{
    FIRST;
    return dig_x__Write_line(digit, type, points);
}
...
{
    FIRST;
    return dig_x__Rewrite_line (digit, offset, type, points);
}
...
static
dig__init_default_heads (do_port)
{
    struct dig_head *head;
...
struct dig_head *
dig__get_default_port_head ()
{
    static int first = 1;

    if (first)
    {
	dig__init_default_heads (1);
	first = 0;
    }

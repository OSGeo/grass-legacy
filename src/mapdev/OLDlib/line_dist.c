...
dig_set_distance_to_line_tolerance (t)
    double t;
{
    if (t <= 0.0)
	t = TOLERANCE;
...
double
dig_distance2_point_to_line (x,y,x1,y1,x2,y2)
    double x,y;         /* point */
...
double
dig_xy_distance2_point_to_line (px,py,x1,y1,x2,y2)
    double *px,*py;         /* point */
    double x1,y1,x2,y2;	/* line segment */
{
    register double dx,dy;
...
double
dig_xy_distance3_point_to_line (px,py,x1,y1,x2,y2, status)
    double *px,*py;         /* point */
    double x1,y1,x2,y2;	/* line segment */
    int *status;
{

...
*/
plus_t **
dig_get_cont_lines (map, line, angle, twoway)
    struct Map_info *map;
    plus_t line;
    double angle;
    int twoway;
{
	...
	for (cnt = 1-i ; ; cnt++)
	{
	    ...
	    {
/*DEBUG*/ debugf ("RAN INTO MYSELF! line %d\n", line);
		lines[i][cnt] = 0;
		break;
	    }
...
plus_t
dig_get_next_cont_line (map, prev_line, angle, twoway)
    struct Map_info *map;
    plus_t prev_line;
    int twoway;
    double angle;
...
/*DEBUG*/ debugf ("Returning closest line: %d\n", closest_line);
    return (closest_line);
}

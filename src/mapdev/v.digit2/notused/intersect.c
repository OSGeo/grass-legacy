    ...
    struct Map_info *map;
{
    ...
    /* find_line_with_mouse  fills Gpoints */
      line_to = find_line_with_mouse (LINE|AREA, "Line to be intersected:", NULL);
      line = line_to;
      if (line_to <= 0)
/*	  return (0);*/
          break;

      get_point (&ux, &uy, "Select intersection point :");
      if ( ux == 0.0 && uy == 0.0)
        {
...
/*	return (0);*/
    ...
    /* let break_line redraw the new lines */
      ...
      if ( (node_num = dig_which_node (CM, &ux, &uy, thresh))  < 0)
         ...
         line_from = find_line_with_mouse (LINE|AREA, " Line to intersect with:", NULL);
         line = line_from;

    ...
    int action;
{
    ...
    *prompt = '\0';
	...
	else
	{
	    ...
	    /* yet.  so write out  Greclass command, then we will start    */
	    /* filling in the input to it 				   */
	    if (!got_rule)
	    {
		char *tmp;
		tmp = tempcell (filecnt);
		sprintf (tmp_names[filecnt], "%s in %s", tmp, G_mapset());
		fprintf(tmpfp, "Greclass '%s' '%s' 'Weight temp %d' << EOF%d\n",
    ...
    fprintf (tmpfp, "Gmapcalc %s = '", result);

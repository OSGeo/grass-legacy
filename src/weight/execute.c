...
/* Find max and min possible values */
    ...
    printf(  "    Maximum possible total weight = %ld\n\n", max_value) ;
    printf(  "    This results in a possible range of %ld categories\n\n", max_value-min_value+1) ;

    while (1) 
    {
	normalize = 0;
	printf("   Hit RETURN to accept this default, or enter a number greater\n") ;
	printf("   than zero to specify an alternate number of potential categories > ") ;
	GETS;
	if (*input_buf == 0)
		break;
...
/* Get a new map name to store results */
    ...
    {
	if ( (map_file = G_open_cell_new(name, mapset) ) == -1)
	{
	    printf("    error: can't open cell file %s\n", name) ;
	    free(mapbuf) ;
	    free(result_buf) ;
	    free(final_buf) ;
	    return(-1) ;
...
/* print out some results */
	if (at_console()) 
	{
	    screen_row = D_draw_cell_row(row, final_buf) ;
	    R_flush() ;
	}
	else
	{
...
/* print analysis row out if saving */
	if (saving)
	    G_put_map_row(map_file, final_buf, row) ; 
    }

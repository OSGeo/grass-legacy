...
#define  USAGE  "elev_layer=name [patt_layer=name] east=value north=value [obs_elev=value] [max_dist=value] out_layer=name"
...
char *argv[];
{
	...
	struct point *heads[16],*segment(),*SEARCH_PT;
	...
	{
		sprintf (buf, 
		    "specified observer location outside database window");
		G_fatal_error (buf);
		exit(1);
	}
	search_mapset = "";
	...
	if(patt_flag == 1)
	{
		...
		if(patt_mapset == NULL)
		{
			sprintf (buf, "%s - not found", patt_layer);
			G_fatal_error (buf);
			exit(1);
		}
	}

    ...
    char Conformat;
{
    ...
    char	fillformat[133],totformat[133];
    char	*mapone;
    FILE	*fopen();
    int col0, row0;

    printf ("preparing report ..."); fflush (stdout);
...
/* Determine if room enough for Table Row Total at end of last panel	*/
	addflag = 0;
	if(at_panel == (num_panels-1) && (last_col-first_col) < (out_cols-2)){
	    addflag = 1;
	    sprintf(fillformat,"%%1.%ds%%s", (out_cols-(last_col-first_col))*12-25);
	}
	if(Conformat == 'x') addflag = 0;
	...
	if(Conformat != 'x') fprintf(dumpfile,"   w cat 0 | w/o cat 0 |");
	if(addflag)
	...
	{
	    if (*(mapone) != NULL)
		fprintf(dumpfile,"|%c %5ld |",*(mapone)++,(long)catlist2[Rndex]);
	    else
    ...
    /* column entries	*/
		...
		print_entry(Conformat,table[Thisone].count,table[Thisone].area);
    ...
    /* 'Total' line at bottom of panel	*/
	    ...
	    if(Conformat != 'x') fprintf(dumpfile,"           |           |");
	    if(addflag)
		fprintf(dumpfile,fillformat,fill,"|           |           |");
	    fprintf(dumpfile,"\n");
    /*========================================================================*/
    ...
    /* Grand Totals of Table Row totals (including/excluding col 0)	*/
		...
		if (col0 >= 0)
		{
		    ...
		    print_entry(Conformat, t_rowcount-count, t_rowarea-area);
		}
		else
		    print_entry(Conformat,t_rowcount, t_rowarea);
	    }

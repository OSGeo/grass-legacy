...
make_coin()
{
    ...
    fprintf(stderr, "Step 1: "); fflush (stderr);
    sprintf (buf, "Gstats -C '%s in %s' '%s in %s'",
	map1name, mapset1, map2name, mapset2);
    ...
    if (fd == NULL)
    {
	G_fatal_error ("can't run Gstats");
	exit(1);
    }

/* let Gstats do the work, but need to find the number of cats in each file */
    count = 0;
    while (fgets (buf, sizeof buf, fd))
    {
	if (sscanf (buf, "%ld:%ld:%lf:%ld",
	    &stats.cat1, &stats.cat2, &stats.area, &stats.count)
	    != 4)
	{
	    pclose (fd);
	    G_fatal_error ("Unexpected output from Gstats");
	    exit(1);
	}
    ...
    fprintf (stderr, "Step 2: reformat"); fflush (stderr);

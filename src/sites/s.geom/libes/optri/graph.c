...
void *
grNewAdvanced (nSiteMax, nofSites, useLIA, nofDeci, X, Y, Z, 
...
void 
grAllocate (g, maxs, maxe, nofDeci)
...

{
  ...
  if (qeTop < 0) {
    printf ("ERROR: grFindPrevEdge: no edge incident to vertex.\n");
    exit (1);
  }
...
static indexType
grLoadConstrainedEdge (g, org, dst)
...

{
  ...
  do {
    while ((sosrCollinear (g, qeORG (q, qe), qeDST (q, qe),
           ...
           (sosrLexoCompare (q, qeORG (q, qe), qeDST (q, qeONEXT (q, qe))) !=
            sosrLexoCompare (q, qeDST (q, qe), qeDST (q, qeONEXT (q, qe))))) {
...
/*  printf ("grCleanConvexHull: %d triangles removed\n", count);*/
}

/*--------------------------------------------------------------------------*/

static int
grEdgesOverlap (g, q, from1, to1, from2, to2)

     void * g, * q;
     indexType from1, to1, from2, to2;
  ...
  if (gt = sosrSlopeCompare (gOverlap, E1->from, E1->to, E2->from, E2->to))
    ...
    if (! sosrCollinear (gOverlap, E1->from, E1->to, E2->from))
      return (sosrLeftTurn (gOverlap, 
			    E1->from, E1->to, E2->from, &dummy) ? 1 : -1);
    else
...

{
  ...
  if (maxW >= 16) {
    printf ("ERROR: readGraph: numbers too large.\n");
    exit (1);
  }
...
/*  if (NE (g) >= 3)
  ...
  /* printf ("readGraph: vertices (sites) read: %d\n", ns); */
}

/*--------------------------------------------------------------------------*/

void 
grPrintGraph (fname, g, nofFlips, nofSuccFlips, runTime)
...
void 
grPrintGraphLong (fname, g, nofFlips, nofSuccFlips, runTime)
...
void
grMaxMinHeightTriangulation2 (g)
...
void
grIncrementalDelaunayTriangulation (g)
...
void
grIncrementalRegularTriangulation (g)

...
/*--------------------------------------------------------------------------*/
...

  {
    ...
    if (((persistQeType *) pq)->magic != PQEMAGIC) {
      printf ("ERROR: pqeMagic: wrong magic.\n");
      X ();
      exit (1);
    }
...
static void
pMarkDeletedEdgeIntern (pq, edge)
...
static indexType
pqeAddTriangleStarIntern (pq, qe, p)

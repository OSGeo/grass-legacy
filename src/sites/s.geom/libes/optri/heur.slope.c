...
static
hsSlopeCashPutAnchor (anchor)

     indexType anchor;

{
  cashTop->anchor = anchor;
}
...
static
hsComputeSlope (origin, from, to, A, B, C, AplusB)
...
static int
hsTestIfAnchor (A, B, candidate, v1, v2)

     Lia_ptr A, B;
     indexType candidate, v1, v2;
...
static int
hsComputeAnchor (A, B, origin, from, to)
...
int
hsSlopeRightTurn (g, origin, from, to)
...
int
hsSlopeGT (g, origin1, from1, to1, origin2, from2, to2)
...

{
  ...
  hsComputeSlope (origin1, from1, to1, &A1, &B1, &C1, &AplusB1, &anchor);

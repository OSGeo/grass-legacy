...
/*--------------------------------------------------------------------------*/
...

  {
    ...
    if (((qeType *) q)->magic != QEMAGIC) {
      printf ("ERROR: qeMagic: wrong magic.\n");
      X ();
      exit (1);
    }
...
void * 
qeNewAdvanced (nMax, nPrescribed, O, D)

     indexType nMax;
     indexType nPrescribed;
     indexType * O, * D;

{
  ...
  if (O == NULL) {
    if (D != NULL) {
      printf ("ERROR: Origin Array not specified.\n");
      exit (1);
    }
    ...
    if (D == NULL) {
      printf ("ERROR: Destination Array not specified.\n");
      exit (1);
    } else {
      tmp->org = O;
      tmp->dst = D;
...

{
  ...
  if (qe + 1 != qeSYM (qe)) {
    printf ("ERROR: qeInsertSiteSite: strange error.\n");
    exit (1);
  }
...
void
qeReinsertSiteSite (q, qe, newA, newB)

     void *q;
     indexType qe, newA, newB;
...

{
  ...
  if (qe + 1 != qeSYM (qe)) {
    printf ("ERROR: qeInsertEdgeSite: strange error.\n");
    exit (1);
  }
...

{
  ...
  if (qe - 1 != qeSYM (qe)) {
    printf ("ERROR: qeInsertEdgeSiteB: strange error.\n");
    exit (1);
  }
  qeSETOPREV (q, qe, qeSYM (a));
  qeSETOPREV (q, aRnext, qe);
...
void
qeReinsertEdgeSite (q, qe, newA, newS)

     void *q;
     indexType qe, newA, newS;
...

{
  ...
  if (qe + 1 != qeSYM (qe)) {
    printf ("ERROR: qeInsertEdgeEdge: strange error.\n");
    exit (1);
  }
  qeSETOPREV (q, qe, qeSYM (a));
  qeSETOPREV (q, aRnext, qe);
...
void
qeReinsertEdgeEdge (q, qe, newA, newB)
...
/*  for (i = 0; i < siNS (s); i++)
  ...
  printf ("qeMarkDeletedDuplicateEdges: %d duplicate edges marked.\n\n",
	  count);
...
static int
qeDoReadEdges (q, fp, examine, nofEdges)

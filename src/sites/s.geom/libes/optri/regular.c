...
static queuesType *
initializePool(h, pool, GTfunction)
...
void
insertNextPoint (h, visual, v, pool, edges, spl)
...
static void
do22flip (p, visual, pool, edges, qe, vertex)
...
static int
do31flip (p, visual, pool, edges, flipQe, vertex)
...
static int
doFlipEdge (h, visual, pool, edges, qe, vertex)
...
static int
inCircVerticesInf (h, s0, s1, s2, s3)
...
static int
regularityTestVerticesInf (h, s0, s1, s2, s3)
...
/*      printf("foundEdge: %d %d %d %d %d", qe, pORG (hdagPQ (h), qe), pDST (hdagPQ (h), qe),
...
void     
incrementalDelaunayTriangulation (dt, printInfo, visual, 
				  nofFlips, nofAttempts, runTime)
...
{
  printf ("Delaunay: Incremental: "); (void) fflush(stdout);
...
void 
regularTriangulation (dt, printInfo, visual, 
		      nofFlips, nofAttempts, runTime)
...
{
  printf ("Regular: Incremental: "); (void) fflush(stdout);

...
int 
sosrLeftTurn (g, s0, s1, s2, detdbl)
...
static int 
regularityMath (i, j, k, l, delaunay)
...
/*printf ("regularityMath (%d %d %d %d, %d) ==> %d, %d\n", 
	i, j, k, l, det, soscase);*/
...
static int 
sosRegularTest (g, s0, s1, s2, s3, doDelaunay)
...
int 
sosrInCircle (g, s0, s1, s2, s3, detdbl)
...

{
  ...
  if (! (inCircle3InfArr [i][j][k] = 
    ...
    printf ("ERROR: sosrInit: 3 infinite points: result == 0!\n");
    exit (1);
  }
  ...
  for (j = 1; j < 4; j++)
	...
	if (! (leftTurn2InfArr [i][j] = - (infP[i][0] - infP[j][0])))
	  if (! (leftTurn2InfArr [i][j] = infP[i][1] - infP[j][1])) {
	    printf ("sosrInit: 2 infinite points: result == 0 !\n");
	    exit (1);
	  }
    }
}
...
int 
sosrInCircleInf (g, s0, s1, s2, s3, doDelaunay, detdbl)
...
int 
sosrLeftTurnInf (g, s0, s1, s2, detdbl)
...
int
sosrLexoCompare (g, s0, s1, detdbl)

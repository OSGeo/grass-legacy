...
/* The getarg routines were posted in alt.sources; article written Jul 11,
   ...
   - changed ANSI function headings to old-style headings;
     otherwise it wouldn't run on Suns;
   - had trouble (segmentation fault, what else) with initarge();
     don't use it!
   - added several type casts to quiet lint;
...
#ifdef sgi
#define strchr(S,C) (S)
#endif  /* don't ask me why, but lint on the SGIs kept complainig  --EPM */
#endif 
...
int basic_getarg_init (argc, argv)
     int argc;
     char **argv;
{
  ...
  /* get or expand space */
  /* I added type casting to quiet lint  --EPM */
  if (pargc == 0)
...
int basic_getarg_inite (argc, argv)
     int argc;
     char **argv;
{
  ...
  if ((env_args[0] = (char*) malloc ((unsigned) strlen (env_str) + 1)) == NULL)
    return(-1);  /* not enough memory for argument pointers */

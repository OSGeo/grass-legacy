    ...
    REPORT *report;
{
    ...
    while (report_read(report))
    {
        ...
        else if (report_record(report,"window north"))
        {
            report->north = ctof(report->field[1]);
            flags |= 004;
        }
        else if (report_record(report,"window south"))
        {
            report->south = ctof(report->field[1]);
            flags |= 010;
        }
        else if (report_record(report,"window east"))
        {
            report->east = ctof(report->field[1]);
            flags |= 020;
        }
        else if (report_record(report,"window west"))
        {
            report->west = ctof(report->field[1]);
            flags |= 040;
        }
        else if (report_record(report,"ns resolution"))
        {
            report->ns_res = ctof(report->field[1]);
            flags |= 0100;
        }
        else if (report_record(report,"ew resolution"))
        {

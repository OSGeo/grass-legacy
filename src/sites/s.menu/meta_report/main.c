...
/* with stats ? */
    ...
    case 'F':	with_stats = 0;
		break;
    default:	printf("%s: illegal stats request: %s\n", argv[0], WITH_STATS);
		exit(-1);
    }
...
/* site list */
    ...
    {
	printf("%s: can't read sites file: %s\n", argv[0], SITE_FILE);
	exit(-1);
    }
...
/* layer list */
    ...
    if (!layer_list)
    {
	printf("%s: can't read layers file: %s\n", argv[0], LAYER_FILE);
	exit(-1);
    }
 ...
 */
    ...
    if (!meta_report)
    {
	printf("%s: can't open report file: %s\n", argv[0], REPORT_FILE);
	exit(-1);
    }

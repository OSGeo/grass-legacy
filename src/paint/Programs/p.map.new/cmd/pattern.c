...
int add_pattern_to_list (char *name, long offset,
    int nlines, int nrows, int ncols, int file)
{
    PLIST *p;
...
int find_pattern (char *name, long *offset,
    int *nlines, int *nrows, int *ncols, int *file)
{
    PLIST *p;
...
int read_pattern (PATTERN *p, int nlines)
{
    char *pp;
    char buf[1024];
    int line, row, col;
...
fprintf (stdout,"read_pattern\n");
    ...
    {
	if (G_getl (buf, sizeof buf, patternfd) == 0)
	    break;
#ifdef DEBUG
fprintf (stdout,"%d\t[%s]\n", row, buf);
#endif
...
int get_pattern (char *name, PATTERN *p)
{
    long offset;
...
*/
int next_pattern (PATTERN *pat, int first)
{
    static PLIST *p ;
...
int open_pattern_file (int file, int mode)
{
    char buf[1024];
    char *name;
    if (patternfile == NULL)

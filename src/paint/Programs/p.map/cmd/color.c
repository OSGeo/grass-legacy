...
static int dither(unsigned char *,int,int,int,short *,short *,short *);
...
static int build (int nlevels,int mult,int *level,int *value,int *level_to_255)
{
    int i;
    int first, last;
...
*/
int 
printer_color_number (register int red, register int grn, register int blu)
{
    if (red < 0) red = 0;
...
int 
red_carryover (register int color)
{
    if (color < 0) color = 0;
...
int 
grn_carryover (register int color)
{
    if (color < 0) color = 0;
    else if (color > 255) color = 255;
...
int 
blu_carryover (register int color)
{
    if (color < 0) color = 0;
    else if (color > 255) color = 255;
...
static int dither (
    unsigned char *color,int row, int col, int ncols,
    short *low,short *hi,short *extra)
{
    short *dp;

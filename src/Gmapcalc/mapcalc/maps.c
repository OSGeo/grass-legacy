...
static uchar *grn = NULL;
static uchar *blu = NULL;
...
initcolors(i)
{
	...
	int ncols;
	ncols = G_window_cols();
	red = (uchar *) G_malloc (ncols);
	grn = (uchar *) G_malloc (ncols);
	blu = (uchar *) G_malloc (ncols);
    }
 ...
 */
    G_get_color_array (cell, red, grn, blu, &list[k].colors, ncols);
    switch(code)
    {
    case '#': /* grey */
	while (--ncols >= 0)
	    cell[ncols] = (18*red[ncols] + 81*grn[ncols] + 11*blu[ncols] + 50)/100;
	    /*

...
thin_step()
{
    ...
    {
        fprintf(stderr,"Can't find drainage direction map layer [%s]\n",
          aspect_name);
        fprintf(stderr,"Unable to run this step\n");
        printf("\nHit return to continue");
        G_gets(buf);
    ...
    V_line(8,"    Will use redirected drainage directions from Step 3");
    V_line(10,"Output:");
    V_line(11,"    Thinned network map layer:");
    V_line(12,"    Number of thinning iterations:");
    V_line(13,"    Accumulation threshold:");

    V_ques(thin_name,'s',11,46,30);
    V_ques(&thin_num,'i',12,46,7);
    V_ques(&acc_thres,'i',13,46,7);
    ...
    if (strncmp(run,"y",1) == 0)
    {
        ...
        sprintf(command,
            "%s/etc/Gthin -v 'accum=%s in %s' 'aspect=%s in %s' thin=%s iters=%d dthres=%d\n",
            G_gisbase(), river_name, river_mapset, aspect_name, aspect_mapset,
              thin_name, thin_num, acc_thres);
    ...
    {
        if (strcmp(save_thin, thin_name) || strcmp(save_aspect, aspect_name)
          || (save_accthres != acc_thres) || (save_thinnum != thin_num))
        {
            for (i=4; i<7; i++)
                complete[i] = 0;
        }

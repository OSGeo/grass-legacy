...
static int aspect_fd;
static int river_fd, thin_fd;
static char aspect_name[40], *aspect_mapset;
...
/* parse command line */
    ...
    if ((!aspect_flag) || (!river_flag) || (!thin_flag) || (!iters_flag) || (!min_flag))
        usage(argv[0]);

    if (verbose)
       printf("\n Running...\n");

    aspect_mapset = G_find_file2("cell",aspect_name,"");
    if (!aspect_mapset)
    {
    ...
    aspect_fd = G_open_cell_old (aspect_name, aspect_mapset);
    if (aspect_fd < 0) exit(2);
    ...
    aspect = (CELL *)G_malloc(nrows*ncols*sizeof(CELL));
	...
	G_get_map_row( aspect_fd, aspect+row*ncols, row);
    }

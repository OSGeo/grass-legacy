/*  %W% %G% */
/* THIS VERSION OF THIN IS FOR ANY GREY IMAGE                         */
/*====================================================================*/
#include "gis.h"
...
/*  temporarily removing this line to experiment with new method 
...
/* check and correct  diamond holes  */
    for (i=1; i<nr1; i++)
    {
        ...
        {
            if (nimg[i*ncols+j] == 0
/*  the following lines are being commented out and replaced
    temporarily for experimenting purposes 
...
*/
            ...
            && (nimg[(i+1)*ncols+j] > 0))
              ...
              nimg[i*ncols+j] = (nimg[i*ncols+j+1] + nimg[(i-1)*ncols+j]
                        + nimg[i*ncols+j-1] + nimg[(i+1)*ncols+j])/4;
        }
    }
...
/*  the following lines are being commented out temporarily for
...
*/
                    ...
                    for (rowinc = -1; rowinc<=1; rowinc++)
                        for (colinc = -1; colinc<=1; colinc++)
                            if ((nimg[i*ncols+j] > 
                              nimg[(i+rowinc)*ncols+(j+colinc)]) &&
                              (nimg[(i+rowinc)*ncols+(j+colinc)] > 0))
                                  maxcnt = maxcnt+1;

                    if (nimg[(i-1)*ncols+j] > 0) n=1;
                    else n=0;
                    if (nimg[(i-1)*ncols+(j-1)] > 0) nw=1;
                    else nw=0;
                    if (nimg[i*ncols+(j-1)] > 0) w=1;
                    else w=0;
                    if (nimg[(i+1)*ncols+(j-1)] > 0) sw=1;
                    else sw=0;
                    if (nimg[(i+1)*ncols+j] > 0) s=1;
                    else s=0;
                    if (nimg[(i+1)*ncols+(j+1)] > 0) se=1;
                    else se=0;
                    if (nimg[i*ncols+(j+1)] > 0) e=1;
                    else e=0;
                    if (nimg[(i-1)*ncols+(j+1)] > 0) ne=1;
                    else ne=0;
...
*/
                    if (nwork[i*ncols+j] == 0) nimg[i*ncols+j] = 0;
        }
    } 
    for (i=0; i<nrows; i++)
        for (j=0; j<ncols; j++)

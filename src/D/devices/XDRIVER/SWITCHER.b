...
#define debug(x)
	...
	opened = 0;
	def = signal(SIGALRM,timeout); /* set to catch alarms */
	/*
	 * this loop tries to open the fifos. If no luck after 1 sec
	 * it will give up, check for any X events to service, then
	 * try to open again
	 */
	while(!opened) {
	    if (setjmp(savenv)) 
	...
	debug("Opened");
	signal(SIGALRM,def);  /* restore orig alarm signal catcher */
...
/*	    case RASTER:
            case ZRASTER:
                ...
                REC (xarray, x * sizeof (*xarray)) ;
		if(c == RASTER)
			Raster(x, y, xarray, 1, 1) ;
		else
			Raster(x, y, xarray, 0, 1) ;
                break ;
*/
            case RGB_RASTER:
                REC (&x, sizeof x) ;
		...
		break ;
            ...
            case RESET_COLORS:
                ...
                if(min > max)
                {
                    sprintf(text,"min > max on call to reset_colors. min:%d max:%d", min, max) ;
                    perror(text) ;
                    exit(-1) ;
                }
		...
		Panel_delete(text) ;
            ...
            lc = c ;
        } /* end of the "while (eof<=0 && !broken_pipe)" block */
        /* read encountered EOF. close fifos now */
        close (_wfd);
        close (_rfd);
    }   /* end of the "while(1)" way back around line 200 */
}
...
/*
 * read the fifo. look for 1 (or more) COMMAND_ESC chars followed by
 * a non-zero command token char. If there is an eof return 1,
 * else return 0.
 */
    ...
    char *buf;
{
    int stat;
#ifdef DEBUG
    fprintf(stderr,"rec: reading %d chars\n",n);
#endif
    while (n-- > 0)
        if ((stat=get1(buf++)) != 0)

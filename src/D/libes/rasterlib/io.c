...
R_open_driver() 
{
	...
	if ((mon = R_parse_monitorcap(MON_NAME,name)) == NULL)
	{
		...
		fprintf(stderr,"Please run \"monitor\" to select a valid graphics monitor.\n");
		exit(-1);
	    }
	    else
	    {
	...
	else
	{
		...
		{
		    if (key_string == NULL)
			fprintf(stderr,"Error - GIS_LOCK variable not set.\n");
		    else
			fprintf(stderr,"Error - GIS_LOCK variable improperly set.\n");
		    exit(-1);
		}
		else
		{
    ...
    char *file;
{
    ...
    *file = 0;
    ...
    if (name && *name)
	sprintf (file, "%s/locks/%s", G_getenv("GISBASE"), name);
}

#include <pwd.h>
static lock_driver (lock_pid)
{
    char file[512];
    int fd;
    int mask;

...
/* Print poly (grip) header information */
	...
	for(;;)
	{
		...
		case 'A':
			...
			fprintf(mapfile,"A %.10lf %.10lf %10d\n",
		...
		case 'L':
			...
			fprintf(mapfile,"L %.10lf %.10lf %10d\n",
				first_uy, first_ux, category) ;
			break ;
...
/* Run the polytocell capabilities on this poly file */
	sprintf(buffer, "%s/etc/poly_to_bmif < %s | sort -t: +0n -1 |  %s/etc/bmif_to_cell %s",
	...
	struct Cell_head *universe ;
{
	...
	fprintf(mapfile,"BOUND     %.10lf %.10lf %.10f %.10f\n",
	...
	double *uy ;
{
	...
	show_instructions_1() ;
	fprintf(stderr, "Right:  quit request\n") ;
...
show_instructions_1()
{
	...
	fprintf(stderr, "Middle: Mark point\n") ;
	fprintf(stderr, "Right:  quit request\n\n") ;
	n_lines = 0 ;
}

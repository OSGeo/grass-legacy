	...
	int hi_mode ;
{
	...
	sprintf(buffer, "       RED: %3d   %3d%%\0", red, (int)((float)red / 2.56) ) ;
	wmove(COLR_WIN,3,3) ;
	waddstr(COLR_WIN,buffer) ;

	sprintf(buffer, "     GREEN: %3d   %3d%%\0", grn, (int)((float)grn / 2.56) ) ;
	wmove(COLR_WIN,4,3) ;
	waddstr(COLR_WIN,buffer) ;

	sprintf(buffer, "      BLUE: %3d   %3d%%\0", blu, (int)((float)blu / 2.56) ) ;
	wmove(COLR_WIN,5,3) ;
	waddstr(COLR_WIN,buffer) ;

	sprintf(buffer, "SHIFT INCR: %3d   %3d%%\0", shift_incr, (int)((float)shift_incr / 2.56)) ;
	wmove(COLR_WIN,7,3) ;
	waddstr(COLR_WIN,buffer) ;

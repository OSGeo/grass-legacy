    ...
    char *gisdbase, *location_name;
{
    ...
    name = G_projection_name(0);
    printf ("        %s (for imagery and other unreferenced data)\n", name);
    for (i = 1; name = G_projection_name(i); i++)
	printf ("        %s\n", name);
    printf ("2. The zone for the database\n");
    printf ("       (except for %s",G_projection_name(0));
    printf (" and %s databases)\n", G_projection_name(PROJECTION_LL));
    printf ("3. The coordinates of the area to become the default window\n");
    printf ("   and the grid resolution of this window\n");
	...
	printf ("Please specify the coordinate system for location <%s>\n\n",
		location_name);
	for (i = 0; name = G_projection_name(i); i++)
	    printf ("%4d %s\n", i, name);
	...
	printf ("Please specify the %s zone for location <%s>\n",
	    G_projection_name(window.proj), location_name);

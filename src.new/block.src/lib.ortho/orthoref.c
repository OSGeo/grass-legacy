...
I_compute_ortho_equations (cpz, cam_info, init_info, XC, YC, ZC, Omega, Phi, Kappa)
...
/* DEBUG */
    debug = fopen("/users/mbaba/debug", "w");
...
/*  Weights set to identity matrix unless have run i.ortho.initial */
    WT1.nrows = 6;
    WT1.ncols = 6;
    zero ( &WT1);
   ...
   else  /* set from mean values of active control points */
   {
      ...
      *Omega = *Phi = *Kappa = 0.0;
       ...
       *YC = meany / n;
       ...
       /* set initial ZC from:                               */ 
       /*    scale ~= (x1 -x2)/(X1 - X2) ~= (CFL)/(Z1 - Zc)  */
       /*       Zc ~= Z1 + CFL(X1-X2)/(x1-x2)                */
...
I_ortho_ref (e1, n1, z1, e2, n2, z2, cam_info, 
             XC, YC, ZC, Omega, Phi, Kappa)
    double e1, n1, z1;
    double *e2, *n2, *z2;
    struct Camera_File_Ref *cam_info;
    double Omega, Phi, Kappa, XC, YC, ZC;
{
    MATRIX UVW, XYZ, M;
    double U,V,W;
    double Xp,Yp,CFL;
...
I_inverse_ortho_ref (e1, n1, z1, e2, n2, z2, cam_info, 
                     XC, YC, ZC, Omega, Phi, Kappa)
    double e1, n1, z1;
    double *e2, *n2, *z2;
    struct Camera_File_Ref *cam_info;

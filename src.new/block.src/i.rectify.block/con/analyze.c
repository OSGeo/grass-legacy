...
/* lets do it */
	...
	while(1)
	{
		...
		break;
	    line++;
	    if(group.con_equation_stat > 0 && group.photo_points.status[curp] > 0)
	    {
		color = BLACK;
		...
		FMT2(buf,
		    group.con_points.e1[curp],
		    group.con_points.n1[curp],
		    group.con_points.e2[curp],
		    group.con_points.n2[curp],
                    group.con_points.z2[curp]);
...
/* all done. restore what was under the window */
    ...
    for (i = 0; i < group.con_points.count; i++)
        group.con_points.status[i] = group.photo_points.status[i];
...
compute_transformation()
{
    ...
    FILE *debug2;
    debug2 = fopen("/users/mbaba/debug_inv", "r+");
...
/*
	I_ortho_ref (group.photo_points.e2[n], group.photo_points.n2[n], group.photo_points.z2[n], &e1, &n1, &z1, &group.cam_info, group.XC, group.YC, group.ZC, group.omega, group.phi, group.kappa);
*/

    fprintf (debug2, "\tPhoto coordinates\n\t\tx = %lf \t y = %lf\n",
            group.photo_points.e1[n], group.photo_points.n1[n]);
    fprintf (debug2, "\tCamera \n\t\txc = %lf \t yc = %lf \t zc = %lf\n                    \t\t omega = %lf \t phi = %lf \t kappa = %lf \n",                              group.XC, group.YC, group.ZC, group.omega, group.phi, group.kappa);

	I_inverse_ortho_ref (group.photo_points.e1[n], group.photo_points.n1[n], group.photo_points.z2[n], &e1, &n1, &z1, &group.cam_info, group.XC, group.YC, group.ZC, group.omega, group.phi, group.kappa);

fprintf (debug2, "\tGround coordinates\n\t\tx = %lf \t y = %lf\n", e1,n1);

/*	I_ortho_ref (group.photo_points.e1[n], group.photo_points.n1[n], group.photo_points.z1[n], &e2, &n2, &z2, group.E12, group.N12, group.Z12);
*/

	if((d = xres[n] = e1-group.photo_points.e2[n]) < 0)
	    d = -d;
	if (d > xval) 
	{
	    xmax = n;
	...
	if ((d = yres[n] = n1-group.photo_points.n2[n]) < 0)
	    d = -d;
	if (d > yval)
	{
	    ymax = n;
...
/* compute target error (ie along diagonal) */
	d1 = e1 - group.photo_points.e2[n];
	d2 = n1 - group.photo_points.n2[n];
	d = d1*d1 + d2*d2;
    ...
    FILE *fd;
{
	...
	FMT0(buf,n+1);
	fprintf (fd, "%s", buf);
	if(group.con_equation_stat > 0 && group.photo_points.status[n] > 0)
	{
...
show_point (n, true_color)
{
	...
	R_standard_color (RED);
    display_one_point (VIEW_MAP1, group.con_points.e1[n], group.con_points.n1[n]);
}

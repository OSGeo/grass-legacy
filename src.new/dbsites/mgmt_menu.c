...
/* create screen */
	V_clear();
	V_line( 1,"      GRASS-RIM  DBSITES  DATA  BASE  MANAGEMENT  MENU");
...
do_backup()  /* dbsites driver to backup the current data base */
{
   char buffer[70];
...
do_pack()  /* dbsites driver to pack a data base */
{
   printf("packing...\n");
   if (backup("PACK") == -1 ) 
...
reload()
{
   V_clear();
   V_line( 1,"                   RELOAD");
   V_line( 4,"   See the dbsites manual for instructions on reloading a");
   V_line( 6,"   data base from a RIM backup text file.");
   V_line( 8,"   (This is done external to dbsites for reasons of data");

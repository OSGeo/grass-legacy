...
char *p,*p1, cmd[100], filename[200], desc[80], extra_field[20];
...
#ifdef DBSITES
fprintf(fp,"\n# Next %d sites added by dbsites from %s data base: %s\n",
		(Last_site-Site_list)+1, File_name, G_date() );
#else
fprintf(fp,"\n# Next %d sites added by Gdbsites from %s data base: %s\n",
		(Last_site-Site_list)+1, File_name, G_date() );
#endif

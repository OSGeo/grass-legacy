...
FILE	*dxf_file;
{
    ...
    {
	make_arc(centerx,centery,radius,360.0,0.0,0);
...
make_arc(centerx,centery,radius,start_angle,finish_angle,flag)
	...
	*/
	if (radius < 0) 
	{
		...
		pt_array[arr_size].x = radius*cos(alpha) + centerx;
		pt_array[arr_size].y = radius*sin(alpha) + centery;
	...
	arr_size++;
	 ...
	 /*need to check extent of plotted arcs and circles */
		...
		for(x=0; x<arr_size; x++)
		    dxf_check_ext(pt_array[x].x,pt_array[x].y);

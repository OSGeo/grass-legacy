...
/* If zeros are to be included, an entire raster row can be constructed */
...
#endif
			if (l)
				XPutImage(dpy, l->l, l->gc, grimage, 0, 0, cur_x, cur_y + i, num, 1);
			if (!backing_store) {
	 ...
	 * re-write it. */
		...
		for (j = 0; j < num; j++) {
			if (*arr == 0) {
				if (width > 0) {
					for (i = 0; i < nrows; i++) {
						if (l) {
							XDrawLine(dpy,l->m,l->mgc,cur_x + start_col, cur_y + i,cur_x + start_col+width-1,cur_y + i);
							XPutImage(dpy, l->l, l->gc, grimage, 0, 0,
		...
		if (width > 0) {
			for (i = 0; i < nrows; i++) {
				if (l) {
					XDrawLine(dpy,l->m,l->mgc,cur_x + start_col,
							  cur_y + i,cur_x + start_col+width-1,cur_y + i);
					XPutImage(dpy, l->l, l->gc, grimage, 0, 0,
							  cur_x + start_col, cur_y + i, width, 1);
				}
#if DIRECT_DRAW
		...
		for (i = 0; i < nrows; i++) {
/*  			XPutImage(dpy, top_layer, gc, grimage, 0, 0, cur_x, cur_y + i, num, 1); */
/*  			XCopyArea(dpy,top_layer, grwin, gc, cur_x, cur_y + i, num, 1,cur_x, cur_y + i); */
		}
		return 1;
}

...
* the limits (lims[]) were designed for the Calcomp raster plotter 
* using and hpgl filter program 
*          M.L.Holko                 4/4/91 
*/
#ifndef lint static char *SCCSID = "@(#)Dhpgl.c	OEMG v.3.2";
#endif
...
moveit:
		...
		if (Dglobal.reverse) {
			Dt = Dx;
			Dx = -(Dy - (lims[Dglobal.model_no].xlim / 2.0));
			Dy = Dt - (lims[Dglobal.model_no].ylim / 2.0);
			}
		else {
	...
	case D_PEN:
		pen = (va_arg(vap, long));
		/*pen %= 8;   only 8 pens */
		while (pen > NUM_PEN) pen = pen - NUM_PEN;
		if (pen == 0) pen = 1;
		if (pen != oldpen) {
			oldpen = pen;

#ifndef lint
static char *SCCSID = "@(#)plotter.c	OEMG v.2.1";
#endif
#include <signal.h>
#include <varargs.h>
#define PLOTTER
...
bomb(va_alist) va_dcl {
	...
	(void)fputs("plotter failure, Version:",stderr);
	(void)fputs(VERSION,stderr);
	if (opt & PERR) perror(va_arg(vargs, char *));
	opt &= ~PERR;
	while (opt-- > 0)
		(void)fputs(va_arg(vargs, char *), stderr);
	(void)fputc('\n',stderr);
	va_end(vargs);
	onintr();
}
...
main(argc, argv) int argc; char *argv[]; {
	...
	if (!(env_base = getenv(env_name)))
		bomb(3,"environ: ", env_name, " not found");
	...
	while (--argc > 0) { /* process options */
		if (**++argv == '-') for (arg = *argv;;) {
			...
			case 'X': /* x base offset */
				if (--argc > 0)
					base_x = strtol(*++argv,0,0);
				continue;
			case 'Y': /* y base offset */
				if (--argc > 0)
					base_y = strtol(*++argv,0,0);
				continue;
			case 's': /* scale factor */
				if (--argc > 0)
					Dglobal.scale = atof(*++argv);
				continue;
			case 'd': /* device name */
				if (--argc > 0)
					devname = *++argv;
			...
			case 'i': /* interactive */
				if (--argc > 0) {
					...
					aargv[aargc++] = pstr;
/*DEBUG fprintf(stderr,"plotter %s\n",pstr);*/
					continue;
				} else
					bomb(1,"no interactive link args\n");
				continue;
			case 'p': /* pen mapping */
				aargv[aargc++] = *argv;
				break;
	...
	if ((p = (*device)(D_INIT)) == NULL)
		bomb(1,"device initialization failure");
	windinit((long)(p->x), (long)(p->y));
	while (aargc--) {
		if (aargv[0][0] == '-' && aargv[0][1] == 'p')
			pmapin(*(aargv++)+2);
		else {
			initincl(*aargv++);
			pltparse();
...
initincl(file) char *file; {
	...
	if (direct = (*file == '-' && file[1] != '\0')) {
		if ((ilist[0] = fdopen(atoi(++file),"r")) == NULL)
		      bomb(PERR, "pipe input open failure");
		file = strchr(file, '.');
		if (!file || (answr = fdopen(atoi(++file),"w")) == NULL)
		      bomb(PERR, "pipe output open failure\n");
	}
	else if (*file == '-')
		ilist[0] = stdin;
	else if ((ilist[0] = fopen(file,"r")) == NULL)
		bomb(PERR+2, "input file","\nrunline name:",file);
	file_no = 0;
	base.x = base_x;
	base.y = base_y;
}

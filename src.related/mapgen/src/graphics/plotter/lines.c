#ifndef lint
static char *SCCSID = "@(#)lines.c	OEMG v.3.1";
#endif
...
flset(s) char *s; {
	...
	for (pen->f_nosym = 0; pen->f_nosym < 8 && (c = *s++) ; ++pen->f_nosym)
		...
		if (dx || dy) {
			...
			for (t = 0; t <= l; ) {
				if ( !pen->f_rdist ) {
					m = t * rd;
					pen->cflags |= C_SYM;
					if (pen->f_syms[pen->f_cycle] & 0x80)
						pen->f_pline ^= _FLIP;
					s = .0625 * pen->f_size * (pen->f_pline
					   & _FLIP ? -rd : rd);
					c = dx * s;
					s *= dy;
					if (setchar(pen->f_syms[pen->f_cycle],
						&T,.0625*pen->f_size,c,s,1)) {
						xt =x0 + lrnd(dx * m),
						yt =y0 + lrnd(dy * m);
					...
					do {
						...
						pen->f_rdist+= pen->f_dist;
					} while ((pen->f_syms[pen->f_cycle]
						& 0x7f) == 0x7f);
				}
				if (( t += pen->f_rdist ) <= l) {
					pen->f_rdist = 0;

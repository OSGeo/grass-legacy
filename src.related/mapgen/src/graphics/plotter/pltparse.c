#ifndef lint
static char *SCCSID = "@(#)pltparse.c	OEMG v.2.1";
#endif
/* pltparse	*/
# define PLOTTER
# include "graphics.h"
...
/*DEBUG fprintf(stderr,"pltparse COORD+REL %ld %ld\n",xv,yv);*/
			position(cmd, xv, yv);
			break;
		case _NOARG:
			noarg(cmd & _LBMASK);
...
/*DEBUG fprintf(stderr,"pltparse default %ld %ld\n",xv,yv);*/
...
position(cmd, x, y) int cmd; long x, y; {
	...
	if (!pen->bezierm) {
		if (!(cmd & _PENUP)) {
/*DEBUG fprintf(stderr,"!bez_clip %ld,%ld  %ld,%ld\n",X,Y,x,y);*/
		...
		X = x;
		Y = y;
/*DEBUG fprintf(stderr,"!bez %ld,%ld  %ld,%ld\n",X,Y,x,y);*/
		if (pen->sym) {
			symbol();
/*DEBUG fprintf(stderr,"call symbol\n");*/
			pen->pendown = 0;
		}
	} else {
/*DEBUG fprintf(stderr,"bez \n");*/
		if (cmd & _PENUP)
...
noarg(cmd) int cmd; {
	...
	caseof(FPLOT):		/* select fancy line mode */
		...
		pen->f_pline |= _FLINE;
fplotc:
		if (pen->f_dist > 0 && pen->f_nosym && !pen->dsline) {
			pen->dsline = pen->line;
			pen->line = fline;
		}
		break;

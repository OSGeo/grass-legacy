...
/*DEBUG dpr = fopen ("/tmp/dpr", "w");*/
/*DEBUG fprintf (dpr, "init called version 1\n");*/
/*DEBUG fprintf (dpr, "*argv=  %s\n", *argv);*/
...
/*DEBUG fprintf (dpr, "type= %d\n", type);*/
	do {
		if (setpltr(1, lastpl, argv, zoom))
			exit(1);
		for ( j = i ; j < argc ; j++){
/*DEBUG fprintf (dpr, "do plotopt2 arg[%d]=  %s\n", j,argv[j]);*/
			plotopt(INCL, argv[j]);}
		if (! zoom) {
/*DEBUG fprintf (dpr, "plotend\n");*/
			plotend();
			break;
		} else
			plotopt(NEWPEN, "T");
/*DEBUG fprintf (dpr, "start for loop\n");*/
		for (;;) {

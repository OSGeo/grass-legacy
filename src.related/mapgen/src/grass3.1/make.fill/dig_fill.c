...
/*DEBUG fprintf(stderr,"\n%d %lf %lf %d\n",islands,*ix,*iy,in[0]);*/
	...
	if (angle > 0) {
		MARX = .01 * scale/100 * rot_cos;
		MARY = .01 * scale/100 * rot_sin;
		}
	else {
		MARY = .01 * scale/100 * rot_cos;
		MARX = .01 * scale/100 * rot_sin;
		}
...
/* ALLOW MARGIN to avoid pen overlap on plot */
		if (x0 < x1) {x0 += MARX; x1 -= MARX;}
		else {x0 -= MARX; x1 += MARX;}
		if (y0 < y1) {y0 += MARY; y1 -= MARY;}
		else {y0 -= MARY; y1 += MARY;}

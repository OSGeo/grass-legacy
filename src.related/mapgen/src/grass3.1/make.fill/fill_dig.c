...

{
        ...
        double *X, *Y, *IX, *IY, scale = 1.0;
        int n_points, *i_points;
        struct pattern pattern;
        int SELECT=0, n_cat, *catlst, *pcatlst;

if (argc <= 2 ) {
        fprintf (stderr,"\n fill_dig [-sp space -c pen -a angle -sc scale] num_cats mapset dig_file\n");
        exit(ERR);
}
else
...
/* -------------   input file name --------------------- */    
        ...
        if (sscanf (argv[cnt++], "%s",mapset) != 1) 
                ...
                fprintf (stderr,"\n fill_dig [-sp space -c pen -a angle -sc scale] mapset dig_file\n");
                exit(ERR);
                }
...
if (!SELECT)
*pcatlst = cat = Map.Att[Map.Area[1].att].cat;
SELECT = 0;
...
dig_fill(n_points,X,Y,&pattern,Map.Area[area_num].n_isles,IX,IY,i_points,scale);

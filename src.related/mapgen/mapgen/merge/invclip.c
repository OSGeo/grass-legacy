#ifndef lint
static char *SCCSID = "@(#)invclip.c	OEMG v.1.1";
#endif
#include "gen.h"
/* provides basic inverse windowing of vectors.  */
	void /* clip line to range outside of window */
invclip(p1, p2, limits) IXY p1, p2; struct LIMITS *limits; {
	register c;
	int c1, c2;
	IXY p, ps, pe;
	...
	while (c1 | c2) {
		...
		if (c == c1) {
			if (!(c1 = outside(p, limits)))
				(*limits->clip)(ps, p, limits->next);
			p1 = p;
		} else {
			if (!(c2 = outside(p, limits)))
				(*limits->clip)(p, pe, limits->next);
			p2 = p;
		}
	}
}

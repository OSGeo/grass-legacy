...
zscale(p, axis) struct AXIS *p; char *axis; {
	...
	if (p->wmin <0 || p->wmin >= p->wmax)
		emess(1, "invalid window values for %s axis",axis);
	if (p->dmin == p->dmax)
		emess(1, "data limits equal for %s axis", axis);
	if (p->log) {
		if (p->dmin <= 0. || p->dmax <= 0.)
			emess(1, "range <= 0. for %s log axis", axis);
...
noarg:				emess(1,"arg -%c has no value",*arg);
			continue;
		...
		case 'w':	/* set window size */
			if (argc-- > 0)
				if (type == 'x')
					getreal(*argv++, &p_def.x.wmin);
				else {
					getreal(*argv++, &p_def.y.wmin);
					if (type != 'y') {
						p_def.x.wmin = p_def.y.wmin;
						p_def.x.wmax = p_def.y.wmax;
					}
				}
			else goto noarg;
			continue;
		case 'd':	/* set data range */
			if (argc-- > 0)
				if (type == 'x')
					getreal(*argv++, &p_def.x.dmin);
				else {
					getreal(*argv++, &p_def.y.dmin);
					if (type != 'y') {
						p_def.x.dmin = p_def.y.dmin;
						p_def.x.dmax = p_def.y.dmax;
					}
				}
			else goto noarg;
			continue;
		...
		case 'n':
			...
			continue;
		default:
			emess(0,"invalid scaling command:-%s\n",arg);
		}
		break;

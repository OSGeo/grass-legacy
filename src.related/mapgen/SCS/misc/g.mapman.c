    ...
    char *entry;
{
    ...
    sprintf (buf, "%s/mapgen/man/%s", G_gisbase(), entry);
...
/* copy entry to temp file, squeezing multiple blank lines into 1 */
    ...
    sprintf (buf, "lpr %s/mapgen/man/%s", G_gisbase(), entry);
    system (buf);
    return 1;
}
...
"------------------------------------------------------------------------\n");
    fclose (fd);
    sprintf (buf, "ls -C %s/mapgen/man >> %s", G_gisbase(), tempfile);
    system (buf);
    fd = opentemp ("a");
    fprintf (fd, "\n");
    fclose (fd);
...
{
    char buf[1024];
    sprintf (buf, "%s/mapgen/man", G_gisbase());

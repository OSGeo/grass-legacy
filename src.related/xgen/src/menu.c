    ...
    Boolean isDynamic;
{
    ...
    int numChildren;       /* how many kids does our menu have ? */
    ...
    if ((NULL != (oneW = CreateObject(s,row_col,isDynamic,&numChildren,False))) && 
        ...
        n = 0;
        XtSetArg(args[n],XmNshadowThickness,&sthick); n++;
        ...
        if ( NULL != (resource = IndexResource((char *)s,SHELL,"visibleitems"))) {
            double oneHeight, rcHeight, offsetHeight, kids;
        ...
        if ( taller ) {
            ...
            XtSetArg(args[n],XmNwidth,width + 2*spacing + 
                sthick*2 + preferred.width); n++;
            if ( !wider ) {
        ...
        if ( wider ) {
            ...
            XtSetArg(args[n],XmNheight,height + spacing*2 + 
                sthick*2 + preferred.height); n++;
            if ( !taller ) {

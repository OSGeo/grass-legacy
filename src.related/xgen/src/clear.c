    ...
    InterfaceObject                *o;
{
    ...
    for (r = 0; r < rows; r++)
        for (c = 0; c < columns; c++)
            XbaeMatrixSetCell((XbaeMatrixWidget) o->widget, r, c, "");
}
    ...
    char                           *spec;
{
    /* index past '(' */
    spec++;
    if (NULL != strrchr(spec, ',')) {
        int                             r = atoi((char *) strtok(spec, ",")), c = atoi((char *) strtok(NULL, ")"));

        XbaeMatrixSetCell((XbaeMatrixWidget) o->widget, r-1, c-1, "");
    } else {
        ...
        for (i = 0; i < columns; i++) {
            XbaeMatrixSetCell((XbaeMatrixWidget) o->widget, r-1, i, "");
        }
    }
}

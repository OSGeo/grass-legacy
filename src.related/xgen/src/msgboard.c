    ...
    Shell                          *s;
{
	...
	XgenWarning("create commandboard",
	"Sorry...I can't use forcesize on commandboard with pane option on...");
	verbose = save;
    }
    ...
    SetGlobalArgs(&n, NOFONTS);
    SetShellColorArgs(s, &n);
    if ( (resource = IndexResource((char *) s, SHELL, "panetype")) != NULL) {
	if ( resource->variable)
            ExpandVariable(resource, (char *) s, SHELL);
	if ( !strcmp(resource->val.cval,"pack")) {
	    XtSetArg(args[n], XmNpacking, XmPACK_COLUMN);
	    n++;
	} else if ( !strcmp(resource->val.cval,"squish")) {
    ...
    if ( (resource = IndexResource((char *) s, SHELL, "orientation")) != NULL ) {
    ...
    if ( (resource = IndexResource((char *) s, SHELL, "columns")) != NULL) {
	XtSetArg(args[n], XmNnumColumns, resource->val.ival);
	n++;
    } else {

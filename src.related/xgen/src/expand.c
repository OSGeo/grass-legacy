    ...
    int max;        /* max characters to process */
{
    char save[1024],    /* for a copy of the incoming string */
         env[256],        /* room for environment variables */
         obj[256],        /* room for object values */
         tableinfo[256],/* room for table row,col specs */
 ...
 **************************************************************/
     ...
     **************************************************************/
        if (*savePtr == '\134' && *(savePtr + 1) == '\042' ) 
            savePtr++;
        if (*savePtr == '\134' && (*(savePtr + 1) == '\133' ||
            *(savePtr + 1) == '\135') ) {
            savePtr++;
            *expandPtr++ = *savePtr++;
        }

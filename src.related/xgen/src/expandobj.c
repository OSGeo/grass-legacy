/**********************************************************************
   expandobj.c  - control object reference expansion
 *********************************************************************/
/*******************************************************************************
...
void
ExpandObjectValues(resource, ptr, ptrType)
    Resource                       *resource;
    caddr_t                         ptr;
    int                             ptrType;
{
    ...
    if (resource->type == STRING) {
        ...
        case -1:
            sprintf(errorbuf,
                    "a object in string [%s] has no value ",
                    resource->varValue);
            XgenWarning("expand object value", errorbuf);
        ...
        case 1:
            sprintf(errorbuf, "string [%s] has been truncated\n",
                    resource->val.cval);
            XgenWarning("expand object", errorbuf);
            ExpandError(resource, ptr, ptrType);
            return;
        }
    }

...
char **argv;
{
	...
	if (argc < 2 || !(argc == 4 || argc == 5)) {
		fprintf (stderr,"gshhs_dp v. 1.0 Line reduction using the Douglas-Peucker algorithm\n\n");
	...
	x = (int *) get_memory (CNULL, 1, sizeof (int), "gshhs_dp");
	y = (int *) get_memory (CNULL, 1, sizeof (int), "gshhs_dp");
	index = (int *) get_memory (CNULL, 1, sizeof (int), "gshhs_dp");
	
	while (fread((char *)&h, sizeof (struct GSHHS), 1, fp_in) == 1) {
	
		if (verbose) fprintf (stderr, "Poly %6d", h.id);	
		
		x = (int *) get_memory ((char *)x, h.n, sizeof (int), "gshhs_dp");
		y = (int *) get_memory ((char *)y, h.n, sizeof (int), "gshhs_dp");
		index = (int *) get_memory ((char *)index, h.n, sizeof (int), "gshhs_dp");
		
		for (k = 0; k < h.n; k++) {
			if (fread ((char *)&p, sizeof(struct POINT), 1, fp_in) != 1) {
		...
		if (n > 2) {
			...
			h.id = n_out;
			h.n = n;
			if (fwrite ((char *)&h, sizeof (struct GSHHS), 1, fp_out) != 1) {
			...
			for (k = 0; k < n; k++) {
				p.x = x[index[k]];
				p.y = y[index[k]];
				if (fwrite((char *)&p, sizeof(struct POINT), 1, fp_out) != 1) {
...
int	x_source[];	/* Input coordinates in micro-degrees */
int	y_source[];
int     n_source;
...
int	index[];	/* output co-ordinates indeces */
{
	...
	double  x12, y12, d12, x13, y13, d13, x23, y23, d23;
 	...
 	sig_start = (int *) get_memory (NULL, n_source, sizeof (int), "Douglas_Peucker_i");
	sig_end   = (int *) get_memory (NULL, n_source, sizeof (int), "Douglas_Peucker_i");
...
char *get_memory (prev_addr, n, size, progname)

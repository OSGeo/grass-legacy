...
#define PAT_TEST 9
    ...
    if ( pattern == PAT_NONE )
	pm_error( "a base pattern must be specified", 0,0,0,0,0 );
...
int sq_radius, sq_circlecount;
pixel sq_colors[SQ_MAXCIRCLE_POINTS];
int sq_xoffs[SQ_MAXCIRCLE_POINTS], sq_yoffs[SQ_MAXCIRCLE_POINTS];
...
rnduni( )
    {
    return random() % 1000000L / 1000000.0;
    }
...
test( pixels, cols, rows, maxval )
    ...
    ppmd_line( pixels, cols, rows, maxval, cols/8, rows/8, cols-cols/8, rows/4, ppmd_fill_drawproc, fh );
    ppmd_line( pixels, cols, rows, maxval, cols-cols/8, rows/4, cols/2, rows/2, ppmd_fill_drawproc, fh );

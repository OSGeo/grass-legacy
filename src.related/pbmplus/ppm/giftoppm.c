...
int	imageNumber;
{
	...
	if (! ReadOK(fd,buf,6))
		pm_error("error reading magic number", 0,0,0,0,0);

	if (strncmp(buf,"GIF",3) != 0)
		pm_error("not a GIF file", 0,0,0,0,0);

	strncpy(version, buf + 3, 3);
	version[3] = '\0';

	if ((strcmp(version, "87a") != 0) && (strcmp(version, "89a") != 0))
		pm_error("bad version number, not '87a' or '89a'", 0,0,0,0,0);

	if (! ReadOK(fd,buf,7))
		pm_error("failed to read screen descriptor", 0,0,0,0,0);
	...
	if (BitSet(buf[4], LOCALCOLORMAP)) {	/* Global Colormap */
		if (ReadColorMap(fd,GifScreen.BitPixel,GifScreen.ColorMap))
			pm_error("error reading global colormap", 0,0,0,0,0);
	}
	...
	for (;;) {
		if (! ReadOK(fd,&c,1))
			pm_error("EOF / read error on image data", 0,0,0,0,0);

		if (c == ';') {		/* GIF terminator */
			if (imageCount < imageNumber)
				pm_error("only %d image%s found in file",
					 imageCount, imageCount>1?"s":"", 0,0,0);
			return;
		}
		...
		if (c != ',') {		/* Not a valid start character */
			pm_message("bogus character 0x%02x, ignoring", (int) c, 0,0,0,0);
			continue;
		}
		...
		if (! useGlobalColormap) {
			if (ReadColorMap(fd, bitPixel, localColorMap))
				pm_error("error reading local colormap", 0,0,0,0,0);
...
unsigned char	buffer[3][MAXCOLORMAPSIZE];
{
	...
	for (i = 0; i < number; ++i) {
		if (! ReadOK(fd, rgb, sizeof(rgb)))
			pm_error("bad colormap", 0,0,0,0,0);
...
int	label;
{
	...
	case 0xfe:		/* Comment Extension */
		...
		while (GetDataBlock(fd, buf) != 0) {
			if (showComment)
				pm_message("gif comment: %s", buf, 0,0,0,0);
		}
		return FALSE;
...
int	interlace, ignore;
{
	...
	*/
	if (ignore) {
		if (verbose)
			pm_message("skipping image...", 0,0,0,0,0);
	...
	if (verbose)
		pm_message("writing output", 0,0,0,0,0);

...
static int colorstobpp(), GetPixel(), GIFNextPixel();
static void BumpPixel(), GIFEncode(), Putword(), compress(), output(),
    cl_block(), cl_hash(), writeerr(), char_init(), char_out(), flush_char();
...
writeerr()
{
        pm_error( "error writing output file", 0,0,0,0,0 );
}

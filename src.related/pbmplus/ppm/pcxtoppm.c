    ...
    char	*argv[];
{
    ...
    if (GetByte(ifp) != PCX_MAGIC)
	 pm_error("%s is not a PCX file", ifname, 0,0,0,0);

    Version	= GetByte(ifp);	 /* get version #			*/

    if (GetByte(ifp) != 1)	 /* check for PCX run length encoding	*/
	 pm_error("%s has unknown encoding scheme", ifname, 0,0,0,0);
	...
	if (GetByte(ifp) != PCX_256_COLORS)
	    pm_error("bad color map signature", 0,0,0,0,0);
    ...
    int	 	Height;
{
    ...
    while (nbytes > 0)
    {
	 ...
	 c    = GetByte(fp);
	 if (count > nbytes)
	     pm_error("repeat count spans end of image, count = %d, nbytes = %d", count,nbytes,0,0,0);
...
int	 	bitsperpixel;
{
    ...
    if (planes > 4)
	 pm_error("can't handle more than 4 planes", 0,0,0,0,0);
    if (bitsperpixel != 1)
	 pm_error("can't handle more than 1 bit per pixel", 0,0,0,0,0);
...
int	 	bitsperpixel;
{
    ...
    if (planes != 1)
	 pm_error("can't handle packed pixels with more than 1 plane", 0,0,0,0,0);
    if (bitsperpixel == 8)
    {
...
FILE    *fp;
{
    ...
    if ((c = getc(fp)) == EOF)
	 pm_error("unexpected end of file", 0,0,0,0,0);

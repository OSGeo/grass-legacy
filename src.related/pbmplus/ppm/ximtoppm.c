...
char *argv[];
    ...
    if ( ! ReadXim( ifp, &xim ) )
	pm_error( "can't read Xim file", 0,0,0,0,0 );
    rows = xim.height;
    cols = xim.width;
	...
	pm_error(
	    ...
	    xim.nchannels, xim.bits_channel, 0,0,0 );
...
XimImage *xim;
{
    if (!ReadXimHeader(in_fp, xim)) {
        pm_message("can't read xim header", 0,0,0,0,0);
	return(0);
    }
    if (!ReadXimImage(in_fp, xim)) {
        pm_message("can't read xim data", 0,0,0,0,0);
	return(0);
    }
    return(1);
}
...
XimImage  *header;
{
	...
	*cp++ = 0;
    ...
    if (fread((char *)&a_head, sizeof(ImageHeader), 1, in_fp) != 1) {
        pm_message("ReadXimHeader: unable to read file header", 0,0,0,0,0);
        return(0);
    }
    if (atoi(a_head.header_size) != sizeof(ImageHeader)) {
        pm_message("ReadXimHeader: header size mismatch", 0,0,0,0,0);
        return(0);
    }
    if (atoi(a_head.file_version) != IMAGE_VERSION) {
        pm_message("ReadXimHeader: incorrect Image_file version", 0,0,0,0,0);
        return(0);
    }
...
*/
    ...
    if (strlen(a_head.author)) {
        if (!(header->author = calloc((unsigned int)strlen(a_head.author)+1,
                1))) {
            pm_message("ReadXimHeader: can't calloc author string", 0,0,0,0,0);
            return(0);
        }
    header->width = atoi(a_head.image_width);
    ...
    if (strlen(a_head.date)) {
        if (!(header->date =calloc((unsigned int)strlen(a_head.date)+1,1))){
            pm_message("ReadXimHeader: can't calloc date string", 0,0,0,0,0);
            return(0);
        }
    header->width = atoi(a_head.image_width);
    ...
    if (strlen(a_head.program)) {
        if (!(header->program = calloc(
            ...
            pm_message("ReadXimHeader: can't calloc program string", 0,0,0,0,0);
            return(0);
        }
    header->width = atoi(a_head.image_width);
    ...
    if (header->nchannels == 3 && header->bits_channel == 8)
        ...
        if (header->colors == NULL) {
            pm_message("ReadXimHeader: can't calloc colors", 0,0,0,0,0);
            return(0);
        }
...
XimImage  *xim;
{
    ...
    if (!(xim->data = (byte *)calloc(xim->datasize, 1))) {
        pm_message("ReadXimImage: can't malloc pixmap data", 0,0,0,0,0);
        return(0);
    }
    if (!ReadImageChannel(in_fp, xim->data, &xim->datasize, 0)) {
        pm_message("ReadXimImage: end of the images", 0,0,0,0,0);
        return(0);
    }
    if (xim->nchannels == 3) {
        ...
        if (xim->grn_data == NULL || xim->blu_data == NULL) {
            pm_message("ReadXimImage: can't malloc rgb channel data", 0,0,0,0,0);
            free((char *)xim->data);
    ...
    if (xim->alpha_flag) {
        if ((xim->other = (byte *)malloc(xim->datasize)) == NULL) {
            pm_message("ReadXimImage: can't malloc alpha data", 0,0,0,0,0);
            return(0);
        }
...
int  encoded;
{
    ...
    if (!encoded)
        ...
        if ((line=(byte *)malloc((unsigned int)BUFSIZ)) == NULL) {
            pm_message("ReadImageChannel: can't malloc() fread string", 0,0,0,0,0);
            return(0);
        }
        ...
        if (fseek(infp, marker, 0) == -1) {
            pm_message("ReadImageChannel: can't fseek to location in image buffer", 0,0,0,0,0);
            return(0);
        }
        free((char *)line);
    }
    if (j != *bufsize) {
        pm_message("unable to complete channel: %u / %u (%d%%)",
            j, *bufsize, (int)(j*100.0 / *bufsize), 0,0);
        *bufsize = j;
    }
    return(1);
}

...
char *argv[];
	...
	pm_error( "image is larger than %dx%d - sorry", COLS, ROWS, 0,0,0 );
    ...
    if ( maxval > MAXVAL )
	pm_message(
	    "maxval is not %d - automatically rescaling colors", MAXVAL, 0,0,0,0 );
	...
	for ( col = 0, pP = pixels[row]; col < cols; ++col, ++pP )
	    {
	    register int color, index, b, plane;
	    ...
	    index = 80 * row + ( ( col >> 4 ) << 2 );
	    b = 0x8000 >> (col & 0xf);
	    for ( plane = 0; plane < 4; ++plane )
		if ( color & (1 << plane) )
		    screen[index+plane] |= b;
	    }
	}

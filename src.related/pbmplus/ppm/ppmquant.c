...
static int redcompare(), greencompare(), bluecompare(), sumcompare();
...
#define FS_SCALE 1024
	...
	{
	if ( pm_keymatch( argv[argn], "-quiet", 2 ) )
	    verbose = 0;
	else if ( pm_keymatch( argv[argn], "-fs", 2 ) ||
	...
	*/
	if ( verbose )
	    pm_message( "choosing %d colors...", newcolors, 0,0,0,0 );
	...
	*/
	if ( mapmaxval != maxval )
	    {
	    if ( mapmaxval > maxval )
		pm_message( "rescaling colormap colors", 0,0,0,0,0 );
	    for ( row = 0; row < rows; ++row )
		for ( col = 0, pP = mappixels[row]; col < cols; ++col, ++pP )
	...
	if ( colormap == (colorhist_vector) 0 )
	    pm_error( "too many colors in colormap!", 0,0,0,0,0 );
	ppm_freearray( mappixels, maprows );
	if ( verbose )
	    pm_message( "%d colors found in colormap", newcolors, 0,0,0,0 );
	}
    ...
    */
    if ( verbose )
	pm_message( "mapping image to new colors...", 0,0,0,0,0 );

...
char *argv[];
	...
	{
	pm_message(
	    "too many colors - proceeding to write a HAM file", 0,0,0,0,0 );
        pm_message(
            "if you want a non-HAM file, try doing a 'ppmquant %d'",
            MAX_HAMIF_COLORS, 0,0,0,0 );
        ham = 1;
        colors = 16;
        nPlanes = 6;
	}
    else if ( chv == (colorhist_vector) 0 )
	pm_error( "too many colors - try doing a ppmquant", 0,0,0,0,0 );
    else if ( hamif && colors > MAX_HAMIF_COLORS )
	{
	pm_message(
	  "%d colors found - proceeding to write a HAM file", colors, 0,0,0,0 );
	pm_message(
	    "if you want a non-HAM file, try doing a 'ppmquant %d'",
	    MAX_HAMIF_COLORS, 0,0,0,0 );
	ham = 1;
    ...
    /* Write out the CMAP. */
    if ( maxval > 255 )
	pm_message( "maxval is not 255 - automatically rescaling colors", 0,0,0,0,0 );

...
ReadXPMFile( stream, widthP, heightP, ncolorsP, chars_per_pixelP, colorsP, dataP )
    ...
    *widthP = *heightP = *ncolorsP = *chars_per_pixelP = format = -1;
	...
	pm_message(
	    "warning, chars_per_pixel > 2 uses a lot of memory", 0,0,0,0,0 );
	...
	*colorsP = ppm_allocrow( *ncolorsP);
	...
	if ( t2 - t1 - 1 != *chars_per_pixelP )
	    pm_error(
		"wrong number of chars per pixel in color table", 0,0,0,0,0 );
	...
	while ( *t1 != '"' )
		...
		*ptr++ = v;
		...
		for ( j = 0; j < *ncolorsP; ++j )
		    if ( ptab[j] == v )
			goto gotit;
		pm_error( "unrecognized pixel in line \"%s\"", line, 0,0,0,0 );
	    gotit:
		*ptr++ = j;
		}
	    ++i;

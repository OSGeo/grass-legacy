...
char *argv[];
{
	...
	for (y = 0; y < nlines; ++y)
	{
		for (x = 0; x < cols; ++x)
		{
			...
			{
			case 0:
				pm_error("incorrect color type", 0,0,0,0,0);
...
unsigned long *cols, *nlines, *bytesperline, *bitsperpixel, *NB;
{
	...
	if (fread(headerblk, 512, 1, infile) == 0)
		pm_error("cannot read gould header", 0,0,0,0,0);
	...
	*NB           = (headerblk[23]<<24)| (headerblk[22]<<16) | (headerblk[21]<<8) | (headerblk[20]);
	...
	if (numheaderrec > 1)
		for (i = 1 ; i <numheaderrec; ++i)
			if (fread(headerblk, 512, 1, infile) == 0)
				pm_error("cannot read gould header(2nd)", 0,0,0,0,0);

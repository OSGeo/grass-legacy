#ifndef lint
static char rcsid[] = "$Header: /usr/people/sam/tiff/libtiff/RCS/tif_picio.c,v 1.21 90/10/08 17:17:54 sam Exp $";
#endif
...
#if USE_PROTOTYPES
static	int PicioStripEncode(TIFF*);
static	int PicioEncode(TIFF*, u_char *, int);
static	int PicioDecodeStrip(TIFF*);
static	int PicioCleanup(TIFF*);
static	int pic8DecodeScanline(TIFF*, u_char *, int);
static	int pic16DecodeScanline(TIFF*, u_short *, int);
static	void endpacket(struct piciostate*);
#else
	...
	TIFF *tif;
{
	...
	case 16:
		...
		break;
	default:
		TIFFError("PicioDecode", "Cannot handle %d-bit pictures",
...
static u_char *startpacket(TIFF*, u_char *, short);
#else
...
#define	OVERRUN(ps,cp,len)	(cp > ps->CAT(len,eobuffer))
	...
	register TIFF *tif;
{
	...
	tif->tif_data = malloc(sizeof (struct piciostate));
	if (tif->tif_data == NULL) {
 ...
 */
static u_char *
DECLARE3(startpacket, TIFF*, tif, u_char*, ptr, short, type)
{
	register struct piciostate *ps;
	register u_char *dp;

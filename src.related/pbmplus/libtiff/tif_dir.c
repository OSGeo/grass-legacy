#ifndef lint
static char rcsid[] = "$Header: /usr/people/sam/tiff/libtiff/RCS/tif_dir.c,v 1.64 90/10/08 17:17:36 sam Exp $";
#endif
...
"TIFF directory is missing required \"%s\" field, calculating from imagelength",
		...
		if (td->td_compression != COMPRESSION_NONE) {
			...
			for (dp = dir, n = dircount; n > 0; n--, dp++) {
				int cc =
				    dp->tdir_count * datawidth[dp->tdir_type];
				if (cc > sizeof (long))
					space += cc;
			}
			td->td_stripbytecount[0] =
			    TIFFGetFileSize(tif->tif_fd) - space;
		} else {
			u_long rowbytes;

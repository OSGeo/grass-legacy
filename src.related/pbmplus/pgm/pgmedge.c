...
char *argv[];
    ...
    int argn, rows, cols, orows, ocols, format, row;
    register int col;
    gray maxval;
    ...
    if ( cols < 3 || rows < 3 )
	pm_error( "the image is too small", 0,0,0,0,0 );
    orows = rows - 2;
    ocols = cols - 2;
    ...
    orow = pgm_allocrow( ocols );

    pgm_writepgminit( stdout, ocols, orows, maxval, 0 );
    ...
    */
	...
	/* Write out a row. */
	pgm_writepgmrow( stdout, orow, ocols, maxval, 0 );

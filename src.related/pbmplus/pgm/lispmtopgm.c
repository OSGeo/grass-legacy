/* lispmtopgm.c - read a file written by the TV:READ-BIT-ARRAY-FILE function
...
void main( argc, argv )
	...
	pm_error( "maxval of %d is too large - try recompiling with a larger gray type", maxval, 0,0,0,0 );
...
long item, bitmask;
unsigned int bitsperitem, maxbitsperitem, bitshift;
...
getinit( file, colsP, rowsP, depthP, padrightP )
    ...
    if (0 != strcmp(LISPM_MAGIC, magic))
        pm_error( "bad id string in Lispm file", 0,0,0,0,0 );
...
/*    pm_message( "inconsistent input: Width and Width(mod32) fields don't agree", 0,0,0,0,0 );  */
...
{				/* is stored that way in the file.			   */
    if (depth==0 || depth==1)	return ( 1);
      ...
      pm_error( "depth was %d, which is not in the range 1-32.", depth, 0,0,0,0 );
}

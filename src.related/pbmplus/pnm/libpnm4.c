...
pr_dump( p, out, colormap, type, copy_flag )
	...
	{
	case RT_OLD:
	pm_error( "old rasterfile type is not supported", 0,0,0,0,0 );

	case RT_FORMAT_TIFF:
	pm_error( "tiff rasterfile type is not supported", 0,0,0,0,0 );

	case RT_FORMAT_IFF:
	pm_error( "iff rasterfile type is not supported", 0,0,0,0,0 );

	case RT_EXPERIMENTAL:
	pm_error( "experimental rasterfile type is not supported", 0,0,0,0,0 );
	...
	default:
	pm_error( "unknown rasterfile type", 0,0,0,0,0 );
	}
	...
	switch ( colormap->type )
	    ...
	    default:
	    pm_error( "unknown colormap type", 0,0,0,0,0 );
	    }
	}

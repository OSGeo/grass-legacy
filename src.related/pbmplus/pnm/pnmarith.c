...
void main( argc, argv )
	...
	pm_error(
	    "the two anymaps must be the same width and height", 0,0,0,0,0 );
	...
	switch ( PNM_FORMAT_TYPE(format3) )
	    ...
	    case PPM_TYPE:
	    if ( PNM_FORMAT_TYPE(format1) != format3 )
		pm_message( "promoting first file to PPM", 0,0,0,0,0 );
	    if ( PNM_FORMAT_TYPE(format2) != format3 )
		pm_message( "promoting second file to PPM", 0,0,0,0,0 );
	    break;
#endif /*PPM*/
#ifdef PGM
	    case PGM_TYPE:
	    if ( PNM_FORMAT_TYPE(format1) != format3 )
		pm_message( "promoting first file to PGM", 0,0,0,0,0 );
	    if ( PNM_FORMAT_TYPE(format2) != format3 )
		pm_message( "promoting second file to PGM", 0,0,0,0,0 );
	    break;
#endif /*PGM*/
	    }
	}

...
void main( argc, argv )
	...
	if ( pm_keymatch( argv[argn], "-xscale", 4 ) )
	    {
	    if ( specxscale )
		pm_error( "already specified an x scale", 0,0,0,0,0 );
	    if ( specxsize )
		pm_error(
	    ...
	    {
	    if ( specyscale )
		pm_error( "already specified a y scale", 0,0,0,0,0 );
	    if ( specysize )
		pm_error(
	    ...
	    {
	    if ( specxsize )
		pm_error( "already specified a width", 0,0,0,0,0 );
	    if ( specxscale )
		pm_error(
	    ...
	    {
	    if ( specysize )
		pm_error( "already specified a height", 0,0,0,0,0 );
	    if ( specyscale )
		pm_error(

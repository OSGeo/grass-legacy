#ifndef lint
static char *SCCSID = "@(#)UVrobin.c	USGS v.3.1";
#endif
/*  Protype title */
# include	"projects.h"
...
FORWARD(s_forward); /* spheroid */
	...
	i = floor((dphi = fabs(phi)) * C1);
	if (i >= NODES) i = NODES - 1;
	dphi = RAD_TO_DEG * (dphi - RC1 * i);
	x = V(X[i], dphi) * FXC * lam;
	y = V(Y[i], dphi) * FYC;
	if (phi < 0.) y = -y;
	return (xy);
}
...
INVERSE(s_inverse); /* spheroid */
	...
	lam = x / FXC;
	phi = fabs(y / FYC);
	if (phi >= 1.) { /* simple pathologic cases */
		if (phi > ONEEPS) I_ERROR
		else {
		...
		for (i = floor(phi * NODES);;) {
			if (Y[i].c0 > phi) --i;
			else if (Y[i+1].c0 <= phi) ++i;
			else break;
		}
		T = Y[i];
		/* first guess, linear interp */
		t = 5. * (phi - T.c0)/(Y[i+1].c0 - T.c0);
		/* make into root */
		T.c0 -= phi;
...
#endif
ENTRY(robin) { if (inverse) RETURN(s_inverse); else RETURN(s_forward); }

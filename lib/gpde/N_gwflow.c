 ...
 * */
N_gwflow_data3d *
N_alloc_gwflow_data3d (int depths, int rows, int cols)
{
  N_gwflow_data3d *data;

  data = (N_gwflow_data3d *) G_calloc (1, sizeof (N_gwflow_data3d));

  data->phead = N_alloc_array_3d (depths, rows, cols, 1, G3D_DOUBLE);
  data->phead_start = N_alloc_array_3d (depths, rows, cols, 1, G3D_DOUBLE);
  data->status = N_alloc_array_3d (depths, rows, cols, 1, G3D_DOUBLE);
  data->kf_x = N_alloc_array_3d (depths, rows, cols, 1, G3D_DOUBLE);
  data->kf_y = N_alloc_array_3d (depths, rows, cols, 1, G3D_DOUBLE);
  data->kf_z = N_alloc_array_3d (depths, rows, cols, 1, G3D_DOUBLE);
  data->q = N_alloc_array_3d (depths, rows, cols, 1, G3D_DOUBLE);
  data->s = N_alloc_array_3d (depths, rows, cols, 1, G3D_DOUBLE);
  data->nf = N_alloc_array_3d (depths, rows, cols, 1, G3D_DOUBLE);
  data->r = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
...
void
N_free_gwflow_data3d (N_gwflow_data3d * data)
{
 ...
 * */
N_gwflow_data2d *
N_alloc_gwflow_data2d (int rows, int cols)
{
  N_gwflow_data2d *data;

  data = (N_gwflow_data2d *) G_calloc (1, sizeof (N_gwflow_data2d));

  data->phead = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
  data->phead_start = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
  data->status = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
  data->kf_x = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
  data->kf_y = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
  data->q = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
  data->s = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
  data->nf = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
  data->r = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
  data->top = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
  data->bottom = N_alloc_array_2d (rows, cols, 1, DCELL_TYPE);
 ...
 * */
void
N_free_gwflow_data2d (N_gwflow_data2d * data)
{
 ...
 * */
N_les_row_entries *
N_callback_gwflow_3d (void *gwdata, N_geom_data * geom, int depth, int row, int col)
{
  ...
  hc_start = N_get_array_3d_value_double (data->phead_start, depth, row, col);

  kf_x = N_get_array_3d_value_double (data->kf_x, depth, row, col);
  kf_y = N_get_array_3d_value_double (data->kf_y, depth, row, col);
  kf_z = N_get_array_3d_value_double (data->kf_z, depth, row, col);

  kf_xw = N_get_array_3d_value_double (data->kf_x, depth, row, col - 1);
  kf_xe = N_get_array_3d_value_double (data->kf_x, depth, row, col + 1);
  kf_yn = N_get_array_3d_value_double (data->kf_y, depth, row - 1, col);
  kf_ys = N_get_array_3d_value_double (data->kf_y, depth, row + 1, col);
  kf_zt = N_get_array_3d_value_double (data->kf_z, depth + 1, row, col);
  kf_zb = N_get_array_3d_value_double (data->kf_z, depth - 1, row, col);
  ...
  /*inner sources */
  q = N_get_array_3d_value_double (data->q, depth, row, col);
  /*specific yield */
  Ss = N_get_array_3d_value_double (data->s, depth, row, col);
  /*porosity */
  nf = N_get_array_3d_value_double (data->nf, depth, row, col);
  ...
  if (depth == geom->depths - 2)
    {
      r = N_get_array_2d_value_dcell (data->r, row, col);
      V += r * Az;
    }

  G_debug (5, "N_callback_gwflow_3d: called [%i][%i][%i]", depth, row, col);
 ...
 * */
N_les_row_entries *
N_callback_gwflow_2d (void *gwdata, N_geom_data * geom, int row, int col)
{
  ...
  /*read the data from the arrays */
  hc_start = N_get_array_2d_value_dcell (data->phead_start, row, col);
  hc = N_get_array_2d_value_dcell (data->phead, row, col);
  top = N_get_array_2d_value_dcell (data->top, row, col);


  if (hc > top)			/*If the aquifer is confined */
    {
      z = N_get_array_2d_value_dcell (data->top, row, col) - N_get_array_2d_value_dcell (data->bottom, row, col);
      z_xw =
	N_get_array_2d_value_dcell (data->top, row, col - 1) - N_get_array_2d_value_dcell (data->bottom, row, col - 1);
      z_xe =
	N_get_array_2d_value_dcell (data->top, row, col + 1) - N_get_array_2d_value_dcell (data->bottom, row, col + 1);
      z_yn =
	N_get_array_2d_value_dcell (data->top, row - 1, col) - N_get_array_2d_value_dcell (data->bottom, row - 1, col);
      z_ys =
	N_get_array_2d_value_dcell (data->top, row + 1, col) - N_get_array_2d_value_dcell (data->bottom, row + 1, col);
  ...
  else				/* the aquifer is unconfined */
    {
      ...
      z = N_get_array_2d_value_dcell (data->phead, row, col);
      z_xw = N_get_array_2d_value_dcell (data->phead, row, col - 1);
      z_xe = N_get_array_2d_value_dcell (data->phead, row, col + 1);
      z_yn = N_get_array_2d_value_dcell (data->phead, row - 1, col);
      z_ys = N_get_array_2d_value_dcell (data->phead, row + 1, col);
  ...
  /* Inner sources */
  q = N_get_array_2d_value_dcell (data->q, row, col);
  nf = N_get_array_2d_value_dcell (data->nf, row, col);

  /* specific yield */
  Ss = N_get_array_2d_value_dcell (data->s, row, col) * Az;
  /* reacharge */
  r = N_get_array_2d_value_dcell (data->r, row, col);

  /*get the surrounding permeabilities */
  kf_x = N_get_array_2d_value_dcell (data->kf_x, row, col);
  kf_y = N_get_array_2d_value_dcell (data->kf_y, row, col);
  kf_xw = N_get_array_2d_value_dcell (data->kf_x, row, col - 1);
  kf_xe = N_get_array_2d_value_dcell (data->kf_x, row, col + 1);
  kf_yn = N_get_array_2d_value_dcell (data->kf_y, row - 1, col);
  kf_ys = N_get_array_2d_value_dcell (data->kf_y, row + 1, col);

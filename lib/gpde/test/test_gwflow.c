...
create_gwflow_data_3d ()
{
  ...
  data = N_alloc_gwflow_data3d (TEST_N_NUM_DEPTHS_LOCAL, TEST_N_NUM_ROWS_LOCAL, TEST_N_NUM_COLS_LOCAL);

#pragma omp parallel for private (k, i, j) shared (data)
  for (k = 0; k < TEST_N_NUM_DEPTHS_LOCAL; k++)
    for (i = 0; i < TEST_N_NUM_ROWS_LOCAL; i++)
      {
	for (j = 0; j < TEST_N_NUM_COLS_LOCAL; j++)
	  {


	    if (i == 0)
	      {
		N_put_array_3d_value_double (data->phead, k, i, j, 50);
		N_put_array_3d_value_double (data->phead_start, k, i, j, 50);
		N_put_array_3d_value_double (data->status, k, i, j, 2);
	      }
	    else
	      {

		N_put_array_3d_value_double (data->phead, k, i, j, 40);
		N_put_array_3d_value_double (data->phead_start, k, i, j, 40);
		N_put_array_3d_value_double (data->status, k, i, j, 1);
	      }
	    N_put_array_3d_value_double (data->kf_x, k, i, j, 0.0001);
	    N_put_array_3d_value_double (data->kf_y, k, i, j, 0.0001);
	    N_put_array_3d_value_double (data->kf_z, k, i, j, 0.0001);
	    N_put_array_3d_value_double (data->q, k, i, j, 0.0);
	    N_put_array_3d_value_double (data->s, k, i, j, 0.001);
	    N_put_array_2d_value_dcell (data->r, i, j, 0.0);
	    N_put_array_3d_value_double (data->nf, k, i, j, 0.1);
	  }
      }
...
create_gwflow_data_2d ()
{
  ...
  data = N_alloc_gwflow_data2d(TEST_N_NUM_ROWS_LOCAL, TEST_N_NUM_COLS_LOCAL);
...
test_gwflow_3d ()
{
  ...
  /*  
   */
   /*CG*/ les = N_assemble_les_3d (N_SPARSE_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_cg (les, 100, 0.1e-8);
  N_print_les (les);
  N_free_les (les);

   /*CG*/ les = N_assemble_les_3d (N_NORMAL_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_cg (les, 100, 0.1e-8);
  N_print_les (les);
  N_free_les (les);

   /*BICG*/ les = N_assemble_les_3d (N_SPARSE_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_bicgstab (les, 100, 0.1e-8);
  N_print_les (les);
  N_free_les (les);

   /*BICG*/ les = N_assemble_les_3d (N_NORMAL_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_bicgstab (les, 100, 0.1e-8);
  N_print_les (les);
  N_free_les (les);

   /*GUASS*/ les = N_assemble_les_3d (N_NORMAL_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_gauss (les);
  N_print_les (les);
  N_free_les (les);

   /*LU*/ les = N_assemble_les_3d (N_NORMAL_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_lu (les);
  N_print_les (les);
  N_free_les (les);
...
test_gwflow_2d ()
{
  ...
  /*  
   */
   /*CG*/ les = N_assemble_les_2d (N_SPARSE_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_cg (les, 100, 0.1e-8);
  N_print_les (les);
  N_free_les (les);

   /*CG*/ les = N_assemble_les_2d (N_NORMAL_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_cg (les, 100, 0.1e-8);
  N_print_les (les);
  N_free_les (les);

   /*BICG*/ les = N_assemble_les_2d (N_SPARSE_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_bicgstab (les, 100, 0.1e-8);
  N_print_les (les);
  N_free_les (les);

   /*BICG*/ les = N_assemble_les_2d (N_NORMAL_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_bicgstab (les, 100, 0.1e-8);
  N_print_les (les);
  N_free_les (les);

   /*GAUSS*/ les = N_assemble_les_2d (N_NORMAL_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_gauss (les);
  N_print_les (les);
  N_free_les (les);

   /*LU*/ les = N_assemble_les_2d (N_NORMAL_LES, geom, data->status, data->phead_start, (void *) data, call);
  N_solver_lu (les);
  N_print_les (les);
  N_free_les (les);

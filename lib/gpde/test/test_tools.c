...
int test_mean_calc()
{
  ...
  G_message("N_calc_arith_mean: calc a %g and b %g = %g", a, b, mean);
  mean_n = N_calc_arith_mean_n(v, 2);
  G_message("N_calc_arith_mean_n: calc a %g and b %g = %g", v[0], v[1], mean_n);
  if(mean != mean_n)
  	sum++;

  /*geom mean*/
  mean = N_calc_geom_mean(a, b);
  G_message("N_calc_geom_mean: calc a %g and b %g = %g", a, b, mean);
  mean_n = N_calc_geom_mean_n(v, 2);
  G_message("N_calc_geom_mean_n: calc a %g and b %g = %g", v[0], v[1], mean_n);
  if(mean != mean_n)
  	sum++;

  /*harmonic mean*/
  mean = N_calc_harmonic_mean(a, b);
  G_message("N_calc_harmonic_mean: calc a %g and b %g = %g", a, b, mean);
  mean_n = N_calc_harmonic_mean_n(v, 2);
  G_message("N_calc_harmonic_mean_n: calc a %g and b %g = %g", v[0], v[1], mean_n);
  if(mean != mean_n)
  	sum++;
  /*null test*/
  a = 2;
  ...
  G_message("N_calc_quad_mean: calc a %g and b %g = %g", a, b, mean);
  mean_n = N_calc_quad_mean_n(v, 2);
  G_message("N_calc_quad_mean_n: calc a %g and b %g = %g", v[0], v[1], mean_n);
  if(mean != mean_n)
  	sum++;

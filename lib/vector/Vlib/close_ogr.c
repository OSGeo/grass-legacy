...
V1_close_ogr (struct Map_info *Map)
{
    ...
    if ( Map->fInfo.ogr.feature_cache ) OGR_F_Destroy( Map->fInfo.ogr.feature_cache );
...
V2_close_ogr (struct Map_info *Map)
{
	...
	if (0 >= dig__fwrite_port_L ( &length, 1, &fp)) return (1);

	/* Body */
	/* number of records  */
	if (0 >= dig__fwrite_port_I ( &(Map->fInfo.ogr.offset_num), 1, &fp)) return (1);

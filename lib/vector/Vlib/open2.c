...
int 
V2_open_old (struct Map_info *Map, char *name, char *mapset)
{
  ...
  /* not needed, done in Vect__P_init()
...
int 
V2__open_new_1 (		/* for digit  (unsupported) */
		 struct Map_info *Map,
		 char *name,
		 int with_z)
{

  if (0 > V1_open_new (Map, name, with_z))	/* open dig for write */
    return -1;
  ...
  /* not needed, done in V1_open_new()
...
int 
V2_open_update (		/* for digit, and v.cad.label */
		 struct Map_info *Map,
		 char *name)
{
}
			   ...
			   char *name)
{
  ...
  /* not needed, done in Vect__P_initi_new_plus()
  G__file_name (name_buf, "dig", name, G_mapset ());
  Map->digit_file = G_store (name_buf);
  */

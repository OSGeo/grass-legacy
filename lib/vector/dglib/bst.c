/* Produced by texiweb from libavl.w on 2002/02/19 at 20:02. */
...
bst_probe (struct bst_table *tree, void *item)
{
  ...
  int dir, height;        /* Side of |q| on which |p| is located. */
  struct bst_node *n;     /* Newly inserted node. */

  assert (tree != NULL && item != NULL);

  for (height = 0, q = NULL, p = tree->bst_root; p != NULL; q = p, p = p->bst_link[dir], height++ )
    {
      int cmp;

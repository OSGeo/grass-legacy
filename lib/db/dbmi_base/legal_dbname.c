...
int db_legal_tablename (char *s)
{
    ...
    if (*s == '.' || *s == 0) {
	fprintf(stderr, _("Illegal table map name <%s>. May not contain '.' or 'NULL'.\n"), buf);
	return DB_FAILED;
    }

    /* file name must start with letter */
    if (! ((*s >= 'A' && *s <= 'Z') || (*s >= 'a' && *s <= 'z')) ) {
	fprintf(stderr, _("Illegal table map name <%s>. Must start with a letter.\n"), buf);
	return DB_FAILED;
    }

    for (s++ ; *s; s++)
	if (! ((*s >= 'A' && *s <= 'Z') || (*s >= 'a' && *s <= 'z') || (*s >= '0' && *s <= '9') || *s == '_' || *s == '@' ) ) {
		fprintf(stderr, _("Illegal table map name <%s>. Character <%c> not allowed.\n"), buf, *s);
	    return DB_FAILED;
	}

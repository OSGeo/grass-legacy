...
int iso_get_cube_value(geovol_isosurf *isosurf, int desc, int x, int y, int z, float *v)
{
    ...
    if (type == VOL_DTYPE_FLOAT) {
        gvl_file_get_value(vf, (int) (x * ResX), (int) (y * ResY), (int) (z * ResZ), v);
    } else if (type == VOL_DTYPE_DOUBLE) {
        gvl_file_get_value(vf, (int) (x * ResX), (int) (y * ResY), (int) (z * ResZ), &d);
        *v = (float) d;
    } else {
        return 0;
    }
    ...
    switch (desc) {
        case (ATT_TOPO) :
			*v = (*v) - isosurf->att[desc].constant;
			break;
        case (ATT_MASK) :
            if (isosurf->att[desc].constant) ret = !ret;
            break;
    }
...
int iso_get_cube_values(geovol_isosurf *isosurf, int desc, int x, int y, int z, float *v)
{
	...
	for(p = 0 ; p < 8 ; ++p ) {
    	if (iso_get_cube_value(isosurf, desc, x+((p^(p>>1))&1), y+((p>>1)&1), z+((p>>2)&1), &v[p]) == 0) {
			ret = 0;
		}
	}
...
void iso_get_cube_grads(geovol_isosurf *isosurf, int x, int y, int z, float (*grad)[3])
{
	...
	for(p = 0 ; p < 8 ; ++p ) {
		i = x+((p^(p>>1))&1); j = y+((p>>1)&1); k = z+((p>>2)&1);
...
void iso_calc_cube(geovol_isosurf *isosurf, int x, int y, int z, data_buffer *dbuff)
{
	...
	/* get color values */
	if (isosurf->att[ATT_COLOR].changed && isosurf->att[ATT_COLOR].att_src == MAP_ATT) {
		iso_get_cube_values(isosurf, ATT_COLOR, x, y, z, val[ATT_COLOR]);
	}

	/* get transparency values */
	if (isosurf->att[ATT_TRANSP].changed && isosurf->att[ATT_TRANSP].att_src == MAP_ATT) {
		iso_get_cube_values(isosurf, ATT_TRANSP, x, y, z, val[ATT_TRANSP]);
	}

	/* get shine values */
	if (isosurf->att[ATT_SHINE].changed && isosurf->att[ATT_SHINE].att_src == MAP_ATT) {
		iso_get_cube_values(isosurf, ATT_SHINE, x, y, z, val[ATT_SHINE]);
	}

	/* get emit values */
	if (isosurf->att[ATT_EMIT].changed && isosurf->att[ATT_EMIT].att_src == MAP_ATT) {
	...
	for (i = 0; i < cell_table[c_ndx].nedges; i++) {
		...
		if (isosurf->att[ATT_TOPO].changed) {
			/* interior vertex */
			if (crnt == 12) {
				FOR_0_TO_N(3, WRITE((d3[FOR_VAR] = d_sum[FOR_VAR]/((float) (cell_table[c_ndx].nedges)))*255));
				GS_v3norm(n_sum);
				FOR_0_TO_N(3, WRITE((n_sum[FOR_VAR]/((float) (cell_table[c_ndx].nedges)) + 1.) * 127));
			/* edge vertex */
			} else {
				/* set egdes verts */
				v1 = edge_vert[crnt][0]; v2 = edge_vert[crnt][1];
				...
				FOR_0_TO_N(3, n[FOR_VAR] = LINTERP(d, grad[v1][FOR_VAR], grad[v2][FOR_VAR]));
				GS_v3norm(n);
			...
			if (crnt == 12) {
				WRITE(c = READ()); d3[0] = ((float) c) / 255.0;
				WRITE(c = READ()); d3[1] = ((float) c) / 255.0;
				WRITE(c = READ()); d3[2] = ((float) c) / 255.0;
			} else {
				/* set egdes verts */
				v1 = edge_vert[crnt][0]; v2 = edge_vert[crnt][1];
		...
		if (isosurf->att[ATT_COLOR].changed && isosurf->att[ATT_COLOR].att_src == MAP_ATT) {
			...
			if (IS_IN_DATA(ATT_COLOR)) SKIP(3);
		} else {
		...
		if (isosurf->att[ATT_TRANSP].changed && isosurf->att[ATT_TRANSP].att_src == MAP_ATT) {
			...
			WRITE(c);
			if (IS_IN_DATA(ATT_TRANSP)) SKIP(1);
		} else {
		...
		if (isosurf->att[ATT_SHINE].changed && isosurf->att[ATT_SHINE].att_src == MAP_ATT) {
			...
			WRITE(c);
			if (IS_IN_DATA(ATT_SHINE)) SKIP(1);
		} else {
    ...
    for (i = 0; i < gvol->n_isosurfs; i++) {
		...
		if (need_update[i]) {
			if (dbuff[i].num_zero != 0)
				gvl_write_char(dbuff[i].ndx_new++, &(dbuff[i].new), dbuff[i].num_zero);
...
void gvl_write_char(int pos, unsigned char **data, unsigned char c)
{
	...
	if ((pos % BUFFER_SIZE) == 0) {
		if ((*data = (char *) G_realloc(*data, sizeof(char) * ((pos / BUFFER_SIZE) + 1) * BUFFER_SIZE)) == NULL) {
			gs_err("can't malloc");
			return;
		}
...
float slice_get_value(geovol *gvl, int x, int y, int z)
{
	...
	if (x < 0 || y < 0 || z < 0 || (x > gvl->cols - 1)  || (y > gvl->rows - 1) || (z > gvl->depths - 1))
		return 0.;
...
int slice_calc(geovol *gvl, int ndx_slc, void *colors)
{
	...
	if (slice->dir == X) {
		modx = ResY; mody = ResZ; modz = ResX;
		p_x = &k; p_y = &i; p_z = &j;
		p_ex = &ek; p_ey = &ei; p_ez = &ej;
	} else if (slice->dir == Y) {
		modx = ResX; mody = ResZ; modz = ResY;
		p_x = &i; p_y = &k; p_z = &j;
		p_ex = &ei; p_ey = &ek; p_ez = &ej;
	} else {
		modx = ResX; mody = ResY; modz = ResZ;
		p_x = &i; p_y = &j; p_z = &k;
		p_ex = &ei; p_ey = &ej; p_ez = &ek;
	}

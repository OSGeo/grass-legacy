...
int
Gs_loadmap_as_float(wind, map_name, buff, nullmap, has_null)
...
/*
    if(NULL == (nullflags = (char *)malloc(wind->cols*sizeof(char)))){
        sprintf(err_buff,"Not able to allocate null buffer for [%s]", map_name);
        Gs_warning(err_buff);
	return(-1);
    }
...
int
Gs_loadmap_as_int(wind, map_name, buff, nullmap, has_null)
struct Cell_head *wind;
char *map_name;
int *buff;
struct BM *nullmap;
...
int
Gs_loadmap_as_short(wind, map_name, buff, nullmap, has_null)
struct Cell_head *wind;
char *map_name;
short *buff;
struct BM *nullmap;
...
Gs_pack_colors_float(filename, fbuf, ibuf, rows, cols)
char *filename;
float *fbuf;
int *ibuf;
int rows, cols;
...
Gs_get_cat_label(filename,drow,dcol,catstr)
char *filename, *catstr;
int drow, dcol;
{
...
Gs_save_3dview(vname, gv, gd, w, defsurf)
char *vname;
geoview *gv;
geodisplay *gd;
struct Cell_head *w;
geosurf *defsurf;
{
    ...
    if(gv->lights[0].position[W] == 1){       /* local */
...
/*
    strcpy((v.pgm_id), G_program_name());  
*/
...
Gs_load_3dview(vname, gv, gd, w, defsurf)
char *vname;
geoview *gv;
geodisplay *gd;
struct Cell_head *w;
...
/*
	if(strcmp((v.pgm_id), G_program_name()))
*/
	...
	int dmode=0;
	    ...
	    while(v.display_type >= 10){ /* globe stuff not used */
		v.display_type -= 10;
	    }
	    if(v.colorgrid)
		dmode &= DM_COL_WIRE;

...
/*- prototypes --------------------------------------------------------------*/
void fatalError (void *map, char *errorMsg);	/*Simple Error message */
...
/* ************************************************************************* */
void
fatalError (void *map, char *errorMsg)
{
...
setParams ()
{
  ...
  param.input->description = _("3dcell map(s) to be converted to 2D raster slices");
...
/* ************************************************************************* */
void
G3dToRaster (void *map, G3D_Region region, int *fd)
{
  double d1 = 0;
  double *d1p;
  float *f1p;
  ...
  G_debug (3, _("G3dToRaster: Writing %i raster maps with rows %i cols."), depths, rows, cols);
  ...
  /*for (z = 0; z < depths; z++)
    {*/
      G_debug (2, _("Writing raster map %i\n"), z + 1);
      for (y = 0; y < rows; y++)
	{
	  G_percent (y, rows, 2);
	  ...
	  if (typeIntern == G3D_FLOAT)
	    G_put_f_raster_row (fd[pos], fcell);

	  if (typeIntern == G3D_DOUBLE)
	    G_put_d_raster_row (fd[pos], dcell);
	}
      G_debug (2, _("\nDone\n"));
      pos++;
    }
...
main (int argc, char *argv[])
{
  ...
  map = G3d_openCellOld (param.input->answer,
			 ...
			 G3D_DEFAULT_WINDOW, G3D_TILE_SAME_AS_FILE, G3D_USE_CACHE_DEFAULT);
  ...
  if (output_type == G3D_FLOAT || output_type == G3D_DOUBLE)
	...
	{
	  fatalError (map, _("Resolution of raster and raster3d maps should be equal!"));
	}
      ...
      if (fd == NULL)
	fatalError (map, _("out of memory!"));

      if (G_legal_filename (param.output->answer) < 0)
	  fatalError (map, _("Illegal output file name"));
    ...
    {
      fatalError (map, _("Wrong G3D Datatype! Cannot create raster maps."));
    }

  /* Close files and exit */
  if (!G3d_closeCell (map))
    fatalError (map, _("Error closing g3d file"));
...
/* ************************************************************************* */
int
open_output_map (const char *name, int res_type)
{
  int fd;

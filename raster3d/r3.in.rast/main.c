...
setParams ()
{
  ...
  param.output->description = _("output 3dcell map which will be filled with the 2D raster slices");
}
...
/* ************************************************************************* */
void
RasterToG3D (void *map, G3D_Region region, int *fd)
{
  int x, y, z;
  ...
  G_debug (3, _("RasterToG3D: Writing %i raster maps with rows %i cols."), depths, rows, cols);
...
main (int argc, char *argv[])
{
  ...
  module->description = _("Converts 2D raster map slices to one 3D raster map");
  ...
  if (rows != region.rows || cols != region.cols)
    {
      fatalError (map, NULL, 0, _("The resolution of raster and raster3d maps should be equal!"));
    }
  ...
  for (i = 0; i < region.depths; i++)
	...
	{
	  fatalError (map, fd, opencells, _("Input maps have to be from the same type. CELL, FCELL or DCELL!"));
	}
    }
  ...
  if (global_rastMapType == CELL_TYPE)
    {
      map = G3d_openCellNew (param.output->answer, G3D_FLOAT, G3D_USE_CACHE_DEFAULT, &region);
    ...
    {
      map = G3d_openCellNew (param.output->answer, G3D_DOUBLE, G3D_USE_CACHE_DEFAULT, &region);
      global_g3dMapType = G3D_DOUBLE;
    }
...
/* ************************************************************************* */
int
open_input_map (char *name, char *mapset)
{
  int fd;

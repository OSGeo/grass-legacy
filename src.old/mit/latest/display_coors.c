	...
	int *nparams;
{
	...
	if(x < ((int) xl) || x > ((int) xr)
		|| y < ((int) yt) || y > ((int) yb))
		{	
		XClearWindow(the_display, coor_win);	
		return;
		}
	easting = (int) (window.west +
		(x - xl) * e_minus_w / (xr - xl));
	northing = (int) (window.north -
		(y - yt) * n_minus_s / (yb - yt));
	...
	XClearWindow(the_display, coor_win);	
	XDrawString(the_display, coor_win, coor_gc,
		4, 11, str, strlen(str));
	XFlush(the_display);

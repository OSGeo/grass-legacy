...
int isosurf_get_att(int id, Tcl_Interp *interp, int argc, char *argv[])
{
    ...
    GVL_isosurf_get_att(id, isosurf_id, isosurf_att_atoi(argv[4]), &set, &c, mapname);
...
int isosurf_set_att(int id, Tcl_Interp *interp, int argc, char *argv[])
{
    ...
    if (att < 0) {
        Tcl_SetResult(interp, "Internal Error: unknown attribute name in set_att",
            TCL_VOLATILE);
        return (TCL_ERROR);
    }
...
int isosurf_unset_att(int id, Tcl_Interp *interp, int argc, char *argv[])
{
    ...
    return ((0 > GVL_isosurf_unset_att(id, isosurf_id, att)) ? TCL_ERROR : TCL_OK);
}
...
int isosurf_set_mask_mode(int id, Tcl_Interp *interp, int argc, char *argv[])
{
    ...
    Tcl_SetResult(interp, "Error: must be BOOLEAN",
              TCL_VOLATILE);
    return (TCL_ERROR);
    }
    else {
...
int slice_get_pos(int id, Tcl_Interp *interp, int argc, char *argv[])
{
	...
	if (GVL_slice_get_pos(id, slice_id, &x1, &x2, &y1, &y2, &z1, &z2, &dir) == -1) {
...
int slice_add(int id, Tcl_Interp *interp, int argc, char *argv[])
{
    if (GVL_slice_add(id) == -1) {
        Tcl_SetResult(interp, "Error: unable to add slice",
            TCL_VOLATILE);
        return (TCL_ERROR);
    }
...
int slice_del(int id, Tcl_Interp *interp, int argc, char *argv[])
{
    ...
    if (argc != 4) {
    Tcl_SetResult(interp,
            "Usage: <map_obj> slice del slice_id",
            TCL_VOLATILE);
    return (TCL_ERROR);
    }
    ...
    if (GVL_slice_del(id, slice_id) == -1) {
        Tcl_SetResult(interp, "Error: unable to delete slice",
            TCL_VOLATILE);
        return (TCL_ERROR);
    }

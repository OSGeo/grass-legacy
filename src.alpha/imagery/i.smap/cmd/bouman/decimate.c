...
void make_pyramid(ll_pym,region,M,alpha,vlevel)
LIKELIHOOD ****ll_pym;  /* log likelihood pyramid, ll_pym[scale][i][j][class] */
...
int vlevel;             /* verbose operation */
{
    ...
    while((wd>2)&&(ht>2)) {
      if(vlevel>=2)
        printf("D = %d  alpha = %lf; 1-alpha = %lf\n",D,alpha[D],1-alpha[D]);
...
static void decimate(ll1,region1,M,ll2,alpha)
...
static void up_ll(pt1,M,alpha,pt2)
LIKELIHOOD  *pt1; /* array of log likelihood values, pt1[class] */
...
double  alpha;
{
    ...
    if(alpha!=1.0) {
      ...
      for(m=0; m<M; m++) pt2[m] += pt1[m];
    }
}

char ***get_pyramid(w0,h0,size)
int w0,h0; 
unsigned   size;
{
    char ***pym;
...
int wd,ht;
{
    ...
    D=levels(wd,ht);
    for(i=0; i<=D; i++) free_img(pt[i]);
    free(pym);
}

char ****get_cubic_pyramid(w0,h0,M,size)
int w0,h0;
int M; 
unsigned   size;
{
...
void free_cubic_pyramid(pym,wd,ht,M)
char *pym;
int wd,ht;
int M;
{
    ...
    pt = (char **)pym;
    D=levels(wd,ht);
    for(i=0; i<=D; i++) multifree(pt[i],M);
    free(pym);
}

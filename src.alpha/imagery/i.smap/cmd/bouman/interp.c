...
void seq_MAP(sf_pym,region,ll_pym,M,alpha_dec,vlevel)
...
int  vlevel;                /* verbose output */
{
    ...
    for(repeat=0; repeat<2; repeat++) {
      ...
      if(vlevel>=2) printf("pyramid constructed\n");
...
static void seq_MAP_routine(sf_pym,region,ll_pym,M,alpha_dec,vlevel)
unsigned char ***sf_pym;    /* pyramid of segmentations */
LIKELIHOOD ****ll_pym;      /* pyramid of class statistics */
struct Region *region;      /* specifies image subregion */
int  M;                     /* number of classes */
...
int  vlevel;                /* verbose output */
{
    ...
    for(D--; D>=0; D-- ) {
      ...
      do { 
        ...
        if(vlevel>=4) printf("log likelihood = %lf\n\n",log_like(N,alpha,M));
...
static void print_N(N)
/* prints out class transition statistics */
double ***N;
{
    int n0,n1,n2;

    printf("Class transition statistics\n");
    for(n0=0; n0<2; n0++) {
      for(n1=0; n1<3; n1++) {
        for(n2=0; n2<2; n2++) printf("%lf ",N[n0][n1][n2]);
        printf("\n");
      }
      printf("\n");
    }
...
static void print_alpha(alpha)
/* prints out transition parameters. */
double *alpha;
{
    printf("Transition probabilities %lf %lf %lf;"
      ,alpha[0],alpha[1],alpha[2]);  
    printf(" %lf\n",1.0-alpha[0]-2*alpha[1]-alpha[2]);
}


static void interp(sf1,region,sf2,ll,M,alpha,period,N,statflag)
...
void MLE(sf,ll,region,M)
/* computes maximum likelihood classification */
...
static int up_char(i,j,region,img,pt)
...
int i,j;                    /* fine resolution pixel location */

...
#define FMT2(buf,e1,n1,e2,n2) \
	sprintf (buf, "%9.1lf %9.1lf %9.1lf %9.1lf ", e1,n1,e2,n2)
...
/* allocate predicted values */
    xres = (double *) G_calloc (group.ref_points.count, sizeof (double));
    yres = (double *) G_calloc (group.ref_points.count, sizeof (double));
    gnd  = (double *) G_calloc (group.ref_points.count, sizeof (double));
...
/* lets do it */
	...
	while(1)
	{
	    if (line >= nlines || curp >= group.ref_points.count)
		break;
	    line++;
	    if(group.ref_equation_stat > 0 && group.ref_points.status[curp] > 0)
	    {
		color = BLACK;
	    ...
	    else if (group.ref_points.status[curp] > 0)
		...
		FMT2(buf,
		    group.ref_points.e1[curp],
		    group.ref_points.n1[curp],
		    group.ref_points.e2[curp],
		    group.ref_points.n2[curp]);
	...
	report.bottom = cury;
	downarrow (&more, curp < group.ref_points.count ? BLACK : BACKGROUND);
...
pick(x,y)
{
    ...
    if (inbox(&more,x,y))
    {
	if (curp >= group.ref_points.count)
	    return 0;
	first_point = curp;
	pager = 1;
	return 1;
...
/*      debug ("have second click  n = %d", n);  */
	group.ref_points.status[first_point+n] = !group.ref_points.status[first_point+n];
	compute_transformation();
 ...
 */
    ...
    rms = 0.0;
    count = 0;
    for (n = 0; n < group.ref_points.count; n++)
    {
	if (group.ref_points.status[n] <= 0) continue;
	count++;
	I_georef (group.ref_points.e2[n], group.ref_points.n2[n], &e1, &n1, group.E21, group.N21);
	I_georef (group.ref_points.e1[n], group.ref_points.n1[n], &e2, &n2, group.E12, group.N12);

	if((d = xres[n] = e1-group.ref_points.e1[n]) < 0)
	    d = -d;
	if (d > xval) 
	{
	    xmax = n;
	...
	if ((d = yres[n] = n1-group.ref_points.n1[n]) < 0)
	    d = -d;
	if (d > yval)
	{
	    ymax = n;
...
/* compute ground error (ie along diagonal) */
	d1 = e2 - group.ref_points.e2[n];
	d2 = n2 - group.ref_points.n2[n];
	d = d1*d1 + d2*d2;
    ...
    FILE *fd;
{
	...
	FMT0(buf,n+1);
	fprintf (fd, "%s", buf);
	if(group.ref_equation_stat > 0 && group.ref_points.status[n] > 0)
	{
	    FMT1(buf, xres[n], yres[n], gnd[n]);
	    fprintf (fd, "%s", buf);
	}
	else if (group.ref_points.status[n] > 0)
	    printcentered (fd, "?", width);
	else
	    printcentered (fd, "not used", width);
	FMT2(buf,
	    group.ref_points.e1[n],
	    group.ref_points.n1[n],
	    group.ref_points.e2[n],
	    group.ref_points.n2[n]);
	fprintf (fd, "   %s\n", buf);
    }
    fprintf (fd, "\n");
    if (group.ref_equation_stat < 0)
...
show_point (n, true_color)
{
    ...
    else if(group.ref_points.status[n])
	R_standard_color (GREEN);
    else
	R_standard_color (RED);
    display_one_point (VIEW_MAP1, group.ref_points.e1[n], group.ref_points.n1[n]);
}

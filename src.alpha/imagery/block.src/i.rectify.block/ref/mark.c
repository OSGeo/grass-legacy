...
/* convert x,y to east,north at center of cell */
    ...
    sprintf (buf, "Point %d marked at IMAGE COORDINATES:", group.ref_points.count+1);
    ...
    else
    {
        ...
        sprintf (buf, "Point %d referenced to PHOTO COORDINATES:", group.ref_points.count+1);
	...
	I_new_ref_point (&group.ref_points, e1, n1, e2, n2, 1);
...
/* allocate predicted values */
    Xf = (double *) G_calloc (block.camera.num_fid, sizeof (double));
    Yf = (double *) G_calloc (block.camera.num_fid, sizeof (double));
...
/*    curp = first_point = 0;   */
	...
	while(1)
	{
	    if (line >= nlines || curp >= block.camera.num_fid)
		break;
	    line++;
        	color = BLACK;
	    if (pager)
	    {
		FMT1(buf, block.camera.fiducials[curp].fid_id, block.camera.fiducials[curp].Xf, block.camera.fiducials[curp].Yf);
	...
	downarrow (&more, curp < block.camera.num_fid ? BLACK : BACKGROUND);
...
/* debug ("Geting point %d   E = %f  N = %f",n, E, N);*/
        ...
        E = block.camera.fiducials[first_point+n].Xf;
        N = block.camera.fiducials[first_point+n].Yf;

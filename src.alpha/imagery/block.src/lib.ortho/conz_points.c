    ...
    struct Con_Points *cp;
{
	...
	if (cp->status[i] >= 0)
	    fprintf (fd, "  %15lf %15lf %15lf %15lf %15lf %15lf %4d\n",
		cp->e1[i], cp->n1[i], cp->z1[i], cp->e2[i], cp->n2[i],  cp->z2[i], cp->status[i]);
}

I_get_con_points (group, cp)
    char *group;
    struct Con_Points *cp;
{
    FILE *fd;
...
I_convert_con_points (group, con_cp, photo_cp, E12, N12)
    char *group;
    struct Con_Points *con_cp, *photo_cp;
    double E12[3], N12[3];
{
    FILE *fd;
    char msg[100];

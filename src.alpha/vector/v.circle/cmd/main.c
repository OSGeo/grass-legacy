...
Area = %lf square feet\n",radius,((double)M_PI*(pow(radius,(double)2.0))) );
          ...
          */
          ...
          area = (double)M_PI * (pow(radius,(double)2.0)); 
          fprintf(stderr,"Radius = %lf meters  \
Area = %lf square meters\n",radius,area);
         }
        else
         {
...
Please choose a different vector file name.\n",
  ...
  local_head = (struct dig_head *) malloc (sizeof(struct dig_head)); 
  Date(today);
    ...
    if ( count == 1 )
     {
      ...
      fprintf(stderr,
      "%d circle was produced in vector file <%s>.\n",
      count, output);
     }
    else
     {
      /* Provide user with number of circles produced in vector file. */
      fprintf(stderr,
      "%d circles were produced in vector file <%s>.\n",
      count, output);
     }
   }

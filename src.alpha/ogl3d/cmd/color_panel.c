...
/*
	n = 0;
	SetPositionArgs(wargs, &n, 25, 40, 30, 50, NULL);
	XtSetArg(wargs[n], XmNbackground,dc->cells[SITES_CELL]); n++;
	dc->toggle_id[S_COLOR] = XtCreateManagedWidget("", 
	...
	n = 0;
	SetPositionArgs(wargs, &n, 40, -1, 30, -1, XmATTACH_NONE);
...
/*--- set the current color and make the colorbar for the background ---*/	
	...
	n = 0;
	SetPositionArgs(wargs, &n, 60, 80, 30, 70, NULL);

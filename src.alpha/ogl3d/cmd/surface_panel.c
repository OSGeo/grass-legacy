...
void surface_panel(w, dc, call_data)
Widget w;
data_cell *dc;
caddr_t call_data;
{
...
------------------------------------------------------------*/
	...
	n = 0;
	SetPositionArgs(wargs, &n, 19,32,-1,50, XmATTACH_FORM);
...
-------------------------------------------------------------------*/
	n = 0;
	SetPositionArgs(wargs, &n, 33,38,60,90, XmATTACH_FORM);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 39,50,60,-1, XmATTACH_FORM);
...
-------------------------------------------------------------------*/
	n = 0;
	SetPositionArgs(wargs, &n,  5,30,60,99, NULL);
	XtSetArg(wargs[n],XmNradioBehavior,TRUE); n++;
...
-------------------------------------------------------------------*/
	n = 0;
	SetPositionArgs(wargs, &n, 35,50,1,50, NULL);
	XtSetArg(wargs[n],XmNradioBehavior,TRUE); n++;
...
/* seperator for current surface status */
	...
	n = 0;
	SetPositionArgs(wargs, &n, 50,52,-1,-1, XmATTACH_FORM);
	sep = XtCreateManagedWidget("",xmSeparatorWidgetClass,
...
/* make new & delete buttons */
	...
	n = 0;
	SetPositionArgs(wargs, &n, 54,60,62,78, NULL);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 54,60,82,98, NULL);
	wtmp = XtCreateManagedWidget("Delete", 
		  xmPushButtonWidgetClass, dc->panels[SURFACE].form, wargs, n);
	XtAddCallback(wtmp, XmNactivateCallback,delete_surf,dc);	

	n = 0;
	SetPositionArgs(wargs, &n, 61, -1, 5, -1, XmATTACH_NONE);
...
*/
	...
	for(i = 1 ; i < MAX_ATTS; i++){
	    ...
	    n = 0;
	    SetPositionArgs(wargs, &n, t, b, 5, 30, NULL);
...
*/
	    ...
	    n = 0;
	    SetPositionArgs(wargs, &n, t, b, 32, 75, NULL);
	    XtSetArg(wargs[n],XmNalignment, XmALIGNMENT_BEGINNING); n++;
	    ...
	    if(i == ATT_TOPO){
		n = 0;
		SetPositionArgs(wargs, &n, t, b, 76, 99, NULL);

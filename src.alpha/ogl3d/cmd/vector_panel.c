...
/* make new & delete buttons */
	...
	n = 0;
	SetPositionArgs(wargs, &n, 10,25,62,78, NULL);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 10,25,82,98, NULL);
...
/* make Arrows and text for selecting line width */
	...
	n = 0;
	SetPositionArgs(wargs, &n, 30, -1, 4, -1, XmATTACH_NONE);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 60, -1, 4, -1, XmATTACH_NONE);
...
/* make ScrolledWindow with RowCol containing check boxes to mark
   which surfaces current vector is to be displayed on */
	...
	n = 0;
	SetPositionArgs(wargs, &n, 30, -1, 45, -1, XmATTACH_NONE);

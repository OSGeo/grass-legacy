...
/***********************************************************************/
void
cplane_translate(w, dc, call_data)
    Widget w;
    data_cell *dc;
    caddr_t    *call_data;
{
...
/* make widgets */
	...
	n = 0;
	SetPositionArgs(wargs, &n, 10, -1, 60, -1, XmATTACH_NONE);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 32, -1, 55, -1, XmATTACH_NONE);
	...
	XtSetArg(wargs[n],XmNscaleWidth,25); n++;
	...
	n = 0;
	SetPositionArgs(wargs, &n, 32, -1, 75, -1, XmATTACH_NONE);
	...
	n = 0;
        SetPositionArgs(wargs, &n, 20,30,50,-1, XmATTACH_FORM);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 10, -1, 0, -1, XmATTACH_NONE);
	...
	n = 0;
	XtCreateManagedWidget("Z:", xmLabelGadgetClass, trc, wargs, n);
	
	n = 0;
	dc->slider_txt[CPL_ZTR] = make_text(trc, "", 8,"ztr_txt", 
	...
	/* XY positioner */
	n = 0;
	SetPositionArgs(wargs, &n, 10, -1, 20, -1, XmATTACH_NONE);
	XtSetArg(wargs[n], XmNwidth, 120); n++;
	XtSetArg(wargs[n], XmNheight, 120); n++;
	...
	n = 0;
	SetPositionArgs(wargs, &n, 75, -1, 22, -1, XmATTACH_NONE);
	XtCreateManagedWidget("X:", xmLabelGadgetClass,
		    dc->panels[CPLANE].form, wargs, n);
	
	n = 0;
	SetPositionArgs(wargs, &n, 72, -1, 29, -1, XmATTACH_NONE);
	X_txt = make_text(dc->panels[CPLANE].form, "", 8,"cp_xtr_txt", 
		    update_xytrans, dc,wargs,n);
	
	n = 0;
	SetPositionArgs(wargs, &n, 85, -1, 22, -1, XmATTACH_NONE);
	XtCreateManagedWidget("Y:", xmLabelGadgetClass,
		    dc->panels[CPLANE].form, wargs, n);
	
	n = 0;
	SetPositionArgs(wargs, &n, 82, -1, 29, -1, XmATTACH_NONE);

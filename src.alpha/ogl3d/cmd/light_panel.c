...
void light_panel(w,dc,call_data)
Widget w;
data_cell *dc;
caddr_t call_data;
{
	...
	n = 0;
	SetPositionArgs(wargs, &n, 5, 10, 3, -1, XmATTACH_NONE);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 10, 15, 3, -1, XmATTACH_NONE);
...
----------------------------------------------------------------------*/
	...
	n = 0;
	SetPositionArgs(wargs, &n, 15, -1, 2, 45, XmATTACH_NONE);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 15, -1, 46, -1, XmATTACH_NONE);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 25, -1, 2, 45, XmATTACH_NONE);
	XtSetArg (wargs[n], XmNorientation, XmHORIZONTAL); n++;
	...
	n = 0;
	SetPositionArgs(wargs, &n, 25, -1, 45, -1, XmATTACH_NONE);
	...
	/*-- Red slider and text --*/
	n = 0;
	SetPositionArgs(wargs, &n, 45, -1, 2, 40, XmATTACH_NONE);
	XtSetArg (wargs[n], XmNorientation, XmHORIZONTAL); n++;
	...
	n = 0;
	XtSetArg(wargs[n],XmNscaleHeight,20); n++;
	XtSetArg(wargs[n],XmNscaleWidth,120); n++;
	...
	n = 0;
	SetPositionArgs(wargs, &n, 45, -1, 40, -1, XmATTACH_NONE);
	dc->slider_txt[LITE_RED] = make_text(dc->panels[LIGHT].form, "", 
		5, "bright", update_sliders, dc, wargs, n);

	/*-- Green slider and text --*/
	n = 0;
	SetPositionArgs(wargs, &n, 55, -1, 2, 40, XmATTACH_NONE);
	XtSetArg(wargs[n], XmNorientation, XmHORIZONTAL); n++;
	...
	n = 0;
	SetPositionArgs(wargs, &n, 65, -1, 40, -1, XmATTACH_NONE);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 42, 75, 65, 75, NULL);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 42, -1, 75, -1, XmATTACH_NONE);
	XtCreateManagedWidget("Height", xmLabelGadgetClass, dc->panels[LIGHT].
		form, wargs, n);

	n = 0;
	SetPositionArgs(wargs, &n, 47, -1, 75, -1, XmATTACH_NONE);
	...
	n = 0; 
	SetPositionArgs(wargs, &n, 5, 33, 63, 98, NULL);
	...
	n = 0;
	SetPositionArgs(wargs, &n, 35, -1, 63, 98, XmATTACH_NONE);

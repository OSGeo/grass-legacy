...
main (argc, argv) char *argv[];
{
	...
	int nrows, ncols;
	int i;
	static int missingval,mval;
	double mult_fact ;
	double x;
	struct
	{
		struct Option *input, *output, *title, *mult, *miss;
	} parm;
	...
	parm.input->description = "ARC/INFO ascii raster file to be imported";
	...
	parm.miss = G_define_option();
	parm.miss->key = "missval";
	parm.miss->type = TYPE_INTEGER;
	parm.miss->answer = "0";
	parm.miss->required = NO;
	parm.miss->description = "Missing value" ;
	...
	sscanf(parm.miss->answer,"%d",&mval) ;

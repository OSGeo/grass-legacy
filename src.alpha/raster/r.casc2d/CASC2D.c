...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
     ...
     int    *chn_row;
     int    *chn_col;
     double wid,depth,z,topwid,volex,dhch,qtoch,htop;
     double elev1,elev2;
     double manout,wetpout,areaout,qoutch,qout;

     int    itmp,jtmp,nodetmp;
     double dummy;
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
     ...
     FILE  *iunit,*yunit,*qunit,*iunit9,*phydunit,*qla;
 ...
 */
	...
	if((rgint=(double*) malloc(nrg*sizeof(double))) == NULL) fprintf(stderr,"cannot allocate memory for intensity array of raingage rainfall");
        fprintf(stderr,"Raingage rainfall intensities are modeled.\n");
     }
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
     ...
     /* fprintf(stderr,"K=%s H=%s P=%s M=%s\n",soil_K_map,soil_H_map,soil_P_map,
              soil_M_map); */
	...
	soil_M_map!=NULL) yes_inf=TRUE;
     ...
     if(yes_inf) 
       {
       fprintf(stderr,"Infiltration losses are computed\n");
       if(soil_K_map == NULL) 
         { 
 ...
 */
        ...
        for(row=0; row<nrows; row++)
	{
	   ...
	   for(col=0; col<ncols; col++)
	   {
	      ...
	      { 
                fprintf(stderr, "SERIOUS WARNING/POSSIBLE ERROR: The inintial water surface elevation value of [%lf] at lake category [%d] is lower than the bed elevation value of [%f] at row= [%d] and col=[%d]. Islands cannot be simulated.\n", lake_el[lake_cat[vect]],lake_cat[vect],elev[vect],row,col); 
	      }	
              else
              {
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
     ...
     if(yes_priess || yes_channel)
        ...
        if((iunit=fopen(channel_file,"r")) == NULL)
           {
           printf("Can't open channel_file named: [%s]",channel_file);
           return(-2);
           }
  	   ...
  	   if(yes_priess)
	      {
	         ...
	         {
                 printf("Can't open yprof_file named:[%s]",yprof_file);
                 return(-2);
                 }
              if((qunit=fopen(qprof_file,"r"))==NULL)
	         {
                 printf("Can't open qprof_file named: [%s]",qprof_file);
                 return(-2);
                 }
	      }
           if(yes_phyd)
	      ...
	      if((phydunit=fopen(phyd_file,"r"))==NULL)
	         {
	         printf("Can't open phyd_file named: [%s]",phyd_file);
	         return(-2);
	         }
	      if((iunit9=fopen(hydout_file,"w"))==NULL)
	         {
	         printf("Can't open hydout_file named: [%s]",hydout_file);
	         return(-2);
                 }
	      }
           }
	   ...
	   if(yes_priess)
	      {
              if((yunit=fopen(yprof_file,"w"))==NULL)
                {
                printf("Can't open yprof_file named: [%s] for writing",yprof_file);
                return(-2);
                }
	      if((qunit=fopen(qprof_file,"w"))==NULL)
	        {
                printf("Can't open qprof_file named: [%s] for writing",qprof_file);
                return(-2);
		}
              }
        }
	 ...
	 *   READ THE INPUT FILE
	 */
         printf("Reading channel x-section data from file: %s\n",channel_file);
	 read_input(iunit,yunit,qunit,&grav,&alpha,&beta,
	     ...
	     &marea1,&parea1,&mtw1,&ptw1,&mk1,&pk1,&numqpipe,yes_priess,
	    ...
	    {
               fprintf(stderr,"Can't open table_file named: [%s]\n",table_file);
               return(-2);
            }
	 ...
	 {
	    fprintf(stderr,"ERROR: The time step %f in [%s] is not equal to the computational overland time step %lf\n",delt,channel_file,dt);
            exit(1);
         }
	 ...
	 if((con_node=(int*) malloc((GNUM+1)*sizeof(int))) == NULL)
	 {
	    ...
	    rewind(phydunit);  /* changed by FLO 5/19/96 */
	    ptot=ptot-1;
	           ...
	           if(vect==vectout) 
	              ...
	              linkout=con_link[vect];
                      if(linkout==0)
                         {
                         fprintf(stderr,
"There is no channel link at the specified outlet.  Check that the outlet\n");
                         fprintf(stderr,
"is at the end of the most downstream link.  The explicit channel routing\n");
                         fprintf(stderr,
"option will not work unless the outlet coordinates specified match with\n");
                         fprintf(stderr,
"coordinates of the outlet of the channel network\n");
                         exit(-1);
                         }
		      nodeout=con_node[vect];
                      if(nodeout==0)
                         {
                         fprintf(stderr,
"There is no channel node at the specified outlet.  Check that the outlet\n");
                         fprintf(stderr,
"is at the end of the most downstream link.  The explicit channel routing\n");
                         fprintf(stderr,
"option will not work unless the outlet coordinates specified match with\n");
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
     ...
     fprintf(discharge_fd,"number of rows:     %d\n",nrows);
     fprintf(discharge_fd,"number of columns:  %d\n",ncols);
     fprintf(discharge_fd,"cell size: %lf\n",w);
     fprintf(discharge_fd,"outlet row number:  %d\n",rowout);
     fprintf(discharge_fd,"outlet col. number: %d\n",colout); 
     fprintf(discharge_fd,"outlet bed slope: %lf\n",sout);

     if(!yes_dist_rough) fprintf(discharge_fd,"uniform Manning n: %lf\n",rman);
     if(yes_dist_rough) fprintf(discharge_fd,"simulating spatially distributed roughness\n");

     if(yes_channel) fprintf(discharge_fd,"using explicit channel routing from x-section file: %s\n",channel_file);

     if(yes_priess) fprintf(discharge_fd,"using implicit channel routing from x-section file: %s\n",channel_file);

     if(!yes_gage && !yes_radar) fprintf(discharge_fd,"uniform rainfall intensity in mm/hr: %lf\n",1000*3600*crain);
     ...
     fprintf(discharge_fd,"computational time step: %6.2f seconds\n",dt);
     fprintf(discharge_fd,"total simulation time: %i seconds\n",totime);
     fprintf(discharge_fd,"rainfall duration:     %i seconds\n",raintime);
...
!!!!!!!!!!!!!!!!!!!!!!!!*/
     ...
     for(row=0; row<nrows; row++)
        ...
        for(col=0; col<ncols; col++)
	      ...
	      if(yes_lake && (lake_cat[vect] != 0 || lake_cat[vect2] !=0))
	         {
                 OV_LAKE(rman,vect,vect2,dmin,yes_dist_rough,elev,rough,
			 h,dqov); 
	         }
	      else
	         {
                 if(h[vect]>=dmin || h[vect2]>=dmin) 
	            OV_FLOW(dt,rman,row,col,vect,vect2,dmin,vectout,
			    yes_dist_rough,yes_flow_out,set_flow_out_basin,
	                    elev,rough,h,dqov,space,&vout_region);
                 }
              }
           }
        }
...
!!!!!!!!!!!!!!!!!!!!!!!!!!*/
     ...
     if(yes_channel) 
        ...
        for(link=1; link<=nlinks; link++)
           {
           /*****************************************************************/
           /* NOTE: this portion was changed to use the channel bed elevations
                    specified in the channel input file rather than the DEM
                    elevations to define the longitudinal profile of the 
                    channel talweg.  FLO 5/16/96 */
           /*****************************************************************/
	    nodetmp=nx1[link]-1;
	    if(ltype[link]==4) nodetmp=nx1[link];
	       ...
	       if(node<nodetmp || link==linkout)
	          ...
	          col2=chn_col[1+backdep[link]*NODES];
                  elev2=bel[1+backdep[link]*NODES];
	          itmp=backdep[link];
                  }
	    ...
	    /*   elev1=(double)elev[vect];
                 elev2=(double)elev[vect2]; THE OLD WAY */
	    ...
	    for(node=1; node<=ptot; node++)
	       {
	       fprintf(iunit9,"%9.2f ",dqch[pnode[node]+plink[node]*NODES]);
               }
            }
         }
...
!!!!!!!!!!!!!!!!!!!!!!!!*/
	...
	qoutch=qp[nx1[nlinks]+nlinks*NODES];
     ...
     if(!yes_quiet) fprintf(stderr,
                    "step, time (min), & discharge : %d  %f %lf\n",
...
if(yes_channel || yes_priess) fclose(channel_file);

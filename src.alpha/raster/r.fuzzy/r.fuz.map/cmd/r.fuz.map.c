...
/* de la classe "Flou" en fonction de SeuilF ppour un set non defini	*/
...
/* Calcul l'appartenance seuillee de Cell pour la premiere variable de	*/
/* de la classe "Flou" en fonction de SeuilF et pour un set defini				*/
...
/* Cree la carte Class de classification seuillee a partir de Raster	*/
	  ...
	  if ( flag==0 && buffer_Var==NULL)
	    {
	      ...
	      /* printf("\nLe nom de la variable ne correspond pas a celui du fichier des variables.\n"); */

	      printf("\n   List of available fuzzy variables :\n");	    
	      /* printf ("Nom des variables dans le fichier :\n"); */
...
char * argv[])
{
	...
	/* Initialisation des structures pour le parser de ligne de commande */
	    ...
	    OptRaster -> key		= "input" ;
	    OptRaster -> description	= "Input fuzzified raster map" ;
	    ...
	    if (Flag1 -> answer) 
	      {
		classe_floue *	Floue ;
		Floue = charge_fichier_flou(OptRaster -> answer) ;
		Print_FuzzyClass(Floue) ;
	      }

	    if (OptMu -> answer) 
	      {
		CarteMu(OptRaster -> answer,OptMu -> answer,OptVar -> answer,
	    ...
	    if (OptClass -> answer) 
	      {
		CarteClass(OptRaster -> answer,OptClass -> answer,
			   OptVar->answer, OptH->answer, OptForce -> answer,
			   Flag2->answer);
	      }

	    if (OptInter -> answer) 
	      {
		CarteInter(OptRaster -> answer,OptInter -> answer,

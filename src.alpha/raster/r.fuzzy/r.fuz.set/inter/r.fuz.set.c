	      ...
	      switch ( (Noeud -> nombre).type ) 
		  {
		    ...
		    /* allocation memoire pour la forme triangle */
			  ...
			  {  
			    ...
			    if (X==NULL || Y==NULL) 
			      {
	       			printf("!!! Error in allocating memory for TFN curve\n");
	       			exit(1);
			      }
		    ...
		    /* allocation memoire pour la forme LR */
			  ...
			  /* allocation memoire pour la forme LRG */
		      case lrg :
			  {
			    X = (float *) malloc(NB_POINTS_LR*sizeof(float)) ;
			    Y = (float *) malloc(NB_POINTS_LR*sizeof(float)) ;
...
char * name;
{
  char *mapset;
	  ...
	  Flag3 -> description	= "Print content of fuzzy variable file " ;
       ...
       /* Demande a l utilisateur d entrer le nom d un raster */
	  mapset = G_ask_old("",name,"cell","raster");
	  if(mapset==NULL)
	    exit(0);
	  ...
	  if (OptFuzzy -> answer) 
	    {
	      ...
	      Assoc_MapFuzzy(name, OptFuzzy -> answer) ;
	    }

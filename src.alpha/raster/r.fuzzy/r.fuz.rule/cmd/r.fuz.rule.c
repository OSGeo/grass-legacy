...
void Lire_variables(char * Fichier_Regles,char ** variables, int nb_rast)
{
  FILE * Fregles;
  char carac;
  int i,j;
...
/* Creer le fichier de variable commun */
...

{
	...
	while ((v<nb_rast)) 
	  {
	    ...
	    if ( (FuzzyFile = G_fopen_old("fuzzy",Rasters[v],G_mapset())) == NULL ) 
	      {
		/* Impossible d ouvrir le fichier variable floue */
	...
	if ( (FVarSortie = fopen(Sortie,"r"))==NULL ) 
	  {
	    /* Impossible d ouvrir le fichier flou de variable de sortie */ 
	    Erreur(42,0,Sortie);
;
	    exit(1);
	  }
	...
	/* Recupere toutes les informations sur les fichiers floues associes au Rasters et les place dans une seule varaible floue Floue de type classe_floue */
			...
			int drapeau)
{
  ...
  if(Mu != NULL)
	...
	{
	  /* Impossible d ouvrir raster ind1 en  sortie */
	  Erreur(43,0,Mu);
	  exit(1);
	}
    }
  ...
  if(Dist != NULL)
	...
	{
	  /* Impossible d ouvrir raster ind2 en  sortie */
	  Erreur(43,0,Dist);
	  exit(1);
	}
    }
  ...
  for(y=0;y<nrows;y++)
      ...
      for(x=0;x<ncols;x++)
	{
	      ...
	      if(drapeau==0 && Rang_Cell[i][x] == 0.0)
		{
	       ...
	       if(drapeau==1)/* Mode prise en compte des zero */
		 {
	      ...
	      /* index_passage est sup a 1 si aucun pixel est nul */
	       index_passage=passage+index_passage;	     
	    } 
	  
	  if(index_passage == nb_var)
	    {	      
	      ...
	      if (Result ==NULL)
		{
		  ...
		  /*  printf("Aucune regles s applique pour le point de coordonnees:\n" );*/
      ...
      /* Place ligne par ligne les valeurs dans le raster de sortie Cell_Sortie */
...
char * argv[] ;
{
  ...
  /* initialisation du tableau qui va contenir les variables floues */
	...
	OptRasters -> key		= "input" ;
	OptRasters -> description	= "Input fuzzified raster maps" ;
	OptRasters -> type		= TYPE_STRING ;
	OptRasters -> required		= YES ;
	OptRasters -> multiple		= YES ;
...
/* Initialisation */
  ...
  while(OptRasters->answers[nb_rast]!=NULL)
    {
      nb_rast++;
    }
...
/* Place dans Floue les differents para de logique floue associés au fichiers Rasters */
  ...
  Lire_Fichier_Rasters(OptRasters->answers,OptVarSortie->answer,Floue,nb_rast,var_utilise);


  /* Renvoie Post et list_regle regles */
  Lire_Regles (OptRule -> answer,Floue,&Post,&regles);

  Traitement_donnees(Floue,&Post,&regles,OptResult->answer,OptRasters->answers,Opt_config,Opt_defuz,Opt_comb,OptMu->answer,OptDist->answer,Flag1->answer);

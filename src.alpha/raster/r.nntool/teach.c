...
BEGIN
   ...
   int    num_cycles=0, scroll_cycle=0, QUIT=0;
   float  t1, t2, grad_err;
   char cycv[1];
   int e_left, e_top, k=0;

   cur_error  = desired_err * 2; k =0;
   sprintf(IO_str, "*** Learning; please wait ***");
   Menu_msg(IO_str);
   ...
   while (cur_error > desired_err && QUIT == 0) BEGIN
	...
	Erase_view(VIEW_HISTO);
      ...
      if(scroll_cycle == 20) {
		Curses_prompt_gets("Enter c to continue; q to stop : ",cycv);
                scroll_cycle = 0;
      ...
      if (num_cycles > stop_cycle && cycv[0] != 'q') BEGIN
         sprintf(IO_str, "*** Timeout failure ***");
         Menu_msg(IO_str);
      ...
      if (print_mod != 0)
         if ((num_cycles % print_mod) == 0) BEGIN
            sprintf(IO_str, " Cycle : %d", num_cycles);
            ...
            write_window(IO_str,e_left+20,e_top+k); 
	    k += 13; scroll_cycle++;
         ENDIF
      ...
      /*-----------------------------------------*/
      if (SAVE_MAXERR == TRUEE || SAVE_RMSERR == TRUEE 
          || SAVE_WEIGHTS == TRUEE || SAVE_TESTS == TRUEE)
   ...
   if (num_cycles <= stop_cycle) BEGIN
      sprintf(IO_str, "Net learned after %d cycles", num_cycles);
      Menu_msg(IO_str);
      sprintf(IO_str, "Learning time: %7.1f seconds", (t2 - t1));
      Menu_msg(IO_str);
   ENDIF

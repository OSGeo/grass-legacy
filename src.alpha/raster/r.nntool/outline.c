...
#define A     Region.saved_point /* [Uj] */
#define AN    Region.saved_npoints /* [Uj] */
...
outline()
{
  ...
  for (cur = 0; cur < AN; cur++) {
    temp.y = view_to_row(Region.saved_view, A[cur].y);
    temp.x = view_to_col(Region.saved_view, A[cur].x);
    tmp_n = row_to_northing(&(Region.saved_view->cell.head), temp.y, 0.5);
    tmp_e = col_to_easting(&(Region.saved_view->cell.head), temp.x, 0.5);
    tmp[cur].y = northing_to_row(&Band_cellhd, tmp_n);
    tmp[cur].x = easting_to_col(&Band_cellhd, tmp_e);
  }
  ...
  do
    {
      if (signalflag.interrupt) return(0);
      if (!skip)
	{
	  V[VN].x = tmp[cur].x;
	  V[VN].y = tmp[cur].y;
      ...
      prev = cur++;
      if (cur >= AN) cur = 0;
      if ((next = cur+1) >= AN) next = 0;

      skip = ( (tmp[prev].y == tmp[cur].y) &&
	      (tmp[next].y == tmp[cur].y) ) ;
    }
  while (cur != first);

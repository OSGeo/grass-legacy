	...
	char *argv[] ;
{
	...
	{
		fprintf(stderr,
		  "ERROR bmif_to_cell: bmif and cellhd rows and cols do not match\n") ;
		fprintf(stderr,
		  "   bmif_to_cell:        rows: %d   cols: %d\n", num_rows, num_cols) ;
		fprintf(stderr,
		  "   Cellhd for <%s>: rows: %d   cols: %d\n", argv[1], wind.rows, wind.cols) ;
		quit() ;
	}
	...
	{
	    fprintf(stderr, "ERROR bmif_to_cell: can't open cell file %s\n", argv[1]) ;
	    quit() ;
	}

...
FILE	*dxf_file;
{
	...
	fgets (dxf_line, 80, dxf_file);
	layer_fd = dxf_which_layer (dxf_line);
	fgets (dxf_line, 80, dxf_file);
	...
	arr_size++;
	fgets (dxf_line, 80, dxf_file);
	while (strcmp (dxf_line, eelev) != 0)
	{
		if (strcmp (dxf_line, zzero) == 0)
		{
			fprintf (stderr, "error looking for eleven\n");
			exit (-1);
		}
		fgets (dxf_line, 80, dxf_file);
	}
	...
	sscanf (dxf_line, "%lf", &(pt_array[arr_size].x));
	fgets (dxf_line, 80, dxf_file);
	while (strcmp (dxf_line, ttwentyone) != 0)
	{
		if (strcmp (dxf_line, zzero) == 0)
		{
			fprintf (stderr, "error looking for twentyone\n");
			exit (-1);
		}
		fgets (dxf_line, 80, dxf_file);
	}

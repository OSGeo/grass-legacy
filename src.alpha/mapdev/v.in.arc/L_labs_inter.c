...
GenToLabelledDigLines(lin_file,txt_file,atts_file,VectMap, cats_filename)
FILE *lin_file,
*txt_file,
*atts_file;
...
*/
    ...
    */
		...
		do {
			...
			printf("for line IDs (probably the column with -ID) : ");
			gets(txtbuf);
			IDCol = atoi(txtbuf);
		...
		do {
			printf("\nEnter the number of the column that should be used\n");
			printf("for GRASS attribute text [<CR> for none]: ");
			gets(txtbuf);
			if (strlen(txtbuf) != 0)
			{
				AttCol = atoi(txtbuf);

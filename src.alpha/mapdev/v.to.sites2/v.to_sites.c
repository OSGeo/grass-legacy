...
**
**  GRASS 4.1.5 - add -n flag  - J. Hinthorne 3/6/96
...
#define MAIN
#define  USAGE  "v.to.sites [-a][-n][-c][-i] input=dig file input output=site file output [dmax=value]\n"
    ...
    char **argv;
{
    ...
    struct Flag   *cat, *Cat, *all, *nod, *interp;
...
/*  check args and set flags  */
    ...
    all = G_define_flag ();
    all->key		= 'a';
    all->description	= "Output all verticies and then nodes to site file";

    nod = G_define_flag ();
    nod->key		= 'n';
    nod->description	= "Output all nodes to site file";
	...
	G_fatal_error (errmsg);
      ...
      bin_to_asc (nod->answer, &Map, out, (cat->answer || Cat->answer) ? &cats : NULL, interp->answer ? dist : 0., (cat->answer != NULL));
      Vect_close (&Map);
    }
    else {
...
bin_to_asc(nodes_only,Map,ascii,cats,dmax,num)
    int nodes_only;
    struct Map_info *Map;
    FILE *ascii;
    struct Categories *cats;
...
if (!nodes_only) /* do vertices first */

...
vect_to_rast (vector_map, raster_map, nrows)
    char *vector_map, *raster_map;
{
	...
	if (nlines)
	{
	    ...
	    if((nlines = do_lines (&Map, Points)) < 0)
	    {
    ...
    update_hist(raster_map, vector_map, vector_mapset);
    inform(NULL);
    stop_clock(NULL);

    printf ("\n");
    sprintf (msg, "Raster map <%s> done. Total processing time:", raster_map);
    inform(msg);
    inform(NULL);
    stop_clock (&timer);
    return 0;

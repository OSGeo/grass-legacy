...
cont_reading:
  ...
  while(1){
    if (*field == '-')
      ...
      if (gets(line) == NULL) break; /*exit big loop */
...
verify_tmpfile(in,out,out_type)
char *in, *out;
int out_type;
{
FILE *fp_in, *fp_out;
...
sub_for_mtx(s)  /* substitute Lxxx for MATRIX or ID field name */
char *s;
{
register int i;
int j, modified;
...
get_utms(lon,lat,east,north,zone,sphere)
long lon, lat;
double *east, *north;
int  *zone;
char *sphere;

...
char *result;
{
    ...
    while ( local != NULL ) {
        ...
        case AttributeSymbol:
            ...
            case LayerAttribute:
                if ( (local->element.att->mapset = G_find_file("cell",
                    local->reclassname,"")) == NULL ) {
                ...
                if ( local->element.att->cellbuf == NULL ) {
                    sprintf(errmsg,"Error allocating memory for [%s]\n",
                        local->name);
                    fatal(errmsg);
                }
                break;
            ...
            case InferredAttribute:
                if ( combinedmap ) {
                    ...
                    case AspectColors:
                        ...
                        break;
                    case GreyScale:
                        G_make_grey_scale(&color,min,max);
                        break;
                    case HistoGreyScale:
                        G_read_histogram(result,G_mapset(),&histo);
                        G_make_histo_grey_scale(&color,&histo);
                        break;
                    case Rainbow:
                        G_make_rainbow_colors(&color,min,max);
                        break;
                    case Ramp:
                        G_make_color_ramp(&color,min,max);
                        break;
                    case Random:
                        G_make_random_colors(&color,min,max);
                        break;
                    case RYG:
                        G_make_red_yel_grn(&color,min,max);
                        break;
                    case Wave:
                        G_make_color_wave(&color,min,max);
                        break;
                    }
        ...
        case ValueSymbol:
            if ( local->element.val->v_type == InferredAttribute 
                ...
                case GreyScale:
                    ...
                    break;
                case HistoGreyScale:
                    G_read_histogram(local->name,G_mapset(),&histo);
                    G_make_histo_grey_scale(&color,&histo);
                    break;
                case Rainbow:
                    G_make_rainbow_colors(&color,min,max);
...
struct symtab *ptr;
{
    ...
    while(local != (struct symtab *)0 ) {
        ...
        for(i = 1; i <= 100; i++ ) {
            sprintf(title,"probabilty of %.2f",(double)i/100.0);
            G_set_cat((CELL)i,title,&cats);
        }

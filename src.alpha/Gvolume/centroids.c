...
centroids(fd, e, n, method, max)
int fd;     /* File descriptor of map layer to process */
            /* This file is assumed to be opened before calling */
            /*   centroids. */
int *e, *n; /* Pointers to arrays at least max+1 long */
int method; /* 0, 1, or 2; see above. */
int max;    /* Highest positive cat number in map layer */
{
CELL *cell_buf, v;
...
for (row = 0; row < rows; row ++)
	...
	for (col=0; col < cols; col++)
		...
		if (v < 1) continue; /* can't handle 0 or - values */
		count[v]++;
...
if (method == 1)
	...
	for (i=1; i <= max; i++)
		...
		if (count[i])
			...
			v = cell_buf[col];
			if (v > 0)
				if (v==i) count[i] = 0; /* weighted is acceptable */
					else adjusted++;
			}
		}
	}
...
for (row=0; row < rows; row++)
	...
	for (col=0; col < cols; col++)
		...
		if ((--count[v]) == 0) /* then this is middle cell */
			...
			/* left edge first */
			while (left > 0)
				if ( cell_buf[--left] != v ) { left++; break;}
			/* then right edge */
			while (right < cols)
				if ( cell_buf[++right] !=v ) {right--; break;}
			e[v] = (left+right) / 2;
			}
		}
	}

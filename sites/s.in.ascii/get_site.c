    ...
    /* check for prefixed atts first, then without prefix */
      ...
      /* move to beginning of next attribute */
			...
			case '%':			/* decimal attribute */
				if (d >= site->dbl_alloc) {
					site->dbl_alloc++;
					site->dbl_att = (double *) G_realloc (site->dbl_att, site->dbl_alloc * sizeof (double));
				}
			...
			case '@':			/* string attribute */
				if (isnull (*buf) || isnull (*(buf + 1))) {
					*buf = '\0';
					...
					if (c >= site->str_alloc) {
						site->str_alloc++;
						site->str_att = (char **) G_realloc (site->str_att, site->str_alloc * sizeof (char *));
						if (site->str_att == NULL) G_fatal_error ("memory allocation error");
						site->str_att[site->str_alloc - 1] = (char *) G_malloc (MAX_SITE_STRING * sizeof (char));
						if (site->str_att[site->str_alloc - 1] == NULL) G_fatal_error ("memory allocation error");
					}

...
char *argv[];
{
        ...
        if (check_connection(me, fifobuf))  /* are our fifos in use? */
        ...
        /* change process groups to be shielded from keyborad signals */
        ...
        /* Establish connection with first client */
        get_connection(fifobuf,&_rfd, &_wfd) ;
        if (_rfd < 0 || _wfd < 0)
        {
        ...
        notify_set_input_func(_rfd, notify_fd, _rfd);
...
fprintf(stderr,"Graph Close\n") ;
                        ...
                        case POLYGON_ABS:
                                REC (&num, sizeof num) ;
                                xarray = (int *) xalloc (xarray, &n_xarray, num, sizeof (*xarray));
                                yarray = (int *) xalloc (yarray, &n_yarray, num, sizeof (*yarray));
                                REC (xarray, num * sizeof(xarray[0])) ;
                                REC (yarray, num * sizeof(yarray[0])) ;
                                Polygon_abs(xarray,yarray,num) ;
                                break ;
                        case POLYGON_REL:
                                REC (&num, sizeof num) ;
                                xarray = (int *) xalloc (xarray, &n_xarray, num, sizeof (*xarray));
                        ...
                        case POLYLINE_ABS:
                                ...
                                REC (xarray, num * sizeof(xarray[0])) ;
                                REC (yarray, num * sizeof(yarray[0])) ;
                                Polyline_abs(xarray,yarray,num) ;
                                break ;
                        case POLYLINE_REL:
                                REC (&num, sizeof num) ;
                                xarray = (int *) xalloc (xarray, &n_xarray, num, sizeof (*xarray));
                                ...
                                REC (xarray, num * sizeof(xarray[0])) ;
                                REC (yarray, num * sizeof(yarray[0])) ;
                                Polyline_rel(xarray,yarray,num) ;
                                break ;
                        case POLYDOTS_ABS:
                                REC (&num, sizeof num) ;
                                xarray = (int *) xalloc (xarray, &n_xarray, num, sizeof (*xarray));
                                ...
                                REC (xarray, num * sizeof(xarray[0])) ;
                                REC (yarray, num * sizeof(yarray[0])) ;
                                Polydots_abs(xarray,yarray,num) ;
                                break ;
                        case POLYDOTS_REL:
                                REC (&num, sizeof num) ;
                                xarray = (int *) xalloc (xarray, &n_xarray, num, sizeof (*xarray));
                                ...
                                REC (xarray, num * sizeof(xarray[0])) ;
                                REC (yarray, num * sizeof(yarray[0])) ;
                                Polydots_rel(xarray,yarray,num) ;
                                break ;
                        case RESET_COLORS:
                                REC (&x, sizeof x) ;
                                REC (&y, sizeof y) ;
                                num = y - x + 1 ;
                                reda = (unsigned char *)
                                    xalloc (reda, &red_alloc, num,
                                    sizeof (*reda));
                                grna = (unsigned char *)
                                    xalloc (grna, &grn_alloc, num,
                                    sizeof (*grna));
                                blua = (unsigned char *)
                                    xalloc (blua, &blu_alloc, num,
                                    sizeof (*blua));
                                REC (reda, num * sizeof(char)) ;
                                REC (grna, num * sizeof(char)) ;
                                REC (blua, num * sizeof(char)) ;
                                Reset_colors(x, y, reda, grna, blua) ;
                                break ;
                        case RESET_COLOR:
                                REC (&red, sizeof red) ;
                                REC (&grn, sizeof grn) ;

...
char *cp,rbuf[12];
int type2_count,found,record,rtsq;
int i,j,ip;
...
while (fgets(t1buf,250,tiger1) != NULL) {  /* get a type 1 record */
	sscanf(t1buf+RECNUM-1,"%9d", &record);  /* get record number */
fprintf(stderr,"record is:\n%s\n",t1buf);
fprintf(stderr,"recnum is: %d\n",record);
continue;
	if (!(record%50)) fprintf(stderr,"\nDoing RECNUM=%d ",record);
	 else fprintf(stderr,".");
...
{
char *cp,buf[300];
int shape_rec,rec_seq,i;
double d;
	fgets(buf,250,p1);
	sscanf(buf+RECNUM,"%9d",&shape_rec);

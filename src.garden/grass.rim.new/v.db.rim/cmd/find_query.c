...
static int append_list;         /* flag for append new records to list */
...
char *inp_buf;
{
  struct query_record *s1, *new_list;
  int i, count, error, number, offset, n_original;
  int add_del_count;
  CELL in_mask();
  char *p1;

  /* initialize for new or appended record list */
  if (append_list && (Last_record != NULL))
        n_original = 1 + (Last_record - Record_list);
  else {
        n_original = 0;
        Last_record = NULL;
  }
  count = Number_of_records+1;  /* more than max number of records */
  number = 0; pass = 0;
  new_list = NULL;
  add_del_count = 0;
    ...
    case 1:
      if (n_original) { /* only if have some already, else do case 0 */
        ...
        if (number>0) /* add them, if not duplicates */
          for (s1=new_list; s1<(new_list+number); s1++)
            if (!dup_rec(Record_list,Last_record,s1->record_number)) {
              copy_list_entry(++Last_record,s1);
              add_del_count++;
            }
        /* sort them in */
        if ((number=add_del_count) > 1)
          qsort((char *)Record_list+n_original,number,
                sizeof(struct query_record),cmp_distance);
      break;/* exit switch */
      }
    case 0:
     if ((add_del_count=get_n_closest(Record_list,count)) < 0)
       goto abort_point;
     if (add_del_count > 0) Last_record = Record_list+(add_del_count-1);
} /* end of switch */

#ifndef DBVECT
  report_records(add_del_count);
  fprintf(Outfile,
...
if (append_list!=1 || Last_record==NULL) return 0;
for (s1=q1; s1<=q2; s1++)
  if (s1->record_number == rec) return 1;
return 0;
}

delete_rec(rec)  /* delete a record from Record_list */
int rec;            /* and update Last_record */
{
struct query_record *s1, *s2;

  if (Last_record != NULL)
   for (s2=Record_list; s2<=Last_record; s2++)
    if (s2->record_number == rec) {
      for (s1=s2; s1<Last_record; s1++)
         copy_list_entry(s1,s1+1);
      if (Last_record > Record_list) Last_record--;
...
char *p1, *p2;
 p2 = (char *) dest;
 p1 = (char *) src;
 for (i=0; i<sizeof(struct query_record); i++) *p2++ = *p1++;
}

get_n_closest(start,count)
struct query_record *start;/* start is start of place to store */
...
save_s_e_n_t_m(s1) /* put new entry on query list */
struct query_record *s1;
{
  s1->record_number = s;
  s1->north = n;
...
int count;
        ...
        switch (dist_clause(inp_buf) ) {
                case -1: {
                        if (! get_loc_of_record(target_vect) )
                        { fprintf(Outfile,
                          ".find: target record %d not in data base %s.\n",
                                target_vect,File_name);
                        target_dist = 0.0;
                        }
                return;
...
CELL in_mask();
  ...
  /* initialize for new or appended record list */
  if (append_list && (Last_record != NULL))
        n_original = 1 + (int) (Last_record - Record_list);
  else {
        n_original = 0;
        Last_record = NULL;
  }
 ...
 SLEEP3;
   ...
   if (error == 0) {/* do the work if successful select */
        ...
        get_s_e_n_t_m(); /* get coordinates, etc. */

        /* check mask, window and target circle conditions */
        if (coord_good())
          if (append_list < 0) {  /* delete record */
            number += delete_rec(s);
            auto_print();
          }
          else
           if (!dup_rec(Record_list,Last_record,s)) {
             /* really add one to list */
             Last_record = Record_list+(n_original+number);
             save_s_e_n_t_m(Last_record);
             number++;
             auto_print();
            }
        }
...
int n;
 ...
 switch (append_list) {
  case -1: fprintf(Outfile,
               "\n%d records deleted from selected set.",number);
           break;
  case 1:  fprintf(Outfile,
               "\n%d records appended to seleted set.",number);
           break;
  case 0:
  default: break;
 }
 ...
 fprintf(Outfile,
        "\n%d records now selected from data base <%s>\n",n,File_name);
}

...
struct cmd_info {
     ...
     char desc[67]; /* help text */
      ...
      {"help", 1, HELP_OP, P_CMD_STATE, TRUE,
        ...
        "List available data bases by mapset."},
                {"make", 1, MAKE_OP, P_MAKE_STATE, FALSE,
        ...
        "Print site records in 'query list' after .find or .query."},
                {"query", 1, QUERY_OP, P_QUERY_STATE, TRUE,
        "Create 'query list' of sites using 'SQL' request format."},
                {"read_site", 2, READ_SITE_OP, P_CMD_STATE, FALSE,
        "Read an site list into the data base as records."},
                {"remove", 6, REMOVE_OP, P_CMD_STATE, FALSE,
        "Remove (Delete, Trash) an entire data base.  Be careful!"},
                {"show", 2, SHOW_OP, P_CMD_STATE, TRUE,
        "Print the site record format."},
                {"site_list", 2, SITE_LIST_OP, P_CMD_STATE, TRUE,
        "Create a GRASS site list in current mapset from 'query list'."},
...
char *inp_line;
{
   ...
   /* copy the input line and change it to lowr case */
           ...
           if (opcode == SYNTAX_ERR)
              if (buffer[0]==SHELL_ESCAPE_CHAR) {
               ...
               if (read_only_mode==TRUE && commands[cmd_num].read_only==FALSE) {
                  ...
                  if (opcode==MAKE_OP) {
                     fprintf(Outfile,"Warning: Database already exists. Make com mand ignored.\n");
                     opcode = NOP;

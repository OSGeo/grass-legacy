      ...
      if (pos<inp_length) {
         ...
         if (field_type==S_FIELD_CHAR) {
            if ((Required_fields & S_FIELD)!=0) {
               fprintf(Outfile,"\nForm Definition Error: More than one field of type 's' (sequence #).\n");
               Make_OK = FALSE;
            }
            else
         ...
         if (field_type==X_FIELD_CHAR) {
            if ((Required_fields & X_FIELD)!=0) {
               fprintf(Outfile,"\nForm Definition Error: More than one field of type 'x' (easting).\n");
               Make_OK = FALSE;
            }
            else
         ...
         if (field_type==Y_FIELD_CHAR) {
            if ((Required_fields & Y_FIELD)!=0) {
               fprintf(Outfile,"\nForm Definition Error: More than one field of type 'y' (northing).\n");
               Make_OK = FALSE;
            }
            else
         ...
         if (field_type==V_FIELD_CHAR) {
            if ((Required_fields & V_FIELD)!=0) {
               fprintf(Outfile,"\nForm Definition Error: More than one field of type 'v' (vector type).\n");
               Make_OK = FALSE;
            }
            else
               Required_fields += V_FIELD;
         }
         if (field_type==M_FIELD_CHAR) {
            if ((Required_fields & M_FIELD)!=0) {
               fprintf(Outfile,"\nForm Definition Error: More than one field of type 'm' (reference map).\n");
               Make_OK = FALSE;
            }
            else
               Required_fields += M_FIELD;
         }

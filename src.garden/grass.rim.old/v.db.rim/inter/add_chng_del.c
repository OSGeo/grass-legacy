...
{
   add_init();
   *((int *) Field_info[Sequence_field].value) = number;
   V_clear();
...
add_a_record()
{
   int field, largest_record;
   char buf[80], buf2[80], tempstr[200];

   largest_record = get_max_seq();
   sprintf(buf,"           Enter record number (%s) to add",
                        Field_info[Sequence_field].column_name);
   while (1) {
      sprintf(buf2, "    %d is the largest %s in the current data base.",
              largest_record, Field_info[Sequence_field].column_name);
      field = largest_record + 1;
      V_clear();
      ...
      /* cannot have more than one record with a given record number */
      sprintf(tempstr, "select from data where %s = %d",
              Field_info[Sequence_field].column_name, field);
      if (crim(DATA_TABLE, tempstr) != RIM_EOT) {
         sprintf(tempstr, "Sequence number (%d) duplicated one in the database, not added.\n", *((int *) Field_info[Sequence_field].value));
         G_warning(tempstr);
         SLEEP3;
      }
      else if (v_add(field)==FALSE) return;
      if (field>largest_record) largest_record = field;
   }
}
...
change_a_record()
{
   ...
   field = 0;
   sprintf(buf,"           Enter sequence number (%s) to change",
                        Field_info[Sequence_field].column_name);
   while (1) {
      V_clear();
      V_line(1, make_line_2() );
      ...
      /* record should already be defined */
      sprintf(tempstr, "select from data where %s = %d",
              Field_info[Sequence_field].column_name, field);
      if (crim(DATA_TABLE, tempstr) == RIM_EOT) {
...
delete_a_record()
{
   ...
   sprintf(buf,"           Enter sequence number (%s) to delete",
                        Field_info[Sequence_field].column_name);
   *record = '\0';
   while (1) {
      V_clear();

             ...
             case BIT8 : /* 8 bit per pixel format */
               ...
               case BIT24: /* 24 bit per pixel format */
                 ...
                 default   : /* unsupported format */
                     fprintf (stdout,"Error : Unsupported Format \n");
                 exit(1);
             } /* end of switch */
...
#endif
{
    ...
    for (i=0; i<16; i++)
    {
       ...
       /*    fprintf (stdout,"compress: y[%d] is %f\n",i,y[i]);*/
       y_av = y_av + y[i];
    }
    y_av = y_av / 16.0;
    /*  fprintf (stdout,"y_av is %f\n",y_av); */
...
#endif
{
    ...
    for (i=0; i<(2*blocks); i++)
    {
       ...
       /*    fprintf (stdout,"cnt_color: k is %d\n",k); */
       temp[k] = 0;
    }
...
#endif
{
    ...
    for (i=0; i<(2*blocks); i++)
    {
       ...
       if (trans[k] == -1)
       {
           ...
           /*
             fprintf (stdout,"map: %d %d %d mapped to %d %d %d\n", r, g, b, new_pal[tran
s[k]*3
   ],
...
#endif
{
    ...
    for (boxes=1; boxes < PALSIZE; boxes++)
    {
       /*
         ...
         while (ptr != NULL)
         {
         fprintf (stdout,"Box %d, distinct %d, total %d\n",j,ptr->nmbr_distinct,
         ptr->nmbr_pts);
         for (i=0; i<ptr->nmbr_distinct; i++)
         fprintf (stdout,"pt %d: %d %d %d",i,distinct_pt[ptr->pts[i]].c[RED],
...
#endif
{
    ...
    if (max == NULL)
    {
       fprintf (stdout,"Error : Number of color points < palette \n");
       exit(1);
    }
...
#endif
{
    ...
    for (ent=0; ent<PALSIZE; ent++)
    {
       ...
       /*
         fprintf (stdout,"Box %d: number of pts %d\n", ent, temp->nmbr_pts);
         */

       for (j=0; j<temp->nmbr_distinct; j++)
       {
           /*
             fprintf (stdout,"pt %d:", j);
             */
           for (k=RED; k<=BLUE; k++)
           {
               /*
                 fprintf (stdout,"%d ",distinct_pt[temp->pts[j]].c[k]);
                 */
               c[k] = c[k] +
...
#endif
{
    ...
    /*
      for (i=0; i<ptr->nmbr_distinct; i++)
      fprintf (stdout,"find_med: sorted list is %d\n",distinct_pt[rank[i]].c[dim]);
      */

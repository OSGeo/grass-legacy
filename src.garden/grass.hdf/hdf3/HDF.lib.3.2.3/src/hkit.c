 ...
 * Revision 1.5  1992/10/23  00:14:11  koziol
...
#else
char *HIstrncpy(dest, source, len)
...
int32 HDspaceleft(void)
{
    ...
    switch(heap_status) {
        ...
        case _HEAPBADBEGIN:
            fprintf (stdout,"%s block at %Fp of size %4.4X\n",(h_info._useflag==_USEDENTRY ? "USED" : "FREE"),h_info._pentry,h_info._size);
            fprintf (stdout,"ERROR - heap is damaged\n\n");
            return((int32)-1);

        case _HEAPBADPTR:
            fprintf (stdout,"%s block at %Fp of size %4.4X\n",(h_info._useflag==_USEDENTRY ? "USED" : "FREE"),h_info._pentry,h_info._size);
            fprintf (stdout,"ERROR - bad pointer to heap\n\n");
            return((int32)-1);

        case _HEAPBADNODE:
            fprintf (stdout,"%s block at %Fp of size %4.4X\n",(h_info._useflag==_USEDENTRY ? "USED" : "FREE"),h_info._pentry,h_info._size);
            fprintf (stdout,"ERROR - bad node in heap\n\n");
            return((int32)-1);
    } /* end switch */

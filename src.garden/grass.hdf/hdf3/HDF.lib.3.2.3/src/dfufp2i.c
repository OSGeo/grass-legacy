...
$Log$
Revision 1.1  1999-12-29 15:12:36  markus
Initial revision

 * Revision 1.2  1992/10/23  00:14:11  koziol
...
#endif /* PROTOTYPE */
{
    ...
    fprintf (stdout,"There %s a palette\n", in->is_pal ? "IS" : "is NOT");
    fprintf (stdout,"color tranform method = %s\n",
                           (in->ct_method==EXPAND)?"expand":"interpolate");
    if ( in->hscale != NULL) {
        fprintf (stdout,"\nHorizontal scale:\n");
        for (i=0; i<(int)(in->hdim); i++)
          fprintf (stdout,"%8.2f",in->hscale[i]);
    }
    else
        fprintf (stdout,"\nNo horizontal scale\n");
    ...
    if ( in->vscale != NULL) {
        ...
        for (i=0; i<(int)(in->vdim); i++)
          fprintf (stdout,"%8.2f",in->vscale[i]);
    }
    else
        fprintf (stdout,"\nNo vertical scale.\n");
    ...
    for(i=0;i<(int)(in->vdim) && i<11; i++) {
        ...
        for (j=0; j< (int)(in->hdim); j++)
            fprintf (stdout,"%6.1f ",in->data[i*in->hdim+j]);
    }
    fprintf (stdout,"\n");
    return 0;
} /* end of print_input */
    ...
    fprintf (stdout,"\n");
      for(i=0;i<(int)(out->vres) && i<20; i++) {
          ...
          for (j=0; j< (int)(out->hres); j++)
             if (j<19)
                fprintf (stdout,"%4d",(uint8) out->image[i*out->hres + j]);
      }
     fprintf (stdout,"\n");
    return 0;
} /* end of printoutput */
...
/*        fprintf (stdout,"midpt[%d]=%8.1f\tscale[%d]=%8.1f\n",i,midpt[i],i,scale[i]);*/
    }

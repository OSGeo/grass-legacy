	...
	int curcat;
{
	...
	if (joinargs)  {	/* two table qry with joinkey */
		fprintf(fp,"SELECT DISTINCT * FROM %s",joinargs[0]);
		fprintf(fp," WHERE %s in",joinargs[1]);
		...
		fprintf(fp,"SELECT DISTINCT * FROM %s",ktab->answer);
		fprintf(fp," WHERE ( %s = %d );\nquit\n",
			keycat->answer, curcat);
	}
	...
	fclose (fp);
        sprintf(buf,"sqlplus -s %s @%s ", G_getenv("DATABASE"), sqlFile);

...
buildSQL(fp,tabname,colname,ctype)
    FILE *fp;
    char *tabname, *colname, *ctype;
{
	/*
	fprintf(fp,SELECT,colname,ctype,colname,ctype,colname,
		ctype,colname,ctype,colname);
	fprintf(fp,FROM,tabname);
	fprintf(fp,WHERE,colname);
	*/
	fprintf(fp,"SELECT (count=count{%s.%s where %s.%s NOTNULL}) \n",
		tabname,colname,tabname,colname);
	fprintf(fp,"SELECT (sum=%ssum{%s.%s where %s.%s NOTNULL}) \n",
		ctype,tabname,colname,tabname,colname);
	fprintf(fp,"SELECT (avg=%save{%s.%s where %s.%s NOTNULL})\n",
		ctype,tabname,colname,tabname,colname);
	fprintf(fp,"SELECT (max=%smax{%s.%s where %s.%s NOTNULL})\n",
		ctype,tabname,colname,tabname,colname);
	fprintf(fp,"SELECT (min=%smin{%s.%s where %s.%s NOTNULL} ) \n",
		ctype,tabname, colname, tabname,colname);

...
buildInfxQry(key, where, table, map, joinargs, mapset, color)
  ...
  int color ;
  {
    ...
    line_cat = 0;
        ...
        if (joinargs != NULL) { 
		fprintf(fp, "SELECT DISTINCT %s FROM %s,%s", key,  
			table, joinargs[0]);
		fprintf(fp, " WHERE %s", where);
		fprintf(fp, " AND %s=%s AND %s IS NOT NULL", 
			joinargs[1], joinargs[2], key);
		fprintf(fp, " ORDER BY %s;\nquit\n", key); 
	}
	else {		   
		fprintf(fp, "SELECT DISTINCT %s FROM %s", key, table);
		fprintf(fp, " WHERE %s AND %s IS NOT NULL", where, key);
		fprintf(fp, " ORDER BY %s;\nquit\n", key);
	}
	...
	/* Execute SQL command				*/
        sprintf(sysbuf,"sqlplus -s %s @%s > %s",

...
echo "Добро пожаловать в GRASS VERSION_NUMBER (VERSION_DATE) VERSION_UPDATE_PKG"
echo
...
echo 
echo "Этот выпуск GRASS VERSION_NUMBER координирует и производит команда"
echo "GRASS Development Team, находящаяся в ITC-irst (Тренто, Италия) при"
echo "поддержке разработчиков всего мира, а также ун-та Baylor"
echo "University и University of Illinois (U.S.A.)."
echo 
echo "Эта программа распространяется для пользы, но"
echo "БЕЗ КАКОЙ-ЛИБО ГАРАНТИИ; даже без подразумеваемой гарантии"
echo "ПОКУПНОЙ СПОСОБНОСТИ или ПРИМЕНИМОСТИ для к.-л. целей.  См. подробно GNU"
echo "General Public License."
echo 
echo "Эта версия работает через $shellname ($SHELL)"
echo "Помощь доступна по команде:      g.help"
echo "Смотри условия лицензии:              g.version -c"

if [ "$GRASS_GUI" = "text" ] ; then
    echo "Запуск графического интерфейса: tcltkgrass&"
else
    echo "Если нужно, снова запустите графический интерфейс: tcltkgrass&"
fi

echo "Чтобы закончить работу:                exit"
...
PS1="
Набор данных <$MAPSET> в области <$LOCATION_NAME>
GRASS-GRID > "

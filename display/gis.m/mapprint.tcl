...
proc GmPrint::window { } {
    ...
    Entry $row.b -width 10 -textvariable PVar(paper_left) -command GmPrint::paper
    Label $row.c -anchor w -text [G_msg "right:"]
    Entry $row.d -width 10 -textvariable PVar(paper_right) -command GmPrint::paper
    Label $row.e -anchor w -text [G_msg "top:"]
    Entry $row.f -width 10 -textvariable PVar(paper_top) -command GmPrint::paper
    Label $row.g -anchor w -text [G_msg "bottom:"]
    Entry $row.h -width 10 -textvariable PVar(paper_bottom) -command GmPrint::paper
    ...
    Button $but.print -text [G_msg "Print"] -command { GmPrint::print "print" }
    Button $but.preview -text [G_msg "Preview"] -command { GmPrint::print "preview" }
...
proc GmPrint::save { } {
    ...
    foreach key { paper_mode paper paper_width paper_height paper_left paper_right paper_top paper_bottom 
        ...
        Gm::rc_write 1 "$key $PVar($key)"
    }
    Gm::rc_write 0 "End"
}

proc GmPrint::set_option { key value } {
    global PWid PVar PPap PView

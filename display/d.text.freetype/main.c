...
int main(int argc, char **argv)
{
	...
	_("Screen position (percentage, [0,0] is bottom left)");
	...
	_("Path to TrueType font (including file name)");
	...
	if (param.at->answer)
	    ...
	    sprintf(buf, "%s at=%f,%f", G_recreate_command(), east, north);
	    D_add_to_list(buf);
	}
    }
    else {
	char *tmpfile, *p, *c, align[3], linefeed, setx, sety, setl;
	FILE *fp;
	int sx, sy, px, py;
	double d, dd;
	...
	while (fgets(buf, 512, stdin)) {
		...
		case 'F':
			...
			*c = 0;
			...
			path = fonts[cur_font].path;
			charset =
			    transform_string(fonts[cur_font].charset,
					     toupper);
		    }
		    else {
			path = p;
			...
			error(_("Unable to create face"));
		    if (FT_Set_Char_Size
			(face, cnv(size), cnv(size), 100, 100))
			error(_("Unable to set size"));
		    break;
		case 'C':
		...
		case 'S':
		    ...
		    size = d + (i ? size : 0);
		    if (face &&
			FT_Set_Char_Size(face, cnv(size), cnv(size), 100,
					 100))
			error(_("Unable to set size"));
		    break;
		case 'B':
		...
		pen.x = x;
		pen.y = y;
		get_ll_coordinates(face, out, ol, align, rotation, &pen);
		pen2 = pen;
		draw_text(win, face, &pen, out, ol, rotation);
...
static void set_color(const char *tcolor)
{
    int r, g, b;
...
get_dimension(FT_Face face, unsigned char *out, int l, FT_Vector * dim)
{
	...
	if (first) {
		...
		minx = slot->bitmap_left;
	    if (maxx < slot->bitmap_left + slot->bitmap.width)
		maxx = slot->bitmap_left + slot->bitmap.width;
	    if (miny > -slot->bitmap_top)
		miny = -slot->bitmap_top;
	    if (maxy < -slot->bitmap_top + slot->bitmap.rows)
		maxy = -slot->bitmap_top + slot->bitmap.rows;
	}
...
static int
draw_character(rectinfo win, FT_Face face, FT_Matrix * matrix,
	       FT_Vector * pen, int ch)
{
    static char *buffer;
    static int nalloc;

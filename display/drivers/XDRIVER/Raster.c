...
static void raster_row(int x0, int y0, int ncols, int nrows, const unsigned char *nul)
{
	int i, j;
	...
	for (i = 0; i < ncols; )
	{
		...
		for (j = 0; j < nrows; j++)
			XPutImage(dpy, bkupmap, gc, img, i0, 0, x0 + i0, y0 + j, i - i0, 1);
	}
}

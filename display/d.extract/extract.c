...
int extract ( struct Map_info *In, struct Map_info *Out, int type, int color, int hcolor )
{
    ...
    mode = M_START;
    fprintf (stderr, "Select vector(s) with mouse\n");
    fprintf (stderr, " - L: draw box with left mouse button to select\n");
    fprintf (stderr, " - M: draw box with middle mouse button to remove from display\n");
    fprintf (stderr, " - R: quit and save selected vectors to new map\n\n");
    while ( 1 ) {
        fprintf (stderr, "L: add  M: remove  R: quit and save\n");
...
display ( struct Map_info *Map, struct ilist *List, int color )
{
	...
	if ( type & GV_POINTS ) 
	    G_plot_icon( Points->x[0], Points->y[0], G_ICON_CROSS, 0, msize);
	else

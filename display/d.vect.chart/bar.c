...
int 
bar ( double cx, double cy, int size, double scale, double *val, int ncols, COLOR *ocolor, COLOR *colors, int y_center, double *max_reference ) 
{
    int i, j;
    double max;
    double x0, y0;
    ...
    if(y_center == 0) /* draw the columns with the bottom at the y value of the point  */
		y0 = cy ; 
	else
		y0 = cy - scale * max * pixel / 2;	/* center the columns around the y value of the point */
	...
	if(max_reference) {
		...
		Vect_append_point ( max_Points, x0 + (i+1)*bw, y0 + scale * max_reference[i] * pixel, 0);
		Vect_append_point ( max_Points, x0 + i*bw, y0 + scale * max_reference[i] * pixel, 0);
		Vect_append_point ( max_Points, x0 + i*bw, y0, 0);
		...
		for (j = 1; j < max_Points->n_points; j++ ) { 
			G_plot_line(max_Points->x[j], max_Points->y[j], max_Points->x[j-1], max_Points->y[j-1]);
		}
		}
	}

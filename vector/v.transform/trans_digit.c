		      ...
		      int field, char** columns)
{
	...
	if (field > 0) { 
		...
		for (j = 0; j <= IDX_ZROT; j++) {
			...
			break;
		    case -1:
			G_fatal_error(_("Missing column <%s> in table <%s>"), columns[j], fi->table);
		    default:
			G_fatal_error(_("Column type of <%s> must be numeric"), columns[j]);
		    }
		    if (db_select_value(driver, fi->table, fi->key, cat, columns[j], &val) != 1) {
			trans_params[j] = trans_params_def[j];
			/*
			G_warning(_("Unable to select record from table <%s> (key <%s>, column <%s>). "
				    "For category %d using default transformation parameter %.3f."),
				    fi->table, fi->key, columns[j], cat, trans_params[j]);
			*/
		    }
		    else {
			if (!db_test_value_isnull(&val))

...
main (int argc, char *argv[])
{
	...
	poly_f->key  = 'p';
        poly_f->description  = "Write areas and isles as closed polygons instead of topo check";
	...
	if ( !poly_f->answer ) { /* do check */
		...
		if ( !Vect_area_alive (  &Out, i ) ) continue;
		nareas2++;
		...
		if ( !Vect_area_alive (  &In, i ) ) continue;

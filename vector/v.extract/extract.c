...
void extract_cats ( struct line_cats *Cats, int type_only, int field, int new )
{
    ...
    if ( !TCats ) TCats = Vect_new_cats_struct ();
...
int areas_new_cats_match ( struct Map_info *In, int area1, int area2, int type_only, int field, int new ) 
{
    ...
    if ( !Cats1 ) Cats1 = Vect_new_cats_struct ();
    if ( !Cats2 ) Cats2 = Vect_new_cats_struct ();

    if ( area1 < 1 || area2 < 1 ) return 0; /* at least one area is missing */
    
    centroid1 = Vect_get_area_centroid ( In, area1 );
    centroid2 = Vect_get_area_centroid ( In, area2 );
    
    if ( centroid1 < 1 || centroid2 < 1 ) return 0; /* at least one centroid is missing */
	...
	found = 0;
        for ( j = 0; j < Cats2->n_cats; j++ ) {
            G_debug ( 5, "%d:%d x %d:%d", Cats1->field[i], Cats1->cat[i], Cats2->field[j], Cats2->cat[j] );
	    if ( Cats1->field[i] == Cats2->field[j] && Cats1->cat[i] == Cats2->cat[j] ) {
		found = 1;
		break;
	    }
	}
             ...
             int new, int select_type, int dissolve, int field, int type_only)
{
	...
	qsort ( cats_array, ncats_array, sizeof(int), cmp ); 
	    ...
	    G_debug ( 3, "Line = %d", line );
	     ...
	     G_debug ( 3, "type_match = %d field_match = %d cat_match = %d", type_match, field_match, cat_match );
	     G_debug ( 3, "left_area = %d left_field_match = %d left_cat_match = %d", 
		           left_area, left_field_match, left_cat_match );

	     G_debug ( 3, "right_area = %d right_field_match = %d right_cat_match = %d", 
		 ...
		 /* centroid */
	         if ( type == GV_CENTROID && (type & GV_AREA) && centroid_in_area ) write = 1;
		 ...
		 /* centroid */
	         if ( type == GV_CENTROID && (type & GV_AREA) && centroid_in_area && field_match ) write = 1;

		 /* areas */ 
		 if ( type == GV_BOUNDARY && ( left_field_match || right_field_match ) ) {
		     if ( !dissolve || !areas_new_cats_match ( In, left_area, right_area, type_only, field, new ) )
		         write = 1;
		 }
	     } 
		 ...
		 G_debug ( 3, "write = %d", write );

		 /* centroid */
	         if ( type == GV_CENTROID && (type & GV_AREA) && centroid_in_area && cat_match ) write = 1;

		 /* areas */ 
		 if ( type == GV_BOUNDARY && ( left_cat_match || right_cat_match ) ) {
		     if ( !dissolve || !areas_new_cats_match ( In, left_area, right_area, type_only, field, new ) )
		         write = 1;
		 }
	     }

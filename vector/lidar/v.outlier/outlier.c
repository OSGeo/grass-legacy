...
void P_Outlier (struct Map_info *Out, struct Map_info *Outlier, struct Map_info *Qgis, struct Cell_head Elaboration, BOUND_BOX \
		General, BOUND_BOX Overlap, double **obs, double *parBilin, double mean, double overlap, int *line_num, \
	...
	if ( Vect_point_in_box (obs[i][0], obs[i][1], mean, &General) ) {
	    ...
	    *point->z += mean;
		...
		if ((*point->x > Overlap.E) && (*point->x != General.E)) {
			...
			if (Select_Outlier (&gradient[0], line_num[i], driver) != DB_OK) 
				G_fatal_error (_("Impossible to read the database"));

			if (UpDate_Outlier (gradient[0], line_num[i], driver) != DB_OK)
			...
			if (Insert_Outlier (interpolation*weight, line_num[i], driver) != DB_OK) 
			...
			if (Insert_Outlier (interpolation*weight, line_num[i], driver)\
				!= DB_OK) 
			    G_fatal_error (_("Impossible to write in the database"));
...
int UpDate_Outlier (double Interp, int line_num, dbDriver *driver)
{
    ...
    db_init_string (&sql);
    sprintf (buf, "UPDATE Auxiliar_outlier_table SET Interp=%lf WHERE ID=%d", Interp, line_num);
    db_append_string (&sql, buf);
...
int Select_Outlier (double *Interp, int line_num, dbDriver *driver)
{
    ...
    db_init_string (&sql);
    sprintf (buf, "SELECT ID, Interp FROM Auxiliar_outlier_table WHERE ID=%d", line_num);
    db_append_string (&sql, buf);

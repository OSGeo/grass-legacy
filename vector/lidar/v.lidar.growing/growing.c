...
void P_Aux_to_Coor (struct Map_info *In, struct Map_info *Out, dbDriver *driver, FILE *fsite) {
	...
	db_append_string (&sql, "select ID, sum(Interp) from Auxiliar_table group by ID");

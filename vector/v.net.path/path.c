...
int path ( struct Map_info *In, struct Map_info *Out, int nfield )
{
    int    i, nlines, line, npoints, type, cat, fcat, tcat, fline, tline, fnode, tnode, count;
    int    ret, sp;
    struct ilist *AList;
    double cost;
    ...
    sprintf ( buf, "create table %s ( cat integer, fcat int, tcat int, " 
	      "sp int, cost double precision )", Fi->table );
	...
	sprintf (buf, "insert into %s values ( %d, %d, %d, %d, %f)", 
		       Fi->table, cat, fcat, tcat, sp, cost );
	db_set_string ( &sql, buf );
    ...
    fprintf ( stderr, "  cost - travelling costs\n");

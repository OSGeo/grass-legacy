...
/*			oor2 = atan2( adz[i], sqrt(grad1) );*/
	...
	curn =  -(adxx[i]*dx2+dxy2+dxz2+dyz2+adzz[i]*dz2+adyy[i]*dy2)/grad2;
		 }
		 if (gcurv!=NULL) {
...
*/
   ...
   if (!wz1)
    ...
    if (cell==NULL) cell = G_allocate_f_raster_buf();
	...
	for (k = ngstr; k <= nszr; k++)
	{
		...
		dzz = 0.;
		  ...
		  if (!once)
		  {
			...
			fprintf (stderr, "Overshoot occures at (%d,%d,%d) cell\n", l, k, i);
			fprintf (stderr, "The w-value is %lf, wmin is %lf,wmax is %lf\n", ww, wmin, wmax);
			  }
		  }
...
/*		printf("\n %f", ww);*/
		...
		adyy[l] = dyy;
                adyz[l] = dyz;
		adzz[l] = dzz;
                if ((gradient != NULL)||(aspect1 != NULL)||(aspect2 != NULL)
	   ...
	   offset2 = offset + ind;    /* rows*cols offset */
           ...
           if ((cellinp != NULL) && (cellout != NULL)&&(i==ngstl)) {
            if (fseek(Tmp_fd_cell,(long)(ind*sizeof(FCELL)),0)==-1) return 0;
	    if(!(fwrite(zero_array_cell+ind1,sizeof(FCELL),nszc-ngstc+1,Tmp_fd_cell))) 
              clean_fatal_error("Not enough disk space--cannot write files");
	   }
	   if (outz != NULL) {    
            if (fseek(Tmp_fd_z,(long)(offset2*sizeof(float)),0)==-1) return 0;
	    if(!(fwrite(zero_array1+ind1,sizeof(float),nszc-ngstc+1,Tmp_fd_z))) 
              clean_fatal_error("Not enough disk space--cannot write files");
	   }
	   if (gradient != NULL) {
	     if (fseek(Tmp_fd_dx,(long)(offset2*sizeof(float)),0)==-1) return 0;
    	     if(!(fwrite(zero_array2+ind1,sizeof(float),nszc-ngstc+1,Tmp_fd_dx))) 
	       clean_fatal_error("Not enough disk space--cannot write files");
	   }
	   if (aspect1 != NULL) {
             if (fseek(Tmp_fd_dy,(long)(offset2*sizeof(float)),0)==-1) return 0;
	     if(!(fwrite(zero_array3+ind1,sizeof(float),nszc-ngstc+1,Tmp_fd_dy))) 
  	       clean_fatal_error("Not enough disk space--cannot write files");
	   }
	   if (aspect2 != NULL) {
 	     if (fseek(Tmp_fd_dz,(long)(offset2*sizeof(float)),0)==-1) return 0;
	     if(!(fwrite(zero_array4+ind1,sizeof(float),nszc-ngstc+1,Tmp_fd_dz))) 
	       clean_fatal_error("Not enough disk space--cannot write files");
	   }
           if (ncurv != NULL) {
	     if (fseek(Tmp_fd_xx,(long)(offset2*sizeof(float)),0)==-1) return 0;
	     if(!(fwrite(zero_array5+ind1,sizeof(float),nszc-ngstc+1,Tmp_fd_xx))) 
	       clean_fatal_error("Not enough disk space--cannot write files");
	   }
           if (gcurv != NULL) {
	     if (fseek(Tmp_fd_yy,(long)(offset2*sizeof(float)),0)==-1) return 0;
	     if(!(fwrite(zero_array6+ind1,sizeof(float),nszc-ngstc+1,Tmp_fd_yy))) 
	       clean_fatal_error("Not enough disk space--cannot write files");
	   }
           if (mcurv != NULL) {
	     if (fseek(Tmp_fd_xy,(long)(offset2*sizeof(float)),0)==-1) return 0;
	     if(!(fwrite(zero_array7+ind1,sizeof(float),nszc-ngstc+1,Tmp_fd_xy))) 
	       clean_fatal_error("Not enough disk space--cannot write files");
	   }

...
int  **buf, nrows, ncols, **value, index, *rich, cnt, cntwhole;
{
  ...
  if (choice->att[0]) {				/* ATTRIBUTE MEASURES */
     if (choice->att[1])
        value[index][0] = (int)(10.0*attr[0]);          /* Mean */
     if (choice->att[2])
        value[index][1] = (int)(10.0*attr[1]);          /* St. dev. */
     if (choice->att[3])
        value[index][2] = (int)attr[2];                 /* Min. */
     if (choice->att[4])
        value[index][3] = (int)attr[3];                 /* Max. */
  }

  if (choice->div[0]) {				/* DIVERSITY MEASURES */
     if (choice->div[1])
	    value[index][4] = (int)diver[0];            /* Richness X 1 */
     if (choice->div[2])
        value[index][5] = (int)(100.0*diver[1]);        /* Shannon X 100 */
     if (choice->div[3]) 
        value[index][6] = (int)(100.0*diver[2]);        /* Dominance X 100 */
     if (choice->div[4]) 
        value[index][7] = (int)(100.0*diver[3]);        /* Inv. Simpson X 100 */
  }

  if (choice->te2) {   				/* TEXTURE MEASURES */
     if (choice->te2[1])                                /* Contagion X 100 */
        value[index][8] = (int)(100.0*tex[0]);
     if (choice->te2[2])                                /* ASM X 1000 */
        value[index][9] = (int)(1000.0*tex[1]);
     if (choice->te2[3])                                /* IDM X 1000 */
        value[index][10] = (int)(1000.0*tex[2]);
     if (choice->te2[4])                                /* Entropy X 100 */
        value[index][11] = (int)(fabs(100.0*tex[3]));
     if (choice->te2[5])                                /* Contrast X 10 */
        value[index][12] = (int)(10.0*tex[4]);  
  }
 
  if (choice->jux[0]) {				/* JUXTA. MEASURES */
     if (choice->jux[1])
        value[index][13] = (int)(1000.0*edge[0]);      /* Mean jux. X 1000 */
     if (choice->jux[2])	
        value[index][14]= (int)(1000.0*edge[2]);       /* St.dev. jux. X 1000 */
  } 

  if (choice->edg[0]) {				/* EDGE MEASURES */
     if (choice->edg[1])
        value[index][15] = (int)(edge[1]);             /* Sum of edges X 1 */
     if (choice->edg[2])
	    value[index][16] = (int)(edge[3]);         /* Sum of edges 
                                                          by type X 1 */
  }
}
...
int  **buf, nrows, ncols, *rich, cnt, cntwhole;
{
  ...
  int           lc, fd, *atts, *edgeatts, *edge1, *edge2, fc; 
  register int  i, j;
  double    	attr[4], diver[4], edge[4], tex[5], **weight, **edgemat;
  ...
  if (choice->z) {
     ...
     for (i = 1; i < nrows + 1; i++) {
        ...
        for(j = 1; j < ncols + 1; j++) {
           if (attr[1] > 0.0)
	          if (buf[i][j]) {
                 *(zscor_buf + j - 1) = (int)(100*((double)(buf[i][j] - 
		           attr[0])/attr[1]));
			  }
        }
...
double  *tex;
{
  ...
  if (i0 > 1) {
     ...
     if (j0 > 1 && (choice->tex == 4 || choice->tex == 6 || 
		choice->tex == 7)){
        if (buf[i0-1][j0-1]) {
     ...
     if (j0 < nc &&(choice->tex == 2 || choice->tex == 6 || 
		choice->tex == 7)){
        if (buf[i0-1][j0+1]) {
           ln = check_order(buf[i0-1][j0+1], rich); 
           GLCM[lc][ln] ++;
  ...
  if (i0 < nr) {
     ...
     if (j0 > 1 && (choice->tex == 2 || choice->tex == 6 || 
		choice->tex == 7)){
        if (buf[i0+1][j0-1]) {
           ln = check_order(buf[i0+1][j0-1], rich); 
           GLCM[lc][ln] ++;
     ...
     if (j0 < nc && (choice->tex == 4 || choice->tex == 6 ||
		choice->tex == 7)){
        if (buf[i0+1][j0+1]) {
           ln = check_order(buf[i0+1][j0+1], rich); 
           GLCM[lc][ln] ++;
  ...
  if (j0 > 1 && (choice->tex == 1 || choice->tex == 5 || 
	 choice->tex == 7)){
     if (buf[i0][j0-1]) {
  ...
  if (j0 < nc && (choice->tex == 1 || choice->tex == 5 || 
	 choice->tex == 7)){
     if (buf[i0][j0+1]) {
        ln = check_order(buf[i0][j0+1], rich); 
        GLCM[lc][ln]++;

...
*/
  ...
  for(i = 0; i < scales; i++) {	
	...
	printf("       Exit to setup option menu   6\n\n"); 
        do {
           printf("                       Which Number?   "); 
           numtrap(1, &dtmp);
...
tryagain:
	      ...
	      if (fmask > 0) {
		    ...
		    cnt = 0;
	         ...
	         printf("\n    Recommended maximum SIZE is");
                 printf(" %d in %d pixel total area\n",
	      ...
	      printf("\n    Recommended maximum SIZE is");
              printf(" %d in %d pixel individual", (int)(w_w*w_l*dtmp/2), 
	      ...
	      printf(" stratum area\n");
           ...
           printf("    What size (in pixels) for each sampling unit of scale %d?  ",
              i+1);
           numtrap(1, &size);	      	  
	   thick = 1;
...
*/
  ...
  for(i = 0; i < tmp; i++){
     ...
     if (!G_yes("\n       (including squares) (y) or circles (n)?   ",1)) {
        ...
        printf("       Left button:     Check unit size\n");
        printf("       Middle button:   Upper left corner of unit here\n");
 	...
 	au_l = ab-at;
	   ...
	   if (ar <= al || ab <= at) {
	      ...
	      goto back1;
	         ...
	         abs(au_w), abs(au_l));  
	      fprintf(stdout,"    Width/length would be %5.2f and size %d pixels\n",

...
qual()
 ...
 while(1)
    ...
    fprintf (stdout,"                CONTANINANT SOURCE AREAS\n\n");
    fprintf (stdout,"    It is possible to make source area maps for several\n");
    fprintf (stdout,"    contaminants: sediment, nitrogen, phosphorous, and COD.\n");
    fprintf (stdout,"    You must produce a sediment source area map to make\n");
    fprintf (stdout,"    the other contaminant source area maps.  However, you may\n");
    fprintf (stdout,"    choose to make only a sediment map, or a sediment map and\n");
    fprintf (stdout,"    and combination of the other maps.\n\n");
    fprintf (stdout," \n\n\n");
...
sleep(2);
	  if (rain == 0)
         ...
         if(!strncmp(rain_map_name, "list",4))
          ...
          if(mapset == NULL)
             {
             fprintf (stdout,"Landcover map `%s' not found.\n",landcover_map_name);
          ...
          if(mapset == NULL)
             {
             fprintf (stdout,"Runoff map `%s' not found.\n",runoff_map_name);
             fprintf (stdout,"Please try again.\n\n");
          ...
          if(mapset == NULL)
             {
             fprintf (stdout,"Slope map `%s' not found.\n",slope_map_name);
             fprintf (stdout,"Please try again.\n\n");
          ...
          if(mapset == NULL)
             {
             fprintf (stdout,"K factor map `%s' not found.\n",Ksoil_map_name);
             fprintf (stdout,"Please try again.\n\n");
          ...
          if(mapset == NULL)
             {
             fprintf (stdout,"Soil texture map `%s' not found.\n",soiltext_map_name);
             fprintf (stdout,"Please try again.\n\n");
          ...
          if(rain == 0)
            {
             ...
             if(mapset == "NULL")
               {
               fprintf (stdout,"Rainfall map `%s' not found.\n",rain_map_name);
               fprintf (stdout,"Please try again.\n\n");
               mapset=G_ask_cell_old("Rainfall map ?",rain_map_name);
               }
              else
                rmap = 1;  
fprintf (stdout,"qual lookd for and found a rain map\n");
sleep(2);
           }
     } /*  end while !ok */
     ...
     while(!ok)
      ...
      if(strcmp(seds_map_name, "_") == 0)
         {
          fprintf (stdout,"You must enter a name for a sediment source area map.\n");
          sleep(4);
          ok = 0;
         }
    ...
    case 2:          /*** contaminant routing  ***/
      ...
      if(strncmp(seds_map_name,"_",1) != 0 )
         {
         seds_mapset = G_find_cell(seds_map_name, "");
         if(seds_mapset == NULL)
           {
           fprintf (stdout," Sorry, sediment source map %s not found.\n",seds_map_name);
           fprintf (stdout," Please try again.\n");
           sleep (4);
           ok = 0;
           }
         }
      if(strncmp(ns_map_name,"_",1) != 0 )
        {
        ns_mapset = G_find_cell(ns_map_name, "");
        if(ns_mapset == NULL)
           {
           fprintf (stdout," Sorry, nitrogen source map %s not found.\n",ns_map_name);
           fprintf (stdout," Please try again.\n");
           sleep (4);
           ok = 0;
           }
         }
      if(strncmp(ps_map_name,"_",1) != 0 )
         {
         ps_mapset = G_find_cell(ps_map_name, "");
         if(ps_mapset == NULL)
           {
           fprintf (stdout,"Sorry, phosphorous source map %s not found.\n",ps_map_name);
           fprintf (stdout," Please try again.\n");
           sleep (4);
           ok = 0;
           }
         }
     if(strncmp(cods_map_name,"_",1) != 0 )
        {
        cods_mapset = G_find_cell(cods_map_name, "");
        if(cods_mapset == NULL)
           {
           fprintf (stdout," Sorry, COD source map %s not found.\n",cods_map_name);
           fprintf (stdout," Please try again.\n");
           sleep (4);
           ok = 0;
           }
...
check_assumptions()
{
  ...
  char location[100], name[110];
    ...
    if ((assump_fd = fopen(name, "r+")) == NULL)
         {
          fprintf (stdout," OOPS !! error opening hidden assumptios file\n\n");
          fprintf (stdout," Please run Modify hidden assumptions option\n");
          fprintf (stdout," in the main menu. Thanks!\n\n");
          sleep(4);
         } 
      else 
         {
      ...
      if (avrain == 42)
         {
          fprintf (stdout,"The average annual rainfall in the hidden assumption file\n");
          fprintf (stdout,"is set at 42 inches, which is the default! \n");
          fprintf (stdout,"Are you sure you don't want to modify that?\n\n");
         }

     if(rei == 150)
         {
          fprintf (stdout,"\n\nThe average annual rainfall energy intensity\n");
          fprintf (stdout,"in the hidden assumption file is set at 150,\n");
          fprintf (stdout,"which is the default! \n");
          fprintf (stdout,"Are you sure you don't want to modify that?\n\n");
         }

   ...
   char **argv;
{
   ...
   /*--------------------------------------*/
   printf("    Creating cross section maps (%d) of water surface elevations...\n",
               nevents );
   ...
   for ( i=0; i < nevents; i++ )
   {
      ...
      if ( !xsect_map( i, &extra_xsect ) )
         printf( "    ERROR: bad results in processing %s flood event.\n",
                      event[i] );
   }
   ...
   if ( splitflow )
      printf("    NOTE: Number of split flow cross sections   = %d.\n", 
                        nsplit );

   printf("    NOTE: Total number of HEC2 cross sections   = %d (SECNO).\n", 
                     nsecno );
   printf("    NOTE: Total number of vector cross sections = %d (XSECT).\n", 
                     nxsect );
   printf("    NOTE: Number of SECNO/XSECT matches = %d\n", nmatches );
   printf("    NOTE: Number of extra SECNO's       = %d\n", extra_secno );
   printf("    NOTE: Number of extra XSECT's       = %d\n", extra_xsect );
   ...
   char  **argv;
{
   ...
   { 
      printf("    Enter HEC2 output file name (full path): ");
      scanf( " %s", hec2_fname );
   }
   ...
   {
      printf("    ERROR: cannot open HEC2 file '%s'.\n", hec2_fname );
      return(0);
   }
   ...
   if ( splitflow )
   {
      ...
      { 
         printf("    Enter HEC2 split flow file name (full path): ");
         scanf( " %s", hec2_fname2 );
      }
      /*-------------------------------*/
      ...
      {
         printf("    ERROR: cannot open HEC2 split flow file '%s'.\n", 
                            hec2_fname2 );
         return(0);
      }
   }
   ...
   int   do_split;
{
   ...
   if ( !find_summary( fid ) )
   {
      printf("    ERROR: summary table not found in HEC2 file.\n");
      return(0);
   }
   ...
   {
      printf("    ERROR: unexpected format of summary table.\n");
      return(0);
   }
   ...
   char *title;
{
   ...
   if ( !found )
   {
      printf("    ERROR: column heading %s not in summary table.\n",
			 title );
      rc = 0;
   }
   else
   ...
   char   *heading;
{
   ...
   secno_match = (int *) calloc( max_secno, sizeof(int) );
   secno_val   = (double *) calloc( max_secno, sizeof(double) );
   cwsel_val   = (double **) calloc( max_secno, sizeof(double *) );
   ...
   while( moredata )
   {
	 ...
	 moredata = next_line( hec2_fid, heading, buffer );
      ...
      {
	 max_secno  += XSECT_INCREMENT;
         secno_match = (int *) realloc( (char *) secno_match,
                                        max_secno * sizeof(int) );
         secno_val   = (double *) realloc( (char *) secno_val,
                                        max_secno * sizeof(double) );
	 cwsel_val   = (double **) realloc( (char *) cwsel_val,
   ...
   {
      printf("    ERROR: unexpected end of summary table\n" );
      printf("           loaded %d values for %d cross sections\n",
			 nval, nsecno );
      rc = 0;
   }
   else
	 ...
	 gotline = fgets( buffer, LINELEN, fid );
      ...
      if ( gotline )
      {
         ...
         if( !strcmp( token, "SPLITFLOW" )  ||  !strcmp( token, "splitflow" ) )
         {
            ...
            {
               splitflow = TRUE;
               printf( "    NOTE: split flow conditions to be processed.\n" );
            }
         ...
         else if( !strcmp( token, "TRUNCATE" ) || !strcmp( token, "truncate" ) )
         {
            ...
            {
               truncate = TRUE;
               printf( "    NOTE: HEC-2 secno's will be truncated.\n" );
            }
            else if( answer[0] == 'N'  ||  answer[0] == 'n' )
               truncate = FALSE;
         }
...
ask_events()
{
   ...
   while ( !valid )
   {
      printf("    How many flood events are modeled in HEC2? : ");
      scanf( " %s", token );
   ...
   int    *extra_xsect;
{
   ...
   *extra_xsect = 0;
   ...
   {
       printf ("   ERROR: Cannot write new attributes for '%s'.\n",
                   fout_map );
       return(0);
   }

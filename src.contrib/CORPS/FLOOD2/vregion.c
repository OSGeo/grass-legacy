...
out_ep (struct Edge *e)
{
  ...
  if (debug)
  {
    fprintf (stdout,"out_ep(): edge %d", e->edgenbr);
    fprintf (stdout," ep %d", e->ep[le] != (struct Site *) NULL ? e->ep[le]->sitenbr : -1);
    fprintf (stdout," ep %d", e->ep[re] != (struct Site *) NULL ? e->ep[re]->sitenbr : -1);
    fprintf (stdout," reg %d", e->reg[le] != (struct Site *) NULL ? e->reg[le]->sitenbr : -1);
...
out_vertex (struct Site *v)
{
  ...
  if (debug)
    fprintf (stdout,"vertex(%d) at %10.7f %10.7f\n", v->sitenbr, v->coord.x, v->coord.y);
}
...
#define EPSILON 1.0
  ...
  fprintf (stdout,"/* xmin, ymin %10.7f %10.7f; xmax, ymax %10.7f %10.7f; */\n", xmin, ymin, xmax, ymax);
  fprintf (stdout,"/* pxmin, pymin %10.7f %10.7f; pxmax, pymax %10.7f %10.7f; crad %10.7f; */\n", pxmin, pymin, pxmax, pymax, cradius);
#endif
}
...
clip_line (struct Edge *e)
{
  ...
  if (!triangulate)
  {
    ...
    if (debug)
      fprintf (stdout,"clip_line puts edge induced by %d and %d\n", r1->sitenbr, r2->sitenbr);
...
int 
vtcomp (VERTTHETA *vt1, VERTTHETA *vt2)
{
 ...
 */
int 
find_vregion (int vsid, float pverts[][2])
{
...
/*============================================================================*/
int 
find_adjacent (int vsid, int *adjlist)
{

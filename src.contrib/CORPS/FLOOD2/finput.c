...
int cl_parse(int, char **);
int read_hec2(FILE *, int);
int find_summary(FILE *);
int find_headings(FILE *, char *);
int find_variables(char *);
int get_column(char *, char *);
int load_data(char *);
int load_split(char *);
int get_value(char *, double *, int);
int next_line(FILE *, char *, char *);
int ctrl_events(char *);
int ask_events(void);
int xsect_map(int, int *);
...
cl_parse (int argc, char **argv)
{
   ...
   event = (char **) G_malloc( count *  sizeof( char *) );
   if ( !event )
      mem_exit();
   for( count=0; parm.event->answers[count] != NULL; count++ )
   {
...
/*==========================================================================*/
int 
read_hec2 (FILE *fid, int do_split)
{
...
/*==========================================================================*/
int 
find_headings (FILE *fid, char *heading)
{
   char buffer[LINELEN];
...
/*==========================================================================*/
int 
get_column (char *buffer, char *title)
{
   char *buff_ptr;
   char  word[WORDLEN];
   int   column;
...
load_data (char *heading)
{
   ...
   while( moredata )
   {
      ...
      /*---------------------------------------------*/
      cwsel_val[nsecno] = (double *) G_malloc( nevents * sizeof(double) );
      if ( !cwsel_val[nsecno] )
         mem_exit();
...
/*==========================================================================*/
int 
get_value (char *buffer, double *value, int column)
{
   int   i;
...
/*==========================================================================*/
int 
next_line (FILE *fid, char *heading, char *buffer)
{
   char  garbage[LINELEN];
   char *status;
   int   rc = 1;
...
/*==========================================================================*/
int 
xsect_map (int event_indx, int *extra_xsect)
{
   int    secno_indx;
   int    old_att;
   int    new_att;

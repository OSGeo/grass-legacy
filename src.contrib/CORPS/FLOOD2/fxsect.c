...
int get_input(void);
int get_memory(void);
int digit_xsect(void);
int add_station(CELL *, int, int, double *, double *);
double point_line_snap(double, double, double, double, double, double, double *, double *);
int rm_flow(char *, int, double *, double *);
int put_line(char *, int, double *, double *, int);
int map_xsect(void);
int get_X1(char *);
int get_GR(char *, int);
int put_field(char *, int, char *);
int xsect_buttons(void);
int flow_buttons(char *);
static int put_box(double, double, int);
...
main (int argc, char **argv)
{
   ...
   if  ( modified )
   {
      fprintf (stdout,"    NOTE: Modified cross section map is '%s'.\n",
                  mod_xsect_vname );
...
get_input (void)
{
   ...
   {
      fprintf (stdout,"    ERROR: cannot open HEC2 description file '%s'.\n", 
                  xsect_hec2_fconv );
      return(0);
   }
   ...
   {
       fprintf (stdout,"    ERROR: Cannot open new cross section map '%s'.\n", 
                    mod_xsect_vname );
       return(0);
   }
   ...
   /*--------------------------------------------*/
   if ((mod_att = G_fopen_new( "dig_att", mod_xsect_vname )) == NULL)
   {
      fprintf (stdout,"    ERROR: Cannot open attribute file for vector map '%s'.\n", 
                   mod_xsect_vname);
      return(0);
   }
   ...
   if ( elev_rid < 0 )
   {
       fprintf (stdout, "   ERROR: cannot open elevation map '%s'.\n",
                           elev_rname);
       return(0);
   }
...
get_memory (void)
{
   ...
   if ( use_orig )
   {
      ...
      for ( i=0; i<orig_vid.n_lines; i++ )
      {
         ...
         /* allocate structure */
         /*--------------------*/
         newptr = (X_INFO *) G_malloc( sizeof(X_INFO) );
         if ( !newptr )
            mem_exit();
         ...
         {
            fprintf (stdout,"    ERROR: reading original cross section attributes.\n" );
            return(0);
         }
   ...
   /*------------------------------------------------*/
   max_gr = GR_INCREMENT;
   elev = (double *) G_calloc( max_gr, sizeof(double) );
   sta  = (int *) G_calloc( max_gr, sizeof(int) );
   if ( !elev  ||  !sta )
      mem_exit();
...
digit_xsect (void)
{
   ...
   while ( TRUE )
   {
      ...
      while ( !valid )
      {
         ...
         if ( sscanf( secno_str, "%d", &secno_int ) == 1 )
         {
            if ( strlen( secno_str ) <= 6 )
               ...
               fprintf (stdout, "    Id limited to field width of 6. Try again.\n" );
         }
         else
            fprintf (stdout, "    Invalid cross section id. Try again.\n" );
      }
      if ( secno_int == 0 )
         break;
      ...
      while ( !valid )
      {
         fprintf (stdout, "Plot all points [A] or only points up to WSEL [W]? ");
         scanf( " %s", usr_buffer );
      ...
      {  
         ...
         /*--------------------------------------------*/
         if ( numst == 89 )
            fprintf (stdout,"    WARNING: approaching HEC2 upper limit for number of stations.\n" );
         else if ( numst == 99 )
         {
            fprintf (stdout,"    WARNING: station limit reached. Identify end of cross section.\n" );
            tailend = TRUE;
         }
         ...
         if ( !add_station( row_buffer, screen_x, screen_y, 
                            x_pair, y_pair ) )
         {
            fprintf (stdout,"    ERROR: cannot add cross section station.\n" );
            return(0);
         }
         ...
         {
            max_gr += GR_INCREMENT;
            elev = (double *) G_realloc( (char *) elev,
                             max_gr * sizeof(double) );
            sta  = (int *) G_realloc( (char *) sta,
                           max_gr * sizeof(int) );
            if ( !elev  ||  !sta )
               mem_exit();
         } 
      ...
      else
      {
         ...
         /*------------------------------------*/
         modified = TRUE;
         new_ptr = (X_INFO *) G_malloc( sizeof(X_INFO) );
         if ( !new_ptr )
            mem_exit();
         new_ptr->source = NEW;
         new_ptr->index  = -1;
         ...
         else
         {
            ...
            if ( !valid )
            {
               fprintf (stdout,"    ERROR: cannot process flow lines.\n" );
               return(0);
            }
         }
...
/*==========================================================================*/
int 
add_station (CELL *row_buffer, int screen_x, int screen_y, double *x_pair, double *y_pair)
{
   int     row, col;
   double  east, north;
   double  dist;
   ...
   {
      fprintf (stdout,"    ERROR: cannot convert screen coordinates.\n");
      return(0);
   }
   ...
   else
   {
      ...
      {
         fprintf (stdout,"    ERROR: station numbers have exceeded field width.\n");
         return(0);
      }
...
which_station (L_PNTS *points)
{
   ...
   while( !accepted )
   {
      ...
      if ( !accepted )
      {
         ...
         fprintf (stdout,"    NOTE: station UN-selected. Try again.\n" );
      }
   }
...
/*==========================================================================*/
int 
snap_station (L_PNTS *points, double utm_x, double utm_y)
{
   int    i;
   int    first;
   int    winner;
...
/*==========================================================================*/
int 
which_flow (X_INFO *new_ptr, int c_check, int *ds_flow_len, int *us_flow_len, X_INFO **us_ptr, X_INFO **ds_ptr)
{
   int     fpts;
   int     erased;
   int     accepted;
   ...
   while( !accepted )
   {
      ...
      /*---------------------------*/
      fprintf (stdout, "       Identify DOWNSTREAM center-of-mass point.\n" );
      ...
      while ( !valid )
      {
         fprintf (stdout, "       Identify NEW center-of-mass point.\n\n" );
      ...
      /*-------------------------*/
      fprintf (stdout, "       Identify UPSTREAM center-of-mass point.\n" );
      ...
      if ( !erased )
      {
         ...
         else
         {
            ...
            {
               fprintf (stdout,"    ERROR: flow lengths have exceeded field width.\n");
               return(0);
            }
         }
      }
...
/*==========================================================================*/
int 
which_xsect (X_INFO *new_ptr, double *xsect_x, double *xsect_y, X_INFO **which_ptr)
{
   int     i;
   int     first;
   int     button;
...
/*==========================================================================*/
double 
point_line_snap (double utm_x, double utm_y, double x1, double y1, double x2, double y2, double *snap_x, double *snap_y)
{
   int    snap_dir;
...
/*==========================================================================*/
int 
rm_flow (char *message, int fpts, double *flow_x, double *flow_y)
{
   int i;
...
/*==========================================================================*/
int 
put_line (char *msg, int npts, double *x, double *y, int color)
{
   int i;
...
/*==========================================================================*/
static int put_box (double x_point, double y_point, int color)
{
   double x1, y1;
   double x2, y2;
   double x3, y3;
...
int map_xsect (void)
{
   ...
   while( xptr )
   {
      ...
      {
         fprintf (stdout,"    ERROR: cannot write out new cross section map.\n");
         return(0);
      }
...
/*==========================================================================*/
int 
put_field (char *buffer, int field, char *token)
{

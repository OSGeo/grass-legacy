...
/* open input, ascii file */
	...
	if (access(buffer,4) != 0)
		{
		fprintf (stdout,"\t ERROR - couldn't open input file <%s>\n",buffer);
	        exit ();
		}
...
/* open output, ascii file */
	...
	if (strlen(buffer) == 0)
  	...
  	if ( access(buffer,2) != 0 && access(buffer,2) != -1 )
	 	{           
		fprintf (stdout,"\t ERROR - couldn't open output file <%s>\n",buffer);
	        fclose(input_file);
	        exit ();
		}
...
/*fprintf (stdout," move %d bytes from col. %d to col. %d, length %d\n",bytes,old,new,end);*/

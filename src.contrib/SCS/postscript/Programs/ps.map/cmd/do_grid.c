...
do_grid()
{
	...
	{
	    if (g == PS.w.north || g == PS.w.south) continue;
	    PS_plot_vect(west, g, east, g, 0);
	    if (j & 1) fprintf(PS.fp, " D\n");
    ...
    {
	if (g == PS.w.east || g == PS.w.west) continue;
        PS_plot_vect(g, PS.w.north, g, PS.w.south, 0);
	if (j & 1) fprintf(PS.fp, " D\n");
	else fprintf(PS.fp, " D ");
    }
}
...
do_grid_numbers(/*panel*/)
{
    double g;
    char num_text[50];
    int grid, vy, vx, hy, hx;
    int first, len, x, y, last_bottom, last_right;
    int i, rounded_grid, margin;
    char *format_northing(), *format_easting();

    if (PS.grid <= 0 || PS.grid_numbers <= 0) return;
...
static char *format_northing(n, round)
double n;
int round;
{
    static char text[50];

    if (PS.w.proj == PROJECTION_LL) G_format_northing(n, text, PS.w.proj);
    else
    {
	n = floor(n / round);
	sprintf(text, "%.0lf", n);
...
static char *format_easting(e, round)
double e;
int round;
{
    static char text[50];

    if (PS.w.proj == PROJECTION_LL) G_format_easting(e, text, PS.w.proj);
    else
    {
	e = floor(e / round);
	sprintf(text, "%.0lf", e);

...
/* open report file for writing*/
           sprintf (Output_name, "%s.rept\0",Mhd->OUTPUT_FILE);
...
/*		  W_bnd = Mhd->UTM_EAST - ((Mhd->ORIGIN_CELL - Mhd->minc) * Mhd->CELL_SIZE); */
      ...
      /* put N, S, E, W, in report file */
	       ...
	       fputs (buffer, rpt_file);
      /* Create a strip name */
       	       sprintf (Output_name,"%s.strip%d\0",Mhd->OUTPUT_FILE,last_strip);
      /* Open  output file */
...
fprintf(stderr," k= %d,  Byte in !%s!  ",k,Bytes);
	...
	/* blank, assign input value to this element */
				    sprintf (category_file[icode],"%s\0",Bytes);
#ifdef  DEBUG 
...
CAT_OUT:
         sprintf (Output_name,"%s.cats\0",Mhd->OUTPUT_FILE);
	 output_file = fopen (Output_name,"w");

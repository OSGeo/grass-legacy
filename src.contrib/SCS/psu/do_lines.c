...
do_lines(Map, Out_Map, psu_att, psu_data, cats, xdist, ydist)
	struct Map_info *Map ;
...
/*    printf( "Step 3: Creating Line information...   ") ;*/
	...
	while (((dummy = fgets(psu_buf, MAXLINE, psu_data)) != NULL)){
		...
		b=strcmp(psu_num , cat_label);
			/*printf("psunum *%s* , catlabel *%s*, cate %d\n", psu_num, cat_label,cur_category);  */
		if (b == 0)
			break;
	}
	if (dummy == NULL){
		printf("catlabel *%s* not found for cat %d\n", cat_label,cur_category); 
		exit(-1);
	}
	rewind (psu_data);
...
/* G_set_cat((CELL) psu_num, psu_cat_buf, cats);*/
	...
	for (a=0;a<psu_pts;a++){
		...
		catlistptr = cats->list;
		catlistptr++;
		sprintf(tmpbuf, "%s%d\0", cat_label, a+1);
		for (x=1;x , cats->count ; x++,catlistptr++){
			G_strcpy( tmpbuf2, catlistptr->label);

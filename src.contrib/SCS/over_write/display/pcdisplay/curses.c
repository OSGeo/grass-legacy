...
#define LINE12	 ""
#define LINE13	 " *  Replot screen  | z  Replot map  | Q  quit"
...
Write_cats(categories, current_cat) 
	...
	int current_cat ;
{
	...
	end_cat = start_cat + 4 ;
	end_cat = end_cat < categories->num ? end_cat : categories->num ;

	werase(CAT_WIN) ;
	wmove(CAT_WIN, 1, 20) ;
	sprintf(buffer,"CATEGORIES:   %3d to %3d of %3d\0", 
		start_cat, end_cat, categories->num) ;
	...
	for (at_cat = start_cat ; at_cat <= end_cat ; at_cat++)
	{
		if (at_cat == current_cat)
			sprintf(buffer,"-> %3d %s\0", at_cat, G_get_cat(at_cat, categories)) ;
		else
			sprintf(buffer,"   %3d %s\0", at_cat, G_get_cat(at_cat, categories)) ;
	...
	int hi_mode ;
{
	...
	if (hi_mode)
		...
		sprintf(buffer, "CATEGORY NUMBER: %d\0", at_cat) ;
	wmove(COLR_WIN,1,3) ;
	waddstr(COLR_WIN,buffer) ;

	sprintf(buffer, "       RED: %3d   %4d%%\0", red, (int)((float)red / 2.56) ) ;
	wmove(COLR_WIN,3,3) ;
	waddstr(COLR_WIN,buffer) ;

	sprintf(buffer, "     GREEN: %3d   %4d%%\0", grn, (int)((float)grn / 2.56) ) ;
	wmove(COLR_WIN,4,3) ;
	waddstr(COLR_WIN,buffer) ;

	sprintf(buffer, "      BLUE: %3d   %4d%%\0", blu, (int)((float)blu / 2.56) ) ;
	wmove(COLR_WIN,5,3) ;
	waddstr(COLR_WIN,buffer) ;

	sprintf(buffer, "SHIFT INCR: %3d   %4d%%\0", shift_incr, (int)((float)shift_incr / 2.56)) ;
	wmove(COLR_WIN,7,3) ;
	waddstr(COLR_WIN,buffer) ;

    ...
    char *file;
{
    ...
    */
	...
	if ((ret=read_att(fp, &type, &Att.x, &Att.y, &Att.cat, &Att.offset))==1)
	   {
       fprintf(stderr,"\b\b\b\b\b%5d", map->n_atts) ;
	   return (cnt); /*EOF */
	   }
#ifdef SCS_MODS
               fprintf(stderr,"\b\b\b\b\b%5d", map->n_atts) ;
	...
	switch (type) {
		...
		break;
	    default:
/*DEBUG*/ fprintf (stderr, "ERROR: READ_ATTS  GOT TYPE %x\n", (int) Att.type);
		break;
	}
    }
    /* return (1); */
...
#else
            fprintf (stderr, "\nWARNING: label %d : %d matched another label: %d.\n", line, map->Att[Line->att].cat, Att->cat);
        	    if (map->Att[Line->att].cat != Att->cat)
        	      {
        	      fprintf (stderr, "\nWARNING: tried to re-label line %d label: %d with another label: %d.\n", line, map->Att[Line->att].cat, Att->cat);
                      fprintf(stderr,"\tLabels attached :     ");
                      }
#endif
    ...
    P_ATT *Att;
{
    ...
    {
#ifndef SCS_MODS
/*DEBUG*/ fprintf (stderr, "PNT_TO_AREA failed: (%lf, %lf) (Category %d)\n", Att->x, Att->y, Att->cat);
#else
                  fprintf (stderr, "\nLabel NOT in ANY area: (Category %d) at (%lf, %lf)\n", Att->cat, Att->x, Att->y);
                  fprintf(stderr,"\tLabels attached :     ");
#endif
	return (area);
    }
...
#else
        	    if (map->Att[Area->att].cat != Att->cat)
        	      {
        	       fprintf (stderr, "\nWARNING: tried to re-label area %d label: %d with another label: %d.\n", area, map->Att[Area->att].cat, Att->cat);
                       fprintf(stderr,"\tLabels attached :     ");
        	       }
#endif

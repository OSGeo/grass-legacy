    ...
    struct Map_info *map;
{
	...
	if (0 > matchup_isle (map, i))
	    return (-1);
#ifdef SCS_MODS
                if (i > 1) fprintf(stderr,"\b\b\b\b\b%5d",i) ;
#endif
    }
    return 0;
}
...
*/
    ...
    {
	match = 0;
/*DEBUG*/ debugf ("ISLE %d, pointing to next area %lf\n", isle, min_dist);
	if (0 == (area = dig_point_to_next_area (map, x, y, &min_dist)))
	{
/*DEBUG*/ debugf ("Could not find area for isle %d (%lf, %lf)\n", isle, x, y);
	    Isle->area = 0;
	    return (0);
	}
/*DEBUG*/ debugf ("point to area returned area %d\n", area);

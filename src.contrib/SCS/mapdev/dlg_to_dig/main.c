	...
	char **argv ;
{
	...
	*/
	if (argc < 4)
	{
		printf("Usage: dlgtodig dlg_file digit_file att_file [ \"line\" ]\n") ;
		exit(-1) ;
	}
...
/*-->  R.Glenn, SCS */
    if (dlg.max_areas > dlg.max_lines)
        linetypes = (char *) G_calloc (dlg.max_areas+1, sizeof (char));
    else linetypes = (char *) G_calloc (dlg.max_lines+1, sizeof (char));

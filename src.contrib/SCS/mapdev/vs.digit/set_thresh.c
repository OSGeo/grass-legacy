    ...
    struct Map_info *map;
{
    ...
    while (1)
     {
        ...
        sprintf(buf,"Current SNAPPING threshold:");
	...
	sprintf ( buf, "  Map : %lf  %s. ", map->snap_thresh, 
        ...
        sprintf(buf, " Enter new Dig threshold or ");
        message[3] = (char *) malloc (strlen (buf) + 1);
        sprintf(message[3],"%s", buf);

        sprintf(buf, "      <RETURN> when finished ");
...
/*DEBUG*/ debugf ("In RESET_THRESH ()\n");
    ...
    /*DEBUG*/ debugf ("reset_thresh: cur_digit = %lf\n", cur_digit);
        ...
        sprintf(buf,"Current thresholds:");
        message[0] = (char *) malloc (strlen (buf) + 1);
        sprintf(message[0],"%s", buf);

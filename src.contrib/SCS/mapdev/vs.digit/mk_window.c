...
set_window_w_mouse ()
{
    ...
    double delta;
    int yn;
    int menu_top, menu_left, ret, chr;
    ...
    buttons[3] = "Middle: Widen view";
    buttons[4] = "Right:  Accept window";
    buttons[5] = "  ";
    buttons[6] = '\0';

    Dchoose(MEN.name) ;
    popup_butns( menu_top, menu_left, buttons, "set_wind", 1) ;
    Dchoose(DIG.name) ;
...
/*DEBUG*/ debugf ("MKWIND default (%lf, %lf, %lf, %lf)\n", sN, sS, sE, sW);
		...
		{
                    sprintf (buffer,"Window is too small to display .") ;
...
foo:
	    ...
	    {
            sprintf(buffer,
		"Window is outside of default\n   Proceed ? ");
	        yn = mouse_yes_no (buffer);
		if (!yn)
		    goto top;
	    }
	    clear_window ();
    ...
    double N, S, E, W;
{
    ...
    if (dummy == 0)
    {
	utm_to_screen ( CM->head.W, CM->head.N, screenx, screeny) ;
	utm_to_screen ( CM->head.E, CM->head.N, screenx+1, screeny+1) ;
	utm_to_screen ( CM->head.E, CM->head.S, screenx+2, screeny+2) ;
	utm_to_screen ( CM->head.W, CM->head.S, screenx+3, screeny+3) ;
	dummy = 1;
    }

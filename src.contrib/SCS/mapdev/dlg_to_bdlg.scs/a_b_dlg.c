...
/* Make each area a new att and create a special dig cats file MLH 6/10/92 */
			if (n_atts)
				{
				fprintf(tmp,"%d:",num_areas);
				for (i=0; i<= n_atts;i++) fprintf(tmp,"%d:", att_buff[i]);
				fprintf(tmp,"\n");
				att_buff[1] = num_areas;
				}
	...
	case 'L':
		...
		case 'E':
			...
			fprintf (stdout,"\n") ;
			fflush(bin) ;
			fprintf(f_cats,"# %d categories\n",num_areas+1);
			fprintf(f_cats,"From DLG.scs\n\n0.00 0.00 0.00 0.00\n0:no data\n");
			fclose(tmp);
			return(tmp_name) ;

		default:
			fprintf (stdout," unknown line: '%s'\n", buff) ;
			num_undef++ ;
		}
	}
}

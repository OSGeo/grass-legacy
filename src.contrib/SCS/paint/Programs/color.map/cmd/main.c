static char rcsid[]="$Header: /gis3/4.0/src/paint/Programs/p.map/cmd/RCS/main.c,v 1.1 1991/09/28 22:01:03 paul Exp $";
#include "gis.h"
#define MAIN
#include "sites.h"
#include "dlg.h"
...
/* get the window */
	...
	if (KEY("setcolor"))
	{
		...
		for (i = 0; i < count; i += 2)
		{
		    ...
		    */
		    G_add_color_rule ((CELL)min, r,g,b, (CELL)max,r,g,b, &parms.pcolr);
		}
		free (list);
	    }
	    continue;
...
*/
    ...
    unlink_pattern_file();
    BLACK = Pcolornum (0.01, 0.01, 0.01);

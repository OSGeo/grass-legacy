...
fprintf (stdout," pcols  is %d \n", pcols);
fprintf (stdout," prows is %d \n", prows);
...
/*
	...
	*/
fprintf (stdout," to get tmpwin\n");

	tmpwin.north	= window->north;
	tmpwin.south	= window->south;
	tmpwin.east	= window->east;
	tmpwin.west	= window->west;
    tmpwin.ns_res = window->ns_res;
    tmpwin.ew_res = window->ew_res;
    tmpwin.rows   = window->rows;
    tmpwin.cols   = window->cols;

	
fprintf (stdout," to get tmpwin\n");
    if (prows)
	unscaled (tmpwin, prows+100-MARGIN, max);
    else
	scale (tmpwin, max+MARGIN, MARGIN, parms.scaletext);

fprintf (stdout," tmpwin.rows is %d\n", tmpwin.rows);
fprintf (stdout," tmpwin.cols is %d\n", tmpwin.cols);
...
tmpwin.cols	= 446;
    ...
    if (prows)
	unscaled (window, prows-MARGIN, max-100);
    else
	scale (window, max-100+MARGIN, MARGIN, parms.scaletext);

fprintf (stdout," window->rows is %d\n", window->rows);
fprintf (stdout," window->cols is %d\n", window->cols);
...
/* compute number of pixels in this panel */
	...
	tpixels = tnpix > (max-100) ? (max-100) : npix;
...
/*
	...
	*/
	window->cols = tpixels;
	window->east = window->west + window->ew_res * tpixels;
...
/* put printer in raster mode, define picture */
	...
	*/
	Ppictsize (tmpwin.rows+extra_lines, pixels+MARGIN);
	Pflush () ;
...
/* print each row of the map */
	...
	next_raster_row = -1;
	for (row = 0+10; row < fullwindow.rows+10; row++)
	{
	    if (colormode == COLORMODE_DIFFUSION)
	    {
		red_carry_right = 0;

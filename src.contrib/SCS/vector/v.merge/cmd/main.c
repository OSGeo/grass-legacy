...
char *argv[];
{
	...
	char errmsg[200];
        char code;
        double x, y;
	FILE *Out, *Subj, *cat, *att, *fopen (), *tmp_file;
	...
	while (map->answers[i])
	    ...
	    tmp_file = fopen (tempname,"w");           
		 ...
		 {         /* use cats list stored in RAM */
                 for (recd=1;recd<cats.count;recd++)
                   {            /* cats search */
                   sprintf(cat_name,"%s",cats.list[recd].label);
		   cptr = cat_name;
		       /* look for field separating colons (SCS version) */
	             while (*cptr != '\0')
		       {
...
fprintf(stderr,"\tmatch %d, %d  change to %d\n",cat_num,area_id,icode);
		    ...
		    search++;
                    ...
                    sprintf(Obuffer, "%c    %10.2lf   %10.2lf    %6d\n",
			      code, x,y,icode);

    ...
    unsigned char *buf;
{
    ...
    if ((pict_row==1) && (header||first_page))
    {
        if (first_page && (ndots>10))
        {
            final_page = 0;
            Pfinish();
            first_page = 0;
    ...
    char ch8in[8],ch8out[8];
{
    ...
    /* maskb[0] = bit-0 (LSB) */
    /* maskb[7] = bit-7 (MSB) */
    static char maskb[8]={0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80};

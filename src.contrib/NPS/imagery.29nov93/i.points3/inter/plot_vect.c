/*=======================================================================
  ...
  vect.c --
    ...
    int _plotvect_warp (name, mapset, color_name, which, E, N, order)
       char *name, *mapset, *color_name;
       int  which;  == 0 - both target&zoom,
                       1 - zoom only 
       double E[]; double N[]; int  order;  -- polynomaial trans coeff.
...
/*---------------------------------------------------------------------*/
int 
_plotvect(name, mapset, color_name, which)
char *name, *mapset, *color_name;
int  which;      
{
...
_plotvect_warp (name, mapset, color_name, which)
 char *name, *mapset, *color_name;
 int  which;          /* 0 - both target&zoom, 1 - zoom only */
{
    struct Cell_head cellhd, window, tmp_window; 
...
/*------------------------------------------------------------------------*/
static
choose_vectcolor (color_name, not_used)

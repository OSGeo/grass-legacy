...
ask_gis_files (type, file, xname, xmapset, position)
    char *type, *file, *xname, *xmapset;
{
    static int use = 1;
    int pick();
...
/* start the mouse in the cancel box */
    ...
    dobox (&less,   "",       COLOR_WHITE, top, right, right+height,0);
    dobox (&more,   "",       COLOR_WHITE, bottom-height, right, right+height,0);
 ...
 */
	...
	*cur_mapset = 0;
	...
	while(1)
	{
		...
		dobox (&dummy, label, COLOR_WHITE, top+line*height, left, right, 0);
		line++;
	    }
	    if (col)
	    {
		dobox (&list[count].box, name, COLOR_GREY, top+line*height, left+width/2, right, 0);
		line++;
		col = 0;
	    }
	    else
	    {
		dobox (&list[count].box, name, COLOR_GREY, top+line*height, left, left+width/2, 0);
		col = 1;
	    }
	...
	downarrow (&more, page+1 < npages ? COLOR_BLACK : COLOR_WHITE);
	uparrow   (&less, page   > 0      ? COLOR_BLACK : COLOR_WHITE);
	which = -1;
...
static
dobox (box, text, color, top, left, right, centered)
    struct box *box;
    char *text;
{
    int bottom;

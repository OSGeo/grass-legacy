...
ask_magnification (magnification)
    int *magnification;
{
    static int use = 1;
    int incr();
    ...
    dotext ("MAGNIFICATION", top, top+height, left, right, COLOR_WHITE);
    dotext ("+", plus.top, plus.bottom, plus.left, plus.right, COLOR_GREY);
    dotext ("-", minus.top, minus.bottom, minus.left, minus.right, COLOR_GREY);
    dotext ("ACCEPT", accept.top, accept.bottom, accept.left, accept.right, COLOR_GREY);
    dotext ("CANCEL", cancel.top, cancel.bottom, cancel.left, cancel.right, COLOR_GREY);
    draw_mag();
...
draw_mag()
{
    ...
    sprintf (buf, "%d", mag);
    dotext (buf, value.top, value.bottom, value.left, value.right, COLOR_WHITE);
}
...
static
dotext (text, top, bottom, left, right, background)
    char *text;
{

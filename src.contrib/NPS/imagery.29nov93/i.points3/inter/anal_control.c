...
#define FMT1(buf,xres,yres,gnd) \
	sprintf (buf, "%7.1lf \t%7.1lf \t%7.1lf ", xres,yres,gnd)

#define LHEAD1 "              error             "
#define LHEAD2 "  #    east   north    distance "

#define FMT2(buf, e1, n1, e2, n2) sprintf (buf, "\t\t%7.0lf \t%7.0lf \t\t%9.1lf \t%9.1lf ", e1, n1, e2, n2)
...
/* lets do it */
	...
	{
	    color = COLOR_BLACK; 
	    sprintf (buf, "Overall rms error: %.2lf", resid.gnd_rms);
	}
...
/* all done. restore what was under the window */
    ...
    /** TODO - clean up free **/
    free (resid.xres); free (resid.yres); free (resid.gnd);

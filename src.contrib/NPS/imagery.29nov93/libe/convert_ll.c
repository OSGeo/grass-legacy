...
/* RETRUNS: 
/*-----------------------------------------------------------------------*/
convert_to_ll (cpll, cptemp)
     Control_Points_LL  *cpll;
     Control_Points_2D  *cptemp;
{
  int i;
  double lat,  lon;
  double east, north;
  struct Cell_head  tmp_window;  /* target window for PROJ, zone */
  ...
  for (i = 0; i < cptemp->count; i++) {
    ...
    if (pj_do_proj (&east, &north, &targ_proj_info, ll_proj_info) <0) {
       sprintf (msg, "Error in pj_do_proj\n");
       G_fatal_error (msg);	
    }

...
static int   write_group_sat (FILE *fd, Satellite sat_info);
#endif
...
-----------------------------------------------------------------------*/
I_get_group_sat (group_name, sat)
char       *group_name;
Satellite  *sat;
{
  FILE *fd;
...
/*-----------------------------------------------------------------------
  ...
  Returns:
    ...
    -2  : Error writing to file.
     1  : o.k.
-----------------------------------------------------------------------*/
I_put_group_sat (group_name, sat)
char       *group_name;
Satellite  sat;
{
  FILE *fd;
...
**/
  ...
  fprintf (fd,"SAT XP     %lf \n",sat_info.Xpix);
  fprintf (fd,"SAT YP     %lf \n",sat_info.Ypix);
...
**/
  ...
  /* 4 corners */
  for (i = 0; i < 4; i++)
    fprintf (fd, "  %15lf %15lf %15lf %15lf \n",

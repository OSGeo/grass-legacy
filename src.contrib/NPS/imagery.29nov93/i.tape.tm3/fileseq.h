...
int FileSeqFF();
#else
int FileSeqInit(FileSequence*, boolean prompt, char *name, char *suffix,
   int blocksize);
int FileSeqNext(FileSequence*, int band);
int FileSeqRead(FileSequence*, unsigned char *buffer, int length);
void FileSeqEnd(FileSequence*);
int FileSeqFF(FileSequence*, int bytes);
#endif

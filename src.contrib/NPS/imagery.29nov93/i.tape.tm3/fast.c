...
#else
static int ExamineFast(Landsat*, unsigned char*, int length, boolean info,
      	       TMinfo*);
static int ExamineFast_A(Landsat*, unsigned char*, int length, boolean info,
      	       TMinfo*);
static int ExamineFast_B(Landsat*, unsigned char*, int length, boolean info,
      	       TMinfo*);
static void ParseGeo(TMgeo*, unsigned char*, int lon, int lat);
static void ParseUTM(TMutm*, unsigned char*, int east, int north);
...
/*======================================================================
			      ImportFast
======================================================================*/
int ImportFast(landsat, header, length)
Landsat *landsat;
	 ...
	 for(k=0; k<num_present && landsat->bands_to_do; ++k) {
	    ...
	    if( landsat->bands_to_do & (1 << bands_present[k])) {
      	       ...
      	       if( bands_present[k] == 8)
      	       	  printf("Looking for band P\n");
      	       else
      	       	  printf("Looking for band %d\n", bands_present[k]);
      	    }
	 }
...
Return Value:
   ...
   1 if error.
======================================================================*/
static int ImportFastBand(landsat, band, tminfo)
Landsat *landsat;
int band;
TMinfo *tminfo;
{
   ...
   for(k=0; k<5 && hist.edlinecnt < MAXEDLINES; ++k)
      if( landsat->info.desc[k][0])
      	 G_strcpy(hist.edhist[hist.edlinecnt++], landsat->info.desc[k],
      	    RECORD_LEN -1);
...
/*======================================================================
			     ExamineFast
======================================================================*/
static int ExamineFast(landsat, header, length, info, tminfo)
Landsat *landsat;
...
/*======================================================================
			    ExamineFast_A
======================================================================*/
static int ExamineFast_A(landsat, header, length, info, tminfo)
Landsat *landsat;
unsigned char *header;	/* Volume descriptor */
int length;    	     	/* length of header. */
boolean info;
...
/*======================================================================
			    ExamineFast_B
======================================================================*/
static int ExamineFast_B(landsat, header, length, info, tminfo)
Landsat *landsat;
unsigned char *header;	/* Volume descriptor */
int length;    	     	/* length of header. */
boolean info;
...
static void ParseGeo(geo, header, lon, lat)
TMgeo *geo;
unsigned char *header;
int lon, lat;
{
...
static void ParseUTM(utm, header, east, north)
TMutm *utm;
unsigned char *header;
int east, north;
{

...
Gs_status("gv_get_new_vect");
    ...
    if(NULL == (nv = (geovect *)G_malloc(sizeof(geovect)))){
...
Gs_status("gv_set_defaults");
    ...
    gv->width = 1;
    gv->color = 0xFFFFFF;
    for(i = 0; i< MAX_SURFS; i++) gv->drape_surf_id[i] = 0;
...
Gs_status("gv_free_vect");
    ...
    if(Vect_top){ 
	if(fv == Vect_top){
            if(Vect_top->next){ /* can't free top if last */
                found = 1;
                Vect_top = fv->next;
            }
	    else{
	    ...
	    for(gv=Vect_top; gv && !found; gv=gv->next){ /* can't free top */
		if(gv->next){
		    if(gv->next == fv){
			found = 1;
			gv->next = fv->next;
...
/***********************************************************************/
gv_set_drapesurfs(gv, hsurfs, nsurfs)
geovect *gv;
int hsurfs[], nsurfs;
{
int i;

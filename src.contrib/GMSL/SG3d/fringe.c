    ...
    int side;
{
    ...
    /* bottom left */ 
    zel = elev_buf[((int)((row+side)*ymod))*X_Size+(int)(col*xmod)];
    if (EMBNULL(zel)) zel = bot;
    else zel = (float)((zel - Zoff) * Z_exag);
    vert_func (X_Min + col*xres, Y_Max - (row+side)*yres, zel);
	    
    cnt = 1;
    for (col = 0 ; col < xcnt-1 ; col++)
    {
	/* bottom right */
	zel = elev_buf[((int)((row+side)*ymod))*X_Size+(int)((col+1)*xmod)];
	if (EMBNULL(zel)) zel = bot;
	else zel = (float)((zel - Zoff) * Z_exag);
	vert_func (X_Min + (col+1)*xres, Y_Max - (row+side)*yres, zel);
	cnt++;
    ...
    int col, ycnt;    /* number of view cells across */
{
    ...
    /* bottom left */ 
    zel = elev_buf[((int)(row*ymod))*X_Size+(int)((col+side)*xmod)];
    if (EMBNULL(zel)) zel = bot;
    else zel = (float)((zel - Zoff) * Z_exag);
    vert_func (X_Min + (col+side)*xres, Y_Max - row*yres, zel);
	    
    cnt = 1;
    for (row = 0 ; row < ycnt-1 ; row++)
    {
	/* top left */
	zel = elev_buf[((int)((row+1)*ymod))*X_Size+(int)((col+side)*xmod)];
	if (EMBNULL(zel)) zel = bot;
	else zel = (float)((zel - Zoff) * Z_exag);
	vert_func (X_Min + (col+side)*xres, Y_Max - (row+1)*yres, zel);
	cnt++;
...
int arrow3d=0; 
	...
	if(Asolidscale->val){
	    ...
	    co = ~(Dcolor[SCALE_COLOR]&0xCCCCCC);
	}
	if(Awirescale->val){
	    lmcolor(LMC_COLOR);   

...
surf_init_displaymode(dc, snum, name, dmode, wcol, pcnt, wcnt, zg, xt, yt, zt)
data_cell *dc;
int snum;
char *name;
int dmode, wcol, pcnt, wcnt;
float zg, xt, yt, zt;
{
surf_dm *dm;
...
static void
_set_displaymode(dm, dc, surfnum)
surf_dm *dm;
data_cell *dc;
int surfnum;
{
...
pack_surfinfo(atts, dms, handles)
att_info atts[MAX_SURFS][MAX_ATTS];
surf_dm dms[MAX_SURFS];
int handles[MAX_SURFS];
{
int i, j, k;

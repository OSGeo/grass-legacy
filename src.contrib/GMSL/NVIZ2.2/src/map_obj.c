...
int Nget_id_from_name_cmd(Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]);
int set_drawmode(Tcl_Interp *interp, int type, int id, Nv_data *data, int argc, char *argv[]);
...
int set_wirecolor(Tcl_Interp *interp, int id, int type, Nv_data *data, int argc, char *argv[]);
...
int set_nozero(Tcl_Interp *interp, int id, int type, Nv_data *data, int argc, char *argv[]);
int get_nozero(int id, int type, Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]);
int select_surf(Tcl_Interp *interp, int id, int type, int argc, char *argv[]);
int unselect_surf(int id, int type, int argc, char *argv[], Tcl_Interp *interp);
int surf_is_selected (int id, int type, Tcl_Interp *interp, int argc, char *argv[]);
...
int load_obj(int id, int type, Nv_data *data, int argc, char *argv[], Tcl_Interp *interp);
int get_att(int id, int type, Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]);
int get_mask_mode(int id, int type, Nv_data *data, Tcl_Interp *interp);
int set_mask_mode(int id, int type, Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]);
int set_att(int id, int type, Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]);
...
int Nget_surf_list_cmd(Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]);
int Nget_vect_list_cmd(Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]);
int Nget_site_list_cmd(Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]);
...
int Nliteral_from_logical_cmd(Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]);
int Nlogical_from_literal_cmd(Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]);
 ...
 */
int Nliteral_from_logical_cmd(Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]) {
  int *surf_list, *vect_list, *site_list, num_surfs, num_vects, num_sites, i, s1_length;
  char err_string[200];
  Nv_clientData *cdata;

  /* Verify number of args */
  if (argc != 2) {
    Tcl_SetResult(interp, "Usage: Nliteral_from_logical logical_id", TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if (num_surfs) {
    for(i=0; i<num_surfs; i++) {
      ...
      if (cdata == NULL) {
	sprintf(err_string, "Internal Error: Can't find client data for surf map %d", surf_list[i]);
  ...
  if (num_vects) {
    for(i=0; i<num_vects; i++) {
      ...
      if (cdata == NULL) {
	sprintf(err_string, "Internal Error: Can't find client data for vect map %d", vect_list[i]);
	Tcl_SetResult(interp, err_string, TCL_VOLATILE);
	return (TCL_ERROR);
      }
      if (!strncmp(cdata->logical_name, argv[1], s1_length)) {
  ...
  if (num_sites) {
    for(i=0; i<num_sites; i++) {
      ...
      if (cdata == NULL) {
	sprintf(err_string, "Internal Error: Can't find client data for site map %d", site_list[i]);
	Tcl_SetResult(interp, err_string, TCL_VOLATILE);
	return (TCL_ERROR);
      }
      if (!strncmp(cdata->logical_name, argv[1], s1_length)) {
  ...
  sprintf(err_string, "Error: can't find map object for logical name %s", argv[1]);
  Tcl_SetResult(interp, err_string ,TCL_VOLATILE);
  return (TCL_ERROR);
}
...
int Nlogical_from_literal_cmd(Nv_data *data, Tcl_Interp *interp, int argc, char *argv[]) {
  ...
  if (argc != 2) {
    Tcl_SetResult(interp, "Usage: Nlogical_from_literal <map_obj>", TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int set_logical_name(int id, Tcl_Interp *interp, int argc, char *argv[]) {
  ...
  /* Check correct syntax */
  if (argc != 3) {
    Tcl_SetResult(interp, "Usage: <map> set_logical_name <string>", TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if (data == NULL) {
    sprintf(err_string, "Internal Error: Can't find client data for map %d",id);
    Tcl_SetResult(interp, err_string, TCL_VOLATILE);
    return (TCL_ERROR);
  }

  /* Free any olf space for logical name */
  if (data->logical_name != NULL) free(data->logical_name);

  /* Allocate new space and copy name */
  data->logical_name = (char *)malloc(sizeof(char)*(strlen(argv[2]) + 1));
  if (data->logical_name == NULL) {
    Tcl_SetResult(interp, "Error: out of memory in set_logical_name - allocating new space", TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int Nnew_map_obj_cmd(Nv_data *data, Tcl_Interp *interp, int argc, char *argv[])
{
  ...
  /* Check for correct syntax */
  if (argc < 2) {
    Tcl_SetResult(interp, "Usage: Nnew_map_obj <type> {default file} {name=logical_name}", TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if (!strcmp (argv[1], "surf")) {
    ...
    new_id = GVL_new_volume();

    /* See if there is a default file name specified */
    if ((argc >= 3) && (strncmp(argv[2],"name=",5))) {
      arglist[2] = argv[2];
      load_obj(new_id, VOL, data, 3, arglist, interp);
      file_used=1;
    }

    /* Initialize display parameters */
    GVL_set_volmode(new_id, 1, 0xFFFFFF, 2);

    sprintf(id, "Nvol%d", new_id);

  } else {
    Tcl_SetResult(interp, "Error: type must be one of surf, site, vect or vol", TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if (new_data == NULL) {
    Tcl_SetResult(interp, "Error: out of memory in Nnew_map_object - creating client data", TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if (argc >= 3) {
	...
	Tcl_SetResult(interp, "Error: logical name syntax should be name=<logical name>", TCL_VOLATILE);
	return (TCL_ERROR);
      }	

      log_name += 5;
      new_data->logical_name = (char *) malloc(sizeof(char)*(strlen(log_name) + 1));
      if (new_data->logical_name == NULL) {
	Tcl_SetResult(interp, "Error: out of memory in Nnew_map_object - copying logical name", TCL_VOLATILE);
	return (TCL_ERROR);
      }
  ...
  if (log_name == NULL) {
    ...
    new_data->logical_name = (char *)malloc(sizeof(char)*(strlen(temp_space) + 1));
    if (new_data->logical_name == NULL) {
      Tcl_SetResult(interp, "Error: out of memory in Nnew_map_object - creating logical name", TCL_VOLATILE);
      return (TCL_ERROR);
    }
...
int set_drawmode(Tcl_Interp *interp, int type, int id, Nv_data *data, int argc, char *argv[])
{
  ...
  if (type != SURF) {
    Tcl_SetResult(interp, "Error: map object must be a surface in order to use set_drawmode",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if (!strcmp (argv[3], "gouraud"))
    ...
    Tcl_SetResult(interp, "Usage: <map_obj> set_drawmode [ poly | wire_poly | wire | col_wire] [ gouraud | flat ]", 
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int get_drawmode(int type, int id, Nv_data *data, Tcl_Interp *interp)
{
  ...
  if (type != SURF) {
    Tcl_SetResult(interp, "Error: map object must be a surface in order to use get_drawmode",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }

  if (GS_get_drawmode(id, &mode) == -1) {
    Tcl_SetResult(interp, "Error: id in get_drawmode is invalid.", TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if ( mode & DM_GOURAUD)
    ...
    Tcl_SetResult(interp, "Internal Error: unknown shade style returned in get_drawmode",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if ( mode & DM_POLY)
    ...
    Tcl_SetResult(interp, "Internal Error: unknown surface style returned in get_drawmode",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int draw_obj(Tcl_Interp *interp, int id, int type)
{
  switch (type) {
    ...
    case VOL:
      ...
      break;
    default:
      Tcl_SetResult(interp, "Internal Error: map_obj type unknown in draw_mode",
		    TCL_VOLATILE);
      return (TCL_ERROR);
      break;
    }
...
int delete_obj(int id, int type, Nv_data *data, Tcl_Interp *interp)     
{
  switch (type) {
    ...
    case VOL:
      ...
      break;
    default:
      Tcl_SetResult(interp, "Internal Error: unknown map object type in delete",
		    TCL_VOLATILE);
      return (TCL_ERROR);
      break;
    }
...
int get_res(int id, int type, Tcl_Interp *interp, int argc, char *argv[])
{
  ...
  if (type != SURF) {
    Tcl_SetResult(interp, "Error: map object must be a surface in order to use get_res",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if (!strcmp(argv[2],"wire"))
    ...
    Tcl_SetResult(interp, "Usage: <map_obj> get_res [ wire | poly | both]",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int set_res(Tcl_Interp *interp, int id, int type, int argc, char *argv[])     
{
  int xres, yres, xwire, ywire;
  if (type != SURF) {
    Tcl_SetResult(interp, "Error: map object must be a surface in order to use set_res",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }

  if (argc < 5) {
    Tcl_SetResult(interp, "Usage: <map_obj> set_res [ [wire xwire ywire] |\n\t\t[poly xres yres] |\n\t\t[both xres yres xwire ywire] ]", TCL_VOLATILE);
    return (TCL_ERROR);
  }
		  ...
		  Nv_data *data, int argc, char *argv[])
{
  ...
  if (type != SURF) {
    Tcl_SetResult(interp, "Error: map object must be a surface in order to use set_wirecolor",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }

  if (argc < 3) {
    Tcl_SetResult(interp, "Usage: <map_obj> set_wirecolor [color / \"UseMap\"]",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int get_trans(int id, int type, Tcl_Interp *interp)
{
  ...
  switch (type)
	...
	break;
      default:
	Tcl_SetResult(interp, "Internal Error: unknown map object type in get_trans",
		      TCL_VOLATILE);
	return (TCL_ERROR);
	break;
      }
...
int set_trans(Tcl_Interp *interp, int id, int type, int argc, char *argv[])     
{
  ...
  switch (type)
	...
	break;
      default:
	Tcl_SetResult(interp, "Internal Error: unknown map object type in set_trans",
		      TCL_VOLATILE);
	return (TCL_ERROR);
      }
  return (TCL_OK);
...
int set_nozero(Tcl_Interp *interp, int id, int type, Nv_data *data, int argc, char *argv[])     
{
  ...
  if (type != SURF) {
    Tcl_SetResult(interp, "Error: map object must be a surface in order to use set_nozero",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }

  if (argc < 4) {
    Tcl_SetResult(interp, "Usage: <map_obj> set_nozero [topo | color] [0 | 1]",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int get_nozero(int id, int type, Nv_data *data, Tcl_Interp *interp, int argc, char *argv[])
{
  ...
  if (type != SURF) {
    Tcl_SetResult(interp, "Error: map object must be a surface in order to use get_nozero",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int select_surf(Tcl_Interp *interp, int id, int type, int argc, char *argv[])
{
  ...
  if (get_type(argv[2]) != SURF) {
    Tcl_SetResult(interp, "Error: argument to select_surf must be a surface map object",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if (type == SITE) {
    ...
    Tcl_SetResult(interp, "Error: expected vect or site map_obj in select_surf",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int unselect_surf(int id, int type, int argc, char *argv[], Tcl_Interp *interp)
{
  ...
  surfid = get_idnum(argv[2]);
  if (get_type(argv[2]) != SURF) {
    Tcl_SetResult(interp, "Error: argument to unselect_surf must be a surface map object",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if (type == SITE) {
    ...
    Tcl_SetResult(interp, "Error: expected vect or site map_obj in unselect_surf",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int surf_is_selected (int id, int type, Tcl_Interp *interp, int argc, char *argv[])
{
  ...
  surfid = get_idnum(argv[2]);
  if (get_type(argv[2]) != SURF) {
    Tcl_SetResult(interp, "Error: argument to surf_is_selected must be a surface map object",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
  ...
  if (type == SITE) {
    ...
    Tcl_SetResult (interp, "Error: expected vect or site map_obj in surf_is_selected",
		   TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int set_exag_obj(int id, int type, int argc, char *argv[], Tcl_Interp *interp)
{
  ...
  if (argc < 3) {
    Tcl_SetResult(interp, "Usage: <map_obj> set_exag value", TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int get_exag_guess(int id, int type, Tcl_Interp *interp)
{
  ...
  if (type != SURF) {
    Tcl_SetResult(interp, "Error: map object must be a surface in order to use get_exag_guess",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
 ...
 *     General load function used to load either a vector
 *     or site file.  The type loaded depends on the
...
int load_obj(int id, int type, Nv_data *data, int argc, char *argv[], Tcl_Interp *interp)
{
  ...
  case VECT:
    ...
    break;
  default:
    Tcl_SetResult(interp,"Error: Map object must be of type vector or site in load_obj",
		  TCL_VOLATILE);
    return (TCL_ERROR);
    break;
  }
...
int get_att(int id, int type, Nv_data *data, Tcl_Interp *interp, int argc, char *argv[])     
{
  ...
  default:
    Tcl_SetResult(interp, "Internal Error: unknown map_obj type in get_att",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int set_att(int id, int type, Nv_data *data, Tcl_Interp *interp, int argc, char *argv[])
{
  ...
  int surf_num, max, xres, yres, xwire, ywire, max2;
  int index;
  float temp2, size;
  float temp;
  long col;
  ...
  case SITE:
	...
	if (!strncmp(argv[3], "3d", 2)) {
	  ...
	  Tcl_SetResult(interp, "Error in set_att display, must be 3d or surfdisp",
			TCL_VOLATILE);
	  return (TCL_ERROR);
	}
      }
      GP_get_sitemode(id, &useatt, &col, &width, &size, &marker);
      col = (strcmp (argv[2], "color")) ? col : tcl_color_to_int(argv[3]);
      width = (strcmp (argv[2], "width")) ? width : atoi(argv[3]);
      size = (strcmp (argv[2], "size")) ? size : (float) atof(argv[3]);
      marker = (strcmp (argv[2], "marker")) ? marker : get_int_marker (argv[3]);
      useatt = (strcmp (argv[2], "useatt")) ? useatt : 
	(!strcmp(argv[3],"z")) ? ST_ATT_SIZE :
	  ((!strcmp (argv[3], "color")) ? ST_ATT_COLOR : ST_ATT_NONE);
  ...
  default:
    Tcl_SetResult(interp, "Internal Error: unknown map_obj type in set_att",
		  TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int unset_att(int id, int type, Tcl_Interp *interp, int argc, char *argv[])     
{
  ...
  if (type != SURF)
    {
      Tcl_SetResult (interp, "Type must be SURF for unset_att", TCL_VOLATILE);
      return (TCL_ERROR);
    }
...
int get_mask_mode(int id, int type, Nv_data *data, Tcl_Interp *interp)
{
  ...
  if (type != SURF) {
    Tcl_SetResult(interp, "Type must be SURF for get_mask_mode", TCL_VOLATILE);
    return (TCL_ERROR);
  }
...
int set_mask_mode(int id, int type, Nv_data *data, Tcl_Interp *interp, int argc, char *argv[])
{
  ...
  if (type != SURF) {
    Tcl_SetResult(interp, "Type must be SURF for set_mask_mode", TCL_VOLATILE);
    return (TCL_ERROR);
  }

  if (Tcl_GetBoolean(interp, argv[2], &mode) != TCL_OK) {
    Tcl_SetResult(interp, "Error: must be surf_id set_mask_mode BOOLEAN", TCL_VOLATILE);
    return (TCL_ERROR);
  } else {

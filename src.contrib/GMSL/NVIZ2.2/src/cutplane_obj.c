...
Nset_fence_color_cmd (Nv_data *data, Tcl_Interp *interp, int argc, char **argv)
{
  /* Verify arguments */
  if (argc != 2) {
    sprintf(interp->result,"Usage: Nset_fence_color ABOVE | BELOW | BLEND | GREY | OFF");
    return (TCL_ERROR);
  }
  ...
  if (!strncmp(argv[1],"ABOVE",5)) {
    ...
    sprintf(interp->result, "Error in Nset_fence_color, second argument must be one of ABOVE, BELOW, BLEND, GREY or OFF.");
    return (TCL_ERROR);
  }
...

{
  ...
  if (!strcmp(argv[1], "draw"))
    ...
    sprintf(interp->result,"Usage: %s \tdraw [surf1 surf2]\n\t\ton\n\t\toff\n\t\tset_rot dx dy dz\n\t\tset_trans dx dy dz\n\t\tget_rot\n\t\tget_trans",argv[0]);
    return (TCL_ERROR);
  }
}
     ...
     
{
  ...
  */
  ...
  data->Cp_trans[num_id][X] = data->Cp_trans[num_id][Y] = data->Cp_trans[num_id][Z] = 0.0;
  data->Cp_on[num_id]=0;

  Tcl_CreateCommand (interp, id, (Tcl_CmdProc *)Ncutplane_obj_cmd, data, NULL);
...
draw_cp_obj (Nv_data *data, Tcl_Interp *interp, int id, char **argv, int argc)
{
  ...
  if (argc == 2) {
    ...
    if (Tcl_GetInt(interp, argv[2], &bound1) != TCL_OK) {
      sprintf(interp->result,"Error in %s draw %s %s, expected integer argument in surf1 field",
	      argv[0], argv[2], argv[3]);
      return (TCL_ERROR);
    }

    if (Tcl_GetInt(interp, argv[3], &bound2) != TCL_OK) {
      sprintf(interp->result, "Error in %s draw %s %s, expected integer argument in surf2 field",
	      argv[0], argv[2], argv[3]);
      return (TCL_ERROR);
    }
  }
...
cp_set_rot (Nv_data *data, Tcl_Interp *interp, int id, char **argv, int argc)
{
  ...
  if (Tcl_GetDouble(interp, argv[2], &dx) != TCL_OK) {
    sprintf(interp->result,"Error in %s set_rot %s %s %s, expected numerical argument in dx field",
	    argv[0], argv[2], argv[3], argv[4]);
    return (TCL_ERROR);
  }

  if (Tcl_GetDouble(interp, argv[3], &dy) != TCL_OK) {
    sprintf(interp->result,"Error in %s set_rot %s %s %s, expected numerical argument in dy field",
	    argv[0], argv[2], argv[3], argv[4]);
    return (TCL_ERROR);
  }

  if (Tcl_GetDouble(interp, argv[4], &dz) != TCL_OK) {
    sprintf(interp->result,"Error in %s set_rot %s %s %s, expected numerical argument in dz field",
	    argv[0], argv[2], argv[3], argv[4]);
    return (TCL_ERROR);
  }
...
cp_set_trans (Nv_data *data, Tcl_Interp *interp, int id, char **argv, int argc)
{
  ...
  /* Extract nummerical arguments */
  if (Tcl_GetDouble(interp, argv[2], &dx) != TCL_OK) {
    sprintf(interp->result,"Error in %s set_trans %s %s %s, expected numerical argument in dx field",
	    argv[0], argv[2], argv[3], argv[4]);
    return (TCL_ERROR);
  }

  if (Tcl_GetDouble(interp, argv[3], &dy) != TCL_OK) {
    sprintf(interp->result,"Error in %s set_trans %s %s %s, expected numerical argument in dy field",
	    argv[0], argv[2], argv[3], argv[4]);
    return (TCL_ERROR);
  }

  if (Tcl_GetDouble(interp, argv[4], &dz) != TCL_OK) {
    sprintf(interp->result,"Error in %s set_trans %s %s %s, expected numerical argument in dz field",
	    argv[0], argv[2], argv[3], argv[4]);
    return (TCL_ERROR);
  }

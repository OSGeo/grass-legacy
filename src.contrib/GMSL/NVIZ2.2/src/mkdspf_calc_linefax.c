...
Tcl_Interp *interp;
{
  ...
  if (strcmp(args[3],"c") == 0)
  ...
  else if (strcmp(args[3],"i") == 0)
    {
      ...
      if (Tcl_SplitList(interp, args[4], &num_thresh_values, &thresh_values) != TCL_OK) {
  ...
  switch(type)
	...
	Tcl_AppendResult(interp, "Max thresh ", temp1, "  Min thresh ", temp2, 
			 "\nInterval ", temp3, "\n", NULL);
      }
      ...
      if(((datarange)/interval) > 126.0) /* means #thres = 127*/
	{
	  ...
	  sprintf(temp," %f\n", interval);
	  Tcl_AppendResult(interp,"Maximum number of thresholds exceeded. \n",
...
/****************************** lit_model ************************************/
int
lit_model(linefax,typeid,interp)
cmndln_info	*linefax;

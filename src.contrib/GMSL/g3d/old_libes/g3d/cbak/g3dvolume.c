...

{
  ...
  nxp = nx * 2 + 1; nyp = ny * 2 + 1; nzp = nz * 2 + 1;

  u000 = (doubleArray *) u[0][0][0]; u001 = (doubleArray *) u[0][0][1]; 
  u010 = (doubleArray *) u[0][1][0]; u011 = (doubleArray *) u[0][1][1];
  u100 = (doubleArray *) u[1][0][0]; u101 = (doubleArray *) u[1][0][1]; 
  u110 = (doubleArray *) u[1][1][0]; u111 = (doubleArray *) u[1][1][1];
  ...
  for (dz = 1; dz < nzp; dz += 2) {
    ...
    for (dy = 1; dy < nyp; dy += 2) {
      s = 1. - (sp = dy / ny / 2.);
      v0[0] = s * v00[0] + sp * v10[0]; v0[1] = s * v00[1] + sp * v10[1];
      v0[2] = s * v00[2] + sp * v10[2];

      v1[0] = s * v01[0] + sp * v11[0]; v1[1] = s * v01[1] + sp * v11[1];
      v1[2] = s * v01[2] + sp * v11[2];

      for (dx = 1; dx < nxp; dx += 2) {
	t = 1. - (tp = dx / nx / 2.);
	v[0] = t * v0[0] + tp * v1[0]; v[1] = t * v0[1] + tp * v1[1];
	v[2] = t * v0[2] + tp * v1[2];
    ...
    
{
  ...
  for (z = 0; z < nz; z++) {
    for (y = 0; y < ny; y++) {
      for (x = 0; x < nx; x++) {
	if (! G3d_putValueRegion (mapVolume, x, y, z, 
			    volumeBuf + (z * ny * nx + y * nx + x) * eltLength,
			    G3d_fileTypeMap (mapVolume)))
	  G3d_fatalError ("G3d_makeAllignedVolumeFile: error in G3d_putValueRegion");
      }
    }
  }

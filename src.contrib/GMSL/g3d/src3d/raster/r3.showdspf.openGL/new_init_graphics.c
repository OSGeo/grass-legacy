...
void winset_main()
{
    glXMakeCurrent(XtDisplay(MainOGLWindow.widget),MainOGLWindow.window,MainOGLWindow.glx_context);
}
...
{
   glXMakeCurrent(XtDisplay(ColormapWindow.widget),ColormapWindow.window,ColormapWindow.glx_context);
}
...
void init_graphics(char *name,int argc,char **argv,struct dspec *dspecptr)
{
   ...
   XtManageChild(form);
   vi = glXChooseVisual(XtDisplay(form),DefaultScreen(XtDisplay(form)),Attributes);
   if(!vi)
   {
   ...
   MultipleToggle = XtVaCreateManagedWidget("Multiple",xmToggleButtonWidgetClass, 
                                                   radio,NULL);
   XtAddCallback(MultipleToggle,XmNarmCallback,SingleMultipleCB,(XtPointer)0);
   ...
   n = 0;
   Button_plus = XtCreateManagedWidget ("Increase",xmPushButtonWidgetClass,buttonrowcol,args,n);
   XtAddCallback(Button_plus,XmNactivateCallback,PlusMinusCB,(XtPointer)1);
   Button_minus = XtCreateManagedWidget ("Decrease",xmPushButtonWidgetClass,buttonrowcol,args,n);
   ...
   if(Headfax.linefax.nthres)
   {
      for(i = 1; i <= Headfax.linefax.nthres; i++)
      {
         ...
         if(i <= MAXTHRESHOLDS)
         {
            ...
            Threshbutton[i-1] = XtVaCreateManagedWidget(buffer,xmToggleButtonWidgetClass, 
                                                   ThresholdRadio,NULL);
   ...
   XtSetArg(args[n],XmNbottomAttachment,XmATTACH_FORM);
   n++;
   MainOGLWindow.widget =  
            GLwCreateMDrawingArea(form,"glwidget",args,n);
   ...
   XtManageChild(form);
   vi = glXChooseVisual(XtDisplay(form),DefaultScreen(XtDisplay(form)),SingleAttributes);
   if(!vi)
   {
      fprintf(stderr,"Fatal error: Couldn't get a visual\n");
      fprintf(stderr,"See program developer.\n");
   ...
   XtSetArg(args[n],XmNbottomAttachment,XmATTACH_FORM);
   n++;
   ColormapWindow.widget =  
            GLwCreateMDrawingArea(form,"glwidget",args,n);

   XtManageChild(ColormapWindow.widget);

   XtAddCallback(ColormapWindow.widget,GLwNginitCallback,glinitCB,(void *)1);
...
void ThresholdCB(Widget widget,XtPointer client_data,XtPointer call_data)
{
   ...
   if(MultipleThresholdFlag) return;
...
void rotateLoop()
{
   ...
   for(;;)
   {
      ...
      if(event.type == KeyPress)
      {
         ...
         sym = XKeycodeToKeysym(XtDisplay(MainOGLWindow.widget), K->keycode, 0);
         if(sym == XK_Escape)
         {
...
void glinitCB(Widget widget,XtPointer client_data,XtPointer cdata)
{
   ...
   {  
      ...
      glXMakeCurrent(XtDisplay(widget),MainOGLWindow.window,MainOGLWindow.glx_context);
   ...
   else
   {
      ...
      glXMakeCurrent(XtDisplay(widget),ColormapWindow.window,ColormapWindow.glx_context);
      clear_screen ();

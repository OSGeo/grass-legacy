...
*/
    ...
    ** but also the data values at these vertices must be stored */
      ...
      side_to_xy (&(Polys[new_pnum].verts[n_verts<<1]),&(Polys[pnum]),zz, side);
      n_verts++;
	...
	Polys[new_pnum].verts[(n_verts<<1)+1] = Polys[pnum].verts[(side <<1)+1];
	n_verts++;
	...
	side = next_higher(side,Polys[pnum].vnum);
      ...
      {
	if (t)
	    draw_cappolys (Headp, B_spec, Cap, &Polys[new_pnum], x, y, 1,tnum);
	drawn = 1;
      }
      else
      {
...
/* if flat shading*/
  ...
  for (y = Cap->miny ; y < Cap->maxy ; y++)		/* for each cell */
  {
    ...
    {
      Polys[0].data[0]=Polys[0].data[4]=(double)Cap->D_buff[row+xdim+x]; /*BL*/
      Polys[0].data[1] = (double)Cap->D_buff[row+xdim+x+1];/* BR */
      Polys[0].data[2] = (double)Cap->D_buff[row+x+1];     /* TR */
      Polys[0].data[3] = (double)Cap->D_buff[row+x];       /* TL */

      /* for each threshold array */
      for (ta = 0 ; ta < 2 && (tp = &(B_spec->threshes[ta]))->nthres ; ta++)	
      {

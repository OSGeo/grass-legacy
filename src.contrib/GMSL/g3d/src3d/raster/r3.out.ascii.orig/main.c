...
/* structs */
typedef	struct {
  struct Option *input, *output, *null_val;
  struct Flag *header;
} paramType;
...
void G3dToascii(FILE *fp, G3D_Region *region);
...
void main(int argc, char *argv[]) {
  ...
  /* Open the map.  Note that currently we require the input map to be
   * in the current mapset.  This is necessary because there is no
   * current G3d support code for parsing out mapsets from the input
   * file name.  I could get around this by writing my own text parser
   * but havn't done so yet.
   */
  map = G3d_openCellOld(input, G_mapset(),

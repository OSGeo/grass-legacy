...
******************************************************************************/
get_color_table (Headp, file, ctable)
    file_info *Headp;
    char *file;
    struct color_entry ctable[];
{
	...
	get_default_table (Headp, ctable);
    }
    return (0);
}
/******************************************************************************
...
new_color_file (Headp, file, cfile, D_spec)
    file_info *Headp;
    char *file, *cfile;
    struct dspec *D_spec;
{
    ...
    else if (0 > get_color_table (Headp, cfile, D_spec->ctable))
    {
...
******************************************************************************/
no_color_file (Headp, D_spec, cfile)
    file_info *Headp;
    struct dspec *D_spec;
    char *cfile;
{
    if (D_spec->cfile != NULL)
        fclose (D_spec->cfile);
    D_spec->cfile = NULL;
    if ( 0 > get_color_table (Headp, cfile, D_spec->ctable))
    {
	fprintf (stderr, "Using default color table\n");
	get_default_table (Headp, D_spec->ctable);
    }
}
/******************************************************************************
    ...
    float spec;
{
   ...
   materials[2] =
   materials[3] = spec;
   lmdef(DEFMATERIAL,1, 15, materials);
   lmbind(MATERIAL,1);
}

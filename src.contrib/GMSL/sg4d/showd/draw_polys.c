...
file_info *Headp;
{
  ...
  t = D_spec->Thresh;
  for (z = 0 ; z < Headp->zdim ; z++)
  {
    zadd = z * Headp->ydim;
    for (y = 0 ; y < Headp->ydim ; y++) 
    {
      yadd = y * D_spec->yscale;
      for (x = 0 ; x < Headp->xdim ; x++)
      {
	if (!read_cube (&Cube, Headp))
	    continue;
	...
	for (t = 0; t < Cube.n_thresh; t++)
	{
	  ...
	  {
	    curr = D_spec->Thresh;       
	    get_cat_color (Headp->linefax.tvalue[curr], D_spec->ctable, color);
	    c3s (color);
	    ...
	    for ( p = 0; p < cubefax[t].npoly; p++)
	    {
		  ...
		  tmp_vect[0] = X_sign * G_sign * (Polyfax->n1[0]/127. - 1.);
		  tmp_vect[1] = X_sign * G_sign * (Polyfax->n1[1]/127. - 1.);
		  tmp_vect[2] = X_sign * G_sign * (Polyfax->n1[2]/127. - 1.);
		  normal(tmp_vect);
		  tmp_vect[0] = Polyfax->v1[0]/255.*D_spec->xscale+xadd; 
		  tmp_vect[1] = Polyfax->v1[1]/255.*D_spec->yscale+yadd;
		  tmp_vect[2] = Polyfax->v1[2]/255.*D_spec->zscale+zadd;
		  v3f(tmp_vect);


		  tmp_vect[0] = Polyfax->v2[0]/255.*D_spec->xscale+xadd; 
		  tmp_vect[1] = Polyfax->v2[1]/255.*D_spec->yscale+yadd;
		  tmp_vect[2] = Polyfax->v2[2]/255.*D_spec->zscale+zadd;
		  v3f(tmp_vect);

		  tmp_vect[0] = Polyfax->v3[0]/255.*D_spec->xscale+xadd; 
		  tmp_vect[1] = Polyfax->v3[1]/255.*D_spec->yscale+yadd;
		  tmp_vect[2] = Polyfax->v3[2]/255.*D_spec->zscale+zadd;
		  v3f(tmp_vect);
...
/* a color data file has been requested */
    ...
    if (color_on)
    {
        level = get_level (&Headp, &chead, 0);
	row[1] = get_row (&Headp, &chead, 0);
	col[1] = get_col (&Headp, &chead, 0);
	if (level < 0  || row[1] < 0 || col[1] < 0)
	{
	    ...
	    color_on = 0;
            ...
            g3read_level(&chead, slice[1], level);/*read in data*/
    }
    if (!color_on)
    {
        get_cat_color (Headp->linefax.tvalue[curr], D_spec->ctable, color);
  ...
  for (z = 0 ; z < Headp->zdim ; z++)
  {
    ...
    if (color_on)
    {
	if ( 0 > (level = get_level (Headp, &chead, z+1)))
	{
    ...
    for (y = 0 ; y < Headp->ydim ; y++) 
    {
	  ...
	  if (y)
	      ...
	      row[0] = get_row (&Headp, &chead, 0);
	  if (0 > (row[1] = get_row (&Headp, &chead, y + 1)))
	  {
		  row[1] = row[0];
	...
	if (color_on)
	{
		...
		col[1] = get_col (Headp, &chead, 0);
	      if (0 > (col[1] = get_col (Headp, &chead, x + 1)))
	      {
		  col[1] = col[0];
...
/* TODO.  What happens for outside, w/ Thresh == max thresh? */
	      ...
	      if (color_on && cubefax[t].npoly)
	          fill_data_cube (data, slice, row, col, D_spec->ctable, xdim);
	      for ( p = 0; p < cubefax[t].npoly; p++)
	      {
		/* center data */ 
		Polyfax = &(cubefax[t].poly[p]);

		{
		  bgnpolygon();
		    ...
		    {
			get_vert_color 
			    (data, Polyfax->v1, D_spec->ctable, color);
			c3s (color);
		    }

		    norm[0] = 
			(X_sign * G_sign * (Polyfax->n1[0]/127. - 1.))/xres;
		    norm[1] = 
		        (X_sign * G_sign * (Polyfax->n1[1]/127. - 1.))/yres;
		    norm[2] = 
			(X_sign * G_sign * (Polyfax->n1[2]/127. - 1.))/zres;
		    normalize(norm);
		    normal(norm);

		    Polyfax->v1[0] = Polyfax->v1[0]*D_spec->xscale+xadd; 
		    Polyfax->v1[1] = Polyfax->v1[1]*D_spec->yscale+yadd;
		    Polyfax->v1[2] = Polyfax->v1[2]*D_spec->zscale+zadd;
		    v3f(Polyfax->v1);
		    ...
		    {
			get_vert_color 
			    (D_spec, data, Polyfax->v2, D_spec->ctable, color);
			c3s (color);
		    }
		    
		    norm[0] = 
			(X_sign * G_sign * (Polyfax->n2[0]/127. - 1.))/xres;
		    norm[1] = 
		        (X_sign * G_sign * (Polyfax->n2[1]/127. - 1.))/yres;
		    norm[2] = 
			(X_sign * G_sign * (Polyfax->n2[2]/127. - 1.))/zres;
		    normalize(norm);
		    normal(norm);

		    Polyfax->v2[0] = Polyfax->v2[0]*D_spec->xscale+xadd; 
		    Polyfax->v2[1] = Polyfax->v2[1]*D_spec->yscale+yadd;
		    Polyfax->v2[2] = Polyfax->v2[2]*D_spec->zscale+zadd;
		    v3f(Polyfax->v2);
		    ...
		    {
			get_vert_color 
			    (D_spec, data, Polyfax->v3, D_spec->ctable, color);
			c3s (color);
		    }
		    norm[0] = 
			(X_sign * G_sign * (Polyfax->n3[0]/127. - 1.))/xres;
		    norm[1] = 
		        (X_sign * G_sign * (Polyfax->n3[1]/127. - 1.))/yres;
		    norm[2] = 
			(X_sign * G_sign * (Polyfax->n3[2]/127. - 1.))/zres;
		    normalize(norm);
		    normal(norm);

		    Polyfax->v3[0] = Polyfax->v3[0]*D_spec->xscale+xadd; 
		    Polyfax->v3[1] = Polyfax->v3[1]*D_spec->yscale+yadd;
		    Polyfax->v3[2] = Polyfax->v3[2]*D_spec->zscale+zadd;
		    v3f(Polyfax->v3);

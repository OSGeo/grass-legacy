...
write_cube (Cube, cur_x, headfax)
    file_info *headfax;
    Cube_data	*Cube;	       /* array of poly info  by threshold */
    int cur_x;
{
    register int i, j;
    ...
    register int poly_size;	/* sizeof (poly_info) * t_cnt */
    poly_info *Poly_info;
    int t_cnt;
...
*/
write_cube_buffer (Buffer, size, cur_x, headfax)
    file_info *headfax;
    ...
    file_info *headfax;
{
    ...
    static zeros_left = 0;	/* move this out if a seek routine is written */

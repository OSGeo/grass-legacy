    ...
    int type;
{
    ...
    */
	...
	if (D_Cap->z == 12)
	{
	    ...
	    xd = D_Cap->Cols;
	    fprintf (stderr, "Z/X\t%8d  %8d  %8d  %8d  %8d  %8d  \n", 
			  14, 15, 16, 17, 18);
	    fprintf (stderr, 
	    "-------------------------------------------------------------\n");
	    for (tmpz = 14; tmpz < 19; tmpz++)
	    {
	         fprintf (stderr, "%d|\t", tmpz+1);
	         for (tmpx = 13; tmpx < 18; tmpx++)
		    fprintf (stderr, "%8f  ", D_Cap->D_buff[tmpz*xd+tmpx]);

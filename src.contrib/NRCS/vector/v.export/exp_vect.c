...
#define		A_DXF		"dxf"
#define		ATT		"dig_att"
#define V_PROG1  "v.out.dlg"   /* non-interactive version */
#define V_PROG2  "v.out.gef"   /* non-interactive version */
...
/*  file1 - binary digit,   file2 - ascii gef  */
	...
	if (level == 99 )
	{
	        ...
	        mapset = G_ask_any( " FILENAME TO STORE ASCII NRCS_GEF FILE", 
...
ask_which_level()
{
	...
	printf("     1  -   Ascii DLG file from GRASS Vector Format\n") ;
	printf("     2  -   Ascii VECTOR file from GRASS Vector Format\n") ;
/*	printf("     3  -   Ascii NRCS_GEF file from GRASS Vector Format\n") ;*/
	printf("     3  -   Ascii ARC/INFO file from GRASS Vector Format\n") ;
	printf("     4  -   Ascii DXF file from GRASS Vector Format\n") ;
...
run_dig_to_dlg( command, name, binary_dig, ascii_dlg)
	char	*command, *name, *ascii_dlg, *binary_dig;
{
...
run_dig_to_gef( command, name, binary_dig, ascii_gef)
	char	*command, *name, *ascii_gef, *binary_dig;
{
...
run_b_a_vect( command, name, ascii_vector, binary_vector)

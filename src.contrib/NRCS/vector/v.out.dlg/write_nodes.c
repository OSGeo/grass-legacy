...
#define NODE_FMT "N%5d%12.2lf%12.2lf%6d%6d%6d%6d%6d            \n"
    ...
    FILE *fp;
{
	...
	for (i = 0 ; i < Node->n_lines ; i++)
	{
	    	...
	    	{
			/* don't write out atts for DOTs but write line refs*/
			if (dlgtype){ 
		    		n_lines++;
			}else{
	...
	if (Node->n_lines)
	{
	    ...
	    {
		/* write out nodes */
		if ((map->Line[abs (Node->lines[i])].type == DOT) && !dlgtype)
		    continue;

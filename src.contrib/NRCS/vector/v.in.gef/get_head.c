...
#define TEXT2	\
	"                 NRCSGEF IMPORT REFERENCE INFORMATION"
#define TEXT4	\
	"          GRASS requires information NOT available in NRCSGEF ."
#define TEXT5	\
   ...
   char *gef, *reg;
{
	...
	char filename[128];
	int  i, ier, stzone ;
	float xdeg, xmin, xsec, ydeg, ymin, ysec, x1, y1;
        float  lat, lon;
...
/* Print warning */
	...
	if ( (gef_file = fopen(filename, "r")) == NULL)
	   {
	   printf("Can't find GEF input file <%s>\n", gef) ;
	   return (-1);
	   }
	...
	if ( (gef_info = fopen(filename, "w")) == NULL)
	   {
	   printf("Can't open new %s info file\n", gef) ;
	   return (-1);
	   }
	...
	if ( (reg_info = fopen(filename, "w")) == NULL)
           {
	   printf("Can't create reg file  %s\n", reg) ;
	   return (-1) ;
           }
...
/*fprintf(stderr,"record 1:\n%s",buff);*/
	...
	if (strchr(scale,':'))
	   ...
	   while(*tpntr != '\0')
           ...
           *(scl++) = '\0';
			  ...
			  /* a 1: may NOT exist, but a , may */
	if (strchr(scale,','))
	   {
	   sprintf(buff,"%s", scale);
	   tpntr = buff;
	   scl = scale;
	   while(*tpntr != '\0')
	      {
	...
	sscanf(scale, "%lf", &dig.orig_scale);
...
/* generate some dig head info */
	strcpy(dig.organization,"USDA, NRCS");
	strcpy(dig.date,date);
	strcpy(dig.your_name,"NRCS GEF Import");
	   ...
	   lon = x1;
	        ...
	        dig.S = x1;
	         ...
	         dig.S = y1;
              ...
              if (strncmp(dig.in_proj,"table",2) == 0)
	         {
...
/* Gather all existing variables **********************************************/
	...
	while (repeat)
	{
		...
		V_ques(HEAD->in_proj ,     's', 14, 64, 10) ;
	...
	if (HEAD->in_proj != 0)
		{
		if (strncmp(HEAD->in_proj,"table",2) == 0) ier = 0;

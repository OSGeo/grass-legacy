...
imp_tig(Map,dig_name, cfclist, tig_file1,tig_file2,dig_att,nrcstig,verbose)
struct Map_info *Map;
char **cfclist;
...

{
	...
	int projection;
        ...
        G_set_key_value("ellps", "clark66", out_proj_keys);
	...
	if (nrcstig == 1) /* skip FIPS code for nrcstig files */
	...
	while((fgets(buff, MAXLINE, tig_file1) != NULL)) {
		...
		if (level == 1 ){
			...
			if (strncmp(cfclist[0], "BOU", 3) == 0){
				if ((*(buf+15) != 49) && (nrcstig != 1)) {
					continue;
				}
				if ((*(buf+10) != 49) && (nrcstig == 1)) {
					continue;
				}
				...
				if (nrcstig == 1) /* use short nrcs tiger file */
		...
		/*	name_to_number(cat_file,cfcc,&cat_num);*/
			...
			if (nrcstig == 1){ /* use short nrcs tiger file */
				...
				G_strncpy(buf1,buf+190,10);    /* long from */
				G_strncpy(buf2,buf+200,9);    /* lat from */
				G_strncpy(buf3,buf+209,10);    /* long to */
				G_strncpy(buf4,buf+219,9);    /* lat to */
			}
...
*/
	/*fprintf(stderr,"\n%f %f %f %f \n",lat1,lon1,lat2,lon2); */
			...
			E2 = lon2;
			N2 = lat2;
			pj_do_proj(&E1, &N1, &info_out, &info_in);
			pj_do_proj(&E2, &N2, &info_out, &info_in);
...
/*			old_pos = ftell(tig_file2); */
			...
			for(x=1;x<11;x++,y++,bufp += 19){
				G_strncpy(buflon, bufp, 10);
				G_strncpy(buflat, bufp + 10, 9);
				sscanf(buflon, "%f", &lon);
				sscanf(buflat, "%f", &lat);
/*fprintf(stderr,"\n%f %f *%s* *%s*\n%s\n",
				...
				if( lon != 0.0 ){
					...
					E = lon;
					N = lat;
					pj_do_proj(&E, &N, &info_out, &info_in);

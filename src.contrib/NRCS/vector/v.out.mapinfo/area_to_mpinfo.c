...

{
	...
	if(category == 0) {
	  printf(" Area %d has no category value, quitting\n",area);
	  exit(-1);
	  }
...
printf("category:%d  label <%s>\n", category,label);
printf("continue ?"); gets(ans);
*/
...
printf("	ISLAND Area # = %d\n",island); 
	    ...
	    if(category == 0) {
	      printf(" Island %d in Area %d has no category value, quitting\n",isle,area);
	      exit(-1);
	      }
            for(ii=0;ii < pcats->count; ii++) {
                if (pcats->list[ii].num == category)
...
+++
printf("island category= %d, label <%s>\n",category,label);
printf("continue ?"); gets(ans);
+++

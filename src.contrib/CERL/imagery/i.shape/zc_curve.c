...
zc_curve(inputfd, zcfd, Width, ds, dalpha, sout, totals)
int inputfd, zcfd, *sout;
float Width;
double dalpha[POINTS],ds[POINTS],*totals;
{
...
-------------------------------------------------------------------
				*/
				...
				k=0;
	printf("zero crossing cell file ... \n");
...
if( *(data[1]+scani[scan]*cols+scanj[scan]) > 0.5 )
	...
	s=s+1;
        ...
        y[s]=(double) yks;
        x[s]=(double) xks;
printf("for larger scan, points=%d, x=%f, y=%f\n", s, x[s], y[s]);
        *(data[1]+scani[scan]*cols+scanj[scan]) = 0;
        kscan=0;
        break;
        }
...
valpha[s-1]=alpha;
printf("s=%d, ls=%lf,ltotal=%lf, valpha=%lf\n",s-1,ls[s-1],ltotal,valpha[s-1]);
}
if(kscan == 2)
{
	s=s+1;
	...
	ds[s]=ltotal;		
	valpha[s-1]=alpha;
printf("last point, s=%d, ls=%lf,ltotal=%lf, valpha=%lf\n",
	s-1,ls[s-1],ltotal,valpha[s-1]);
	ls[s]=ls[0];
	valpha[s]=valpha[0];
}

...
install_hidden_panels ()
{
    ...
    Anewsite->x = 0.5;
    Anewsite->y = 1.0;
    pnl_addact (Anewsite, P_Options);

    Astcolor = pnl_mkact (pnl_discrete_hpalette);
    Astcolor->label = "    sites color = green  ";
    Astcolor->minval = 0;
    Astcolor->maxval = 9;
    Astcolor->x = 0.5;
    Astcolor->y = 0.3;
...
_do_globe()
{
	...
	Afocus->selectable = Ascale->selectable = !LatLon;
    pnl_fixact(Awhat);
    pnl_fixact(Afringe);
    pnl_fixact(Afocus);
    pnl_fixact(Ascale);
...
update_site_color ()
{
    ...
    sprintf (st_buf, "    sites color = %s",cobuf);

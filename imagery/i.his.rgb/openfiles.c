                ...
                CELL *rowbuf[3])
{
    ...
    if ((fd_output[0] = G_open_cell_new (r_name)) < 0)
        G_fatal_error (_("Cannot create raster map <%s>"), r_name);
    if ((fd_output[1] = G_open_cell_new (g_name)) < 0)
        G_fatal_error (_("Cannot create raster map <%s>"), g_name);
    if ((fd_output[2] = G_open_cell_new (b_name)) < 0)
        G_fatal_error (_("Cannot create raster map <%s>"), b_name);
    ...
    if ((fd_input[0] = G_open_cell_old (h_name, mapset)) < 0)
        G_fatal_error (_("Cannot open raster map <%s>"), h_name);
    ...
    if ((fd_input[1] = G_open_cell_old (i_name, mapset)) < 0)
        G_fatal_error (_("Cannot open raster map <%s>"), i_name);
    ...
    if ((fd_input[2] = G_open_cell_old (s_name, mapset)) < 0)
        G_fatal_error (_("Cannot open raster map <%s>"), s_name);

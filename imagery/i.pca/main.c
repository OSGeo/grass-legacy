...
int main (int argc, char *argv[])
{
  ...
  opt3->required   = NO;
  opt3->answer     = "0,255"; 
  opt3->description= _("Rescaling range output (For no rescaling use 0,0)");
  ...
  if (opt3->answer)
  {
	...
	{
            G_message(_("Scale range length should be >0; Using default values: 0,255\n\n"));
            scale_min = 0;
            scale_max = 255;
        }
     }
  ...
  if (bands < 2)
    G_fatal_error(_("Sorry, you must provide at least 2 input bands"));
  ...
  for (i=0 ; i<bands ; i++)
  {
      ...
      if ((mapset=G_find_cell(opt1->answers[i], "")) == NULL)
          G_fatal_error(_("Unable to find cell map <%s>"), opt1->answers[i]);

      if ((inp_file_descr[i] =G_open_cell_old(opt1->answers[i], mapset)) < 0)
          G_fatal_error(_("Error opening %s"), opt1->answers[i]);
  ...
  /*
    for (i=0; i<bands; i++) {
	    ...
	    if (strcmp(inp_names[i], out_names[j]) == 0)
	        G_fatal_error(_("The output file %s will overwrite the input file %s"),
  ...
  */
  ...
  G_message(_("Calculating eigenvalues and eigenvectors..."));
  ...
  */
  ...
  G_message(_("Ordering eigenvalues in descending order..."));
  egvorder2(eigval, eigmat, bands);

  G_message(_("Transposing eigen matrix..."));
  transpose2(eigmat, bands);

  G_message(_("Transforming data..."));
             ...
             if (pass == 1)
	     {
	         fprintf(stdout,"%s:   ", name);
                 ...
                 if ((infd=G_open_cell_new(name)) < 0)
                     G_fatal_error(_("Error opening %s"), G_fully_qualified_name(name, mapset));
             }

 ...
 */
    south = window->south;
    window->rows = (window->north - window->south + window->ns_res/2) / window->ns_res ;
    window->south = window->north - window->rows * window->ns_res;

/* do the same for the west */
    west = window->west ;
    window->cols = (window->east - window->west + window->ew_res/2) / window->ew_res ;
    ...
    {
	fprintf (stderr, "warning - north falls outside the minimal window\n");
	ok = 0;
    }
    if (window->south < minimal.south)
    {
	fprintf (stderr, "warning - south falls outside the minimal window\n");
	ok = 0;
    }
    if (window->east > minimal.east)
    {
	fprintf (stderr, "warning - east falls outside the minimal window\n");
	ok = 0;
    }
    if (window->west < minimal.west)
    {
	fprintf (stderr, "warning - west falls outside the minimal window\n");
	ok = 0;
    }

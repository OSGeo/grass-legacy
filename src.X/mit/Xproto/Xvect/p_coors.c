...
#define X_ADJ(x)    (int)((x - dig_U_west ) * dig_U_to_D_xconv + xl)
#define Y_ADJ(y)     (int)((dig_U_north - y) * dig_U_to_D_yconv  + yt)
...
struct line_pnts *line_p;
{
    ...
    /* dig_First(xptr++, yptr++) ;
     * 
     * for(i=1; i<n_coors; i++) dig_Next(xptr++, yptr++) ; */
    ...
    for (i = 0; i < n_coors; i++) {
        ...
        a_pt->x = (short)X_ADJ(utm_x);
        a_pt->y = (short) Y_ADJ(utm_y);
...
/* check to see if entire line segment is outside window */
    ...
    if (off_a_roll) {
        ...
        if (on_a_roll) {
            ...
            xpos = X_ADJ(current_x_utm);
            ypos = Y_ADJ(current_y_utm);
            Move_abs(xpos, ypos);
            xpos = X_ADJ(new_x);
            ypos = Y_ADJ(new_y);
            Cont_abs(xpos, ypos);

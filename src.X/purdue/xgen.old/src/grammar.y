...
ShellType
    : StaticMenu
		{ $$ = STATICMENU; shellType = SM; }
    | DynamicMenu
		{ $$ = DYNAMICMENU; shellType = DM; }
    | MessageBoard
		{ $$ = MESSAGEBOARD; shellType = MB; }
    | CommandBoard
		{ $$ = COMMANDBOARD; shellType = CB; }
    ;
    ...
    : ResourceType ':' Value
		{
			...
			switch(resourceType) {
				case ENVIRONMENT:
					...
					AddResource(curRes,curEnv,resourceType,$3,$<ival>1);
					break;
				case SHELL:
					...
					AddResource(curRes,curShell,resourceType,$3,$<ival>1);
					break;
				case OBJECT:
					...
					AddResource(curRes,curObject,resourceType,$3,$<ival>1);
					break;
			}
			tailRes = curRes;
		}

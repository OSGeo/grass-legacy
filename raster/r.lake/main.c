 ...
 *               - Add different debug level messages;
 *               - Option to output resulting lake area and volume.
 *
...
/* Saves map file from 2d array. NULL must be 0. */
void save_map (FCELL **out, int out_fd, int rows, int cols, int flag, FCELL *min_depth, FCELL *max_depth)
{
    int row, col;

    G_message(_("Saving new map"));
    for (row = 0; row < rows; row++)
    {
        ...
        G_put_f_raster_row(out_fd, out[row]);
...
int main(int argc, char *argv[])
{
    ...
    for (row = 0; row < rows; row++)
    {
        ...
        G_get_f_raster_row(in_terran_fd, in_terran[row], row);

        if (smap_opt->answer)
          G_get_f_raster_row(out_fd, out_water[row], row);
    ...
    G_message(_("Starting lake filling at level of %8.4f in %d passes. \nPercent done:"),water_level,pases);
    ...
    save_map(out_water, out_fd, rows, cols, negative_flag->answer, &min_depth, &max_depth);

...
void  open_files()
{
  ...
  if (choice->att[1] || choice->att[2] ||
     ...
     fprintf(fp, "Scale  Unit  MN. PIXEL ATT.   S.D. PIXEL ATT.  MN. PATCH ATT.   S.D. PATCH ATT.\n");
     fclose(fp);
  }
  ...
  if (choice->size[1] || choice->size[2]) {
     ...
     fprintf(fp, "Scale  Unit  MN. PATCH SIZE   S.D. PATCH SIZE -- in pixels\n");
     fclose(fp);
  }
  ...
  if (choice->size[5]) {
     ...
     fprintf(fp, "               NUMBER OF PATCHES BY SIZE CLASS -- in pixels\n");
     fprintf(fp, "Scale  Unit ");
  ...
  if (choice->size[6]) {
     ...
     fprintf(fp, "               NUMBER OF PATCHES BY SIZE CLASS BY GROUP -- in pixels\n");
     fprintf(fp, "Scale  Unit ");
     for(i = 0; i < size_cl_no; i++)
        fprintf(fp, "   Class[%2d]",i + 1);
     fprintf(fp, "\n");
  ...
  if (choice->core[1] || choice->core[2] ||
     ...
     fprintf(fp, "Scale  Unit  MEAN CORE SIZE   S.D. CORE SIZE   MEAN EDGE SIZE   S.D. EDGE SIZE\n");
     fclose(fp);
  }
  ...
  if (choice->core[9]) {
     ...
     fprintf(fp, "               NUMBER OF PATCH CORES BY SIZE CLASS -- in pixels\n");
     fprintf(fp, "Scale  Unit ");
     for(i = 0; i < size_cl_no; i++)
        fprintf(fp, "   Class[%2d]",i + 1);
     fprintf(fp, "\n");
     ...
     fprintf(fp, "               NUMBER OF PATCH EDGES BY SIZE CLASS -- in pixels\n");
     fprintf(fp, "Scale  Unit ");
     for(i = 0; i < size_cl_no; i++)
        fprintf(fp, "   Class[%2d]",i + 1);
     fprintf(fp, "\n");
  ...
  if (choice->core[10]) {
     ...
     fprintf(fp, "               NUMBER OF PATCH CORES BY SIZE CLASS BY GROUP -- in pixels\n");
     fprintf(fp, "Scale  Unit ");
     for(i = 0; i < size_cl_no; i++)
        fprintf(fp, "   Class[%2d]",i + 1);
     fprintf(fp, "\n");
     ...
     fprintf(fp, "               NUMBER OF PATCH EDGES BY SIZE CLASS BY GROUP -- in pixels\n");
     fprintf(fp, "Scale  Unit ");
     for(i = 0; i < size_cl_no; i++)
        fprintf(fp, "   Class[%2d]",i + 1);
     fprintf(fp, "\n");
  ...
  if (choice->shape[6]) {
     ...
     fprintf(fp, "               NO. OF PATCHES BY SHAPE INDEX CLASS BY GROUP\n");
     fprintf(fp, "Scale  Unit ");
     for(i = 0; i < shape_cl_no; i++)
        fprintf(fp, "   Class[%2d]",i + 1);
     fprintf(fp, "\n");
  ...
  if (choice->boundary[1] || choice->boundary[2] ||
     ...
     fprintf(fp, "Scale  Unit  MEAN TWIST NUM.  SD. TWIST NUM.   MN. OMEGA INDEX  SD. OMEGA INDEX\n");
     fclose(fp);
  }
  ...
  if (choice->perim[1] || choice->perim[2] ||
     ...
     fprintf(fp, "Scale  Unit  SUM PERIMETER    MEAN PERIMETER   S.D. PERIMETER -- in pixels\n");
     fclose(fp);
  }

  if (choice->perim[4]) {
     fp = fopen0("r.le.out/p4.out", "w");
     fprintf(fp, "               SUM OF PERIMETERS BY GROUP -- in pixels\n");
     fprintf(fp, "Scale  Unit ");
     for(i = 0; i < ntype; i++)
        fprintf(fp, "   Group[%2d]",i + 1);
     fprintf(fp, "\n");
...
char *name, *flag;
{
  ...
  if (!(fp = fopen(name, flag))) {
     fprintf(stderr, "\n");
     fprintf(stderr, "   ******************************************************\n");
     fprintf(stderr, "    You chose a moving window or sampling units analysis \n");
     fprintf(stderr, "       but r.le.patch can't find file \"%s\"             \n",name);
     fprintf(stderr, "       which defines the moving window or sampling units \n");
...
char *name, *flag;
{
  ...
  if (!(fp = fopen(name, flag))) {
     fprintf(stderr, "\n");
     fprintf(stderr, "   **************************************************\n");
     fprintf(stderr, "    You chose a 'by gp' or 'by class' analysis       \n");
     fprintf(stderr, "       but r.le.patch can't find file \"%s\"         \n",name);
...
*/
  ...
  if (nc < 1 || nr < 1) {
     fprintf(stderr, "\n");
     fprintf(stderr, "   *******************************************************\n");
     fprintf(stderr, "    The moving window size specified in file r.le.para/   \n");
  ...
  if (wind.rows < nr + y0 || wind.cols < nc + x0) {
     fprintf(stderr, "\n");
     fprintf(stderr, "   *******************************************************\n");
     fprintf(stderr, "    Moving window search area in file r.le.para/move_wind \n");
     fprintf(stderr, "    does not match the dimensions of the current region.  \n");
     fprintf(stderr, "    You must either rerun r.le.setup to make a new        \n");
  ...
  fprintf(stderr, "If a MASK is not present (see r.mask) a beep may sound and a\n");
  fprintf(stderr, "   warning may be printed or appear in a window; ignore this warning.\n");
...
*/
  ...
  if (choice->att[5] || choice->att[6] || choice->size[3] || choice->size[4]
     ...
     if (!ntype) {
        fprintf(stderr, "\n");
        fprintf(stderr, "   ********************************************************\n");
        fprintf(stderr, "    The attribute group file (r.le.para/recl_tb) seems to  \n");
...
double dist;
  ...
  if (ncols < left + u_w || nrows < top + u_l) {
     fprintf(stderr, "\n");
     fprintf(stderr, "   ******************************************************\n");
     fprintf(stderr, "    Sampling units do not fit within the current region. \n");
     fprintf(stderr, "    Either correct the region or redo the sampling unit  \n");
     fprintf(stderr, "    selection using r.le.setup.  This error message came \n");

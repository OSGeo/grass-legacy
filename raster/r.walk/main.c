...
int main (int argc, char *argv[])
{
	...
	opt11->required   = YES;
	opt11->multiple   = NO;
	opt11->description= _("Lambda coefficients for combining walking energy and friction cost");
	...
	opt13->multiple   = NO;
	opt13->answer     = "-0.2125";
	opt13->description= _("Slope factor determines travel energy cost per height step");
	...
	if ((par_number=sscanf(opt10->answer, "%lf,%lf,%lf,%lf", &a,&b,&c,&d)) != 4)
		G_fatal_error(_( "Missing required value: got %d instead of 4"), par_number);
	else
	{
		if (verbose)
	...
	scols=ncols/nseg + 1 ;
	if (maxmem > 0)
		segments_in_memory = 2 + maxmem * (nrows/srows) * (ncols/scols) / 100;
	else
...

	{
		...
		for( row=0 ; row<nrows ; row++ )
		{
			...
			if(G_get_raster_row(dtm_fd, dtm_cell, row, dtm_data_type)<0)
				G_fatal_error(_("Can't get row %d from raster map %s"),row,dtm_layer);
		/* INPUT NULL VALUES: ??? */
			ptr2 = dtm_cell;
			switch (dtm_data_type) {
				case CELL_TYPE:		
	...
	
	{
		...
		for( row=0 ; row<nrows ; row++ )
		{
			...
			if(G_get_raster_row(cost_fd, cost_cell, row, cost_data_type)<0)
				G_fatal_error(_("Can't get row %d from raster map %s"),row,cost_layer);
		/* INPUT NULL VALUES: ??? */
			ptr2 = cost_cell;
			switch (cost_data_type) {
				case CELL_TYPE:		
	...
	*/
	if (! have_start_points)
	{
		...
		if (cell2 == NULL)
			G_fatal_error(_( "Memory allocation error on reading start points from raster map %s"), cum_cost_layer);
		if (verbose)
		...
		for ( row=0 ; row<nrows ; row++ )
		{
			...
			if ( G_get_raster_row (cum_fd, cell2, row, data_type2) < 0)
				G_fatal_error(_("Can't get row %d from raster map %s"),row,cum_cost_layer);
			ptr2 = cell2;
	...
	else
	{
		...
		while(top_start_pt != NULL)
		{
			...
			segment_put(&out_seg,value_start_pt,top_start_pt->row,top_start_pt->col);
	...
	while ( pres_cell != NULL )
	{
		...
		double NNE_dtm, ENE_dtm, ESE_dtm, SSE_dtm, SSW_dtm, WSW_dtm, WNW_dtm, NNW_dtm ;
		double N_cost, NE_cost, E_cost, SE_cost, S_cost, SW_cost, W_cost, NW_cost ;
		double NNE_cost, ENE_cost, ESE_cost, SSE_cost, SSW_cost, WSW_cost, WNW_cost, NNW_cost ;
		...
		for( neighbor=1;neighbor<=total_reviewed;neighbor++ )
		{
			...
			case 1:
				...
				if(((W_dtm - my_dtm) / EW_fac) >= 0)
							...
							else  fcost_dtm = (double)((double)(W_dtm - my_dtm) * c);
				fcost_cost = ((double)(W_cost + my_cost) / 2.0);
				min_cost = pres_cell->min_cost + fcost_dtm +(EW_fac * a) + lambda*fcost_cost*EW_fac;
				break ;
			case 2:
				dtm_value = &E_dtm ;
				segment_get(&dtm_in_seg,dtm_value,row,col);
				...
				if (((E_dtm - my_dtm) / EW_fac) >= 0)
			              ...
			              else  fcost_dtm = (double)(E_dtm - my_dtm) * c ;
				...
				if (((N_dtm - my_dtm) / NS_fac) >= 0)
			            fcost_dtm = (double)(N_dtm - my_dtm) * b ;
			             ...
			             fcost_dtm = (double)(N_dtm - my_dtm) * d ;
		                    else  fcost_dtm = (double)(N_dtm - my_dtm) * c ;
				fcost_cost = (double)(N_cost + my_cost) / 2.0 ;
				min_cost = pres_cell->min_cost + fcost_dtm + (NS_fac * a) +lambda * fcost_cost *NS_fac ;
				break ;
			case 4:
				dtm_value = &S_dtm;
				segment_get(&dtm_in_seg,dtm_value,row,col);
				...
				if (((S_dtm- my_dtm) / NS_fac) >= 0)
      				   fcost_dtm = (double)(S_dtm- my_dtm) * b ;
		                  ...
		                  else  fcost_dtm = (double)(S_dtm- my_dtm) * c ;
			...
			case 5:
				...
				if (((NW_dtm - my_dtm) / DIAG_fac) >= 0)
				    fcost_dtm = (double)(NW_dtm - my_dtm) * b ;
			         ...
			         else  fcost_dtm = (double)(NW_dtm - my_dtm) * c ;
				fcost_cost = (double)(NW_cost + my_cost) / 2.0 ;
				min_cost = pres_cell->min_cost + fcost_dtm + (DIAG_fac * a) +lambda * fcost_cost *DIAG_fac;
				break ;
			case 6:
				dtm_value = &NE_dtm ;
				segment_get(&dtm_in_seg,dtm_value,row,col);
				...
				if (((NE_dtm - my_dtm) / DIAG_fac) >= 0)
				   fcost_dtm = (double)(NE_dtm - my_dtm) * b ;
			           ...
			           else  fcost_dtm = (double)(NE_dtm - my_dtm) * c ;
			...
			case 7:
				...
				if (((SE_dtm - my_dtm) / DIAG_fac) >= 0)
			        ...
			        else  fcost_dtm = (double)(SE_dtm - my_dtm) * c ;
			...
			case 8:
				...
				if (((SW_dtm - my_dtm) / DIAG_fac) >= 0)
			          ...
			          else  fcost_dtm = (double)(SW_dtm - my_dtm) * c ;
				...
				if (((NNW_dtm - my_dtm) / V_DIAG_fac) >= 0)
				  fcost_dtm = (double)(NNW_dtm - my_dtm) * b ;
			         ...
			         fcost_dtm = (double)(NNW_dtm - my_dtm) * d ;
			          else  fcost_dtm = (double)(NNW_dtm - my_dtm) * c ;
				fcost_cost = (double)(N_cost + NW_cost + NNW_cost + my_cost) / 4.0 ;
				min_cost = pres_cell->min_cost+fcost_dtm+(V_DIAG_fac * a) +lambda * fcost_cost *V_DIAG_fac;
				break ;
			case 10:
				dtm_value = &NNE_dtm ;
				segment_get(&dtm_in_seg,dtm_value,row,col);
				...
				if (((NNE_dtm - my_dtm) / V_DIAG_fac) >= 0)
				  ...
				  else  fcost_dtm = (double)(NNE_dtm - my_dtm) * c ;
				fcost_cost = (double)(N_cost + NE_cost + NNE_cost + my_cost) / 4.0 ;
				min_cost = pres_cell->min_cost+fcost_dtm+(V_DIAG_fac * a) +lambda * fcost_cost *V_DIAG_fac;
				break ;
			case 11:
				dtm_value = &SSE_dtm ;
				...
				if (((SSE_dtm - my_dtm) / V_DIAG_fac) >= 0)
				 ...
				 fcost_dtm = (double)(SSE_dtm - my_dtm) * d ;
			          else  fcost_dtm = (double)(SSE_dtm - my_dtm) * c ;
				fcost_cost = (double)(S_cost + SE_cost + SSE_cost + my_cost) / 4.0 ;
				min_cost = pres_cell->min_cost+fcost_dtm+(V_DIAG_fac * a)+lambda * fcost_cost *V_DIAG_fac ;
				break ;
			case 12:
				dtm_value = &SSW_dtm ;
				...
				if (((SSW_dtm - my_dtm) / V_DIAG_fac) >= 0)
				  ...
				  else  fcost_dtm = (double)(SSW_dtm - my_dtm) * c ;
				fcost_cost = (double)(S_cost + SW_cost + SSW_cost + my_cost) / 4.0 ;
				min_cost = pres_cell->min_cost+fcost_dtm+(V_DIAG_fac * a)+lambda * fcost_cost *V_DIAG_fac ;
				break ;
			case 13:
				dtm_value = &WNW_dtm ;
				...
				if (((WNW_dtm - my_dtm) / H_DIAG_fac) >= 0)
				   ...
				   else  fcost_dtm = (double)(WNW_dtm - my_dtm) * c ;
				fcost_cost = (double)(W_cost + NW_cost + WNW_cost + my_cost) / 4.0 ;
				min_cost = pres_cell->min_cost+fcost_dtm+(H_DIAG_fac * a)+lambda * fcost_cost *H_DIAG_fac ;
				break ;
			case 14:
				dtm_value = &ENE_dtm ;
				segment_get(&dtm_in_seg,dtm_value,row,col);
				...
				if (((ENE_dtm - my_dtm) / H_DIAG_fac) >= 0)
				 ...
				 fcost_dtm = (double)(ENE_dtm - my_dtm) * d ;
				  else  fcost_dtm = (double)(ENE_dtm - my_dtm) * c ;
				fcost_cost = (double)(E_cost + NE_cost + ENE_cost + my_cost) / 4.0 ;
				min_cost = pres_cell->min_cost+fcost_dtm+(H_DIAG_fac * a)+lambda * fcost_cost *H_DIAG_fac ;
				break ;
			case 15:
				dtm_value = &ESE_dtm ;
				...
				if (((ESE_dtm - my_dtm) / H_DIAG_fac) >= 0)
				 fcost_dtm = (double)(ESE_dtm - my_dtm) * b ;
			         ...
			         else  fcost_dtm = (double)(ESE_dtm - my_dtm) * c ;
				fcost_cost = (double)(E_cost + SE_cost + ESE_cost + my_cost) / 4.0 ;
				min_cost = pres_cell->min_cost+fcost_dtm+(H_DIAG_fac * a)+lambda * fcost_cost *H_DIAG_fac;
				break ;
			case 16:
				dtm_value = &WSW_dtm ;
				...
				if (((WSW_dtm - my_dtm) / H_DIAG_fac) >= 0)
				 ...
				 else  fcost_dtm = (double)(WSW_dtm - my_dtm) * c ;
				fcost_cost = (double)(W_cost + SW_cost + WSW_cost + my_cost) / 4.0 ;
				min_cost = pres_cell->min_cost+fcost_dtm+(H_DIAG_fac * a)+lambda * fcost_cost *H_DIAG_fac ;
				break ;
			}
		...
		for ( row=0 ; row<nrows; row++ )
		{
			...
			if (keep_nulls) {
				if(G_get_raster_row(dtm_fd, cell2, row, dtm_data_type)<0)
					G_fatal_error(_("Error getting input null cells at row %d from %s"),row,dtm_layer);
			}
			p = cum_cell;
			p2 = cell2;
			for ( col=0 ; col<ncols; col++ )

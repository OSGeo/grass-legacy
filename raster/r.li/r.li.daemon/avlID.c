...
static avlID_node* critical_node( avlID_node *added, int *pos1, int *pos2, const avlID_node *prec);
...
int avlID_add(avlID_tree *root, const  long k, const  long n)
{
    ...
    /* insert the new node */
    node_temp->father= p;
    if(d == -1) p->left_child= node_temp;
    else
    {
	if( d == 1 ) p->right_child= node_temp;
	else
	{
	    G_free(node_temp);
	    return ERR;
	...
	case AVL_SS: avlID_rotation_ll( critical ); break;
	case AVL_SD: avlID_rotation_lr( critical ); break;
...
static avlID_node* critical_node( avlID_node *added, int *pos1, int *pos2, const avlID_node *prec)
{
  ...
  if(prec==NULL)
      ...
      *pos2 = *pos1;
      if( prec == added->left_child ) *pos1=AVL_S;
      else *pos1=AVL_D; /* prec == added->right_child */
  }


  fdb= abs( avlID_height(added->left_child) - avlID_height(added->right_child) );
...
void avlID_rotation_ll( avlID_node* critical )
{
    ...
    if( r->father!=NULL )
    {
	if( (r->father)->left_child == r ) (r->father)->left_child= s;
	else (r->father)->right_child= s;
    }
...
void avlID_rotation_lr( avlID_node* critical )
{
    ...
    if( r->father!=NULL )
    {
	if( (r->father)->left_child == r ) (r->father)->left_child= t;
	else (r->father)->right_child= t;
    }
    ...
    if(b!=NULL) b->father = s;
    if(g!=NULL) g->father = r;
}

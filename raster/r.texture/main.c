 ...
 **********************************************************************************************/
		    ...
		    measure=(FCELL) h_measure (data, size, size, t_measure, dist);
		...
		if (row==0)
			...
			if (G_put_raster_row (outfd, outrast, out_data_type) < 0)
			    G_fatal_error(_("Failed writing raster map <%s> row %d"), result, row);
	    ...
	    G_close_cell (outfd);
	    G_important_message (_("Calculated measure #%d (56 measures available)"), (t_measure+1));
	}
    G_free(outrast); 
    G_free (data);

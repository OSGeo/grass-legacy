...
static int 
f_if_i(int argc, const int *argt, void **args)
{
	CELL *res = args[0];
	DCELL *arg1 = args[1];
	CELL *arg2 = args[2];
...
static int 
f_if_f(int argc, const int *argt, void **args)
{
	FCELL *res = args[0];
	DCELL *arg1 = args[1];
	FCELL *arg2 = args[2];
...
static int 
f_if_d(int argc, const int *argt, void **args)
{
	DCELL *res = args[0];
	DCELL *arg1 = args[1];
	DCELL *arg2 = args[2];
...
int 
f_if(int argc, const int *argt, void **args)
{
	if (argc < 1)
		return E_ARG_LO;
	if (argc > 4)
	...
	switch (argt[0])
	{
	case CELL_TYPE:		return f_if_i(argc, argt, args);
	case FCELL_TYPE:	return f_if_f(argc, argt, args);
	case DCELL_TYPE:	return f_if_d(argc, argt, args);
	default:		return E_INV_TYPE;
	}
}

int 
c_if(int argc, int *argt)
{
	if (argc < 1)
		return E_ARG_LO;
	if (argc > 4)
	...
	if (argc >= 2 && argt[2] == FCELL_TYPE) argt[0] = FCELL_TYPE;
	if (argc >= 3 && argt[3] == FCELL_TYPE) argt[0] = FCELL_TYPE;
	if (argc >= 4 && argt[4] == FCELL_TYPE) argt[0] = FCELL_TYPE;

	if (argc >= 2 && argt[2] == DCELL_TYPE) argt[0] = DCELL_TYPE;
	if (argc >= 3 && argt[3] == DCELL_TYPE) argt[0] = DCELL_TYPE;

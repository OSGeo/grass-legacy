...
# Before using this a script must:
# source lib/gtcltk/settings.tcl (gui.tcl does this)
# source lib/gtcltk/gmsg.tcl (gui.tcl does this)
# And must define all of these globally:

# execute        Run with a ui
# run_panel      Run a command with no ui.
# term           Run a command that needs a terminal
# spawn          Run a command in the background, don't wait for it to return
# guarantee_xmon Make sure there is an xmon available for the command
...
proc GuiMenu::tree {} {
	...
	# Menus that can be injected in here
	set injected_menus {Menu_File_Top Menu_File_Bottom}
	foreach injected_menu $injected_menus {
		variable $injected_menu
		if {![info exists $injected_menu]} {
			set $injected_menu ""
		}
	}

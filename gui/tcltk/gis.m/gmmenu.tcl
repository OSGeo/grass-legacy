...
set descmenu [subst  {
 [G_msg "&File"] all file $tmenu {
	...
	{cascad {[G_msg "Manage maps and volumes"]} {} "" $tmenu {			
		...
		{command {[G_msg "List maps using expressions and 'wildcards'"]} {} "g.mlist" {} -command {$execom g.mlist }}
	...
	{separator}
	{cascad {[G_msg "Settings"]} {} "" $tmenu {			
		{command {[G_msg "Open..."]} {} "Open gis.m settings file" {} -accelerator $keyctrl-O -command { Gm::OpenFileBox }}
		{command {[G_msg "Save"]} {} "Save gis.m settings file" {} -accelerator $keyctrl-S -command { Gm::SaveFileBox }}
		{command {[G_msg "Save as..."]} {} "Save gis.m settings file as new name" {} -command { set filename($mon) "" ; Gm::SaveFileBox }}
		{command {[G_msg "Close"]} {} "Close gis.m settings" {} -accelerator $keyctrl-W -command { GmTree::FileClose {}}}
	}}
	{separator}
 ...
 [G_msg "&Config"] all options $tmenu {
	{cascad {[G_msg "Region"]} {} "" $tmenu {			
		...
		{command {[G_msg "Zoom to maximum extent of all displayed maps"]} {} "d.extend" {} -command {run d.extend }}
		{separator}
	...
	{cascad {[G_msg "Text"]} {} "" $tmenu {			
		...
		{command {[G_msg "Show standard GRASS fonts"]} {} "show.fonts.sh" {} -command {$execom show.fonts.sh }}
	}}
 ...
 [G_msg "&Raster"] all options $tmenu {
	{cascad {[G_msg "Develop map"]} {} "" $tmenu {			
		{command {[G_msg "Digitize raster"]} {} "r.digit" {} -command {Gm::xmon term r.digit }}
		{separator}
	...
	{cascad {[G_msg "Terrain analysis"]} {} "" $tmenu {			
		...
		{command {[G_msg "Profile analysis"]} {} "d.profile" {} -command {Gm::xmon run d.profile }}
 ...
 [G_msg "&Vector"] all options $tmenu {
	{cascad {[G_msg "Develop map"]} {} "" $tmenu {			
		{command {[G_msg "Digitize"]} {} "v.digit" {} -command {Gm::xmon run v.digit }}
		{separator}
	...
	{cascad {[G_msg "Network analysis"]} {} "" $tmenu {			
		...
		{command {[G_msg "Shortest route (visualization only)"]} {} "d.path" {} -command {Gm::xmon run d.path }}
...
lappend descmenu [subst {
	...
	{command {[G_msg "GIS Manager &help"]} {} "GIS Manager help" {} -command { exec g.manual gis.m > /dev/null & } }
	{command {[G_msg "About &GRASS"]} {} "About GRASS" {} -command { source $env(GISBASE)/etc/gm/grassabout.tcl} }
	{command {[G_msg "About &System"]} {} "About System" {} -command { exec $env(GRASS_WISH) $env(GISBASE)/etc/gm/tksys.tcl --tcltk & }}
 }]

...
*/
 int insert_str ( char *str, int pos, char **strarr ) {
	 ...
	 if (( pos < 0 ) || ( pos > (n) )) {
		 print_error ( ERR_REGISTER_ENTRIES_GISMAN, "insert: invalid line number %i.\n", pos);
	 }	 
...
*/ 
 int delete_str ( int pos, char **strarr ) {
	 ...
	 if (( pos < 0 ) || ( pos > (i) )) {
		 print_error (ERR_REGISTER_ENTRIES_GISMAN,"delete: invalid line number %i.\n", pos);
	 }	
 ...
 void get_package_name ( char *path, char *name ) {
	...
	if ( f == NULL ) {
		...
		if ( nc_fgets_nb (tmp, MAXSTR, f) == NULL ) {
			fclose (f);
			print_error ( ERR_INVALID_EXT,"invalid or missing extension name.\n"); 
		} else {
			chop ( tmp );
			strcpy ( name, tmp );
		}
...
void wget_extension ( char *url ) {
	...
	error = system (str);
	if ( error == -1 ) {
		print_error ( ERR_DOWNLOAD,"could not run 'wget' to download extension. Is it installed?\n");
	}
	if ( error > 0 ) {
